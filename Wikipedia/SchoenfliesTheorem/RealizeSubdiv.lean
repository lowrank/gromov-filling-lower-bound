...
theorem IsArcBetween.closure_diff_eq {A : Set Plane} {p q : Plane} (h : IsArcBetween A p q) :
    closure (A \ {p, q}) = A := by
  refine subset_antisymm (h.isArc.isClosed.closure_subset_iff.2 diff_subset) fun z hz => ?_
  by_cases hzp : z = p
...
theorem diff_pair_union_union {A : Set Plane} {p q : Plane} (hp : p ∈ A) (hq : q ∈ A) :
    A \ {p, q} ∪ {q} ∪ {p} = A := by
  ext z
  simp only [mem_union, mem_diff, mem_insert_iff, mem_singleton_iff, not_or]
  constructor
  · rintro ((⟨h, -, -⟩ | rfl) | rfl)
    exacts [h, hq, hp]
  · intro h
...
`SchoenfliesGraph.IsDrawing` writes. -/
...
noncomputable def realizeGraph (d : S.SubdivData) (R : S.Realization) (t : ℝ) : SchoenfliesGraph Plane γ :=
...
theorem mem_edgeSet_skel_of_ne {f : γ} (hf : f ∈ E(d.realizeGraph R t))
  ...
  simp only [mem_insert_iff, mem_diff, mem_singleton_iff] at hf
...
theorem pointSet_realize :
  ...
  have hedge : (⋃ f ∈ E(R.graph), edgeArc R.drawing f) =
    ...
    rw [← biUnion_insert, insert_diff_singleton,
