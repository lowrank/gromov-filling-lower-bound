...
detour above it, and a connecting arc, and appeal to `SchoenfliesGraph.IsArcK33.elim`.
...
* `Schoenflies.exists_unique_unbounded_connectedComponentIn_compl` — the opening sentence of
  ...
  component.
* `SchoenfliesGraph.isArcK33_of_pieces` — the packaging of `cor:k33-subdivision` (through
  `SchoenfliesGraph.IsArcK33.elim`) that this configuration fits.
...
* `SchoenfliesGraph.isArcK33_of_pieces` belongs in `Schoenflies/SchoenfliesGraph/K33Planar.lean`, next to
  `SchoenfliesGraph.IsArcK33`.
-/
...
This is a packaging lemma: it repackages the `meet` clause of `SchoenfliesGraph.IsArcK33` as the five
...
theorem not_isPreconnected_compl_of_coord {C : Set Plane} (hC : IsJordanCurve C)
  ...
  refine (SchoenfliesGraph.isArcK33_of_pieces (x := ![p₁, p₂, f sp]) (y := ![a, b, f tp])
    (P := ![![A₁, B₁, E₁], ![A₂, B₂, E₂], ![D₁, D₂, L₆]])
...
theorem exists_unique_unbounded_connectedComponentIn_compl {C : Set Plane} (hC : IsCompact C) :
  ...
  refine ⟨base, hbeyond hbase, fun hb => SchoenfliesGraph.not_isBounded_beyondSquare r (hb.subset hsub), ?_⟩
  intro z hz hzu
  ...
  obtain ⟨w, hw1, hw2⟩ : ∃ w, w ∈ connectedComponentIn Cᶜ z ∧ w ∈ beyondSquare r := by
    by_contra hcon
    push Not at hcon
    refine hzu ((SchoenfliesGraph.isBounded_closedSquare r).subset fun w hw => ?_)
    by_contra hout
