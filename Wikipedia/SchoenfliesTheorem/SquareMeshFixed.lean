...
supplies the ear half (as a chain of `SchoenfliesGraph.IsTwoConnected.union`s) and **drops the
...
`SchoenfliesGraph.IsTwoConnected.replace_edge_by_path` — `lem:subdivision-ear-preserve` (a) — iterated,
...
* `meshGraph_outer_cycle` / `squareMesh_outer_cycle_of_subdividesToPath` — clause 3 **as a
  ...
  (`SchoenfliesGraph.IsPath.append_of_disjoint`) glue them into one path once round `S`, and the first step
  of the *reversed* fourth side is peeled off to close the cycle. The result is a
  `SchoenfliesGraph.IsLongCycle` of the mesh whose edges occupy exactly `modelCurve`.
...
segment being cut. `SchoenfliesGraph.IsTwoConnected.replace_edge_by_path` takes a path of any length, so
...
theorem coveredVertices_pair (a q b : Plane) :
    (pieceListGraph [((a, q) : Piece), (q, b)]).coveredVertices [((a, q) : Piece), (q, b)]
      = {a, q, b} := by
  ext x
  simp only [SchoenfliesGraph.mem_coveredVertices_iff, List.mem_cons, List.not_mem_nil, or_false,
...
theorem isPathGraph_pair {a q b : Plane} (haq : a ≠ q) (hqb : q ≠ b) (hab : a ≠ b) :
  ...
  have hcov : (pieceListGraph [((a, q) : Piece), (q, b)]).coveredVertices [((q, b) : Piece)]
      = {q, b} := by
    ext x
    simp only [SchoenfliesGraph.mem_coveredVertices_iff, List.mem_singleton, mem_insert_iff,
      mem_singleton_iff]
    constructor
    · rintro ⟨e, rfl, hinc⟩
      obtain ⟨-, hx⟩ := pieceListGraph_inc_iff.1 hinc
  ...
  have hpath : (pieceListGraph [((a, q) : Piece), (q, b)]).IsPath a [(a, q), (q, b)] b := by
    refine SchoenfliesGraph.IsPath.cons hlink₁ (SchoenfliesGraph.IsPath.single hlink₂ hqb) ?_
    rw [SchoenfliesGraph.walkVertices, hcov]
    simp only [mem_insert_iff, mem_singleton_iff]
    push Not
    exact ⟨haq, haq, hab⟩
  refine ⟨hpath, rfl, ?_⟩
  rw [pieceListGraph_vertexSet, endSet_pair, SchoenfliesGraph.walkVertices, coveredVertices_pair]
  simp
...
theorem pieceListGraph_splitAllAt_eq {l : List Piece} {P : Piece} {q : Plane} (hP : P ∈ l)
  ...
  refine (SchoenfliesGraph.eq_of_le_of_subset_subset (SchoenfliesGraph.union_le ?_ ?_) ?_ ?_).symm
  · -- the untouched edges
    refine ⟨?_, ?_⟩
    · intro v hv
      rw [SchoenfliesGraph.vertexSet_deleteEdges, pieceListGraph_vertexSet] at hv
  ...
  · intro v hv
    ...
    rw [SchoenfliesGraph.vertexSet_union, SchoenfliesGraph.vertexSet_deleteEdges, pieceListGraph_vertexSet,
  ...
  · intro R hR
    ...
    · exact Or.inl (SchoenfliesGraph.mem_edgeSet_deleteEdges_iff.2 ⟨hRl, by simpa using hRP⟩)
    · exact Or.inr hR2
...
theorem pieceListGraph_splitAllAt_isTwoConnected {l : List Piece} {P : Piece} {q : Plane}
  ...
  · -- no edge is shared: a half of `P` has `q` as an end, and `q` is an end of nothing in `l`
    ...
    rw [SchoenfliesGraph.mem_edgeSet_deleteEdges_iff, pieceListGraph_mem_edgeSet] at hR
...
`Schoenflies/SchoenfliesGraph/Walk.lean` has `SchoenfliesGraph.IsWalk.append` and no path analogue, because nothing
...
/-- Reading a path off its first step. `cases` on `SchoenfliesGraph.IsPath` needs the edge list to be a
...
missing, stated as a `Prop` so that the trust audit stays a real guarantee and the defect shows
...
theorem walkVertices_subset_of_edges {pieces : List Piece} {points : List Plane}
  ...
  rcases SchoenfliesGraph.mem_walkVertices_iff.1 hx with rfl | ⟨Q, hQ, hinc⟩
  · exact hu
  · obtain ⟨y, -, hends⟩ := hinc
...
theorem edgesCover_eq_seg {pieces : List Piece} {points : List Plane} {P : Piece}
    ...
    SchoenfliesGraph.edgesCover segmentDrawing W = P.seg := by
  ...
  · intro z hz
    ...
    refine SchoenfliesGraph.mem_edgesCover ((hW (orientPiece Q)).2
...
Peeling is why the fourth side is reversed: `SchoenfliesGraph.IsPath` is built at the *source* end, so its
...
theorem meshGraph_outer_cycle {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
    ...
    ∃ (e : Piece) (u v x : Plane) (D : List Piece),
      (meshGraph N fresh anchors).IsLongCycle e u v D x ∧
        ...
        SchoenfliesGraph.edgesCover segmentDrawing (e :: D) = modelCurve := by
  ...
  have hcovT : SchoenfliesGraph.edgesCover segmentDrawing WT = sideT.seg := edgesCover_eq_seg hmemT hcT
  have hcovL : SchoenfliesGraph.edgesCover segmentDrawing WL = sideL.seg := edgesCover_eq_seg hmemL hcL
  have hcovB : SchoenfliesGraph.edgesCover segmentDrawing WB = sideB.seg := edgesCover_eq_seg hmemB hcB
  have hcovR : SchoenfliesGraph.edgesCover segmentDrawing WR = sideR.seg := edgesCover_eq_seg hmemR hcR
...
by `lem:face-cycles`'s `SchoenfliesGraph.IsLongCycle.isTwoConnected`. What is still missing is the rest of
...
theorem squareMesh_outer_cycleGraph_isTwoConnected_of_subdividesToPath {fresh : List Plane}
  ...
  exact ⟨e, u, D, SchoenfliesGraph.IsLongCycle.isTwoConnected hlc, hcov⟩
