...
open Set
open scoped SchoenfliesGraph

namespace Schoenflies

open SchoenfliesGraph
...
variable {β : Type*} {G A B : _root_.SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
...
theorem IsDrawing.common_vertices_of_disjoint_subgraphs (h : G.IsDrawing drawing)
    ...
    {x : Plane} (hxA : x ∈ _root_.SchoenfliesGraph.pointSet A drawing)
    (hxB : x ∈ _root_.SchoenfliesGraph.pointSet B drawing) :
    x ∈ V(A) ∩ V(B) := by
...
noncomputable def joinedCrosscutOverlay
    ...
    (extra : List Plane) (joins : List Piece) : SchoenfliesGraph Plane Piece :=
...
theorem joinedCrosscutOverlay_pointSet (Q : SourceNonboundarySegmentCover P)
    ...
    SchoenfliesGraph.pointSet (Q.joinedCrosscutOverlay J p s epsilon extra joins) segmentDrawing =
      SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra) segmentDrawing ∪ cover joins := by
...
theorem joinedCrosscutOverlay_isDrawing
    ...
    SchoenfliesGraph.IsDrawing (Q.joinedCrosscutOverlay J p s epsilon extra joins) segmentDrawing :=
...
theorem joinedCrosscutOverlay_old_edge_subset
    ...
    ∀ {A : Piece}, A ∈ E(Q.crosscutOverlay J p s epsilon extra) → ∀ {R : Piece},
      ...
      (SchoenfliesGraph.edgeArc segmentDrawing R ∩
        (SchoenfliesGraph.edgeArc segmentDrawing A \
          V(Q.joinedCrosscutOverlay J p s epsilon extra joins))).Nonempty →
      SchoenfliesGraph.edgeArc segmentDrawing R ⊆ SchoenfliesGraph.edgeArc segmentDrawing A := by
...
theorem joinedCrosscutOverlay_join_subset
    ...
    ∀ {R : Piece}, R ∈ E(Q.joinedCrosscutOverlay J p s epsilon extra joins) →
      ...
      SchoenfliesGraph.edgeArc segmentDrawing R ⊆ cover joins := by
  intro R hR hmeet
  ...
  obtain ⟨R', hR', hzR', hR'A⟩ :=
    exists_overlayPiece_mem_subset
      ...
      hzA
  have hzR'Arc : z ∈ SchoenfliesGraph.edgeArc segmentDrawing R' := by
...
abbrev outerGraph (_u : JoinedCrosscutOverlayRelabeling w joins) :
    _root_.SchoenfliesGraph Plane γ := P.str.outerGraph.map P.src.pos

/-- The freshly relabelled joined inner overlay. -/
noncomputable abbrev innerGraph : _root_.SchoenfliesGraph Plane γ :=
...
noncomputable def graph : _root_.SchoenfliesGraph Plane γ := u.outerGraph.union u.innerGraph
...
theorem compatible : u.outerGraph.Compatible u.innerGraph := by
  apply _root_.SchoenfliesGraph.Compatible.of_disjoint_edgeSet
  rw [Set.disjoint_left, _root_.SchoenfliesGraph.edgeSet_map, _root_.SchoenfliesGraph.edgeSet_relabelEdges]
...
theorem outer_isDrawing : u.outerGraph.IsDrawing u.drawing := by
  apply Schoenflies.SchoenfliesGraph.isDrawing_congr_of_eqOn
    (P.src.isDrawing.mono (P.str.outerGraph_le.map P.src.pos))
  intro e he
  apply u.drawing_of_outer
  rwa [_root_.SchoenfliesGraph.edgeSet_map] at he
...
theorem outer_pointSet :
    _root_.SchoenfliesGraph.pointSet u.outerGraph u.drawing = srcOuter := by
  calc
    _root_.SchoenfliesGraph.pointSet u.outerGraph u.drawing =
        _root_.SchoenfliesGraph.pointSet u.outerGraph P.src.drawing := by
      apply _root_.SchoenfliesGraph.pointSet_congr
      intro e he
      simpa only [_root_.SchoenfliesGraph.edgeArc] using congrArg
        (fun f : ℝ → Plane => f '' unitInterval)
        (u.drawing_of_outer (by rwa [_root_.SchoenfliesGraph.edgeSet_map] at he))
    _ = P.src.outerSet := rfl
    _ = srcOuter := P.src_isWeaklyAdmissible.outerSet_eq

theorem inner_pointSet :
    _root_.SchoenfliesGraph.pointSet u.innerGraph u.drawing =
      _root_.SchoenfliesGraph.pointSet (Q.joinedCrosscutOverlay J p s epsilon extra joins)
        segmentDrawing := by
  calc
    _root_.SchoenfliesGraph.pointSet u.innerGraph u.drawing =
        _root_.SchoenfliesGraph.pointSet u.innerGraph
          ((Q.joinedCrosscutOverlay J p s epsilon extra joins).relabelDrawing
    ...
    _ = _root_.SchoenfliesGraph.pointSet (Q.joinedCrosscutOverlay J p s epsilon extra joins)
        segmentDrawing := _root_.SchoenfliesGraph.pointSet_relabelEdges u.name_inj
...
theorem graph_isDrawing (hs : 0 < s) (hJ : J.Nondeg)
  ...
  apply Schoenflies.SchoenfliesGraph.isDrawing_union_of_common_vertices
  ...
  · have hxOldOuter : x ∈ _root_.SchoenfliesGraph.pointSet w.outerGraph w.drawing := by
      rwa [w.outer_pointSet]
    have hxOldInner' : x ∈ _root_.SchoenfliesGraph.pointSet w.innerGraph w.drawing := by
      rwa [w.inner_pointSet]
    have hOldDisjoint : Disjoint E(w.outerGraph) E(w.innerGraph) := by
      rw [Set.disjoint_left, _root_.SchoenfliesGraph.edgeSet_map,
        _root_.SchoenfliesGraph.edgeSet_relabelEdges]
      intro e heOuter heInner
      obtain ⟨R, hR, hname⟩ := heInner
      rw [← hname] at heOuter
      exact w.name_fresh R hR
        (P.str.mem_cells_of_mem_edgeSet (P.str.outerGraph_le.edgeSet_mono heOuter))
    have hxOldVertices :=
      Schoenflies.SchoenfliesGraph.IsDrawing.common_vertices_of_disjoint_subgraphs
        (w.graph_isDrawing hs hJ hJgeom hwindow)
        (_root_.SchoenfliesGraph.left_le_union _ _) w.compatible.right_le_union hOldDisjoint
        hxOldOuter hxOldInner'
    refine ⟨hxOldVertices.1, ?_⟩
    rw [_root_.SchoenfliesGraph.vertexSet_relabelEdges] at hxOldVertices ⊢
...
theorem graph_finite : u.graph.Finite where
  finite_vertexSet := by
    rw [graph, _root_.SchoenfliesGraph.vertexSet_union, outerGraph, _root_.SchoenfliesGraph.vertexSet_map,
      _root_.SchoenfliesGraph.vertexSet_relabelEdges]
    exact ((P.str.finite_vertexSet.subset P.str.outerGraph_le.vertexSet_mono).image
      P.src.pos).union
      (_root_.SchoenfliesGraph.finite_vertexSet
        (Q.joinedCrosscutOverlay J p s epsilon extra joins))
  finite_edgeSet := by
    rw [graph, _root_.SchoenfliesGraph.edgeSet_union, outerGraph, _root_.SchoenfliesGraph.edgeSet_map,
      _root_.SchoenfliesGraph.edgeSet_relabelEdges]
    exact (P.str.finite_edgeSet.subset P.str.outerGraph_le.edgeSet_mono).union
      ((_root_.SchoenfliesGraph.finite_edgeSet
...
theorem graph_pointSet :
    _root_.SchoenfliesGraph.pointSet u.graph u.drawing =
      _root_.SchoenfliesGraph.pointSet w.graph w.drawing ∪ cover joins := by
  rw [graph, _root_.SchoenfliesGraph.pointSet_union, u.outer_pointSet, u.inner_pointSet,
    Q.joinedCrosscutOverlay_pointSet]
  calc
    srcOuter ∪
        (_root_.SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra)
          segmentDrawing ∪ cover joins) =
        (srcOuter ∪
          _root_.SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra) segmentDrawing) ∪
            cover joins := by ac_rfl
    _ = _root_.SchoenfliesGraph.pointSet w.graph w.drawing ∪ cover joins := by
      rw [CrosscutOverlayRelabeling.graph, _root_.SchoenfliesGraph.pointSet_union,
...
theorem oldVertices_subset_graph (hs : 0 < s) (hJ : J.Nondeg)
  ...
  intro x hx
  rw [CrosscutOverlayRelabeling.graph, _root_.SchoenfliesGraph.vertexSet_union] at hx
  rw [graph, _root_.SchoenfliesGraph.vertexSet_union]
  rcases hx with hxOuter | hxInner
  · exact Or.inl hxOuter
  · exact Or.inr (by
      rw [_root_.SchoenfliesGraph.vertexSet_relabelEdges] at hxInner ⊢
      exact Q.crosscutOverlayVertices_subset_joinedCrosscutOverlay
...
theorem old_edge_subset (hs : 0 < s) (hJ : J.Nondeg)
    ...
    ∀ {e : γ}, e ∈ E(w.graph) → ∀ {f : γ}, f ∈ E(u.graph) →
      (_root_.SchoenfliesGraph.edgeArc u.drawing f ∩
        (_root_.SchoenfliesGraph.edgeArc w.drawing e \ V(u.graph))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc u.drawing f ⊆ _root_.SchoenfliesGraph.edgeArc w.drawing e := by
  intro e he f hf hmeet
  obtain ⟨z, hzf, hze, hznot⟩ := hmeet
  rcases he with heOuter | heInner
  · have heAbstract : e ∈ E(P.str.outerGraph) := by
      rwa [_root_.SchoenfliesGraph.edgeSet_map] at heOuter
    have heNew : e ∈ E(u.graph) := Or.inl heOuter
    have hzeNew : z ∈ _root_.SchoenfliesGraph.edgeArc u.drawing e := by
      have hu := u.drawing_of_outer heAbstract
      have hw := w.drawing_of_outer heAbstract
      rw [_root_.SchoenfliesGraph.edgeArc, hu]
      rw [_root_.SchoenfliesGraph.edgeArc, hw] at hze
      exact hze
    have hef : e = f :=
    ...
    have hw := w.drawing_of_outer heAbstract
    intro y hy
    rw [_root_.SchoenfliesGraph.edgeArc, hu] at hy
    rw [_root_.SchoenfliesGraph.edgeArc, hw]
    exact hy
  · obtain ⟨A, hA, rfl⟩ := heInner
    have hAOld : w.name A ∈ E(w.innerGraph) := ⟨A, hA, rfl⟩
    have hOldArc : _root_.SchoenfliesGraph.edgeArc w.drawing (w.name A) =
        _root_.SchoenfliesGraph.edgeArc segmentDrawing A := by
      calc
        _root_.SchoenfliesGraph.edgeArc w.drawing (w.name A) =
            _root_.SchoenfliesGraph.edgeArc
        ...
        _ = _root_.SchoenfliesGraph.edgeArc segmentDrawing A :=
          _root_.SchoenfliesGraph.edgeArc_relabelDrawing w.name_inj hA
    rcases hf with hfOuter | hfInner
    · exfalso
      have hzOldOuter : z ∈ _root_.SchoenfliesGraph.pointSet w.outerGraph w.drawing := by
        rw [w.outer_pointSet, ← u.outer_pointSet]
        exact _root_.SchoenfliesGraph.edgeArc_subset_pointSet hfOuter hzf
      have hzOldInner : z ∈ _root_.SchoenfliesGraph.pointSet w.innerGraph w.drawing :=
        _root_.SchoenfliesGraph.edgeArc_subset_pointSet hAOld (hOldArc ▸ hze)
      have hOldDisjoint : Disjoint E(w.outerGraph) E(w.innerGraph) := by
        rw [Set.disjoint_left, _root_.SchoenfliesGraph.edgeSet_map,
          _root_.SchoenfliesGraph.edgeSet_relabelEdges]
      ...
      rw [graph, _root_.SchoenfliesGraph.vertexSet_union]
      exact Or.inr (by
        rw [_root_.SchoenfliesGraph.vertexSet_relabelEdges] at hzOldVertex ⊢
        exact Q.crosscutOverlayVertices_subset_joinedCrosscutOverlay
          hs hJ hjoins hzOldVertex)
    · obtain ⟨R, hR, rfl⟩ := hfInner
      have hRNew : u.name R ∈ E(u.innerGraph) := ⟨R, hR, rfl⟩
      have hNewArc : _root_.SchoenfliesGraph.edgeArc u.drawing (u.name R) =
          _root_.SchoenfliesGraph.edgeArc segmentDrawing R := by
        calc
          _root_.SchoenfliesGraph.edgeArc u.drawing (u.name R) =
              _root_.SchoenfliesGraph.edgeArc
                ((Q.joinedCrosscutOverlay J p s epsilon extra joins).relabelDrawing
                  u.name segmentDrawing) (u.name R) := by
            simpa only [_root_.SchoenfliesGraph.edgeArc] using congrArg
              (fun g : ℝ → Plane => g '' unitInterval) (u.drawing_of_inner hRNew)
          _ = _root_.SchoenfliesGraph.edgeArc segmentDrawing R :=
            _root_.SchoenfliesGraph.edgeArc_relabelDrawing u.name_inj hR
      rw [hNewArc, hOldArc]
      ...
      apply hznot
      rw [graph, _root_.SchoenfliesGraph.vertexSet_union]
      exact Or.inr (by rwa [_root_.SchoenfliesGraph.vertexSet_relabelEdges])
...
theorem oldTrace_isTwoConnected (hs : 0 < s) (hJ : J.Nondeg)
    ...
    (_root_.SchoenfliesGraph.traceGraph u.graph u.drawing
      (_root_.SchoenfliesGraph.pointSet w.graph w.drawing)).IsTwoConnected :=
...
theorem joinEnds_subset_graph (hs : 0 < s) (hJ : J.Nondeg)
  ...
  rw [graph, _root_.SchoenfliesGraph.vertexSet_union]
  exact Or.inr (by
    rw [_root_.SchoenfliesGraph.vertexSet_relabelEdges]
...
theorem joins_subset_graph :
    cover joins ⊆ _root_.SchoenfliesGraph.pointSet u.graph u.drawing := by
  rw [u.graph_pointSet]
...
theorem graph_join_subset (hs : 0 < s) (hJ : J.Nondeg)
    ...
    (hjoinsOpen : cover joins ⊆ srcDom \ srcOuter) :
    ∀ {f : γ}, f ∈ E(u.graph) →
      (_root_.SchoenfliesGraph.edgeArc u.drawing f ∩ (cover joins \ V(u.graph))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc u.drawing f ⊆ cover joins := by
  intro f hf hmeet
  ...
  · obtain ⟨R, hR, rfl⟩ := hfInner
    ...
    have hArc : _root_.SchoenfliesGraph.edgeArc u.drawing (u.name R) =
        _root_.SchoenfliesGraph.edgeArc segmentDrawing R := by
      calc
        _root_.SchoenfliesGraph.edgeArc u.drawing (u.name R) =
            _root_.SchoenfliesGraph.edgeArc
...
theorem joinTrace_pointSet (hs : 0 < s) (hJ : J.Nondeg)
    ...
    (hjoins : ∀ R ∈ joins, R.Nondeg)
    (hjoinsOpen : cover joins ⊆ srcDom \ srcOuter) :
    _root_.SchoenfliesGraph.pointSet
      (_root_.SchoenfliesGraph.traceGraph u.graph u.drawing (cover joins)) u.drawing = cover joins :=
  _root_.SchoenfliesGraph.pointSet_traceGraph_eq
...
theorem exists_join_trace (hs : 0 < s) (hJ : J.Nondeg)
    ...
    ∃ D : List γ, u.graph.IsPath x D y ∧
      _root_.SchoenfliesGraph.edgesCover u.drawing D = cover joins := by
  let T := _root_.SchoenfliesGraph.traceGraph u.graph u.drawing (cover joins)
  have hTle : T ≤ u.graph := _root_.SchoenfliesGraph.traceGraph_le _
  letI : u.graph.Finite := u.graph_finite
  letI : T.Finite := _root_.SchoenfliesGraph.Finite.of_le hTle
  have hpoint : _root_.SchoenfliesGraph.pointSet T u.drawing = cover joins :=
  ...
  have hxT : x ∈ V(T) := by
    rw [_root_.SchoenfliesGraph.traceGraph_vertexSet]
    exact ⟨hxGraph, hArc.left_mem⟩
  have hyT : y ∈ V(T) := by
    rw [_root_.SchoenfliesGraph.traceGraph_vertexSet]
    exact ⟨hyGraph, hArc.right_mem⟩
  have hTconn : T.Connected := _root_.SchoenfliesGraph.connected_of_isPreconnected_pointSet
  ...
  have hPathArc : IsArcBetween (_root_.SchoenfliesGraph.edgesCover u.drawing D) x y :=
    (u.graph_isDrawing hs hJ hJgeom hwindow hjoins hjoinsOpen).path_isArcBetween
      hDGraph (hDGraph.ne_nil hxy)
  have hcoverSub : _root_.SchoenfliesGraph.edgesCover u.drawing D ⊆ cover joins := by
    rw [← hpoint]
    exact _root_.SchoenfliesGraph.edgesCover_subset_pointSet fun g hg => hD.edge_mem hg
  exact ⟨D, hDGraph,
...
theorem graph_isTwoConnected_of_join (hs : 0 < s) (hJ : J.Nondeg)
    ...
    {x y : Plane} (hxy : x ≠ y) (hxEnd : x ∈ endSet joins)
    (hyEnd : y ∈ endSet joins) (hxOld : x ∈ _root_.SchoenfliesGraph.pointSet w.graph w.drawing)
    (hyOld : y ∈ _root_.SchoenfliesGraph.pointSet w.graph w.drawing)
  ...
  let T := _root_.SchoenfliesGraph.traceGraph u.graph u.drawing
    (_root_.SchoenfliesGraph.pointSet w.graph w.drawing)
  ...
  have hTC : T.Compatible C :=
    _root_.SchoenfliesGraph.Compatible.of_le_le (_root_.SchoenfliesGraph.traceGraph_le _)
      (_root_.SchoenfliesGraph.pathGraphOf_le hD.isWalk)
  ...
  apply hUnion2.of_le_of_vertexSet_subset
    (_root_.SchoenfliesGraph.union_le (_root_.SchoenfliesGraph.traceGraph_le _)
      (_root_.SchoenfliesGraph.pathGraphOf_le hD.isWalk))
  intro z hz
  rw [_root_.SchoenfliesGraph.vertexSet_union]
  have hzPoint : z ∈ _root_.SchoenfliesGraph.pointSet u.graph u.drawing :=
    _root_.SchoenfliesGraph.vertexSet_subset_pointSet hz
  ...
  · exact Or.inl (by
      ...
      exact ⟨hz, hzOld⟩)
  · exact Or.inr (by
      rw [_root_.SchoenfliesGraph.pathGraphOf_vertexSet]
...
theorem graph_pointSet_subset (hs : 0 < s)
    ...
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter)
    (hjoinsOpen : cover joins ⊆ srcDom \ srcOuter) :
    _root_.SchoenfliesGraph.pointSet u.graph u.drawing ⊆ srcDom := by
  rw [u.graph_pointSet]
  exact Set.union_subset (w.graph_pointSet_subset hs hJgeom hwindow)
    (hjoinsOpen.trans diff_subset)
...
theorem graph_edge_dichotomy (hs : 0 < s) (hJ : J.Nondeg)
    ...
    (hjoins : ∀ R ∈ joins, R.Nondeg)
    (hjoinsOpen : cover joins ⊆ srcDom \ srcOuter) :
    ∀ {e : γ}, e ∈ E(u.graph) → _root_.SchoenfliesGraph.edgeArc u.drawing e ⊆ srcOuter ∨
      (IsPolygonal (_root_.SchoenfliesGraph.edgeArc u.drawing e) ∧
        _root_.SchoenfliesGraph.edgeArc u.drawing e \ V(u.graph) ⊆ srcDom \ srcOuter) := by
  intro e he
  rcases he with heOuter | heInner
  · exact Or.inl (by
      intro x hx
      rw [← u.outer_pointSet]
      exact _root_.SchoenfliesGraph.edgeArc_subset_pointSet heOuter hx)
...
theorem graph_edge_subset (hs : 0 < s) (hJ : J.Nondeg)
    ...
    ∀ {e : γ}, e ∈ E(P.str.skel) → ∀ {f : γ}, f ∈ E(u.graph) →
      (_root_.SchoenfliesGraph.edgeArc u.drawing f ∩ (P.src.cell e \ V(u.graph))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc u.drawing f ⊆ _root_.SchoenfliesGraph.edgeArc P.src.drawing e := by
  intro e he f hf hmeet
...
theorem graph_isConnected_diff_of_join (hs : 0 < s)
    ...
    (hArc : IsArcBetween (cover joins) x y) :
    IsConnected (_root_.SchoenfliesGraph.pointSet u.graph u.drawing \ srcOuter) := by
  let Sset := P.src.skeletonSet \ srcOuter
  let Jset := J.seg \ srcOuter
  let Kset := _root_.SchoenfliesGraph.pointSet
  ...
  have hKconn : IsConnected Kset :=
    Schoenflies.SchoenfliesGraph.IsDrawing.isConnected_pointSet
  ...
  have hcarrier : _root_.SchoenfliesGraph.pointSet u.graph u.drawing \ srcOuter =
      ((Sset ∪ Jset) ∪ Kset) ∪ cover joins := by
    ext z
    rw [Set.mem_diff, Set.mem_union, Set.mem_union, Set.mem_union,
      Set.mem_diff, Set.mem_diff]
    constructor
  ...
  have hA1K : A.1 ∈ Kset := by
    apply _root_.SchoenfliesGraph.edgeArc_subset_pointSet hA
    rw [edgeArc_segmentDrawing]
    exact left_mem_segment ℝ _ _
  have hA1J : A.1 ∈ Jset :=
...
theorem isSourceExtension (hs : 0 < s) (hJ : J.Nondeg)
    ...
    (hconnected : IsConnected
      (_root_.SchoenfliesGraph.pointSet u.graph u.drawing \ srcOuter)) :
...
structure JoinedLocalGridSourceExtensionData
  ...
  localGrid_subset :
    _root_.SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing ⊆
      _root_.SchoenfliesGraph.pointSet joinedRelabeling.graph joinedRelabeling.drawing
...
theorem RefinedCrosscutOverlayData.exists_joinedLocalGridSourceExtensionData
  ...
  have hxOld : x ∈ _root_.SchoenfliesGraph.pointSet o.relabeling.graph o.relabeling.drawing := by
    rw [o.relabeling.graph_pointSet]
    exact Or.inr (Or.inl (Or.inr
      (openSegment_subset_segment ℝ _ _ hxJ)))
  have hyOld : y ∈ _root_.SchoenfliesGraph.pointSet o.relabeling.graph o.relabeling.drawing :=
...
theorem GeneratedPair.exists_localGridEdge_interior_disjoint_of_common_subsingleton
    ...
    (hcommon :
      (P.src.skeletonSet ∩
        _root_.SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon))
  ...
  · have hdisj₁ : Disjoint A₁.interior P.src.skeletonSet := by
      ...
      have hz₀Grid : z₀ ∈ _root_.SchoenfliesGraph.pointSet (localGrid p s k) segmentDrawing := by
...
theorem GeneratedPair.exists_localGridSourceExtensionData_of_common_not_subsingleton
  ...
  have commonVertex : ∀ {x : Plane}, x ∈ [a, b] →
      x ∈ P.src.skeletonSet →
      x ∈ _root_.SchoenfliesGraph.pointSet
        (localGrid p s (localGridCount s epsilon)) segmentDrawing →
      x ∈ V(w.graph) := by
    ...
    have hxCore : x ∈
        _root_.SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing := by
