...
`SchoenfliesGraph.exists_access_segment` is the theorem, and it exports the *construction*: a point `y` of
the 2-cell `F`, as close to `x` as prescribed, whose straight segment from `x` lies in `F` apart
from `x` itself. `SchoenfliesGraph.polygonal_side_accessibility` packages it as
...
because the sector meets `F` at `y` (`SchoenfliesGraph.localSector_subset_cell` — this is the clause the
audit found missing from `SchoenfliesGraph.exists_access_sector`); the straight segment from `x` to `y`
lies in that sector
...
`SchoenfliesGraph.polygonal_side_accessibility_target` and `SchoenfliesGraph.polygonal_side_accessibility_square`.
...
* `Schoenflies.localSectors` and its API, `SchoenfliesGraph.IsDrawing.localSectors_finite`,
  `SchoenfliesGraph.IsDrawing.isOpen_of_mem_localSectors` — the same clause for a finite
  plane graph with polygonal edges.
* `Schoenflies.localDirs_union_of_disjoint`, `Schoenflies.IsLocalRadius.union_of_disjoint`,
  `SchoenfliesGraph.IsDrawing.exists_isLocalRadius_union` — "shrink the disk at `x` until it also misses
...
* `SchoenfliesGraph.localSector_subset_face`, `SchoenfliesGraph.localSector_subset_cell`,
...
* `SchoenfliesGraph.exists_access_segment`, `SchoenfliesGraph.polygonal_side_accessibility` —
  `lem:polygonal-side-accessibility`, source side.
* `SchoenfliesGraph.polygonal_side_accessibility_target`, `SchoenfliesGraph.polygonal_side_accessibility_square`,
...
theorem IsLocalRadius.isOpen_ball_diff (h : IsLocalRadius S x r)
  ...
  have heq : ball x r \ S = ball x r ∩ (closedBall x r ∩ S)ᶜ := by
    ext z
    simp only [mem_diff, mem_inter_iff, mem_compl_iff, mem_ball, mem_closedBall, not_and]
...
theorem frontier_openSquare_subset (c : Plane) (s : ℝ) :
  ...
  exact Set.diff_subset_diff_left
...
clause `SchoenfliesGraph.exists_access_sector` proves under the two-branch hypothesis, here with none — and
...
`SchoenfliesGraph.exists_access_sector` was credited with and does not prove: the sector containing a point
...
`SchoenfliesGraph.polygonal_side_accessibility`.
...
/-- `SchoenfliesGraph.polygonal_side_accessibility_target` with the ambient region the open square. The
