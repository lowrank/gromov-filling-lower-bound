...
theorem exists_finiteGraph_edgeRelabeling_avoiding
    {β : Type*} (γ : Type*) [Infinite γ] (H : SchoenfliesGraph Plane β) [H.Finite]
...
theorem squareMesh_relabelEdges_finite {γ : Type*}
  ...
  SchoenfliesGraph.Finite.relabelEdges hname
...
theorem squareMesh_relabelEdges_isDrawing {γ : Type*}
    ...
    (hname : InjOn name E(squareMesh delta fresh anchors)) :
    SchoenfliesGraph.IsDrawing ((squareMesh delta fresh anchors).relabelEdges name hname)
...
theorem squareMesh_pointSet_relabelEdges {γ : Type*}
    (delta : ℝ) (fresh anchors : List Plane) (name : Piece → γ)
    (hname : InjOn name E(squareMesh delta fresh anchors)) :
    SchoenfliesGraph.pointSet ((squareMesh delta fresh anchors).relabelEdges name hname)
        ((squareMesh delta fresh anchors).relabelDrawing name segmentDrawing) =
      SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing :=
  SchoenfliesGraph.pointSet_relabelEdges hname
...
theorem squareMesh_edge_dichotomy
    ...
    (hpointSet : SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing ⊆ dom) :
  ...
  · refine Or.inr ⟨?_, ?_⟩
    ...
    · intro x hx
      refine ⟨hpointSet (SchoenfliesGraph.edgeArc_subset_pointSet hf hx.1), ?_⟩
      intro hxOuter
...
theorem isSourceExtension_relabelledSquareMesh
    ...
    (hpointSet : SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing ⊆ tgtDom)
  ...
  edge_subset := by
    ...
    obtain ⟨f, hf, rfl⟩ := hd
    rw [SchoenfliesGraph.edgeArc_relabelDrawing hname hf, SchoenfliesGraph.vertexSet_relabelEdges] at hmeet
    rw [SchoenfliesGraph.edgeArc_relabelDrawing hname hf]
  ...
  edge_dichotomy := by
    intro d hd
    obtain ⟨f, hf, rfl⟩ := hd
    rw [SchoenfliesGraph.edgeArc_relabelDrawing hname hf, SchoenfliesGraph.vertexSet_relabelEdges]
    exact hdichotomy hf
  isConnected := by
    rw [squareMesh_pointSet_relabelEdges]
    exact hconnected
...
theorem targetEarEndpointsOuterOnly_squareMesh
    ...
    {B : SchoenfliesGraph Plane Piece} {a b : Plane} {D : List Piece} {par : Piece → Piece}
