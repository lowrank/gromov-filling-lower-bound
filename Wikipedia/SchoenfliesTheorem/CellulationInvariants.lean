...
primitive assumption whatever: nothing says `S.boundary F` is a closed walk, nothing ties it to
`S.sub`, and
...
theorem closure_cell_eq_union (h : R.IsCellDecomposition D) (hτ : τ ∈ S.cells) :
  ...
  rw [h.closure_cell_eq hτ, faceBoundary, ← R.cellUnion_insert,
    Set.insert_diff_singleton, Set.insert_eq_self.2 (h.mem_subcells_self hτ)]
...
theorem faceBoundary_eq_frontier (h : R.IsCellDecomposition D) (hτ : τ ∈ S.cells)
  ...
  rw [hopen.frontier_eq, h.closure_cell_eq_union hτ, Set.union_diff_left,
  ...
  closure_eq := by
    ...
    · rcases hτn with hτn | hτn
      · rcases hτn with ⟨hzV, hznot⟩ | hfE
        ...
        · -- an edge of the ear
          obtain ⟨a, b, hl⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet hfE
