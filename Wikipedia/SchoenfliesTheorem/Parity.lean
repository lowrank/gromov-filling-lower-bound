...
theorem meet_swap (h : hgt u a ≠ hgt u b) (t : ℝ) : meet u b a t = meet u a b t := by
  ...
  have h2 : (t - hgt u b) / (hgt u a - hgt u b)
      = 1 - (t - hgt u a) / (hgt u b - hgt u a) := by
    rw [show hgt u a - hgt u b = -(hgt u b - hgt u a) by ring, div_neg]
    apply (eq_sub_iff_add_eq).2
    rw [add_comm, ← sub_eq_add_neg, ← sub_div]
    exact (div_eq_one_iff_eq hne).2 (by ring)
