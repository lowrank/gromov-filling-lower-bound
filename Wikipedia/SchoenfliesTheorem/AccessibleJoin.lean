...
* `SchoenfliesGraph.pointSet_eq_biUnion`, `SchoenfliesGraph.exists_simple_poly_of_pointSet` — the same extraction on
  the point set of a finite plane graph with polygonal edges, which is the shape a skeleton
  stage arrives in. Root `SchoenfliesGraph` namespace, per the convention of `Schoenflies/SchoenfliesGraph/*`.
...
Following the convention of `Schoenflies/SchoenfliesGraph/*`, these live in the root `SchoenfliesGraph` namespace. -/

namespace SchoenfliesGraph

open Schoenflies Set

variable {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {a b : Plane}
...
theorem pointSet_eq_biUnion (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) :
    pointSet G drawing =
