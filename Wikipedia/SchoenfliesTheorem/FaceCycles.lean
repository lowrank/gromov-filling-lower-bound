...
the graph from a cycle by ears (`SchoenfliesGraph.IsTwoConnected.ear_decomposition`) and applying
...
`SchoenfliesGraph.IsTwoConnected.exists_adj_ne` is the two-neighbours sentence; "take a maximal simple
path" is `SchoenfliesGraph.exists_longest_path`, already in `Schoenflies/SchoenfliesGraph/Tree.lean` and not rebuilt
here; and `SchoenfliesGraph.IsTwoConnected.exists_long_cycle` is the conclusion, packaged as
`SchoenfliesGraph.IsLongCycle`, a cycle together with a *named* third vertex. Naming the third vertex
rather than counting is what both consumers want: the counting clause of
`SchoenfliesGraph.IsTwoConnected`, and — later — the non-degeneracy of the polygon the cycle draws.

`SchoenfliesGraph.cycleGraph` is that cycle as a subgraph, and `SchoenfliesGraph.IsLongCycle.isTwoConnected` is
*"the cycle `C` is 2-connected, so Lemma `lem:relative-ear` builds the graph from `C`"*. This
is the one place "length at least three" is used: a two-vertex cycle is a `SchoenfliesGraph.banana`, and
`SchoenfliesGraph.not_isTwoConnected_banana` says it is not 2-connected.

## One step of the induction

Adding an ear removes the ear's own point set from the exterior and changes nothing else:
`SchoenfliesGraph.pointSet_union`, `SchoenfliesGraph.exterior_union` and `SchoenfliesGraph.face_union_eq_of_disjoint` are
*"all other faces are unchanged"*.

`SchoenfliesGraph.IsDrawing.exists_face_of_ear` is *"its interior is connected and disjoint from the
...
subgraph altogether (`SchoenfliesGraph.IsDrawing.edgesCover_inter_pointSet`).
`SchoenfliesGraph.IsDrawing.ends_mem_frontier_face` is the next sentence, *"the ear's two endpoints are
limits of its interior, which lies in `F`, so they lie on `∂F`"* — which is what makes the ear
a crosscut of that face.

`SchoenfliesGraph.IsDrawing.mono` — a subgraph of a plane graph is a plane graph, with the same drawing —
is what lets the induction talk about the faces of the current subgraph at all.

## An interface repair

`SchoenfliesGraph.IsTwoConnected.ear_decomposition` hands its step the ear's path but **not** the clause
`∀ g ∈ D, g ∉ E(B)`, which `SchoenfliesGraph.IsTwoConnected.relative_ear_exists` proves and which
`SchoenfliesGraph.IsTwoConnected.relative_grows_by_ear` drops on the way. A geometric consumer needs it:
without it the ear may be drawn right on top of the current subgraph, and
`SchoenfliesGraph.IsDrawing.edgesCover_inter_pointSet` is false. `SchoenfliesGraph.ear_edges_notMem_or_union_eq`
...
of a graph cycle arrives here as a **set** — `SchoenfliesGraph.edgesCover drawing (e :: D)` — which
`SchoenfliesGraph.IsDrawing.cycle_isJordanCurve` knows to be a Jordan curve and `SchoenfliesGraph.polygonal_redrawing`
...
Root `SchoenfliesGraph`, as fixed by `Schoenflies/SchoenfliesGraph/Walk.lean`. The two arc lemmas are stated in
`Schoenflies`, where the rest of the arc theory lives.

## Blueprint

* `SchoenfliesGraph.IsTwoConnected.exists_adj_ne` — "every vertex has at least two distinct neighbors".
* `SchoenfliesGraph.IsLongCycle`, `SchoenfliesGraph.IsTwoConnected.exists_long_cycle` — "the resulting closed subpath
  is a cycle of length at least three".
* `SchoenfliesGraph.cycleGraph`, `SchoenfliesGraph.IsLongCycle.isTwoConnected` — "the cycle `C` is 2-connected, so
  Lemma `lem:relative-ear` builds the graph from `C`".
* `SchoenfliesGraph.ear_edges_notMem_or_union_eq` — the ear's edges are new, or the ear is not an ear.
* `SchoenfliesGraph.pointSet_union`, `SchoenfliesGraph.exterior_union`, `SchoenfliesGraph.face_union_subset`,
  `SchoenfliesGraph.face_union_eq_of_disjoint` — "all other faces are unchanged".
* `SchoenfliesGraph.IsDrawing.mono` — the current subgraph is itself a plane graph.
* `Schoenflies.IsArcBetween.isConnected_diff`,
  `Schoenflies.IsArcBetween.left_mem_closure_diff`, `…right_mem_closure_diff`,
  `SchoenfliesGraph.IsDrawing.edgesCover_inter_pointSet`, `SchoenfliesGraph.IsDrawing.exists_face_of_ear`,
  `SchoenfliesGraph.IsDrawing.ends_mem_frontier_face` — "its interior is connected and disjoint from the
...
variable {α β : Type*} {G H K B : SchoenfliesGraph α β} {u v w x y z c : α} {e f g : β} {D W : List β}
...
"Take a maximal simple path" is `SchoenfliesGraph.exists_longest_path`, already in
`Schoenflies/SchoenfliesGraph/Tree.lean` — it is what the three-leaf lemma runs on — so nothing here
rebuilds it. -/
...
structure IsLongCycle (G : SchoenfliesGraph α β) (e : β) (u v : α) (D : List β) (w : α) : Prop where
...
the path first steps to (`SchoenfliesGraph.IsTwoConnected.exists_adj_ne`), and `x` lies on the path:
...
`SchoenfliesGraph.IsTwoConnected.ear_decomposition` starts from a 2-connected subgraph. The subgraph the
...
`SchoenfliesGraph.banana`, and `SchoenfliesGraph.not_isTwoConnected_banana` says it is not 2-connected. -/

/-- **A path graph grows with its edge list.** General, and not in
`Schoenflies/SchoenfliesGraph/PathGraph.lean`; the integrator may want to hoist it. -/
...
def cycleGraph (G : SchoenfliesGraph α β) (u : α) (e : β) (D : List β) : SchoenfliesGraph α β :=
  G.pathGraphOf u (D ++ [e])
...
`SchoenfliesGraph.IsTwoConnected.ear_decomposition` asks of its starting subgraph.
...
(`SchoenfliesGraph.IsPathGraph.reaches_an_end`), an end which — having been reached after the deletion —
...
for `SchoenfliesGraph.IsTwoConnected.ear_decomposition`, so a consumer that can do the base case on
`G.cycleGraph u e D` and the ear step gets the conclusion for `G`. Nothing but the two
theorems is used. -/
example {motive : SchoenfliesGraph α β → Prop} [G.Finite] (hG : G.IsTwoConnected)
    (hnl : ∀ ⦃g x⦄, ¬ G.IsLoopAt g x)
    (base : ∀ (e : β) (u v : α) (D : List β) (w : α), G.IsLongCycle e u v D w →
      motive (G.cycleGraph u e D))
    (step : ∀ (B : SchoenfliesGraph α β) (a b : α) (D : List β), B.IsTwoConnected → B ≤ G → motive B →
...
`SchoenfliesGraph.IsTwoConnected.ear_decomposition` hands its step the ear's path but **not** the clause
`∀ g ∈ D, g ∉ E(B)` that `SchoenfliesGraph.IsTwoConnected.relative_ear_exists` proves and that
`SchoenfliesGraph.IsTwoConnected.relative_grows_by_ear` drops. A *geometric* consumer needs it: the ear's
...
decomposes the path — `SchoenfliesGraph.IsPath.eq_head_of_inc_source` says the same about a list already
...
step of `SchoenfliesGraph.IsTwoConnected.ear_decomposition` has to open before it can do any geometry. -/
...
variable {β : Type*} {G B P : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {z a b : Plane}
  {S : Set Plane} {D : List β}

/-- What a union of two plane graphs occupies is what the two of them occupy. -/
@[simp]
theorem pointSet_union (B P : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) :
...
theorem exterior_union (B P : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) :
...
(`SchoenfliesGraph.IsDrawing.vertex_mem_edgeArc`), and a point of the ear on an *edge* of `B` is a vertex
incident with both (`SchoenfliesGraph.IsDrawing.edge_inter`) — the two edges being distinct because no
...
serves. General, and not in `Schoenflies/SchoenfliesGraph/Drawing.lean`; the integrator may want to hoist
it. -/
