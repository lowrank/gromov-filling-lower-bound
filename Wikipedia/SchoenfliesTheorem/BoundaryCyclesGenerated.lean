...
theorem walkVertices_oldWalk {u v : γ} {W : List γ} (h : S.skel.IsWalk u W v) :
  ...
  · intro z hz
    rcases SchoenfliesGraph.mem_walkVertices_iff.1 hz with rfl | ⟨f, hf, y, hlink⟩
    ...
    · rw [SplitData.skeleton, d.compatible.union_isLink] at hlink
      ...
      · exact SchoenfliesGraph.mem_walkVertices_of_mem_covered ⟨f, hf, y, hlink⟩
      · exact (Set.disjoint_left.1 d.disjoint_edgeSet (h.edge_mem hf)
          hlink.edge_mem).elim
  · exact SchoenfliesGraph.walkVertices_mono_of_le d.le_skeleton
...
theorem walkVertices_earWalk :
  ...
  · rw [d.isPathGraph.vertexSet_eq]
    exact SchoenfliesGraph.walkVertices_mono_of_le d.ear_le_skeleton
...
theorem faceCycle_of_boundaryPath {newFace : γ} {P : List γ}
  ...
  | @cons _ w _ _ _ hlink hR hfresh =>
    ...
    have hfreshEdge : f ∉ R ++ d.earWalk.reverse := by
      ...
      rintro (hfR | hfQ)
      · exact (List.nodup_cons.1 (SchoenfliesGraph.IsPath.nodup (.cons hlink hR hfresh))).1 hfR
      · exact Set.disjoint_left.1 d.disjoint_edgeSet hlink.edge_mem
      ...
      calc
        (S.splitFace d).pathCells d.source ((f :: R) ++ d.earWalk.reverse) =
          ...
          FaceCycle.pathCells_append
            ((SchoenfliesGraph.IsWalk.cons hlink hR.isWalk).mono d.le_skeleton)
            hQ'.isWalk
        _ = S.pathCells d.source (f :: R) ∪ d.earCells := by
          rw [d.pathCells_oldWalk (SchoenfliesGraph.IsWalk.cons hlink hR.isWalk),
...
single-edge path. -/
theorem isPath_of_isWalk_of_isPath {G : SchoenfliesGraph γ γ} {a b x y : γ} {W : List γ}
  ...
  | @cons a w b e W hl hP hfresh ih =>
      ...
      | @cons _ w' _ _ _ hl' hW =>
        ...
        · have hnil : W = [] := by
            ...
            exact hfresh (SchoenfliesGraph.mem_walkVertices_of_mem_covered ⟨f, hf, hinc⟩)
          subst W
          cases hW with
          | nil hu => exact SchoenfliesGraph.IsPath.single hl.symm (SchoenfliesGraph.IsPath.cons_ne hP hfresh).symm
...
theorem walkVertices_eq_of_notMem {u v : γ} {W : List γ} (h : S.skel.IsWalk u W v)
  ...
  apply Set.Subset.antisymm
  · intro z hz
    rcases SchoenfliesGraph.mem_walkVertices_iff.1 hz with rfl | ⟨f, hf, y, hl⟩
    · exact SchoenfliesGraph.mem_walkVertices_self
    · rw [d.skeleton_isLink] at hl
      rcases hl with ⟨hl, -, -, -⟩ | ⟨rfl, -⟩ | ⟨rfl, -⟩
      · exact SchoenfliesGraph.mem_walkVertices_of_mem_covered ⟨f, hf, y, hl⟩
      · exact (d.newEdge₁_notMem_edgeSet (h.edge_mem hf)).elim
      · exact (d.newEdge₂_notMem_edgeSet (h.edge_mem hf)).elim
  · intro z hz
    rcases SchoenfliesGraph.mem_walkVertices_iff.1 hz with rfl | ⟨f, hf, y, hl⟩
    · exact SchoenfliesGraph.mem_walkVertices_self
    · apply SchoenfliesGraph.mem_walkVertices_of_mem_covered
...
theorem old_walkVertices_subset (hsub : d.SubstWalk u W W') :
  ...
  induction hsub with
  | nil u => intro z hz; simpa only [SchoenfliesGraph.walkVertices_nil] using hz
  | forward hs ih =>
      intro z hz
      rcases SchoenfliesGraph.mem_walkVertices_cons d.isLink hz with rfl | hz
      · exact SchoenfliesGraph.mem_walkVertices_self
      · have hlink₁ : d.skeleton.IsLink d.newEdge₁ d.left d.newVertex :=
        ...
        exact SchoenfliesGraph.mem_walkVertices_cons_of_mem hlink₁
          (SchoenfliesGraph.mem_walkVertices_cons_of_mem hlink₂ (ih hz))
  | backward hs ih =>
      intro z hz
      rcases SchoenfliesGraph.mem_walkVertices_cons d.isLink.symm hz with rfl | hz
      · exact SchoenfliesGraph.mem_walkVertices_self
  ...
  | other hl hf hs ih =>
      intro z hz
      rcases SchoenfliesGraph.mem_walkVertices_cons hl hz with rfl | hz
      · exact SchoenfliesGraph.mem_walkVertices_self
      · have hlink : d.skeleton.IsLink _ _ _ := d.skeleton_isLink.2 (Or.inl
        ...
        exact SchoenfliesGraph.mem_walkVertices_cons_of_mem hlink (ih hz)
...
theorem new_walkVertices_subset (hsub : d.SubstWalk u W W') {z : γ}
  ...
  induction hsub with
  | nil u => exact Or.inr (by simpa only [SchoenfliesGraph.walkVertices_nil] using hz)
  | forward hs ih =>
      have hlink₁ : d.skeleton.IsLink d.newEdge₁ d.left d.newVertex :=
        d.skeleton_isLink.2 (Or.inr (Or.inl ⟨rfl, rfl⟩))
      have hlink₂ : d.skeleton.IsLink d.newEdge₂ d.newVertex d.right :=
        d.skeleton_isLink.2 (Or.inr (Or.inr ⟨rfl, rfl⟩))
      rcases SchoenfliesGraph.mem_walkVertices_cons hlink₁ hz with rfl | hz
      · exact Or.inr SchoenfliesGraph.mem_walkVertices_self
      · rcases SchoenfliesGraph.mem_walkVertices_cons hlink₂ hz with rfl | hz
        · exact Or.inl rfl
        · rcases ih hz with rfl | hz
          · exact Or.inl rfl
          · exact Or.inr (SchoenfliesGraph.mem_walkVertices_cons_of_mem d.isLink hz)
  | backward hs ih =>
      have hlink₂ : d.skeleton.IsLink d.newEdge₂ d.right d.newVertex :=
        d.skeleton_isLink.2 (Or.inr (Or.inr ⟨rfl, Sym2.eq_swap⟩))
      have hlink₁ : d.skeleton.IsLink d.newEdge₁ d.newVertex d.left :=
      ...
      · rcases SchoenfliesGraph.mem_walkVertices_cons hlink₁ hz with rfl | hz
        ...
        · rcases ih hz with rfl | hz
          · exact Or.inl rfl
          · exact Or.inr (SchoenfliesGraph.mem_walkVertices_cons_of_mem d.isLink.symm hz)
  | other hl hf hs ih =>
      have hlink : d.skeleton.IsLink _ _ _ := d.skeleton_isLink.2 (Or.inl
        ⟨hl, hf, fun h => d.newEdge₁_notMem_edgeSet (h ▸ hl.edge_mem),
          fun h => d.newEdge₂_notMem_edgeSet (h ▸ hl.edge_mem)⟩)
      ...
      · rcases ih hz with rfl | hz
        · exact Or.inl rfl
        · exact Or.inr (SchoenfliesGraph.mem_walkVertices_cons_of_mem hl hz)
...
theorem isPath (hsub : d.SubstWalk u W W') (hP : S.skel.IsPath u W v) :
  ...
  | forward hs ih =>
      ...
      | @cons _ w _ _ _ hl hP hfresh =>
        ...
        subst w
        have heW : d.edge ∉ _ :=
          (List.nodup_cons.1 (SchoenfliesGraph.IsPath.nodup (SchoenfliesGraph.IsPath.cons hl hP hfresh))).1
        ...
        · intro hz
          rcases SchoenfliesGraph.mem_walkVertices_cons hlink₂ hz with hEq | hz
...
theorem newCells_subset_pathCells (hsub : d.SubstWalk u W W') (he : d.edge ∈ W) :
  ...
  | forward hs ih =>
      ...
      · exact Or.inr (SchoenfliesGraph.mem_walkVertices_cons_of_mem
          (d.skeleton_isLink.2 (Or.inr (Or.inl ⟨rfl, rfl⟩))) SchoenfliesGraph.mem_walkVertices_self)
  ...
  | backward hs ih =>
      ...
      · exact Or.inr (SchoenfliesGraph.mem_walkVertices_cons_of_mem
          (d.skeleton_isLink.2 (Or.inr (Or.inr ⟨rfl, Sym2.eq_swap⟩)))
          SchoenfliesGraph.mem_walkVertices_self)
      · exact Or.inl (List.mem_cons_of_mem _ (List.mem_cons_self ..))
      · exact Or.inl (List.mem_cons_self ..)
  | other hl hne hs ih =>
      simp only [List.mem_cons] at he
      ...
      · intro z hz
        ...
        · have hlink : d.skeleton.IsLink _ _ _ := d.skeleton_isLink.2 (Or.inl
            ⟨hl, hne, fun h => d.newEdge₁_notMem_edgeSet (h ▸ hl.edge_mem),
              fun h => d.newEdge₂_notMem_edgeSet (h ▸ hl.edge_mem)⟩)
          exact Or.inr (SchoenfliesGraph.mem_walkVertices_cons_of_mem hlink hz)
