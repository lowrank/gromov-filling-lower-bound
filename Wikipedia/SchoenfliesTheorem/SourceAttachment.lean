...
theorem of_seg_subset_interior {J : Piece} (hJopen : J.seg ⊆ srcDom \ srcOuter) :
  ...
  seg_subset_domain := hJopen.trans diff_subset
...
noncomputable def crosscutOverlay (J : Piece) (p : Plane) (s epsilon : ℝ)
    (extra : List Plane) : SchoenfliesGraph Plane Piece :=
...
theorem crosscutOverlay_isDrawing {J : Piece} {p : Plane} {s epsilon : ℝ}
    ...
    SchoenfliesGraph.IsDrawing (Q.crosscutOverlay J p s epsilon extra) segmentDrawing :=
...
theorem crosscutOverlay_pointSet (J : Piece) (p : Plane) (s epsilon : ℝ)
    (extra : List Plane) :
    SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra) segmentDrawing =
      (SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing ∪ J.seg) ∪
...
theorem sourceCore_subset_crosscutOverlay (J : Piece) (p : Plane) (s epsilon : ℝ)
    (extra : List Plane) :
    SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing ⊆
      SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra) segmentDrawing := by
...
theorem crosscut_subset_crosscutOverlay (J : Piece) (p : Plane) (s epsilon : ℝ)
    (extra : List Plane) :
    J.seg ⊆ SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra) segmentDrawing := by
  rw [Q.crosscutOverlay_pointSet]
...
theorem sourceCoreVertices_subset_crosscutOverlay {J : Piece} {p : Plane}
  ...
  · apply mem_attachPoints_of_mem
    exact List.mem_append_right extra (by
      rw [Finset.mem_toList, SchoenfliesGraph.mem_vertexFinset]
      exact hx)
  · rw [crosscutPieces, cover_append, cover_append, Q.cover_eq]
    exact Or.inl (Or.inl (SchoenfliesGraph.vertexSet_subset_pointSet hx))
...
theorem crosscutOverlay_pointSet_subset {J : Piece} {p : Plane} {s epsilon : ℝ}
    ...
    SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra) segmentDrawing ⊆ srcDom := by
  rw [Q.crosscutOverlay_pointSet]
  apply Set.union_subset
  · apply Set.union_subset
    · exact (SchoenfliesGraph.pointSet_mono P.sourceNonboundaryGraph_le).trans
        P.src_isWeaklyAdmissible.skeletonSet_subset
    · exact hJdom
  · exact (cover_localGridEdges_subset_closedSquare hs
      (one_le_localGridCount s epsilon)).trans (hwindow.trans diff_subset)
...
theorem crosscutOverlay_edge_dichotomy {J : Piece} {p : Plane} {s epsilon : ℝ}
    ...
    ∀ {R : Piece}, R ∈ E(Q.crosscutOverlay J p s epsilon extra) →
      IsPolygonal (_root_.SchoenfliesGraph.edgeArc segmentDrawing R) ∧
        _root_.SchoenfliesGraph.edgeArc segmentDrawing R \ V(Q.crosscutOverlay J p s epsilon extra) ⊆
          srcDom \ srcOuter := by
  intro R hR
  ...
  · obtain ⟨A, hA, hRA⟩ := hOld
    ...
    have hxe : x ∈ _root_.SchoenfliesGraph.edgeArc P.src.drawing e := hAe (hRA hxSeg)
...
theorem crosscutOverlay_edge_subset {J : Piece} {p : Plane} {s epsilon : ℝ}
    ...
    ∀ {e : γ}, e ∈ E(P.str.skel) → e ∉ E(P.str.outerGraph) → ∀ {R : Piece},
      ...
      (_root_.SchoenfliesGraph.edgeArc segmentDrawing R ∩
        (P.src.cell e \ V(Q.crosscutOverlay J p s epsilon extra))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc segmentDrawing R ⊆
        _root_.SchoenfliesGraph.edgeArc P.src.drawing e := by
  intro e he heOuter R hR hmeet
  obtain ⟨z, hzR, hzCell, hznotOverlay⟩ := hmeet
  obtain ⟨a, b, hab⟩ := P.str.skel.exists_isLink_of_mem_edgeSet he
  have hzOldArc : z ∈ _root_.SchoenfliesGraph.edgeArc P.src.drawing e := by
    rw [P.src.cell_edge hab] at hzCell
    exact hzCell.1
  have hzCore : z ∈ SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing :=
    SchoenfliesGraph.edgeArc_subset_pointSet
  ...
  have hzg : z ∈ _root_.SchoenfliesGraph.edgeArc P.src.drawing g := hAg hzA
  ...
  have hAold : A.seg ⊆ _root_.SchoenfliesGraph.edgeArc P.src.drawing e := by rwa [heg]
  ...
  have hzR'Arc : z ∈ _root_.SchoenfliesGraph.edgeArc segmentDrawing R' := by
...
theorem crosscutOverlay_grid_edge_subset {J : Piece} {p : Plane} {s epsilon : ℝ}
    ...
    ∀ {A : Piece}, A ∈ E(localGrid p s (localGridCount s epsilon)) → ∀ {R : Piece},
      R ∈ E(Q.crosscutOverlay J p s epsilon extra) →
      (_root_.SchoenfliesGraph.edgeArc segmentDrawing R ∩
        (_root_.SchoenfliesGraph.edgeArc segmentDrawing A \
...
theorem crosscutOverlay_crosscut_edge_subset {J : Piece} {p : Plane} {s epsilon : ℝ}
    ...
    ∀ {R : Piece}, R ∈ E(Q.crosscutOverlay J p s epsilon extra) →
      (_root_.SchoenfliesGraph.edgeArc segmentDrawing R ∩
        (_root_.SchoenfliesGraph.edgeArc segmentDrawing J \
          V(Q.crosscutOverlay J p s epsilon extra))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc segmentDrawing R ⊆
        _root_.SchoenfliesGraph.edgeArc segmentDrawing J := by
  intro R hR hmeet
...
theorem pieceListGraph_single_isDrawing {J : Piece} (hJ : J.Nondeg) :
    _root_.SchoenfliesGraph.IsDrawing (pieceListGraph [J]) segmentDrawing where
  edge_param := by
    intro R hR
...
noncomputable def graph : _root_.SchoenfliesGraph Plane γ := w.outerGraph.union w.innerGraph
...
theorem outer_pointSet :
    _root_.SchoenfliesGraph.pointSet w.outerGraph w.drawing = srcOuter := by
  calc
    _root_.SchoenfliesGraph.pointSet w.outerGraph w.drawing =
        _root_.SchoenfliesGraph.pointSet w.outerGraph P.src.drawing := by
      ...
      simpa only [_root_.SchoenfliesGraph.edgeArc] using congrArg
        ...
        (w.drawing_of_outer (by rwa [_root_.SchoenfliesGraph.edgeSet_map] at he))
...
theorem inner_pointSet :
    _root_.SchoenfliesGraph.pointSet w.innerGraph w.drawing =
      _root_.SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra)
        segmentDrawing := by
  calc
    _root_.SchoenfliesGraph.pointSet w.innerGraph w.drawing =
        _root_.SchoenfliesGraph.pointSet w.innerGraph
          ((Q.crosscutOverlay J p s epsilon extra).relabelDrawing
    ...
    _ = _root_.SchoenfliesGraph.pointSet (Q.crosscutOverlay J p s epsilon extra)
        segmentDrawing :=
      _root_.SchoenfliesGraph.pointSet_relabelEdges w.name_inj
...
theorem graph_finite : w.graph.Finite where
  finite_vertexSet := by
    ...
    exact ((P.str.finite_vertexSet.subset P.str.outerGraph_le.vertexSet_mono).image
      P.src.pos).union
      (_root_.SchoenfliesGraph.finite_vertexSet (Q.crosscutOverlay J p s epsilon extra))
  finite_edgeSet := by
...
theorem graph_pointSet :
    _root_.SchoenfliesGraph.pointSet w.graph w.drawing =
      srcOuter ∪ ((
        _root_.SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing ∪ J.seg) ∪
          cover (localGridEdges p s (localGridCount s epsilon))) := by
  rw [graph, _root_.SchoenfliesGraph.pointSet_union, w.outer_pointSet, w.inner_pointSet,
...
theorem sourceSkeleton_subset_graph :
    P.src.skeletonSet ⊆ _root_.SchoenfliesGraph.pointSet w.graph w.drawing := by
  ...
  · rw [graph, _root_.SchoenfliesGraph.vertexSet_union]
    exact Or.inl
      (((P.str.outerGraph_le.map P.src.pos).inc_congr
        (by rwa [_root_.SchoenfliesGraph.edgeSet_map])).2 hinc).vertex_mem
...
theorem graph_pointSet_subset (hs : 0 < s)
    (hJgeom : SourceCrosscutGeometry P J)
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter) :
    _root_.SchoenfliesGraph.pointSet w.graph w.drawing ⊆ srcDom := by
  rw [graph, _root_.SchoenfliesGraph.pointSet_union]
  apply Set.union_subset
  · rw [w.outer_pointSet, ← P.src_isWeaklyAdmissible.outerSet_eq]
    exact (_root_.SchoenfliesGraph.pointSet_mono (P.str.outerGraph_le.map P.src.pos)).trans
      P.src_isWeaklyAdmissible.skeletonSet_subset
  · rw [w.inner_pointSet]
...
theorem graph_edge_dichotomy (hs : 0 < s) (hJ : J.Nondeg)
    (hJgeom : SourceCrosscutGeometry P J)
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter) :
    ∀ {e : γ}, e ∈ E(w.graph) → _root_.SchoenfliesGraph.edgeArc w.drawing e ⊆ srcOuter ∨
      (IsPolygonal (_root_.SchoenfliesGraph.edgeArc w.drawing e) ∧
        _root_.SchoenfliesGraph.edgeArc w.drawing e \ V(w.graph) ⊆ srcDom \ srcOuter) := by
  intro e he
  ...
  · obtain ⟨R, hR, rfl⟩ := heInner
    ...
    have harc : _root_.SchoenfliesGraph.edgeArc w.drawing (w.name R) =
        _root_.SchoenfliesGraph.edgeArc segmentDrawing R := by
      calc
        _root_.SchoenfliesGraph.edgeArc w.drawing (w.name R) =
        ...
        _ = _root_.SchoenfliesGraph.edgeArc segmentDrawing R :=
          _root_.SchoenfliesGraph.edgeArc_relabelDrawing w.name_inj hR
    rw [harc]
...
theorem graph_edge_subset (hs : 0 < s) (hJ : J.Nondeg)
    ...
    ∀ {e : γ}, e ∈ E(P.str.skel) → ∀ {f : γ}, f ∈ E(w.graph) →
      (_root_.SchoenfliesGraph.edgeArc w.drawing f ∩ (P.src.cell e \ V(w.graph))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc w.drawing f ⊆ _root_.SchoenfliesGraph.edgeArc P.src.drawing e := by
  intro e he f hf hmeet
  obtain ⟨z, hzf, hzCell, hznotGraph⟩ := hmeet
  obtain ⟨a, b, hab⟩ := P.str.skel.exists_isLink_of_mem_edgeSet he
  have hze : z ∈ _root_.SchoenfliesGraph.edgeArc P.src.drawing e := by
    rw [P.src.cell_edge hab] at hzCell
    exact hzCell.1
  rcases hf with hfOuter | hfInner
  · have hfAbstract : f ∈ E(P.str.outerGraph) := by
      rwa [_root_.SchoenfliesGraph.edgeSet_map] at hfOuter
    have hzfSrc : z ∈ _root_.SchoenfliesGraph.edgeArc P.src.drawing f := by
      have hdraw := w.drawing_of_outer hfAbstract
      simpa only [_root_.SchoenfliesGraph.edgeArc] using
    ...
    have hef : e = f := P.src.isDrawing.unique_edge_at
      (by rw [_root_.SchoenfliesGraph.edgeSet_map]; exact he)
      (by
        rw [_root_.SchoenfliesGraph.edgeSet_map]
        exact P.str.outerGraph_le.edgeSet_mono hfAbstract)
      hznotOld hze hzfSrc
    subst f
    have hdraw := w.drawing_of_outer hfAbstract
    rw [_root_.SchoenfliesGraph.edgeArc, hdraw]
    intro y hy
    exact hy
  · obtain ⟨R, hR, rfl⟩ := hfInner
    ...
    · exfalso
      ...
      have hzeMixed : z ∈ _root_.SchoenfliesGraph.edgeArc w.drawing e := by
...
theorem sourceTrace_isTwoConnected (hs : 0 < s) (hJ : J.Nondeg)
    (hJgeom : SourceCrosscutGeometry P J)
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter) :
    (_root_.SchoenfliesGraph.traceGraph w.graph w.drawing P.src.skeletonSet).IsTwoConnected :=
...
theorem localGrid_subset_graph :
    _root_.SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing ⊆
      _root_.SchoenfliesGraph.pointSet w.graph w.drawing := by
...
theorem graph_grid_edge_subset (hs : 0 < s) (hJ : J.Nondeg)
    ...
    ∀ {A : Piece}, A ∈ E(localGrid p s (localGridCount s epsilon)) → ∀ {f : γ},
      f ∈ E(w.graph) →
      (_root_.SchoenfliesGraph.edgeArc w.drawing f ∩
        (_root_.SchoenfliesGraph.edgeArc segmentDrawing A \ V(w.graph))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc w.drawing f ⊆
        _root_.SchoenfliesGraph.edgeArc segmentDrawing A := by
...
theorem graph_crosscut_edge_subset (hs : 0 < s) (hJ : J.Nondeg)
    ...
    ∀ {f : γ}, f ∈ E(w.graph) →
      (_root_.SchoenfliesGraph.edgeArc w.drawing f ∩
        (_root_.SchoenfliesGraph.edgeArc segmentDrawing J \ V(w.graph))).Nonempty →
      _root_.SchoenfliesGraph.edgeArc w.drawing f ⊆
        _root_.SchoenfliesGraph.edgeArc segmentDrawing J := by
  intro f hf hmeet
...
theorem localGridTrace_isTwoConnected (hs : 0 < s) (hJ : J.Nondeg)
    (hJgeom : SourceCrosscutGeometry P J)
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter) :
    (_root_.SchoenfliesGraph.traceGraph w.graph w.drawing
...
theorem exists_crosscut_trace (hs : 0 < s) (hJ : J.Nondeg)
    ...
    ∃ D : List γ, w.graph.IsPath J.1 D J.2 ∧
      _root_.SchoenfliesGraph.edgesCover w.drawing D = J.seg := by
  obtain ⟨D, hD, hcover⟩ :=
...
theorem graph_isTwoConnected_of_crosscut_grid_edge (hs : 0 < s) (hJ : J.Nondeg)
  ...
  let T := _root_.SchoenfliesGraph.traceGraph w.graph w.drawing P.src.skeletonSet
  let C := w.graph.pathGraphOf J.1 D
  let K := _root_.SchoenfliesGraph.traceGraph w.graph w.drawing
    (_root_.SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing)
  ...
  have hA1K : A.1 ∈ V(K) := by
    rw [_root_.SchoenfliesGraph.traceGraph_vertexSet]
    exact ⟨hA1Graph,
      (_root_.SchoenfliesGraph.edgeArc_subset_pointSet hA) (by
        rw [edgeArc_segmentDrawing]
        exact left_mem_segment ℝ _ _)⟩
  ...
  have hUKcompat : (T.union C).Compatible K :=
    _root_.SchoenfliesGraph.Compatible.of_le_le
      (_root_.SchoenfliesGraph.union_le (_root_.SchoenfliesGraph.traceGraph_le _)
        (_root_.SchoenfliesGraph.pathGraphOf_le hD.isWalk))
      (_root_.SchoenfliesGraph.traceGraph_le _)
  have hAne : A.1 ≠ A.2 :=
  ...
  apply hAll2.of_le_of_vertexSet_subset
    (_root_.SchoenfliesGraph.union_le
      (_root_.SchoenfliesGraph.union_le (_root_.SchoenfliesGraph.traceGraph_le _)
        (_root_.SchoenfliesGraph.pathGraphOf_le hD.isWalk))
      (_root_.SchoenfliesGraph.traceGraph_le _))
  intro x hx
  rw [_root_.SchoenfliesGraph.vertexSet_union]
  have hxPoint : x ∈ _root_.SchoenfliesGraph.pointSet w.graph w.drawing :=
    _root_.SchoenfliesGraph.vertexSet_subset_pointSet hx
...
theorem graph_isConnected_diff_of_crosscut_grid_edge (hs : 0 < s)
    ...
    {A : Piece} (hA : A ∈ E(localGrid p s (localGridCount s epsilon)))
    (hAJ : A.seg ⊆ J.seg) :
    IsConnected (_root_.SchoenfliesGraph.pointSet w.graph w.drawing \ srcOuter) := by
  ...
  have hcarrier : _root_.SchoenfliesGraph.pointSet w.graph w.drawing \ srcOuter =
      ((P.src.skeletonSet \ srcOuter) ∪ J.seg) ∪ Kset := by
    ext x
    rw [Set.mem_diff, Set.mem_union, Set.mem_union, Set.mem_diff]
    constructor
...
theorem graph_isConnected_diff_of_crosscut_grid_edge_of_end_not_outer (hs : 0 < s)
  ...
  have hcarrier : _root_.SchoenfliesGraph.pointSet w.graph w.drawing \ srcOuter =
    ...
    ext x
    rw [Set.mem_diff, Set.mem_union, Set.mem_union, Set.mem_diff, Set.mem_diff]
    constructor
    · rintro ⟨hxGraph, hxNotOuter⟩
      rw [w.graph_pointSet] at hxGraph
      rcases hxGraph with hxOuter | hxRest
...
theorem isSourceExtension (hs : 0 < s) (hJ : J.Nondeg)
    ...
    (hconnected : IsConnected
      (_root_.SchoenfliesGraph.pointSet w.graph w.drawing \ srcOuter)) :
...
structure LocalGridSourceExtensionData
  ...
  graph : _root_.SchoenfliesGraph Plane γ
  drawing : γ → ℝ → Plane
  isSourceExtension : IsSourceExtension P.src srcOuter srcDom graph drawing
  localGrid_subset :
    _root_.SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing ⊆
      _root_.SchoenfliesGraph.pointSet graph drawing
...
structure RefinedCrosscutOverlayData
  ...
  sourceSkeleton_subset :
    r.subdivision.pair.src.skeletonSet ⊆
      _root_.SchoenfliesGraph.pointSet relabeling.graph relabeling.drawing
  pointSet_subset :
    _root_.SchoenfliesGraph.pointSet relabeling.graph relabeling.drawing ⊆ srcDom
  localGrid_subset :
    _root_.SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing ⊆
      _root_.SchoenfliesGraph.pointSet relabeling.graph relabeling.drawing
