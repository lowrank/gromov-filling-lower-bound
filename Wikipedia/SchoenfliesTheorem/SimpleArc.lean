...
* A simple path in that graph is `SchoenfliesGraph.IsPath`, produced by `SchoenfliesGraph.Reaches.exists_isPath`.
* Its realisation is an arc between its ends, by `SchoenfliesGraph.IsDrawing.path_isArcBetween`.
...
**The realisation of a path is `poly` of a vertex list.** `SchoenfliesGraph.IsDrawing.path_isArcBetween`
delivers `SchoenfliesGraph.edgesCover`, a union of edge arcs with no order in it, while `IsPolygonal` and
...
`SchoenfliesGraph.edgesCover` is a union with no order in it. The walk supplies the order, and the
...
theorem exists_poly_eq_edgesCover {u v : Plane} {W : List Piece}
    ...
    ∃ vs : List Plane, ∃ h : vs ≠ [], vs.head h = u ∧ vs.getLast h = v ∧
      poly vs = SchoenfliesGraph.edgesCover segmentDrawing W := by
  induction hW with
  | nil => exact absurd rfl hne
  | @cons u w v e W hl hW ih =>
    -- The edge's arc is the segment from the source to the waypoint, in whichever order the
    -- edge names its ends.
    have hedge : SchoenfliesGraph.edgeArc segmentDrawing e = segment ℝ u w := by
    ...
    · have hwv : w = v := hW.eq_of_nil
      ...
      rw [poly_pair, SchoenfliesGraph.edgesCover_cons, SchoenfliesGraph.edgesCover_nil, union_empty, hedge]
    · obtain ⟨vs, hvs, hhead, hlast, hpoly⟩ := ih hWne
      refine ⟨u :: vs, by simp, rfl, ?_, ?_⟩
      · rw [List.getLast_cons hvs]; exact hlast
      · rw [poly_cons_of_ne_nil hvs, hhead, hpoly, SchoenfliesGraph.edgesCover_cons, hedge]
...
theorem overlayGraph_reaches_end {P : Piece} (hP : P ∈ overlayPieces pieces points)
  ...
  · exact Iff.rfl
  · exact ⟨fun h => h.trans (SchoenfliesGraph.Reaches.of_isLink hlink),
      fun h => h.trans (SchoenfliesGraph.Reaches.of_isLink hlink).symm⟩
...
theorem overlayGraph_reaches (hnd : ∀ P ∈ pieces, P.Nondeg)
  ...
  classical
  have hdraw : SchoenfliesGraph.IsDrawing (overlayGraph pieces points) segmentDrawing :=
  ...
  have haS : a ∈ S := by
    ...
    refine mem_biUnion (show P ∈ _ from
      ⟨hP, (overlayGraph_reaches_end hP hend).2 (SchoenfliesGraph.Reaches.refl haV)⟩) ?_
...
theorem exists_simple_poly_of_cover (hnd : ∀ P ∈ pieces, P.Nondeg)
  ...
  have hdraw : SchoenfliesGraph.IsDrawing (overlayGraph pieces pts) segmentDrawing :=
  ...
  have harc : IsArcBetween (SchoenfliesGraph.edgesCover segmentDrawing W) a b :=
  ...
  · rw [hpoly, ← overlayGraph_pointSet pieces pts]
    exact SchoenfliesGraph.edgesCover_subset_pointSet fun e he => hpath.edge_mem he
  · rw [hpoly]; exact harc
...
theorem subset_of_finite_diff {U C : Set Plane} (hU : IsPreconnected U) (hC : IsClosed C)
  ...
  have hDclosed : IsClosed D := hC.union ((hfin.subset Set.diff_subset).isClosed)
