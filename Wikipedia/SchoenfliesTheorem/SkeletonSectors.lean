...
* `SchoenfliesGraph.IsDrawing.exists_edge_radial` and `SchoenfliesGraph.IsDrawing.edge_radial_unique` — each radial
  segment lies inside exactly one edge arc. The relative interior of the segment is connected
  and contains no vertex, so `SchoenfliesGraph.IsDrawing.unique_edge_at` makes "the edge through this
  point" locally constant on it, and the edge arcs being closed and finitely many makes it
  constant.
* `SchoenfliesGraph.IsDrawing.not_three_localDirs_on_edge` — one edge carries at most two of them. The
...
* `SchoenfliesGraph.IsDrawing.localDirs_ncard_le` — the two together, counted: there are at most twice as
...
well contain other vertices, sitting out along the radial segments. `SchoenfliesGraph.IsLocalDisk` adds
that clause and `SchoenfliesGraph.IsDrawing.exists_isLocalDisk` produces it.
...
segment from `x` into it. `SchoenfliesGraph.exists_access_sector` hands over all four about one named
...
The sector decomposition and `SchoenfliesGraph.exists_access_sector` assume that `x` has **at least two**
...
consumer form is `SchoenfliesGraph.IsDrawing.exists_access_region`, which returns the connected component
...
* `SchoenfliesGraph.IsDrawing.exists_edge_radial`, `SchoenfliesGraph.IsDrawing.edge_radial_unique`,
  `SchoenfliesGraph.IsDrawing.not_three_localDirs_on_edge`, `SchoenfliesGraph.IsDrawing.localDirs_ncard_le` — "two of
...
* `SchoenfliesGraph.IsLocalDisk`, `SchoenfliesGraph.IsDrawing.exists_isLocalDisk` — the radius `r₀` of the same lemma,
  with the vertex clause the drawing axioms need.
* `SchoenfliesGraph.exists_access_sector`, `SchoenfliesGraph.IsDrawing.exists_access_region` — "each sector is
...
`SchoenfliesGraph.exists_access_region` and `SchoenfliesGraph.exists_access_sector` two readings of one theorem. -/
...
variable {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {x d : Plane} {r : ℝ}
...
structure IsLocalDisk (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (x : Plane) (r : ℝ) :
    Prop where
...
theorem IsDrawing.exists_isLocalDisk [G.Finite] (h : IsDrawing G drawing)
  ...
  have hfinV : (V(G) \ {x}).Finite := (SchoenfliesGraph.finite_vertexSet G).diff
...
theorem IsDrawing.exists_edge_radial [G.Finite] (h : IsDrawing G drawing)
  ...
  have hrest : IsClosed (⋃ e ∈ E(G) \ {e₀}, edgeArc drawing e) :=
    ((SchoenfliesGraph.finite_edgeSet G).diff (t := {e₀})).isClosed_biUnion
...
theorem IsDrawing.localDirs_ncard_le [G.Finite] (h : IsDrawing G drawing)
  ...
  have hmaps : ∀ d ∈ hfin.toFinset, f d ∈ G.edgeFinset := fun d hd =>
    SchoenfliesGraph.mem_edgeFinset.2 (hf d (hfin.mem_toFinset.1 hd)).1
  have hfib : ∀ e ∈ G.edgeFinset,
    ...
    exact h.not_three_localDirs_on_edge hr (SchoenfliesGraph.mem_edgeFinset.1 he)
  ...
  have hE : E(G).ncard = (G.edgeFinset).card :=
    Set.ncard_eq_toFinset_card _ (SchoenfliesGraph.finite_edgeSet G)
...
`SchoenfliesGraph.exists_access_sector`; with fewer it is the whole punctured disk, which is not an arc
