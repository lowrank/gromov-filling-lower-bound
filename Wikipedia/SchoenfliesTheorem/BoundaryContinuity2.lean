...
`lem:skeleton-crosscuts` (`SchoenfliesGraph.IsStageOn.exists_crosscut`), for one finite
...
* **`lem:skeleton-crosscuts` + `prop:skeleton-agreement`** enter as
  ...
  produces once `SchoenfliesGraph.IsStageOn.exists_crosscut` has been run in the stage containing `a`
  and `b` (see `SchoenfliesGraph.IsStageOn.exists_isCrosscut` below, which does the packaging).
...
* `SchoenfliesGraph.IsStageOn.exists_isCrosscut` — **`lem:skeleton-crosscuts`** repackaged as
...
`SchoenfliesGraph.IsStageOn.exists_crosscut` extracts from a stage drawn in the closed Jordan domain is a
...
theorem IsStageOn.exists_isCrosscut {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
...
theorem image_sdiff_eq_of_eqOn {Sk : Set Plane} {g : Plane → Plane} (hPSk : P ⊆ Sk)
  ...
  rw [hFg.image_eq, (hinj.mono hPSk).image_diff_subset (pair_subset ha hb), image_pair]
...
theorem image_inside_sdiff (hF : IsHomeoOn F F' (inside C) (Plane.openSquare 0 1))
  ...
  rw [inside_sdiff_crosscut hP, hF.injOn.image_diff_subset hP.sdiff_subset, hagree, hFin,
