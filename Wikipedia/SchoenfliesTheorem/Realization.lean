...
* `Schoenflies.exists_closedPolygon_corners`, `Schoenflies.exists_closedPolygon_split`,
  ...
  The last is the shape `SchoenfliesGraph.IsHexRealization` asks for.
...
theorem, and is exactly the freedom `SchoenfliesGraph.IsK33Config.not_isDrawing` reserves for itself.
-/
...
This is the form the plane-graph layer wants — compare `SchoenfliesGraph.IsHexRealization`. The corner
...
`SchoenfliesGraph.IsHexRealization` asks for. -/
