...
`Schoenflies.isLink_of_le_of_mem_edgeSet`. The second belongs in `Schoenflies/SchoenfliesGraph/`; nothing
on `main` states it.
...
def subdivGraph (H : SchoenfliesGraph γ γ) (e x y v e₁ e₂ : γ) (hne : e₁ ≠ e₂)
    (h₁ : e₁ ∉ E(H)) (h₂ : e₂ ∉ E(H)) : SchoenfliesGraph γ γ where
  ...
  isLink_symm := by
    intro f hf a b
    rintro (⟨hl, hfe, hf₁, hf₂⟩ | ⟨rfl, hl, hs⟩ | ⟨rfl, hl, hs⟩)
    · exact Or.inl ⟨hl.symm, hfe, hf₁, hf₂⟩
  ...
  edge_mem_iff_exists_isLink f := by
    constructor
    · rintro (⟨hf, hfe⟩ | ⟨hf, hl⟩)
      · obtain ⟨a, b, hab⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet hf
...
variable {H K : SchoenfliesGraph γ γ} {e x y v e₁ e₂ : γ}
...
theorem subdivGraph_eq_self {hne h₁ h₂} (he : e ∉ E(H)) :
  ...
  refine SchoenfliesGraph.ext ?_ fun f a b => ?_
  · simp [hno]
...
theorem isLink_of_le_of_mem_edgeSet (hHK : H ≤ K) (he : e ∈ E(H)) (h : K.IsLink e x y) :
    H.IsLink e x y := by
  obtain ⟨a, b, hab⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet he
...
def skeleton : SchoenfliesGraph γ γ :=
...
def outer : SchoenfliesGraph γ γ :=
...
The ear enters as a *graph* `ear` together with `SchoenfliesGraph.IsPathGraph`, rather than as a bare list
...
theorem compatible : S.skel.Compatible d.ear :=
  SchoenfliesGraph.Compatible.of_disjoint_edgeSet d.disjoint_edgeSet

/-- The skeleton after the split: the old skeleton with the ear glued in along its two ends. -/
def skeleton : SchoenfliesGraph γ γ := S.skel.union d.ear
...
theorem le_skeleton : S.skel ≤ d.skeleton := SchoenfliesGraph.left_le_union _ _
...
theorem source_mem_cells₁ : d.source ∈ d.cells₁ := Or.inr SchoenfliesGraph.mem_walkVertices_self
...
theorem exists_ear_edge : ∃ f, f ∈ E(d.ear) := by
  ...
  · exfalso
    ...
    rw [d.isPathGraph.vertexSet_eq, hw, SchoenfliesGraph.walkVertices_nil, Set.mem_singleton_iff] at hmem
...
theorem IsCrosscut.inside_eq_split (hjordan : ∀ S : Set Plane, IsJordanCurve S → IsSeparating S)
  ...
  conv_lhs => rw [← Set.diff_union_inter (inside C) P]
