...
There is no `K33 : SchoenfliesGraph α β`. A copy of `K(3,3)` inside `G` is `SchoenfliesGraph.IsK33Config G x y e`:
...
hypothesis land correctly: `SchoenfliesGraph.IsDrawing G drawing` is a hypothesis about all of `G`, so
taking `G` larger only strengthens it.

Two economies follow from stating the configuration this way. The nine edge names are
automatically distinct (`SchoenfliesGraph.IsK33Config.eq_of_e_eq`): an edge has only two ends, so two
...
by their index pairs as well (`SchoenfliesGraph.arcAPairs`, `SchoenfliesGraph.arcBPairs`); `SchoenfliesGraph.arcA` and
`SchoenfliesGraph.arcB` are the corresponding edge lists, `rfl`-equal to the mapped pair lists so that
both presentations are available with no conversion lemma.

## Blueprint

* `SchoenfliesGraph.IsK33Config` — a copy of the utility graph.
* `SchoenfliesGraph.IsK33Config.hexagon_isCycleThrough` — §"Plane graphs and the utility graph": "regard
  `K(3,3)` as the six-cycle `x₁y₁x₂y₂x₃y₃x₁` together with the three remaining edges".
* `SchoenfliesGraph.IsK33Config.hexagon_isJordanCurve` — `lem:k33`, "the six-cycle is a polygonal Jordan
  curve". Polygonality plays no part; it is needed only where the *polygonal* Jordan curve
  theorem is applied to the result.
* `SchoenfliesGraph.IsK33Config.chord_inter_hexSet`, `SchoenfliesGraph.IsK33Config.chord_openArc_eq`,
  `SchoenfliesGraph.IsK33Config.chord_openArc_subset_connectedComponentIn` — `lem:k33`, "each remaining
  ...
  `cor:alternating-crosscuts`.
* `SchoenfliesGraph.IsK33Config.exists_two_chords_same_side` — `lem:k33`, "two of the three lie on the
...
* `SchoenfliesGraph.IsK33Config.chords_alternate` — `lem:k33`, "their endpoints alternate on the
...
* `SchoenfliesGraph.IsK33Config.chords_disjoint` — the disjointness that `cor:alternating-crosscuts`
...
make it polygonal (`SchoenfliesGraph.polygonal_redrawing`, `lem:polygonal-redrawing`); the six-cycle is
...
variable {α β : Type*} {G : SchoenfliesGraph α β} {x y : Fin 3 → α} {e : Fin 3 → Fin 3 → β}
variable {i j k l s t : Fin 3} {z : α}

/-- A copy of the utility graph `K(3,3)` inside `G`. -/
structure IsK33Config (G : SchoenfliesGraph α β) (x y : Fin 3 → α) (e : Fin 3 → Fin 3 → β) : Prop where
...
/-- **The nine edges carry nine different names.** This is not a primitive rule of the configuration:
...
remaining edges. The cycle is presented as `Schoenflies/SchoenfliesGraph/Cycle.lean` presents one:
...
uses more geometry than the three clauses of `SchoenfliesGraph.IsDrawing`. -/
