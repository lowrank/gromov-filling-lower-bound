...
-/
import SchoenfliesCompat.Graph.Delete
...
Deleting a vertex from Mathlib's multigraph `SchoenfliesGraph α β`, and the blueprint's notion of a
2-connected graph.

## Vertex deletion is Mathlib's

Mathlib already has it: `SchoenfliesGraph.deleteVerts G X = G.induce (V(G) \ X)`, in
`Mathlib/Combinatorics/SchoenfliesGraph/Delete.lean`, with `V(G.deleteVerts X) = V(G) \ X` and
...
Two of those transfers are the whole story, and both are corollaries of `SchoenfliesGraph.IsWalk.anti`:

* `SchoenfliesGraph.IsWalk.deleteVerts_singleton` — a walk that never visits `x` is a walk of `G - x`.
  The edges of such a walk automatically survive, because the ends of an edge of a walk are
  vertices the walk visits.
* `SchoenfliesGraph.IsWalk.notMem_of_inc_of_deleteVerts` — a walk **in** `G - x` cannot have used an
  edge at `x`; there are none left to use. This is how an argument that routes around a
  vertex ends up avoiding a named edge, and it is what makes `SchoenfliesGraph.IsTwoConnected.no_bridge`
  short.

Deleting a vertex the graph does not have changes nothing — `G.deleteVerts {x} = G` on the
nose (`SchoenfliesGraph.deleteVerts_singleton_eq_self`), since `V(G) \ {x} = V(G)`. That equation is
what makes the union lemma's hard case disappear.

## At least three vertices

The counting clause of 2-connectivity is carried by `SchoenfliesGraph.HasThreeVertices`, "three pairwise
distinct vertices", rather than by `3 ≤ V(G).ncard`. The two agree on a finite graph, but the
existential form is **monotone with no finiteness hypothesis** (`SchoenfliesGraph.HasThreeVertices.mono`),
and monotonicity is precisely what `SchoenfliesGraph.IsTwoConnected.union` needs: the union of two graphs
...
What consumers actually use is never the clause itself but `SchoenfliesGraph.HasThreeVertices.exists_ne_ne`
— any two vertices leave a third over. `SchoenfliesGraph.hasThreeVertices_iff_ncard` is the bridge back to
the count, for a consumer that has one.

## The convention, verbatim, and what it costs

`SchoenfliesGraph.IsTwoConnected` is the blueprint's definition word for word (§"Plane graphs and the
...
Both are instances of `SchoenfliesGraph.not_isTwoConnected_banana`: `SchoenfliesGraph.banana u v F` has only the two
...
Mathlib's `Mathlib/Combinatorics/SchoenfliesGraph/Lattice.lean` provides only the *infimum* of two
graphs — there is no union in this version of Mathlib — so `SchoenfliesGraph.union` is defined here. On
...
graphs are `SchoenfliesGraph.Compatible` — in particular when both are subgraphs of one graph, via
`SchoenfliesGraph.Compatible.of_le_le`, which is the only way the plane layer forms unions — the
disjunction collapses to the naive one (`SchoenfliesGraph.Compatible.union_isLink`) and both graphs are
subgraphs of the union.

## Blueprint

* `SchoenfliesGraph.IsTwoConnected` — the definition in §"Plane graphs and the utility graph": "a finite
...
* `SchoenfliesGraph.IsTwoConnected.no_cut_vertex` — "with this convention a 2-connected graph has no cut
  vertex".
* `SchoenfliesGraph.not_isTwoConnected_banana` — "the one-edge graph is not 2-connected", and the same
  computation for the two-vertex cycle of the next paragraph.
* `SchoenfliesGraph.IsTwoConnected.no_bridge` — "A 2-connected graph has no bridge", the step of
...
* `SchoenfliesGraph.IsTwoConnected.union` — `lem:union-two-connected`, "if two finite 2-connected graphs
  have at least two vertices in common, their union is 2-connected".
-/

open Set

variable {α β : Type*} {G H K G₁ G₂ : SchoenfliesGraph α β} {a b s t u v w x y z : α} {e f : β}
...
theorem deleteVerts_singleton_eq_self (hx : x ∉ V(G)) : G.deleteVerts {x} = G := by
  rw [deleteVerts, diff_singleton_eq_self hx, induce_vertexSet]
...
theorem deleteVerts_mono (h : G ≤ H) (X : Set α) : G.deleteVerts X ≤ H.deleteVerts X where
  vertexSet_mono := by
    ...
    exact diff_subset_diff_left h.vertexSet_mono
...
def IsCutVertex (G : SchoenfliesGraph α β) (x : α) : Prop :=
...
def HasThreeVertices (G : SchoenfliesGraph α β) : Prop :=
...
structure IsTwoConnected (G : SchoenfliesGraph α β) : Prop where
...
`SchoenfliesGraph.Compatible` graphs, which is the only case the development forms, the disjunction
collapses to the naive one. -/
protected def union (G H : SchoenfliesGraph α β) : SchoenfliesGraph α β where
  vertexSet := V(G) ∪ V(H)
  edgeSet := E(G) ∪ E(H)
  IsLink e x y := G.IsLink e x y ∨ (e ∉ E(G) ∧ H.IsLink e x y)
  isLink_symm := by
    intro e he x y h
    rcases h with hxy | ⟨heG, hxy⟩
    · exact Or.inl hxy.symm
    · exact Or.inr ⟨heG, hxy.symm⟩
...
@[simp]
theorem vertexSet_union (G H : SchoenfliesGraph α β) : V(G.union H) = V(G) ∪ V(H) := rfl

@[simp]
theorem edgeSet_union (G H : SchoenfliesGraph α β) : E(G.union H) = E(G) ∪ E(H) := rfl

theorem union_isLink :
    (G.union H).IsLink e x y ↔ G.IsLink e x y ∨ (e ∉ E(G) ∧ H.IsLink e x y) := Iff.rfl

theorem left_le_union (G H : SchoenfliesGraph α β) : G ≤ G.union H where
...
deleted from it, and `SchoenfliesGraph.deleteVerts_singleton_eq_self` makes that a rewriting step rather
than an argument. -/
