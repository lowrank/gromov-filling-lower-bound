...
`SchoenfliesGraph.Reaches.reroute` says the obvious thing in the one form every consumer wants: if every
...
`e` is not an edge of `G` at all: it is then vacuous. `SchoenfliesGraph.Reaches.across_edge_replacement`
...
* reroute around the old graph — the deleted vertex *is* on the path, so the old graph is
  untouched and the replaced edge was not a bridge (`SchoenfliesGraph.IsTwoConnected.no_bridge`);
...
`SchoenfliesGraph.IsTwoConnected.replace_edge_by_path` replaces an edge by a whole path, not by a single
...
`SchoenfliesGraph.IsTwoConnected.ear` attaches an ear, and the blueprint's "whose internal vertices are
new" is **not** assumed — the proof does not use it. What carries the argument is
`SchoenfliesGraph.IsPathGraph.reaches_an_end`: whatever vertex is deleted, every remaining vertex of the
...
`SchoenfliesGraph.IsTwoConnected.ear_exists` needs neither the components of `G - V(H)` nor a shortest
...
(`SchoenfliesGraph.IsWalk.crossing`), and walk back from that edge's outer end to a *different* vertex of
...
`SchoenfliesGraph.pathGraphOf` turns it into a graph and `SchoenfliesGraph.IsTwoConnected.grows_by_ear` glues it on.
...
`SchoenfliesGraph.IsTwoConnected.ear_exists` is the one statement here that has to assume the graph has
...
* `SchoenfliesGraph.IsTwoConnected.replace_edge_by_path` — `lem:subdivision-ear-preserve` (a), "every
  subdivision of `G` is 2-connected", stated for a replacement path of any length.
* `SchoenfliesGraph.IsTwoConnected.ear` — `lem:subdivision-ear-preserve` (b), "if `P` is a path whose
  distinct endpoints lie in `G` … then `G ∪ P` is 2-connected".
* `SchoenfliesGraph.IsTwoConnected.ear_exists`, `SchoenfliesGraph.IsTwoConnected.grows_by_ear` — the step of
  `lem:relative-ear`, "`G` is obtained from `H` by repeatedly adding paths whose distinct
  endpoints are already present".
* `SchoenfliesGraph.Reaches.reroute`, `SchoenfliesGraph.Reaches.across_edge_replacement` — the blueprint's "the
...
variable {α β : Type*} {G H K P : SchoenfliesGraph α β} {a b c s t u v x y z : α} {e f : β}
  {W D : List β}

/-! ### The union of two subgraphs

`SchoenfliesGraph.union` and the two inclusions *into* it live in `Schoenflies/SchoenfliesGraph/TwoConnected.lean`;
...
`hends` premise of `SchoenfliesGraph.Reaches.reroute` is vacuous.
...
(`SchoenfliesGraph.IsPathGraph.reaches_an_end`) — an end which, having been reached after the deletion, is
...
old side reaches it by `SchoenfliesGraph.Reaches.across_edge_replacement`, whose `routed` premise is
...
theorem IsTwoConnected.replace_edge_by_path (hG : G.IsTwoConnected) (hl : G.IsLink e u v)
  ...
  have key : ∀ x ∈ V(G), x ≠ c → (S.deleteVerts {c}).Reaches x z := by
    ...
    · rw [vertexSet_deleteVerts, vertexSet_deleteVerts]
      exact Set.diff_subset_diff_left hVGS
    · intro g p q hg hge
...
theorem IsTwoConnected.grows_by_ear (hG : G.IsTwoConnected) (hK : K.IsTwoConnected)
    ...
    (hproper : (∃ x ∈ V(G), x ∉ V(K)) ∨ (∃ g ∈ E(G), g ∉ E(K))) :
    ∃ B : SchoenfliesGraph α β, B.IsTwoConnected ∧ K ≤ B ∧ B ≤ G ∧ ∃ g ∈ E(B), g ∉ E(K) := by
