...
This file sits directly on `Schoenflies/SchoenfliesGraph/Walk.lean` and answers one question: which
...
hand, every repetition that a closed walk can contain, whereas `SchoenfliesGraph.IsPath` already forbids
...
Mathlib's `SchoenfliesGraph` allows an edge with both ends equal. **A loop is taken to be a cycle of
...
not use `e`, is a cycle through `e`. So `SchoenfliesGraph.liesOnCycle_of_isLoopAt` needs no proof, no
graph is acyclic while carrying a loop, and no bridge is a loop. Nothing in this file
hypothesises looplessness.

## Deletion

Edge deletion is Mathlib's `SchoenfliesGraph.deleteEdges`, not a new definition; `G.deleteEdges {e}` is
...
subgraph can answer differently. `SchoenfliesGraph.IsWalk.anti` from `Walk.lean` is the engine;
`SchoenfliesGraph.IsPath.anti` is proved here, since freshness has to be carried down too.

## Blueprint

* `SchoenfliesGraph.liesOnCycle_iff_deleteEdges_reaches` — `lem:cycle-criterion`, "in a finite graph, an
  ...
  `SchoenfliesGraph.LiesOnCycle.deleteEdges_reaches` and `SchoenfliesGraph.LiesOnCycle.of_deleteEdges_reaches`.
* `SchoenfliesGraph.IsBridge`, `SchoenfliesGraph.isBridge_iff_not_reaches` — the bridges of
...
* `SchoenfliesGraph.IsAcyclic`, `SchoenfliesGraph.Connected.deleteEdges_singleton` — the acyclicity underneath
...
* `SchoenfliesGraph.IsPath.chord_liesOnCycle`, `SchoenfliesGraph.IsAcyclic.mem_of_isLink_of_mem_walkVertices` — the
...
variable {α β : Type*} {G H : SchoenfliesGraph α β} {u v w x y : α} {e f : β} {F : Set β} {W P D : List β}
...
`SchoenfliesGraph.deleteEdges` is Mathlib's. These are the four transfer lemmas every deletion argument
below reduces to. -/
...
def IsCycleThrough (G : SchoenfliesGraph α β) (e : β) (u v : α) (D : List β) : Prop :=
  G.IsLink e u v ∧ G.IsPath u D v ∧ e ∉ D

/-- `G.LiesOnCycle e` : the edge `e` lies on a cycle of `G`. -/
def LiesOnCycle (G : SchoenfliesGraph α β) (e : β) : Prop := ∃ u v D, G.IsCycleThrough e u v D
...
around: `SchoenfliesGraph.IsLink` already forces it. -/
...
is how a tree consumes it — every edge is a bridge — while `SchoenfliesGraph.IsAcyclic.not_liesOnCycle`
drops the membership hypothesis for the uses that do not have it to hand. -/
def IsAcyclic (G : SchoenfliesGraph α β) : Prop := ∀ ⦃e⦄, e ∈ E(G) → ¬ G.LiesOnCycle e
...
(`SchoenfliesGraph.isBridge_iff_not_reaches`). -/
def IsBridge (G : SchoenfliesGraph α β) (e : β) : Prop := e ∈ E(G) ∧ ¬ G.LiesOnCycle e
