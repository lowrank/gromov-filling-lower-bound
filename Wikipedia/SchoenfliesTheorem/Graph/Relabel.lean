...
Mathlib's `SchoenfliesGraph.map` changes vertex names and deliberately leaves edge names fixed.  The ear
...
variable {α β δ : Type*} {G : SchoenfliesGraph α β} {f : β → δ}

/-- Relabel every edge of `G` by a map injective on `E(G)`, without changing its vertices. -/
def relabelEdges (G : SchoenfliesGraph α β) (f : β → δ) (hf : InjOn f E(G)) : SchoenfliesGraph α δ where
  vertexSet := V(G)
  edgeSet := f '' E(G)
...
@[simp] theorem vertexSet_relabelEdges (G : SchoenfliesGraph α β) (f : β → δ) (hf : InjOn f E(G)) :
    V(G.relabelEdges f hf) = V(G) := rfl

@[simp] theorem edgeSet_relabelEdges (G : SchoenfliesGraph α β) (f : β → δ) (hf : InjOn f E(G)) :
    E(G.relabelEdges f hf) = f '' E(G) := rfl

@[simp] theorem relabelEdges_isLink (G : SchoenfliesGraph α β) (f : β → δ) (hf : InjOn f E(G))
    (d : δ) (x y : α) :
...
old edge. -/
theorem relabelEdges_inc (G : SchoenfliesGraph α β) (f : β → δ) (hf : InjOn f E(G))
    (d : δ) (x : α) :
...
theorem IsPathGraph.relabelEdges {P : SchoenfliesGraph α β} (hf : InjOn f E(P))
...
theorem Finite.relabelEdges [G.Finite] (hf : InjOn f E(G)) :
  ...
  finite_vertexSet := by
    ...
    exact _root_.SchoenfliesGraph.finite_vertexSet G
  finite_edgeSet := by
    rw [edgeSet_relabelEdges]
    exact (_root_.SchoenfliesGraph.finite_edgeSet G).image f
...
variable {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}

/-- The drawing with its edge argument translated back through an injective relabelling. -/
noncomputable def relabelDrawing [Nonempty β] (G : SchoenfliesGraph α β) (f : β → δ)
