...
Degree counting on Mathlib's multigraph `SchoenfliesGraph α β`, for graphs with finitely many vertices
...
```
class SchoenfliesGraph.Finite (G : SchoenfliesGraph α β) : Prop
```
...
* It is a class, so it is silent at the call site — the statements below read like ordinary
  ...
  price is that a subgraph does not get an instance for free; `SchoenfliesGraph.Finite.of_le` produces
...
* `SchoenfliesGraph.sum_degree_eq_two_mul_ncard_edgeSet` — "the degree sum is twice the number of
  edges", the counting step in the proof of Lemma `lem:three-leaf-tree` (Trees with three
  leaves). The whole content is `SchoenfliesGraph.ncard_inc_add_ncard_isLoopAt`: an edge has exactly two
  ends.
* `SchoenfliesGraph.IsLeaf` — "the vertices of degree one are the leaves", same lemma.

## Namespace

Everything lives in the root `SchoenfliesGraph` namespace rather than in `Schoenflies`, so that dot
notation `G.degree`, `G.IsLeaf`, `G.vertexFinset` works on a `G : SchoenfliesGraph α β`. Later graph
...
variable {α β : Type*} {G H : SchoenfliesGraph α β} {e f : β} {x y : α}
...
class Finite (G : SchoenfliesGraph α β) : Prop where
...
theorem finite_vertexSet (G : SchoenfliesGraph α β) [G.Finite] : V(G).Finite := Finite.finite_vertexSet

theorem finite_edgeSet (G : SchoenfliesGraph α β) [G.Finite] : E(G).Finite := Finite.finite_edgeSet
...
recovered from the goal — so install it with `haveI := SchoenfliesGraph.Finite.of_le h`. -/
theorem Finite.of_le (h : H ≤ G) [G.Finite] : H.Finite :=
  ⟨(_root_.SchoenfliesGraph.finite_vertexSet G).subset h.vertexSet_mono,
   (_root_.SchoenfliesGraph.finite_edgeSet G).subset h.edgeSet_mono⟩

/-- The vertex set of a finite graph, as a `Finset`. -/
noncomputable def vertexFinset (G : SchoenfliesGraph α β) [G.Finite] : Finset α :=
  (finite_vertexSet G).toFinset

/-- The edge set of a finite graph, as a `Finset`. -/
noncomputable def edgeFinset (G : SchoenfliesGraph α β) [G.Finite] : Finset β :=
...
theorem incidenceSet_eq_setOf (G : SchoenfliesGraph α β) (x : α) : G.incidenceSet x = {e | G.Inc e x} :=
  rfl

theorem loopSet_eq_setOf (G : SchoenfliesGraph α β) (x : α) : G.loopSet x = {e | G.IsLoopAt e x} :=
  rfl
...
noncomputable def degree (G : SchoenfliesGraph α β) (x : α) : ℕ :=
  (G.incidenceSet x).ncard + (G.loopSet x).ncard

theorem degree_def (G : SchoenfliesGraph α β) (x : α) :
...
theorem ncard_inc_add_ncard_isLoopAt (G : SchoenfliesGraph α β) (he : e ∈ E(G)) :
...
(`SchoenfliesGraph.ncard_inc_add_ncard_isLoopAt`). Loops are allowed — that is exactly what the loop
term in `SchoenfliesGraph.degree` buys. -/
theorem sum_degree_eq_two_mul_ncard_edgeSet (G : SchoenfliesGraph α β) [G.Finite] :
...
theorem sum_degree_eq_two_mul_card_edgeFinset (G : SchoenfliesGraph α β) [G.Finite] :
...
theorem even_sum_degree (G : SchoenfliesGraph α β) [G.Finite] :
...
def IsLeaf (G : SchoenfliesGraph α β) (x : α) : Prop := x ∈ V(G) ∧ G.degree x = 1
