...
`SchoenfliesGraph.IsIsomorphism` is built or needed.

## Three things the design did not have, and this module supplies

**Simple polygonal paths.** `SchoenfliesGraph.IsDrawing.edge_param` asks for an *injective*
...
**The contact point is on the frame.** `SchoenfliesGraph.IsDrawing.exists_core` states that the core meets
...
* `PolyReaches.exists_arcList` — **every polygonal path contains a simple one**; the deferred
  half of Lemma 1.1.
* `SchoenfliesGraph.polygonal_redrawing` — `lem:polygonal-redrawing`, with bricks B7 and B8 inside its
...
`SchoenfliesGraph.IsDrawing`. The induction is over the inductive relation `PolyReaches`, and the step
...
clause of `SchoenfliesGraph.IsDrawing`.
...
of `Schoenflies/SchoenfliesGraph/VertexSquares.lean` is exactly that. The old path is truncated there and
...
`SchoenfliesGraph.IsDrawing.exists_core` says the core meets the square about `v` in exactly one point.
...
isomorphism is the identity and no `SchoenfliesGraph.IsIsomorphism` appears.
...
theorem polygonal_redrawing {β : Type*} (G : SchoenfliesGraph Plane β) [G.Finite]
  ...
  have hMloc : IsLocallyPolyConn' M :=
    isLocallyPolyConn'_compl_biUnion (SchoenfliesGraph.finite_vertexSet (G := G)) (fun _ => r) hsqdisj
  ...
  obtain ⟨ε, hε, hεall⟩ : ∃ ε > 0, ∀ e ∈ E(G), ∀ f ∈ E(G), f ≠ e →
    ...
    refine exists_pos_forall_of_finite (SchoenfliesGraph.finite_edgeSet (G := G))
