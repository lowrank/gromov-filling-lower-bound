...
Stated for an ambient graph `K` that the two paths cover, rather than for `SchoenfliesGraph.union`, so
...
* `SchoenfliesGraph.isTwoConnected_of_two_paths` — the cycle base case of `lem:union-two-connected`.
-/
...
variable {α β : Type*} {K P Q : SchoenfliesGraph α β} {u v c : α} {W₁ W₂ : List β}
...
(`SchoenfliesGraph.IsPathGraph.reaches_an_end`).  If `c` is neither end, then `c` misses one of the two
