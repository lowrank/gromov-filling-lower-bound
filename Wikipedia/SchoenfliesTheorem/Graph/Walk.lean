...
-/
import SchoenfliesCompat.Graph.Subgraph
...
Mathlib's `SchoenfliesGraph α β` carries incidence (`G.IsLink e x y`) and nothing that moves along it.
...
`SchoenfliesGraph.IsWalk.contains_path`, the blueprint's "shorten an arbitrary walk at repeated
vertices".
...
(`SchoenfliesGraph.IsLink.right_unique` upstream, `SchoenfliesGraph.IsWalk.target_unique` here).
...
at; concatenation (`SchoenfliesGraph.IsWalk.append`) and reversal (`SchoenfliesGraph.IsWalk.reverse`) are proved
...
vertex of the graph with *no* side hypothesis, because Mathlib's `SchoenfliesGraph` already forces the
...
Mathlib's `SchoenfliesGraph` allows an edge with both ends equal; the blueprint's graphs have none.
...
not a loop (`SchoenfliesGraph.IsPath.not_isLoopAt`). Imposing looplessness on the graph instead would
put a hypothesis on every statement in the layer to rule out something the definitions
already rule out.

## Namespace

Declarations live in the root `SchoenfliesGraph` namespace, extending Mathlib's, rather than in
...
* `SchoenfliesGraph.IsWalk.contains_path` — shortening a walk to a path; used at
...
* `SchoenfliesGraph.Reaches`, `SchoenfliesGraph.Connected` — the connectedness those three lemmas invoke.
* `SchoenfliesGraph.IsPath.nodup` — a simple path repeats no edge; underneath the finiteness arguments
  that bound a path's length.
* `SchoenfliesGraph.IsPath.reverse`, `SchoenfliesGraph.IsPath.split` — a path backwards, and a path cut in two at a
  vertex it visits; what `lem:subdivision-ear-preserve` and `lem:relative-ear` run on.
-/

open Set

variable {α β : Type*} {G H : SchoenfliesGraph α β} {u v w x y z : α} {e f : β} {W W₁ W₂ P : List β}
...
def coveredVertices (G : SchoenfliesGraph α β) (W : List β) : Set α := {x | ∃ e ∈ W, G.Inc e x}
...
def walkVertices (G : SchoenfliesGraph α β) (u : α) (W : List β) : Set α := insert u (G.coveredVertices W)
...
inductive IsWalk (G : SchoenfliesGraph α β) : α → List β → α → Prop
...
edge has only two ends, `SchoenfliesGraph.IsLink.right_unique`, which holds in Mathlib's `SchoenfliesGraph` with no
...
inductive IsPath (G : SchoenfliesGraph α β) : α → List β → α → Prop
...
engine of `SchoenfliesGraph.IsWalk.contains_path`: when the next vertex is already on the path built so
...
`SchoenfliesGraph.IsPath.from_visited` is the suffix alone; the prefix is the awkward half, since the
...
cut the path already built (`SchoenfliesGraph.IsPath.from_visited`). -/
...
condition — and a path is recovered on demand (`SchoenfliesGraph.Reaches.exists_isPath`). -/
def Reaches (G : SchoenfliesGraph α β) (u v : α) : Prop := ∃ W, G.IsWalk u W v
...
true as stated. -/
def Connected (G : SchoenfliesGraph α β) : Prop :=
...
the companion of `SchoenfliesGraph.IsWalk.anti`, which it is otherwise identical to. Freshness comes
