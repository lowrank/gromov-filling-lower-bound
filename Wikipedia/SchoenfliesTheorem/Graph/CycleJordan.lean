...
A cycle of a plane graph arrives as `Schoenflies/SchoenfliesGraph/Cycle.lean` presents it: an edge, and a
...
Exactly once, in `SchoenfliesGraph.IsDrawing.first_edge_meets_rest`: a common point of the first edge's
arc and the rest of the path is a vertex incident with both edges
(`SchoenfliesGraph.IsDrawing.arcs_meet_at_vertex`), hence one of the two ends of the first edge, and the
...
Nothing here hypothesises looplessness, and nothing needs to: `SchoenfliesGraph.IsDrawing.ne_of_isLink`
...
degenerate cycle `SchoenfliesGraph.liesOnCycle_of_isLoopAt` — a loop with an empty detour — whose
realisation is a single arc and not a Jordan curve.

## Blueprint

* `SchoenfliesGraph.edgesCover` — what a *list* of edges occupies. `SchoenfliesGraph.pointSet` is the vertices
  together with this, taken over all the edges of the graph.
* `SchoenfliesGraph.IsDrawing.path_isArcBetween` — the realisation of a path is an arc between its two
  ends.
* `SchoenfliesGraph.IsDrawing.cycle_isJordanCurve` — §"Plane graphs and the utility graph": "in a plane
...
* `SchoenfliesGraph.IsDrawing.cycle_inter` — the meeting condition in full: the detour and the edge meet
...
step is `SchoenfliesGraph.IsDrawing.first_edge_meets_rest`. -/

/-- **The first edge's arc meets the rest of the path only at the waypoint.** Two cases, and
neither is geometric beyond the workhorse `SchoenfliesGraph.IsDrawing.arcs_meet_at_vertex`: if the rest
...
(`SchoenfliesGraph.IsDrawing.first_edge_meets_rest`), so `Schoenflies.IsArcBetween.concatenate` glues
them. -/
...
`SchoenfliesGraph.IsDrawing.detour_meets_edge` is what `Schoenflies.IsJordanCurve.of_two_arcs` consumes;
