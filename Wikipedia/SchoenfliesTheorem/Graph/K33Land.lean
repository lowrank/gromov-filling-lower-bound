...
`Schoenflies/SchoenfliesGraph/K33Planar.lean` proves the nonplanarity of `K(3,3)` from
`SchoenfliesGraph.IsHexRealization`; `Schoenflies/SchoenfliesGraph/K33Closed.lean` reduces that to `SchoenfliesGraph.Bendable` —
...
`Schoenflies/SchoenfliesGraph/K33Closed.lean` found three realizations independently and matched them with
...
* `Schoenflies.IsPrePolygonalCrosscut` and its API up to
  ...
  corners.
* `SchoenfliesGraph.IsPreHexCrosscut`, `SchoenfliesGraph.IsK33Config.isPreHexCrosscut` — the six-cycle of a polygonally
  drawn `K(3,3)`, cut at the two ends of one remaining edge, as such a crosscut. **This is what
  `SchoenfliesGraph.IsHexRealization` was assuming, and it is proved here.**
* `SchoenfliesGraph.IsK33Config.false_of_isPreHexCrosscut`, `…false_of_polygonal`,
  `SchoenfliesGraph.IsK33Config.not_exists_isDrawing` — `lem:k33`.
* `SchoenfliesGraph.IsArcK33.elim`, `SchoenfliesGraph.IsK33Subdivision.elim` — `cor:k33-subdivision`.
* `SchoenfliesGraph.k33Graph_not_exists_isDrawing` — `lem:k33` for the concrete graph.
...
|---------------------------------------------------|--------------------------------------------|
| `SchoenfliesGraph.IsK33Config.not_isDrawing` (K33Planar)      | `SchoenfliesGraph.IsK33Config.not_exists_isDrawing`   |
| `SchoenfliesGraph.IsK33Config.not_isDrawing_of_bendable`      | `SchoenfliesGraph.IsK33Config.not_exists_isDrawing`   |
| `SchoenfliesGraph.IsArcK33.false_of_realization`/`_bendable`  | `SchoenfliesGraph.IsArcK33.elim`                      |
| `SchoenfliesGraph.IsK33Subdivision.false_of_realization`/`…`  | `SchoenfliesGraph.IsK33Subdivision.elim`              |
| `SchoenfliesGraph.k33Graph_not_isDrawing`                     | `SchoenfliesGraph.k33Graph_not_exists_isDrawing`      |

With this module in place `SchoenfliesGraph.IsHexRealization`, `SchoenfliesGraph.IsHexCrosscut`, `SchoenfliesGraph.IsHexGeneric`
and `SchoenfliesGraph.Bendable` have no consumers left.
-/
...
`Schoenflies.ClosedPolygon` version is in `Schoenflies/SchoenfliesGraph/K33Closed.lean`, and this is the
...
variable {β : Type*} {G : SchoenfliesGraph Plane β} {x y : Fin 3 → Plane} {e : Fin 3 → Fin 3 → β}
variable {drawing : β → ℝ → Plane} {s : Fin 3} {R : Set Plane}

/-! ## The six-cycle and one remaining edge, as a crosscut

`SchoenfliesGraph.IsHexCrosscut` of `Schoenflies/SchoenfliesGraph/K33Planar.lean` is the same statement with
...
`SchoenfliesGraph.IsK33Config.not_isDrawing` of `Schoenflies/SchoenfliesGraph/K33Planar.lean` with its realization
hypothesis discharged, and `SchoenfliesGraph.IsK33Config.not_isDrawing_of_bendable` of
`Schoenflies/SchoenfliesGraph/K33Closed.lean` with `SchoenfliesGraph.Bendable` discharged: no drawing has to be bent,
...
has a plane drawing. -/
theorem IsK33Subdivision.elim {H : SchoenfliesGraph Plane β} {W : Fin 3 → Fin 3 → List β}
...
`SchoenfliesGraph.k33Graph x y`, whose nine edges are the index pairs, with nothing assumed beyond the six
