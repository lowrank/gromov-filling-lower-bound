...
`Schoenflies/SchoenfliesGraph/K33Planar.lean` proves the nonplanarity of `K(3,3)` from one assumption,
`SchoenfliesGraph.IsHexRealization`: that the six-cycle, the two arcs the chord cuts it into, and the two
...
* `Schoenflies.exists_poly_anchored`, `Schoenflies.IsPolygonal.union`,
  `SchoenfliesGraph.IsDrawing.isPolygonal_edgesCover` — §1: a polygonal set is the carrier of a vertex list
...
* `SchoenfliesGraph.IsK33Config.isPolygonal_hexSet`, `…chord_inter_arcA`, `…chord_inter_arcB` — the three
  curves the crosscut is built from, as polygonal Jordan curves.
* `SchoenfliesGraph.IsHexGeneric` — the general-position hypothesis that survives: at each of the two cut
  points, the six-cycle and both spliced curves turn.
* `SchoenfliesGraph.IsK33Config.isHexRealization` — `SchoenfliesGraph.IsHexRealization`, discharged from it. **This is
  the theorem this module exists for.**
* `SchoenfliesGraph.Bendable`, `SchoenfliesGraph.IsK33Config.false_of_hexGeneric`, `…not_isDrawing_of_bendable`,
  `SchoenfliesGraph.IsArcK33.false_of_bendable`, `SchoenfliesGraph.IsK33Subdivision.false_of_bendable`,
  `SchoenfliesGraph.k33Graph_not_isDrawing` — `lem:k33` and `cor:k33-subdivision` with `IsHexRealization`
  replaced by `IsHexGeneric`.

## What is still assumed, and why — READ THIS BEFORE USING THE THEOREMS BELOW

**Milestone H8 is not closed here.** `SchoenfliesGraph.IsHexRealization` is gone, but a strictly smaller
...
crosscut forms with the two arcs. `SchoenfliesGraph.IsHexGeneric` says exactly that and nothing more, and
...
genuinely about changing the drawing, not about presentations. `SchoenfliesGraph.Bendable` is that
...
`SchoenfliesGraph.IsHexRealization` asks for three closed polygons that agree on the arcs they share. They
...
/-- **`SchoenfliesGraph.IsHexRealization`, discharged.** The six-cycle and the two curves the remaining edge
...
to, assuming `SchoenfliesGraph.Bendable`. -/
...
drawing, assuming `SchoenfliesGraph.Bendable`, and only for the *contracted* graph `k33Graph x y`,
whose nine edges are the branch paths. -/
theorem IsK33Subdivision.false_of_bendable {H : SchoenfliesGraph Plane β} {W : Fin 3 → Fin 3 → List β}
...
`SchoenfliesGraph.k33Graph x y`, whose nine edges are the index pairs. Assuming `SchoenfliesGraph.Bendable`, the
