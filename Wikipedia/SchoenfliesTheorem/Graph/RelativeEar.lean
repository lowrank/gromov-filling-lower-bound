...
import Wikipedia.SchoenfliesTheorem.Graph.Tree
...
* `SchoenfliesGraph.IsTwoConnected.relative_ear_exists` — the **relative** ear: the blueprint's own
  construction, through a component of `G - V(K)`, which is what makes the internal vertices
  new. `SchoenfliesGraph.IsTwoConnected.ear_exists` in `Schoenflies/SchoenfliesGraph/Ear.lean` finds *an* ear by a
...
* `SchoenfliesGraph.IsTwoConnected.ear_decomposition` — the **iterated** form, as an induction principle.
  `SchoenfliesGraph.IsTwoConnected.grows_by_ear` is deliberately one step; the finiteness induction the
...
All of that is already proved: `SchoenfliesGraph.IsTwoConnected.exists_path_through_component` in
`Schoenflies/SchoenfliesGraph/Component.lean` packages the component argument and the path through it.
...
`SchoenfliesGraph.IsTwoConnected.ear_exists`, that branch — and only that branch — needs `G` to be
...
process stops exactly when `B = G`. The stopping step is `SchoenfliesGraph.eq_of_le_of_subset_subset`: a
...
`Schoenflies/SchoenfliesGraph/Tree.lean` is imported for a single two-line fact, `SchoenfliesGraph.IsPath.ne_nil` —
...
`SchoenfliesGraph.IsPath.ne_nil` into `Schoenflies/SchoenfliesGraph/Walk.lean`, where it belongs, and drop the
...
* `SchoenfliesGraph.IsTwoConnected.relative_ear_exists` — `lem:relative-ear`, the ear itself: "a shortest
...
* `SchoenfliesGraph.IsTwoConnected.relative_grows_by_ear` — the same, glued on: one step of "`G` is
  obtained from `H` by repeatedly adding paths".
* `SchoenfliesGraph.IsTwoConnected.ear_decomposition` — `lem:relative-ear` in full, including
  "finiteness terminates the process", as an induction principle.

## Namespace

Root `SchoenfliesGraph`, as fixed by `Schoenflies/SchoenfliesGraph/Walk.lean`.
-/
...
variable {α β : Type*} {G K B : SchoenfliesGraph α β} {a b x y : α} {e f : β} {D : List β}

/-! ### Two facts about a subgraph inclusion

Both are general and neither is in `Schoenflies/SchoenfliesGraph/Walk.lean`; the integrator may want to
hoist them. -/
...
distinguishes it from `SchoenfliesGraph.IsTwoConnected.ear_exists`. The first branch takes a component
`L` of `G - V(K)` and a path across it between two of its neighbours in `K`
(`SchoenfliesGraph.IsTwoConnected.exists_path_through_component`); the two distinct vertices of `V(K)`
...
`SchoenfliesGraph.IsTwoConnected.relative_ear_exists`, glued on with `SchoenfliesGraph.IsTwoConnected.ear`: the
...
`Schoenflies/SchoenfliesGraph/TwoConnected.lean` and `Schoenflies/SchoenfliesGraph/PathGraph.lean`; they are here
only so that no consumer has to rediscover which two to combine. -/

@[simp]
theorem vertexSet_union_pathGraphOf (B G : SchoenfliesGraph α β) (a : α) (D : List β) :
...
theorem IsTwoConnected.ear_decomposition {motive : SchoenfliesGraph α β → Prop} [G.Finite]
    (hG : G.IsTwoConnected) (hnl : ∀ ⦃g x⦄, ¬ G.IsLoopAt g x)
    (hK : K.IsTwoConnected) (hKG : K ≤ G) (base : motive K)
    (step : ∀ (B : SchoenfliesGraph α β) (a b : α) (D : List β), B.IsTwoConnected → K ≤ B → B ≤ G →
  ...
  have hstep : ∀ B : SchoenfliesGraph α β, B ≠ G → B.IsTwoConnected → K ≤ B → B ≤ G → motive B →
      ∃ B' : SchoenfliesGraph α β, B'.IsTwoConnected ∧ K ≤ B' ∧ B' ≤ G ∧ motive B' ∧
    ...
    refine Set.ncard_lt_ncard ⟨fun f hf ↦ ⟨hf.1, fun hfB ↦ hf.2 (hBB'.edgeSet_mono hfB)⟩, ?_⟩
      ((finite_edgeSet G).subset diff_subset)
  ...
  have key : ∀ n, ∀ B : SchoenfliesGraph α β, (E(G) \ E(B)).ncard ≤ n → B.IsTwoConnected → K ≤ B →
