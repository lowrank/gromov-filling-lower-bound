...
a vertex of the graph (`SchoenfliesGraph.IsWalk.nil`), so `G.Reaches u v` carries `v ∈ V(G)` with it, and
...
`SchoenfliesGraph.iUnion_component` true with no side condition.
...
(`SchoenfliesGraph.IsWalk.walkVertices_subset_component`), so it is already a walk of the induced
subgraph, by `SchoenfliesGraph.IsWalk.anti`.

## Shortest walks need no finiteness

The set of lengths of walks from `u` to `v` is a nonempty set of naturals, so it has a least
element — `Nat.find`, with no `SchoenfliesGraph.Finite` hypothesis anywhere. That is worth noticing: the
...
A minimum-length walk is automatically a path (`SchoenfliesGraph.IsWalk.isPath_of_minLength`). The proof
is the induction of `SchoenfliesGraph.IsWalk.contains_path` run once more, with lengths attached: if the
step's source were revisited later, `SchoenfliesGraph.IsPath.from_visited` would cut the tail down to a
strictly shorter walk with the same ends, and a path's edge list repeats nothing
(`SchoenfliesGraph.IsPath.nodup`), so the cut piece really is no longer than what it came from
...
Here the two neighbours are *constructed*, in the shape `SchoenfliesGraph.IsTwoConnected.no_bridge`
already uses. A walk from the component out to a vertex of `S` crosses the boundary
(`SchoenfliesGraph.IsWalk.crossing`), and the crossing edge's far end is in `S` — because a neighbour
...
* `SchoenfliesGraph.component`, `SchoenfliesGraph.induce_component_connected` — "a component `L` of `G - V(H₀)`" in
  the proof of `lem:relative-ear`.
* `SchoenfliesGraph.exists_minLength_isPath` — "a shortest path through `L`", same proof.
* `SchoenfliesGraph.IsTwoConnected.component_two_neighbors` — "if a component `L` of `G - V(H₀)` contains
  a vertex, it has at least two distinct neighbors in `H₀`; otherwise its unique neighbor
  would be a cut vertex of `G`".
* `SchoenfliesGraph.IsTwoConnected.exists_path_through_component` — "a shortest path through `L` between
...
variable {α β : Type*} {G H : SchoenfliesGraph α β} {u v w x y z a b c d p q : α} {e f : β}
  {W W' P Q : List β} {S : Set α}

/-! ### Components -/

/-- The component of `u` in `G`: the vertices `u` reaches. A comprehension over the vertices,
not an indexed union of connected subgraphs; the membership test is `SchoenfliesGraph.Reaches` itself,
which is why every closure property below is one line of the reachability API.

No `v ∈ V(G)` clause: `SchoenfliesGraph.Reaches` already carries it, since the empty walk demands a
vertex of the graph. For `u ∉ V(G)` the component is empty. -/
def component (G : SchoenfliesGraph α β) (u : α) : Set α := {v | G.Reaches u v}
...
theorem iUnion_component (G : SchoenfliesGraph α β) : ⋃ u ∈ V(G), G.component u = V(G) :=
...
tail could be cut there (`SchoenfliesGraph.IsPath.from_visited`) and the result would be a strictly
...
theorem Connected.exists_minLength_isPath (h : G.Connected) (hu : u ∈ V(G)) (hv : v ∈ V(G)) :
    ∃ P, G.IsPath u P v ∧ ∀ W, G.IsWalk u W v → P.length ≤ W.length :=
  _root_.SchoenfliesGraph.exists_minLength_isPath (h.reaches hu hv)
...
This is `SchoenfliesGraph.IsWalk.crossing` plus the previous lemma: the crossing edge's far end is
outside the component, so it cannot be outside `S`. -/
theorem exists_isLink_out_of_component {K : SchoenfliesGraph α β} (hKG : K ≤ G)
