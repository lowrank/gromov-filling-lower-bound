...
There is no `PlaneGraph` type. A plane graph is an abstract `G : SchoenfliesGraph Plane β` **together
with** a drawing `drawing : β → ℝ → Plane`, related by `IsDrawing G drawing`. The abstract
graph stays itself, so every combinatorial theorem of `Schoenflies/SchoenfliesGraph/` applies with
...
variable {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {base : Plane}
...
structure IsDrawing (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) : Prop where
...
def pointSet (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) : Set Plane :=
...
theorem IsDrawing.isCompact_pointSet [G.Finite] (h : IsDrawing G drawing) :
  ...
  refine (SchoenfliesGraph.finite_vertexSet (G := G)).isCompact.union ?_
  exact (SchoenfliesGraph.finite_edgeSet (G := G)).isCompact_biUnion fun e he => h.isCompact_edgeArc he
...
def exterior (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) : Set Plane :=
...
def face (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (base : Plane) : Set Plane :=
  connectedComponentIn (exterior G drawing) base

theorem face_subset_exterior (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (base : Plane) :
