...
`Schoenflies/SchoenfliesGraph/K33.lean` built the whole configuration: the six-cycle of a drawn `K(3,3)`
...
**One hypothesis is assumed and not proved: `SchoenfliesGraph.IsHexRealization`. See "The gap" below.**
The remaining results use only the hypotheses in their statements.

## Blueprint

* `SchoenfliesGraph.IsHexCrosscut` — the six-cycle, together with one of the three remaining edges, in the
...
* `SchoenfliesGraph.IsK33Config.endpoints_subset_arcA`, `…_arcB`, `…exists_reference_point`,
...
* `SchoenfliesGraph.IsHexRealization` — that combinatorial realization, and the one thing assumed here.
* `SchoenfliesGraph.IsK33Config.exists_two_chords_same_region` — `lem:k33`, "two of the three lie on the
...
* `SchoenfliesGraph.IsK33Config.false_of_isHexCrosscut` — the contradiction at a *fixed* pair of remaining
  edges: `cor:alternating-crosscuts` makes them meet, `chords_disjoint` says they do not.
* `SchoenfliesGraph.IsK33Config.false_of_hexCrosscuts`, `…false_of_hexRealizations` — `lem:k33` for a
  drawing that is already polygonal.
* `SchoenfliesGraph.IsK33Config.not_isDrawing` — `lem:k33`: a graph carrying a copy of `K(3,3)` has no
...
* `SchoenfliesGraph.IsArcK33`, `SchoenfliesGraph.k33Graph`, `SchoenfliesGraph.IsArcK33.arcDrawing`, `…isDrawing`,
...
* `SchoenfliesGraph.IsK33Subdivision`, `…isArcK33`, `…false_of_realization` — `cor:k33-subdivision`: in a
...
vertices first, and `SchoenfliesGraph.IsK33Config.not_isDrawing` gives it that freedom: it hands the
...
`SchoenfliesGraph.IsK33Config.chords_alternate` returns the two arcs existentially. The crosscut corollary
...
theorem chord_openArc_subset_edgeArc (h : IsK33Config G x y e) (hd : IsDrawing G drawing)
  ...
  rw [h.chord_openArc_eq hd s]; exact Set.diff_subset

/-- **Two of the three remaining edges lie in one and the same region of the complement of the
six-cycle.** This is `SchoenfliesGraph.IsK33Config.exists_two_chords_same_side` with the two sides read as
...
(`SchoenfliesGraph.IsHexRealization`). -/
...
`K(3,3)` (`SchoenfliesGraph.IsArcK33`), then the reduction of a drawn subdivision to nine such arcs. -/
...
def k33Graph (x y : Fin 3 → Plane) : SchoenfliesGraph Plane (Fin 3 × Fin 3) where
  vertexSet := Set.range x ∪ Set.range y
  edgeSet := Set.univ
  IsLink p u v := (u = x p.1 ∧ v = y p.2) ∨ (u = y p.2 ∧ v = x p.1)
  isLink_symm := by
    rintro ⟨i, j⟩ - u v (⟨rfl, rfl⟩ | ⟨rfl, rfl⟩)
...
them to. This assumes the realization, exactly as `SchoenfliesGraph.IsK33Config.not_isDrawing` does. -/
...
variable {H : SchoenfliesGraph Plane β} {W : Fin 3 → Fin 3 → List β}
...
structure IsK33Subdivision (H : SchoenfliesGraph Plane β) (x y : Fin 3 → Plane)
...
drawing. This assumes the realization, exactly as `SchoenfliesGraph.IsK33Config.not_isDrawing` does, and
