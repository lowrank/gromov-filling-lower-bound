...
`Schoenflies/SchoenfliesGraph/Walk.lean` says what it means for an edge list to *run* through a graph.
...
inside one. `SchoenfliesGraph.IsPathGraph P u W v` says that `P`'s own edges, listed in the order `W`,
...
Mathlib's `SchoenfliesGraph α β` carries a `Set β` of edges, not a list, so the correspondence between
the two is a clause of the definition (`SchoenfliesGraph.IsPathGraph.edgeSet_eq`) rather than something
a list-based graph would give for free.

## Constructing one

`SchoenfliesGraph.pathGraphOf G u W` is the subgraph of `G` spanned by the walk `W` from `u`: the
...
reaches one of the two ends** (`SchoenfliesGraph.IsPathGraph.reaches_an_end`). The proof is uniform in
the deleted vertex `c`, with no case split on whether `c` is an end, an interior vertex or
off the path at all:

* if `c` is on the path, cut there (`SchoenfliesGraph.IsPath.split`); each remaining vertex lies on one
...
Both branches go through the same two lemmas, `SchoenfliesGraph.IsPath.reaches_target_of_ne_source` and
its mirror, so the interior case has no separate content.

## Blueprint

* `SchoenfliesGraph.IsPathGraph` — the object called "a path `P`" in Lemma
...
* `SchoenfliesGraph.IsPathGraph.reaches_an_end` — the blueprint's "every component of `P - x` is
...
* `SchoenfliesGraph.pathGraphOf` — the "shortest path through `L` between two such neighbors is an ear"
...
Root `SchoenfliesGraph`, as fixed by `Schoenflies/SchoenfliesGraph/Walk.lean`, so that `h.reaches_an_end` and
...
variable {α β : Type*} {G H P : SchoenfliesGraph α β} {u v w x y c : α} {e : β} {W W₁ W₂ : List β}

/-! ### Walks and vertex deletion

Mathlib's `SchoenfliesGraph.deleteVerts` takes a set; a single deleted vertex is `{c}`. -/
...
whole content of `SchoenfliesGraph.IsPath.anti`. -/
...
this is not `SchoenfliesGraph.walkVertices_reverse`, which keeps it. -/
...
structure IsPathGraph (P : SchoenfliesGraph α β) (u : α) (W : List β) (v : α) : Prop where
...
`SchoenfliesGraph.restrict` and `SchoenfliesGraph.induce`, so that `SchoenfliesGraph.pathGraphOf_le` is almost free. -/
def pathGraphOf (G : SchoenfliesGraph α β) (u : α) (W : List β) : SchoenfliesGraph α β :=
...
for: the object every `SchoenfliesGraph.IsPathGraph` hypothesis elsewhere is discharged by. -/
...
Stated as a `Set.Finite` rather than as a `SchoenfliesGraph.Finite` instance so that this file does not
have to depend on `Schoenflies/SchoenfliesGraph/Degree.lean`; the two halves build one in a line. -/
