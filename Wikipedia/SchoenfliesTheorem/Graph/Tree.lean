...
-/
import Wikipedia.SchoenfliesTheorem.Graph.Cycle
import Wikipedia.SchoenfliesTheorem.Graph.Degree
...
* deleting a leaf leaves a tree (`SchoenfliesGraph.IsTree.delete_leaf`),
* a tree on `n` vertices has `n - 1` edges (`SchoenfliesGraph.IsTree.edge_count`), by peeling leaves,
* **a tree with exactly three leaves has one vertex of degree three**
  (`SchoenfliesGraph.IsTree.three_leaves`), which is where the handshake lemma finally pays.
...
path repeats no edge (`SchoenfliesGraph.IsPath.nodup`) and the graph has finitely many. That needs
`SchoenfliesGraph.Finite` and nothing else — in particular no well-founded recursion and no choice of a
maximal element in an order. The bound is the only place finiteness enters the longest-path
argument.

## Vertex deletion

`SchoenfliesGraph.deleteVerts` is Mathlib's. What this file adds is the transfer lemma every deletion
argument reduces to — `SchoenfliesGraph.IsWalk.deleteVerts`, a walk that never visits a deleted vertex
survives — together with the reason the hypothesis is ever available:
`SchoenfliesGraph.IsLeaf.path_avoids`, a path whose two ends are elsewhere never visits a vertex of
...
* `SchoenfliesGraph.IsTree.three_leaves` — `lem:three-leaf-tree` ("a finite tree with exactly three
  ...
  tree `T_i`.
* `SchoenfliesGraph.IsTree.edge_count` — the same lemma's "a finite tree with `n` vertices has exactly
  `n - 1` edges".
* `SchoenfliesGraph.IsTree.has_leaf`, `SchoenfliesGraph.IsTree.delete_leaf` — its "a tree with an edge has a leaf,
...
* `SchoenfliesGraph.IsTree` — the trees of `lem:two-components` ("take a minimal connected subgraph
  `T_i` spanning the three terminals. It is a tree").

## Namespace

The root `SchoenfliesGraph` namespace, as in `Walk.lean`, `Degree.lean` and `Cycle.lean`.
-/

open Set
open scoped SchoenfliesGraph

variable {α β : Type*} {G H : SchoenfliesGraph α β} {u v w x y : α} {e f : β} {P W : List β}
...
nonemptiness (`SchoenfliesGraph.Connected`), which is what makes the counting statements below true as
stated. -/
def IsTree (G : SchoenfliesGraph α β) : Prop := G.Connected ∧ G.IsAcyclic
...
(`SchoenfliesGraph.IsAcyclic.mem_of_isLink_of_mem_walkVertices`), and an edge to a vertex it does not
...
theorem edgeSet_deleteVerts_singleton (G : SchoenfliesGraph α β) (x : α) :
    E(G.deleteVerts {x}) = E(G) \ G.incidenceSet x := by
  ext g
  simp only [edgeSet_deleteVerts, Set.mem_setOf_eq, Set.mem_diff, mem_incidenceSet,
...
clear of the leaf (`SchoenfliesGraph.IsLeaf.path_avoids`). -/
...
/-- The induction behind `SchoenfliesGraph.IsTree.edge_count`, with a bound on the number of vertices to
...
private theorem isTree_edge_count_aux :
    ∀ (n : ℕ) (G : SchoenfliesGraph α β), G.Finite → V(G).ncard ≤ n → G.IsTree →
  ...
  | succ n ih =>
    ...
    · obtain ⟨x, hx⟩ := hT.has_leaf h2
      ...
      have hV : V(G.deleteVerts {x}).ncard = V(G).ncard - 1 := by
        rw [vertexSet_deleteVerts, Set.ncard_diff_singleton_of_mem hx.mem_vertexSet]
      ...
      have hE : E(G.deleteVerts {x}).ncard = E(G).ncard - 1 := by
        rw [edgeSet_deleteVerts_singleton, he₀, Set.ncard_diff_singleton_of_mem he₀G]
...
(`SchoenfliesGraph.IsTree.edge_count`), hence degree sum `2k + 4` (the handshake lemma). The leaves
