...
* `SchoenfliesGraph.IsDrawing.exists_square_at` — B2 at one vertex.
* `SchoenfliesGraph.IsDrawing.exists_vertexSquares` — B2 as consumed: one radius for every vertex, with
  distinct squares disjoint.
* `SchoenfliesGraph.IsDrawing.exists_core` — B4, the core of one edge.
* `SchoenfliesGraph.IsDrawing.disjoint_of_subset_edgeArc` — B4's "distinct cores are disjoint".
* `SchoenfliesGraph.IsDrawing.closedSquare_disjoint_edgeArc_of_ne` — B4's "meets no other vertex square",
...
theorem IsDrawing.exists_square_at [G.Finite] (h : IsDrawing G drawing) {v : Plane}
  ...
  obtain ⟨r₁, hr₁, hvert⟩ :
    ...
    refine exists_pos_forall_of_finite (SchoenfliesGraph.finite_vertexSet (G := G))
