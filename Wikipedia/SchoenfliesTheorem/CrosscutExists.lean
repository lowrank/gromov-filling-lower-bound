...
`SchoenfliesGraph.CrosscutExists`, the combinatorial half of the descent step of `lem:outer-chain`,
...
(`SchoenfliesGraph.IsCycleThrough.split_at_edges`): the closed walk reads
...
* the property of maximal subpaths that the argument actually consumes — that no subpath of
  `C ∩ Γ (j-1)` joins two of them — becomes `SchoenfliesGraph.exists_isCycleCrosscut`'s hypothesis
...
minimality clauses of `SchoenfliesGraph.IsCycleCrosscut` fall out of one minimisation: among all walks of
`Γ (j-1)` from `S₁` to `S₂`, take one of least length.

* it is a path, because any walk between its own two ends is a competitor
  (`SchoenfliesGraph.IsWalk.isPath_of_minLength`);
...
`SchoenfliesGraph.IsPath.split_at_edge` (cut a path at one of its edges, with the two freshness clauses
`SchoenfliesGraph.IsCycleThrough.split_aux` asks for) and `SchoenfliesGraph.IsCycleThrough.rotate` (present a cycle
through any one of its edges) are general facts about walks with nothing to do with this
development; they are the missing companions of `SchoenfliesGraph.IsPath.split_meet` and
`SchoenfliesGraph.IsCycleThrough.split_at` and belong in `Schoenflies/SchoenfliesGraph/Walk.lean` and
`Schoenflies/FaceCyclesProof.lean` respectively.

## Blueprint

* `SchoenfliesGraph.IsPath.split_at_edge`, `SchoenfliesGraph.IsCycleThrough.rotate`,
  `SchoenfliesGraph.IsCycleThrough.split_at_edges` — "choose points `a, b` in the relative interiors of
  these two edges. The two arcs of the cycle `C` from `a` to `b` are internally disjoint",
  read combinatorially (`lem:outer-chain`).
* `SchoenfliesGraph.exists_isCycleCrosscut` — "since `Γ_{j-1}` is connected, there is a path in `Γ_{j-1}`
...
* `SchoenfliesGraph.IsPlaneChain.crosscutExists` — **`SchoenfliesGraph.CrosscutExists`**, the combinatorial half of
...
`SchoenfliesGraph.IsCycleThrough.split_aux` asks for: the near half never returns to the far end of the
cut edge, and a vertex reached by both halves is the near end of it.

This is the edge-indexed companion of `SchoenfliesGraph.IsPath.split_meet`, which cuts at a vertex. -/
...
/-- **A cycle presented through any one of its edges.** `SchoenfliesGraph.IsCycleThrough` names one edge
...
the other is a crosscut of the cycle in the sense of `SchoenfliesGraph.IsCycleCrosscut`.
...
being outside `F`. -/
theorem exists_isCycleCrosscut {H F : SchoenfliesGraph α β} {e : β} {u v : α} {D : List β}
...
/-- **`SchoenfliesGraph.CrosscutExists`, discharged.** From a cycle of the block
...
The two named edges cut the cycle into two arcs (`SchoenfliesGraph.IsCycleThrough.split_at_edges`). Each
...
`SchoenfliesGraph.IsPlaneChain.disjoint_block_far` forbids), so it belongs to `Γ (i+m+1)`. The rest is
`SchoenfliesGraph.exists_isCycleCrosscut`. -/
