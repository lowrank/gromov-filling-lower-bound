...
-/
import Wikipedia.SchoenfliesTheorem.CommonSubdivision
import Wikipedia.SchoenfliesTheorem.CrosscutExists
import Wikipedia.SchoenfliesTheorem.FreshAccess
...
structure IsTargetPartialTransferOf
    ...
    (B : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) (par : γ → γ) : Prop where
...
theorem IsTargetPartialTransferOf.targetSkeletonSet_subset
    {T P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom}
    {B : SchoenfliesGraph Plane γ} {Hdraw : γ → ℝ → Plane} {par : γ → γ}
...
structure IsTargetTransferOf
    (T P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
    (H : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) (par : γ → γ) : Prop
...
def TargetCommonSubdivision
    ...
    (H : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) : Prop :=
  ∃ (K : SchoenfliesGraph Plane γ) (T₀ : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
      (par₀ : γ → γ),
...
def TargetEarStep [Infinite γ]
    (P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
    (H : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) : Prop :=
  ∀ (B : SchoenfliesGraph Plane γ) (a b : Plane) (D : List γ), B.IsTwoConnected → B ≤ H →
...
structure TargetEarStepData (T : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
    (B H : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) (a : Plane) (D : List γ) where
  ...
  srcEdgePolygonal : ∀ ⦃e⦄, e ∈ E(splitData.ear) →
    IsPolygonal (SchoenfliesGraph.edgeArc srcDraw e)
  tgtEdgePolygonal : ∀ ⦃e⦄, e ∈ E(splitData.ear) →
    IsPolygonal (SchoenfliesGraph.edgeArc tgtDraw e)
  /-- The target ear realizes exactly the ambient target path. -/
  tgtEarSet_eq : splitData.earSet tgtPos tgtDraw = SchoenfliesGraph.edgesCover Hdraw D
...
variable {T : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom}
  {B H : SchoenfliesGraph Plane γ} {Hdraw : γ → ℝ → Plane} {a : Plane} {D : List γ}
...
theorem isTargetPartialTransferOf_pair
  ...
  skeletonSet_eq := by
    ...
    rw [w.splitData.skeletonSet_realize, hT.skeletonSet_eq, w.tgtEarSet_eq,
      SchoenfliesGraph.pointSet_union, hdraw.pointSet_pathGraphOf hpath.isWalk (hpath.ne_nil hab)]
  vertexSet_subset := by
...
theorem targetEarStep_of_data [Infinite γ]
  ...
  rcases SchoenfliesGraph.ear_edges_notMem_or_union_eq hBH hpath hab haB hbB hint with hnew | hsame
...
theorem exists_target_face_of_ear
    ...
    {B H : SchoenfliesGraph Plane γ} {Hdraw : γ → ℝ → Plane}
    ...
    ∃ u v F, u ∈ V(T.str.skel) ∧ v ∈ V(T.str.skel) ∧ u ≠ v ∧
      ...
      SchoenfliesGraph.edgesCover Hdraw D \ {a, b} ⊆ T.tgt.cell F ∧
  ...
  have huv : u ≠ v := by
    ...
    apply hab
    rw [← hua, ← hvb, huv]
  have harc : IsArcBetween (SchoenfliesGraph.edgesCover Hdraw D) a b :=
    hH.isDrawing.path_isArcBetween hpath (hpath.ne_nil hab)
  let N := SchoenfliesGraph.edgesCover Hdraw D \ {a, b}
  ...
  have hND : N ⊆ tgtDom := by
    ...
    exact hH.pointSet_subset
      (SchoenfliesGraph.edgesCover_subset_pointSet (fun g hg => hpath.edge_mem hg) hx.1)
...
theorem target_ear_edge_not_outer
    ...
    (hinside : SchoenfliesGraph.edgesCover Hdraw D \ {a, b} ⊆ T.tgt.cell F) :
    ∀ e ∈ D, ¬ SchoenfliesGraph.edgeArc Hdraw e ⊆ tgtOuter := by
  intro e he houter
  ...
  have harcPair : edgeArc Hdraw e ⊆ ({a, b} : Set Plane) := by
    ...
    have hzCell : z ∈ T.tgt.cell F :=
      hinside ⟨SchoenfliesGraph.mem_edgesCover he hz, hzpair⟩
...
theorem target_ear_edge_polygonal
    ...
    (hpath : H.IsPath a D b) (hF : F ∈ T.str.faces)
    (hinside : SchoenfliesGraph.edgesCover Hdraw D \ {a, b} ⊆ T.tgt.cell F) :
    ∀ e ∈ D, IsPolygonal (SchoenfliesGraph.edgeArc Hdraw e) := by
  intro e he
...
def TargetBoundaryAnchored {β : Type*}
    (P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
    (H : SchoenfliesGraph Plane β) (Hdraw : β → ℝ → Plane) : Prop :=
...
def NewTargetBoundaryAnchored {β : Type*}
    ...
    (H : SchoenfliesGraph Plane β) (Hdraw : β → ℝ → Plane) : Prop :=
  ∀ {B : SchoenfliesGraph Plane β}, B ≤ H → base ⊆ pointSet B Hdraw →
...
theorem TargetBoundaryAnchored.new
    ...
    {H : SchoenfliesGraph Plane β} {Hdraw : β → ℝ → Plane}
...
def NonouterIncidenceUniqueAtBoundary {β : Type*}
    (H : SchoenfliesGraph Plane β) (Hdraw : β → ℝ → Plane) (outer : Set Plane) : Prop :=
...
def NoNewNonouterIncidenceAtBoundary {β : Type*}
    (base : Set Plane) (H : SchoenfliesGraph Plane β) (Hdraw : β → ℝ → Plane)
...
theorem NonouterIncidenceUniqueAtBoundary.noNew
    {β : Type*} {H : SchoenfliesGraph Plane β} {Hdraw : β → ℝ → Plane} {outer : Set Plane}
...
theorem edge_mem_of_edgeArc_subset_pointSet
    {β : Type*} {G B : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
...
theorem eq_of_edgeArc_subset
    {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
...
theorem TargetBoundaryAnchored.relabelEdges {β δ : Type*} [Nonempty β]
    {P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom}
    {H : SchoenfliesGraph Plane β} {Hdraw : β → ℝ → Plane} {f : β → δ}
  ...
  obtain ⟨g, hg, hge, hginc⟩ :=
    (SchoenfliesGraph.relabelEdges_inc H f hf (f e) y).1 hinc
  have hgeq : g = e := hf hg he hge
  subst g
  apply h he hginc hy
  intro hsub
  apply hnot
  rwa [SchoenfliesGraph.edgeArc_relabelDrawing hf he]
...
theorem NonouterIncidenceUniqueAtBoundary.relabelEdges {β δ : Type*} [Nonempty β]
    {H : SchoenfliesGraph Plane β} {Hdraw : β → ℝ → Plane} {outer : Set Plane}
  ...
  obtain ⟨e', he', he'f, he'inc⟩ :=
    (SchoenfliesGraph.relabelEdges_inc H f hf (f e) z).1 hdinc
  obtain ⟨g', hg', hg'f, hg'inc⟩ :=
    (SchoenfliesGraph.relabelEdges_inc H f hf (f g) z).1 hkinc
  ...
  · intro hsub
    apply hknot
    rwa [SchoenfliesGraph.edgeArc_relabelDrawing hf hg]
...
structure TargetSideEarStepData (T : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
    (B H : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) (a b : Plane) (D : List γ) where
  splitData : T.str.SplitData
  tgtPos : γ → Plane
  tgtDraw : γ → ℝ → Plane
  tgtCrosscut : splitData.EarCrosscut T.tgt tgtPos tgtDraw
...
theorem exists_targetSideEarStepData [Infinite γ]
  ...
  let Q : SchoenfliesGraph Plane γ := H.pathGraphOf a D
  have hQle : Q ≤ H := SchoenfliesGraph.pathGraphOf_le hpath.isWalk
  have hQpath : Q.IsPathGraph a D b := hpath.isPathGraph_pathGraphOf
  have haQ : a ∈ V(Q) := SchoenfliesGraph.mem_vertexSet_pathGraphOf_self
  have hbQ : b ∈ V(Q) := by
  ...
  have hnewVertices_fin : newVertices.Finite := hQfinV.diff.image vname
  ...
  let relabelled : SchoenfliesGraph Plane γ := Q.relabelEdges ename ename_inj
  let ear : SchoenfliesGraph γ γ := relabelled.map vname
  ...
  let tgtDraw : γ → ℝ → Plane := SchoenfliesGraph.relabelDrawing Q ename Hdraw
  have hQdraw : SchoenfliesGraph.IsDrawing Q Hdraw := hH.isDrawing.mono hQle
  have hrelDraw : SchoenfliesGraph.IsDrawing relabelled tgtDraw := hQdraw.relabelEdges ename_inj
  have hearGraph : d.earGraph tgtPos = relabelled := by
    ...
    simpa [tgtPos, relabelled] using
      (SchoenfliesGraph.map_map_invFunOn (G := relabelled) (f := vname)
        (by simpa [relabelled] using vname_inj))
  have htgtSet : d.earSet tgtPos tgtDraw = SchoenfliesGraph.edgesCover Hdraw D := by
    rw [CellStructure.SplitData.earSet, hearGraph, SchoenfliesGraph.pointSet_relabelEdges ename_inj]
  ...
  have htgtEdgePoly : ∀ ⦃e⦄, e ∈ E(d.ear) →
      IsPolygonal (SchoenfliesGraph.edgeArc tgtDraw e) := by
    intro e he
    change e ∈ E(ear) at he
    rw [hEear] at he
    obtain ⟨f, hfQ, rfl⟩ := he
    rw [SchoenfliesGraph.edgeArc_relabelDrawing ename_inj hfQ]
    apply htgtEdgeOrig f
    rwa [SchoenfliesGraph.pathGraphOf_edgeSet hpath.isWalk] at hfQ
  ...
  · apply Or.inr
    rw [hearGraph, SchoenfliesGraph.vertexSet_relabelEdges]
    exact hxQ
...
theorem targetEarEndpointStronglyAccessible_of_newBoundaryAnchored
  ...
  let d := w.splitData
  have hinside : SchoenfliesGraph.edgesCover Hdraw D \ {a, b} ⊆ T.tgt.cell d.face := by
    intro x hx
...
theorem targetEarEndpointsOuterOnly_of_noNewNonouterIncidence
    ...
    {B : SchoenfliesGraph Plane γ} {a b : Plane} {D : List γ} {par : γ → γ}
    (hBH : B ≤ H) (hpath : H.IsPath a D b) (hab : a ≠ b)
    (haB : a ∈ V(B)) (hbB : b ∈ V(B))
    (hnew : ∀ g ∈ D, g ∉ E(B))
  ...
  letI : B.Finite := SchoenfliesGraph.Finite.of_le hBH
  have hBdraw := hH.isDrawing.mono hBH
  have hinside : SchoenfliesGraph.edgesCover Hdraw D \ {a, b} ⊆
...
def GeneratedPair.sourceNonboundaryGraph
    (T : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom) : SchoenfliesGraph Plane γ :=
...
theorem GeneratedPair.sourceNonboundaryGraph_le
  ...
  apply (SchoenfliesGraph.induce_le ?_).trans SchoenfliesGraph.deleteEdges_le
  rintro x ⟨e, -, -, hinc⟩
...
instance GeneratedPair.sourceNonboundaryGraph_finite
  ...
  SchoenfliesGraph.Finite.of_le T.sourceNonboundaryGraph_le
...
the drawing hypothesis turns that case back into incidence with the same edge. -/
theorem GeneratedPair.source_pos_notMem_nonboundaryGraph_of_outerOnlyAt
  ...
  have abstract_incident : ∀ {e : γ}, T.src.graph.Inc e (T.src.pos v) →
    ...
    rw [SchoenfliesGraph.map_inc] at hinc
...
theorem GeneratedPair.skeletonSet_eq_sourceNonboundaryGraph_union
  ...
  have hdecomp : T.src.skeletonSet =
    ...
    · intro x hx
      ...
      · obtain ⟨e, he, hinc⟩ : ∃ e, e ∈ E(T.src.graph) ∧ T.src.graph.Inc e x := by
        ...
        · apply Or.inr
          ...
          exact ((hOle.inc_congr (by rwa [SchoenfliesGraph.edgeSet_map])).2 hinc).vertex_mem
        · apply Or.inl
          apply SchoenfliesGraph.vertexSet_subset_pointSet
          exact ⟨e, he, heOuter, hinc⟩
      · obtain ⟨e, he, hxe⟩ := Set.mem_iUnion₂.1 hxE
        by_cases heOuter : e ∈ E(T.str.outerGraph)
        · exact Or.inr (SchoenfliesGraph.edgeArc_subset_pointSet (by
            rw [SchoenfliesGraph.edgeSet_map]
            exact heOuter) hxe)
        · apply Or.inl
          exact SchoenfliesGraph.edgeArc_subset_pointSet (by
    ...
    · exact Set.union_subset
        (SchoenfliesGraph.pointSet_mono T.sourceNonboundaryGraph_le)
...
theorem GeneratedPair.source_polyAccessible_of_notMem_outer
  ...
  have hGunion : G.union O = T.src.graph := by
    apply SchoenfliesGraph.eq_of_le_of_subset_subset (SchoenfliesGraph.union_le SchoenfliesGraph.deleteEdges_le hOle)
    · intro z hz
      exact Or.inl (by rwa [SchoenfliesGraph.vertexSet_deleteEdges])
    · intro e he
      by_cases heOuter : e ∈ E(T.str.outerGraph)
      · exact Or.inr (by rwa [SchoenfliesGraph.edgeSet_map])
      · exact Or.inl (SchoenfliesGraph.mem_edgeSet_deleteEdges_iff.2 ⟨he, heOuter⟩)
  have hK : T.src.skeletonSet = pointSet G T.src.drawing ∪ srcOuter := by
    change pointSet T.src.graph T.src.drawing = _
    rw [← hGunion, SchoenfliesGraph.pointSet_union]
    change pointSet G T.src.drawing ∪ T.src.outerSet = _
    rw [T.src_isWeaklyAdmissible.outerSet_eq]
  letI : G.Finite := SchoenfliesGraph.Finite.of_le SchoenfliesGraph.deleteEdges_le
  have hGdraw : G.IsDrawing T.src.drawing := T.src.isDrawing.mono SchoenfliesGraph.deleteEdges_le
  have hGpoly : ∀ e ∈ E(G), IsPolygonal (edgeArc T.src.drawing e) := by
    intro e he
    rw [SchoenfliesGraph.mem_edgeSet_deleteEdges_iff] at he
  ...
  letI : O.Finite := SchoenfliesGraph.Finite.of_le hOle
  ...
  exact SchoenfliesGraph.polygonal_side_accessibility hGdraw hGpoly
...
noncomputable def targetCommonSubdivisionData [Infinite γ]
  ...
  classical
  let K := SchoenfliesGraph.traceGraph H Hdraw P.tgt.skeletonSet
