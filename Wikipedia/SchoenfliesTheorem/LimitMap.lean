...
theorem pointSet_map_eq_iUnion_cell {H : SchoenfliesGraph γ γ} (hH : H ≤ S.skel) :
    SchoenfliesGraph.pointSet (H.map R.pos) R.drawing = ⋃ κ ∈ V(H) ∪ E(H), R.cell κ := by
  refine Set.Subset.antisymm (fun z hz => ?_) (Set.iUnion₂_subset fun κ hκ => ?_)
  · rcases hz with hz | hz
    · -- a drawn 0-cell
      rw [SchoenfliesGraph.vertexSet_map] at hz
      obtain ⟨v, hv, rfl⟩ := hz
    ...
    · -- a point of a drawn 1-cell: either interior to it, or one of its two ends
      rw [SchoenfliesGraph.edgeSet_map] at hz
      obtain ⟨e, he, hze⟩ := Set.mem_iUnion₂.1 hz
      obtain ⟨a, b, hl⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet he
  ...
  · rcases hκ with hv | he
    · rw [R.cell_vertex (hH.vertexSet_mono hv)]
      exact Set.singleton_subset_iff.2 (SchoenfliesGraph.vertexSet_subset_pointSet
        (by rw [SchoenfliesGraph.vertexSet_map]; exact ⟨κ, hv, rfl⟩))
    · obtain ⟨a, b, hl⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet he
      rw [R.cell_edge (hl.mono hH)]
      exact Set.Subset.trans Set.diff_subset
        (SchoenfliesGraph.edgeArc_subset_pointSet (by rw [SchoenfliesGraph.edgeSet_map]; exact he))
...
theorem region_subset_dom : L.region ⊆ L.dom := Set.diff_subset

theorem region'_subset_dom' : L.region' ⊆ L.dom' := Set.diff_subset
