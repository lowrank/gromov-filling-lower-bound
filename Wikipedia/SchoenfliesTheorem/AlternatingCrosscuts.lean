...
verbatim the conclusion of `SchoenfliesGraph.IsK33Config.chords_alternate`, and derives the
...
`alternating_inter_nonempty` was written against `SchoenfliesGraph.IsK33Config.chords_alternate`, and the
fit was checked: with `hK33 : SchoenfliesGraph.IsK33Config G x y e`, `hd : SchoenfliesGraph.IsDrawing G drawing`,
...
`SchoenfliesGraph.IsK33Config.exists_two_chords_same_side`, the last step is one term:
...
* `IsPolygonalCrosscut.alternating_inter_nonempty` — the same in the two-arcs shape that
  `SchoenfliesGraph.IsK33Config.chords_alternate` produces.
...
interior to `B`. This is verbatim the data `SchoenfliesGraph.IsK33Config.chords_alternate` returns. -/
