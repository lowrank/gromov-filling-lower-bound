...
theorem IsArcBetween.closure_diff {A : Set Plane} {p q : Plane} (h : IsArcBetween A p q) :
    closure (A \ {p, q}) = A := by
  refine Subset.antisymm (h.isArc.isClosed.closure_subset_iff.2 diff_subset) fun z hz => ?_
  by_cases hzp : z = p
...
theorem cellSet_edge_subset (i : Fin 6) : H.cellSet (.edge i) ⊆ H.outer i '' I := diff_subset
...
theorem cellSet_chord_subset : H.cellSet .chord ⊆ H.chordSet := diff_subset
...
noncomputable def generatedPair :
  ...
  tgt_isPolygonal := by
    ...
    change IsPolygonal (SchoenfliesGraph.edgeArc d.tgt.draw e)
