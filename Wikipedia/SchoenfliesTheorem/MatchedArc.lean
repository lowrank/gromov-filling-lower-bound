...
* Two named arcs have an endpoint-preserving homeomorphism, obtained by matching their
  parameters.
...
/-- Any two named arcs have an endpoint-preserving homeomorphism. -/
...
variable {β : Type*} {A B : Set Plane} {a b c d : Plane} {G : SchoenfliesGraph Plane β}
  {drawing : β → ℝ → Plane}
...
theorem pointSet_map_mapDrawing (m : Schoenflies.ArcHomeo A B a b c d) :
  ...
  rw [pointSet, pointSet, SchoenfliesGraph.vertexSet_map, SchoenfliesGraph.edgeSet_map, Set.image_union,
    Set.image_iUnion₂]
  congr 1
...
theorem IsDrawing.map_arcHomeo (h : IsDrawing G drawing)
  ...
  vertex_mem_edgeArc := by
    ...
    rw [SchoenfliesGraph.vertexSet_map] at hv
    obtain ⟨v', hv', rfl⟩ := hv
  ...
  edge_inter := by
    ...
    rw [SchoenfliesGraph.edgeSet_map] at he hf
    ...
    subst p
    exact ⟨by rw [SchoenfliesGraph.vertexSet_map]; exact ⟨x, hxV, rfl⟩,
