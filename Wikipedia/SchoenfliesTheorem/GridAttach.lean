...
* `lem:subdivision-ear-preserve` — `SchoenfliesGraph.SameLinks` and
...
`SchoenfliesGraph.SameLinks` isolates — same vertex set, same joined pairs — and it transfers `Connected`,
...
* `hΓ` of `gridAttachGraph_isTwoConnected` — the skeleton of `Γ`, with the case's crosscut and
  ...
  polygonal part and `SchoenfliesGraph.IsTwoConnected.replace_edge_by_path` for the arcs of `C`; the ears
  are `lem:subdivision-ear-preserve` (b), which is `SchoenfliesGraph.IsTwoConnected.ear`, and
...
that duplicates another — none of which a `SchoenfliesGraph` isomorphism captures, since the edge type is
...
This is general graph theory and belongs in `Schoenflies/SchoenfliesGraph/Walk.lean` beside `IsWalk.mono`;
it is here only because it was needed here first. -/

namespace SchoenfliesGraph

variable {α β : Type*} {G H : SchoenfliesGraph α β} {u v x y : α} {e : β} {X : Set α}
...
def SameLinks (G H : SchoenfliesGraph α β) : Prop :=
  V(G) = V(H) ∧ ∀ x y, (∃ e, G.IsLink e x y) ↔ (∃ e, H.IsLink e x y)

namespace SameLinks

@[refl] theorem refl (G : SchoenfliesGraph α β) : SameLinks G G := ⟨rfl, fun _ _ => Iff.rfl⟩
...
theorem sameLinks_overlayGraph (pieces : List Piece) (points : List Plane) :
    SchoenfliesGraph.SameLinks (pieceListGraph (subdivide pieces points)) (overlayGraph pieces points) := by
  constructor
  · ext v
...
theorem frontier_face_subset_pointSet {β : Type*} {G : SchoenfliesGraph Plane β} [G.Finite]
    {drawing : β → ℝ → Plane} (h : SchoenfliesGraph.IsDrawing G drawing) (base : Plane) :
    frontier (SchoenfliesGraph.face G drawing base) ⊆ SchoenfliesGraph.pointSet G drawing :=
...
theorem isPathGraph_single {q₀ q₁ : Plane} (hne : q₀ ≠ q₁) :
  ...
  vertexSet_eq := by
    ...
    · rintro ⟨P, hP, hx⟩
      ...
      subst hP
      simp only [SchoenfliesGraph.walkVertices, mem_insert_iff, SchoenfliesGraph.coveredVertices, mem_setOf_eq]
      rcases hx with h | h
      · exact Or.inl h
      · exact Or.inr ⟨(q₀, q₁), hmem, pieceListGraph_inc hmem (Or.inr h)⟩
    · intro hx
      simp only [SchoenfliesGraph.walkVertices, mem_insert_iff, SchoenfliesGraph.coveredVertices, mem_setOf_eq] at hx
...
noncomputable def attachGraph (pieces : List Piece) (extra : List Plane) : SchoenfliesGraph Plane Piece :=
  overlayGraph pieces (attachPoints pieces extra)

instance attachGraph_finite (pieces : List Piece) (extra : List Plane) :
    SchoenfliesGraph.Finite (attachGraph pieces extra) := overlayGraph_finite _ _

theorem attachGraph_isDrawing {pieces : List Piece} (hnd : ∀ P ∈ pieces, P.Nondeg)
    (extra : List Plane) : SchoenfliesGraph.IsDrawing (attachGraph pieces extra) segmentDrawing :=
...
@[simp] theorem attachGraph_pointSet (pieces : List Piece) (extra : List Plane) :
    SchoenfliesGraph.pointSet (attachGraph pieces extra) segmentDrawing = cover pieces :=
...
theorem isConnected_cover_diff_of_joins {l : List Piece} {C : Set Plane} {r₀ : Plane}
  ...
  have hEq : cover (l ++ reps.flatMap J) \ C = (cover l \ C) ∪ ⋃ r ∈ reps, cover (J r) := by
    ...
    ext z
    simp only [Set.mem_diff, mem_union, mem_iUnion, exists_prop]
    constructor
...
theorem localGrid_subset_attachGraph_pointSet {p : Plane} {s : ℝ} {k : ℕ} (l : List Piece)
    ...
    cover (localGridEdges p s k) ⊆
      SchoenfliesGraph.pointSet (attachGraph (l ++ localGridEdges p s k) extra) segmentDrawing := by
...
noncomputable def gridAttachGraph (gsegs : List Piece) (reps : List Plane)
    (Jarc : Plane → List Piece) (p : Plane) (s ε : ℝ) (extra : List Plane) : SchoenfliesGraph Plane Piece :=
...
instance gridAttachGraph_finite (gsegs : List Piece) (reps : List Plane)
    ...
    SchoenfliesGraph.Finite (gridAttachGraph gsegs reps Jarc p s ε extra) := attachGraph_finite _ _
...
theorem gridAttachGraph_isDrawing (hs : 0 < s)
    (hg : ∀ P ∈ gsegs, P.Nondeg) (hj : ∀ P ∈ reps.flatMap Jarc, P.Nondeg) :
    SchoenfliesGraph.IsDrawing (gridAttachGraph gsegs reps Jarc p s ε extra) segmentDrawing :=
  attachGraph_isDrawing (gridAttachPieces_nondeg hs hg hj) extra

@[simp] theorem gridAttachGraph_pointSet :
    SchoenfliesGraph.pointSet (gridAttachGraph gsegs reps Jarc p s ε extra) segmentDrawing =
...
theorem localGrid_subset_gridAttachGraph :
    cover (localGridEdges p s (localGridCount s ε)) ⊆
      SchoenfliesGraph.pointSet (gridAttachGraph gsegs reps Jarc p s ε extra) segmentDrawing := by
...
of `C` and for the ears it is `SchoenfliesGraph.IsTwoConnected.replace_edge_by_path` and
`SchoenfliesGraph.IsTwoConnected.ear` iterated (`pieceListGraph_append_crosscut` above is the single ear
...
theorem gridAttachGraph_isConnected_diff
    ...
    IsConnected
      (SchoenfliesGraph.pointSet (gridAttachGraph gsegs reps Jarc p s ε extra) segmentDrawing \ C) := by
