...
variable {β : Type*} {G H : SchoenfliesGraph Plane β} {drawing drawing' : β → ℝ → Plane}
...
theorem isDrawing_union_of_common_vertices (hG : G.IsDrawing drawing)
  ...
  have hGle : G ≤ G.union H := SchoenfliesGraph.left_le_union _ _
  have hHle : H ≤ G.union H := hcompat.right_le_union
  have inc_of_mem (K : SchoenfliesGraph Plane β) (hK : K.IsDrawing drawing)
  ...
  · intro e x y v hxy hv hve
    ...
    · have hvG : v ∈ V(G) := by
        ...
        · exact hvG
        · exact (hcross (SchoenfliesGraph.edgeArc_subset_pointSet hxyG.edge_mem hve)
            (SchoenfliesGraph.vertexSet_subset_pointSet hvH)).1
      exact hG.vertex_mem_edgeArc hxyG hvG hve
    · have hvH : v ∈ V(H) := by
        rcases hv with hvG | hvH
        · exact (hcross (SchoenfliesGraph.vertexSet_subset_pointSet hvG)
            (SchoenfliesGraph.edgeArc_subset_pointSet hxyH.edge_mem hve)).2
        · exact hvH
  ...
  · intro e f he hf hef p hpe hpf
    ...
    · obtain ⟨hpVG, hpVH⟩ := hcross
        (SchoenfliesGraph.edgeArc_subset_pointSet heG hpe)
        (SchoenfliesGraph.edgeArc_subset_pointSet hfH hpf)
      exact ⟨Or.inl hpVG, (inc_of_mem G hG heG hpVG hpe).mono hGle,
        (inc_of_mem H hH hfH hpVH hpf).mono hHle⟩
    · obtain ⟨hpVG, hpVH⟩ := hcross
        (SchoenfliesGraph.edgeArc_subset_pointSet hfG hpf)
        (SchoenfliesGraph.edgeArc_subset_pointSet heH hpe)
...
theorem IsDrawing.isConnected_edgesCover_of_isWalk (h : G.IsDrawing drawing)
    ...
    IsConnected (SchoenfliesGraph.edgesCover drawing W) := by
  induction hW with
  | nil => exact (hne rfl).elim
  | @cons u w v e W hlink htail ih =>
      rw [SchoenfliesGraph.edgesCover_cons]
      by_cases hWnil : W = []
      · subst W
        rw [SchoenfliesGraph.edgesCover_nil, Set.union_empty]
      ...
      · apply IsConnected.union
        ...
        apply SchoenfliesGraph.vertexSet_subset_pointSet
        rw [SchoenfliesGraph.pathGraphOf_vertexSet]
        exact SchoenfliesGraph.mem_walkVertices_self
...
theorem IsDrawing.isConnected_pointSet (h : G.IsDrawing drawing) (hG : G.Connected) :
    IsConnected (SchoenfliesGraph.pointSet G drawing) := by
  obtain ⟨c, hc⟩ := hG.nonempty
  have bridge : ∀ {u : Plane}, u ∈ V(G) →
      ∃ A : Set Plane, A ⊆ SchoenfliesGraph.pointSet G drawing ∧ c ∈ A ∧ u ∈ A ∧ IsConnected A := by
    intro u hu
    by_cases hcu : c = u
    · subst u
      exact ⟨{c}, fun z hz => by
        rw [Set.mem_singleton_iff] at hz
        exact hz ▸ SchoenfliesGraph.vertexSet_subset_pointSet hc,
    ...
    · obtain ⟨W, hW⟩ := hG.reaches hc hu
      ...
      have hsubset : SchoenfliesGraph.edgesCover drawing W ⊆ SchoenfliesGraph.pointSet G drawing := by
        intro z hz
        obtain ⟨e, heW, hze⟩ := SchoenfliesGraph.mem_edgesCover_iff.1 hz
        exact SchoenfliesGraph.edgeArc_subset_pointSet (hW.edge_mem heW) hze
      have hcW : c ∈ SchoenfliesGraph.edgesCover drawing W := by
        rw [← h.pointSet_pathGraphOf hW hWne]
        apply SchoenfliesGraph.vertexSet_subset_pointSet
        rw [SchoenfliesGraph.pathGraphOf_vertexSet]
        exact SchoenfliesGraph.mem_walkVertices_self
      have huW : u ∈ SchoenfliesGraph.edgesCover drawing W := by
        rw [← h.pointSet_pathGraphOf hW hWne]
        apply SchoenfliesGraph.vertexSet_subset_pointSet
        rw [SchoenfliesGraph.pathGraphOf_vertexSet]
        exact hW.target_mem_walkVertices
      exact ⟨SchoenfliesGraph.edgesCover drawing W, hsubset, hcW, huW,
        Schoenflies.SchoenfliesGraph.IsDrawing.isConnected_edgesCover_of_isWalk h hW hWne⟩
  refine ⟨⟨c, SchoenfliesGraph.vertexSet_subset_pointSet hc⟩, isPreconnected_of_forall c ?_⟩
  intro y hy
  ...
  · obtain ⟨e, he, hye⟩ := Set.mem_iUnion₂.1 hyE
    ...
    let B := SchoenfliesGraph.edgeArc drawing e
    have hBconn : IsConnected B := (h.edge_isArcBetween huv).isArc.isConnected
    have huB : u ∈ B := h.inc_mem_edgeArc huv.inc_left
    refine ⟨A ∪ B, Set.union_subset hA (SchoenfliesGraph.edgeArc_subset_pointSet he),
...
theorem exists_sourceNonboundarySegmentCover
  ...
  have hincident : ∀ z ∈ V(G), ∃ e, G.Inc e z := by
    ...
    have hdeleted :
      ...
      rw [← SchoenfliesGraph.restrict_edgeSet_sdiff_eq_deleteEdges, SchoenfliesGraph.restrict_isLink]
  ...
  have hpoly : ∀ e ∈ E(G), IsPolygonal (edgeArc P.src.drawing e) := by
    ...
    have heDeleted : e ∈ E(P.src.graph.deleteEdges E(P.str.outerGraph)) :=
      hxy.1.edge_mem
    obtain ⟨heSrc, heOuter⟩ := SchoenfliesGraph.mem_edgeSet_deleteEdges_iff.1 heDeleted
...
theorem localOverlay_isDrawing {p : Plane} {s epsilon : ℝ} (hs : 0 < s)
    (extra : List Plane) :
    SchoenfliesGraph.IsDrawing (Q.localOverlay p s epsilon extra) segmentDrawing :=
...
theorem localOverlay_pointSet (p : Plane) (s epsilon : ℝ) (extra : List Plane) :
    SchoenfliesGraph.pointSet (Q.localOverlay p s epsilon extra) segmentDrawing =
      SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing ∪
...
theorem sourceCore_subset_localOverlay (p : Plane) (s epsilon : ℝ)
    (extra : List Plane) :
    SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing ⊆
      SchoenfliesGraph.pointSet (Q.localOverlay p s epsilon extra) segmentDrawing := by
...
theorem sourceCoreVertices_subset_localOverlay {p : Plane} {s epsilon : ℝ}
  ...
  · rw [localPieces, cover_append, Q.cover_eq]
    exact Or.inl (SchoenfliesGraph.vertexSet_subset_pointSet hx)
...
theorem localOverlay_pointSet_subset {p : Plane} {s epsilon : ℝ} (hs : 0 < s)
    ...
    SchoenfliesGraph.pointSet (Q.localOverlay p s epsilon extra) segmentDrawing ⊆ srcDom := by
  rw [Q.localOverlay_pointSet]
...
theorem sourceCore_inter_outer_vertices {x : Plane}
    (hxCore : x ∈ SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing)
    (hxOuter : x ∈ srcOuter) :
  ...
  have hxOuterSet : x ∈ P.src.outerSet := by
    ...
    exact hxOuter
  change x ∈ SchoenfliesGraph.pointSet (P.str.outerGraph.map P.src.pos) P.src.drawing at hxOuterSet
  have hxCoreV : x ∈ V(P.sourceNonboundaryGraph) := by
    ...
    · obtain ⟨e, heCore, hxe⟩ := Set.mem_iUnion₂.1 hxE
      ...
      have heDeleted : e ∈ E(P.src.graph.deleteEdges E(P.str.outerGraph)) :=
        habCore.1.edge_mem
      obtain ⟨heSrc, heNotOuter⟩ := SchoenfliesGraph.mem_edgeSet_deleteEdges_iff.1 heDeleted
      have hxSrcV : x ∈ V(P.src.graph) := by
        ...
        · obtain ⟨f, hfOuterGraph, hxf⟩ := Set.mem_iUnion₂.1 hxOuterE
          have hfOuter : f ∈ E(P.str.outerGraph) := by
            rwa [SchoenfliesGraph.edgeSet_map] at hfOuterGraph
...
abbrev outerGraph (_w : Q.LocalOverlayRelabeling p s epsilon extra) : SchoenfliesGraph Plane γ :=
  P.str.outerGraph.map P.src.pos

/-- The finite inner overlay after allocation of fresh abstract edge names. -/
noncomputable abbrev innerGraph : SchoenfliesGraph Plane γ :=
  (Q.localOverlay p s epsilon extra).relabelEdges w.name w.name_inj

/-- The mixed source extension graph. -/
noncomputable def graph : SchoenfliesGraph Plane γ := w.outerGraph.union w.innerGraph
...
theorem compatible : w.outerGraph.Compatible w.innerGraph := by
  apply SchoenfliesGraph.Compatible.of_disjoint_edgeSet
  rw [Set.disjoint_left, SchoenfliesGraph.edgeSet_map, SchoenfliesGraph.edgeSet_relabelEdges]
...
theorem outer_isDrawing : w.outerGraph.IsDrawing w.drawing := by
  ...
  rwa [SchoenfliesGraph.edgeSet_map] at he
...
theorem outer_pointSet : SchoenfliesGraph.pointSet w.outerGraph w.drawing = srcOuter := by
  calc
    SchoenfliesGraph.pointSet w.outerGraph w.drawing =
        SchoenfliesGraph.pointSet w.outerGraph P.src.drawing := by
      apply SchoenfliesGraph.pointSet_congr
      intro e he
      simpa only [SchoenfliesGraph.edgeArc] using congrArg (fun f : ℝ → Plane => f '' unitInterval)
        (w.drawing_of_outer (by rwa [SchoenfliesGraph.edgeSet_map] at he))
...
theorem inner_pointSet :
    SchoenfliesGraph.pointSet w.innerGraph w.drawing =
      SchoenfliesGraph.pointSet (Q.localOverlay p s epsilon extra) segmentDrawing := by
  calc
    SchoenfliesGraph.pointSet w.innerGraph w.drawing =
        SchoenfliesGraph.pointSet w.innerGraph
          ((Q.localOverlay p s epsilon extra).relabelDrawing w.name segmentDrawing) := by
      apply SchoenfliesGraph.pointSet_congr
      intro e he
      simpa only [SchoenfliesGraph.edgeArc] using congrArg (fun f : ℝ → Plane => f '' unitInterval)
        (w.drawing_of_inner he)
    _ = SchoenfliesGraph.pointSet (Q.localOverlay p s epsilon extra) segmentDrawing :=
      SchoenfliesGraph.pointSet_relabelEdges w.name_inj
...
theorem graph_isDrawing (hs : 0 < s)
  ...
  · obtain ⟨hxCoreV, hxOuterV⟩ :=
    ...
    refine ⟨hxOuterV, ?_⟩
    rw [SchoenfliesGraph.vertexSet_relabelEdges]
...
theorem graph_finite : w.graph.Finite where
  finite_vertexSet := by
    rw [graph, SchoenfliesGraph.vertexSet_union, outerGraph, SchoenfliesGraph.vertexSet_map,
      SchoenfliesGraph.vertexSet_relabelEdges]
    exact ((P.str.finite_vertexSet.subset P.str.outerGraph_le.vertexSet_mono).image
      P.src.pos).union (SchoenfliesGraph.finite_vertexSet (Q.localOverlay p s epsilon extra))
  finite_edgeSet := by
    rw [graph, SchoenfliesGraph.edgeSet_union, outerGraph, SchoenfliesGraph.edgeSet_map,
      SchoenfliesGraph.edgeSet_relabelEdges]
    exact (P.str.finite_edgeSet.subset P.str.outerGraph_le.edgeSet_mono).union
      ((SchoenfliesGraph.finite_edgeSet (Q.localOverlay p s epsilon extra)).image w.name)
...
theorem graph_pointSet :
    SchoenfliesGraph.pointSet w.graph w.drawing =
      srcOuter ∪ (SchoenfliesGraph.pointSet P.sourceNonboundaryGraph P.src.drawing ∪
        cover (localGridEdges p s (localGridCount s epsilon))) := by
  rw [graph, SchoenfliesGraph.pointSet_union, w.outer_pointSet, w.inner_pointSet,
...
theorem sourceSkeleton_subset_graph :
    P.src.skeletonSet ⊆ SchoenfliesGraph.pointSet w.graph w.drawing := by
...
theorem sourceVertices_subset_graph (hs : 0 < s) :
  ...
  · rw [graph, SchoenfliesGraph.vertexSet_union]
    exact Or.inl
      (((P.str.outerGraph_le.map P.src.pos).inc_congr
        (by rwa [SchoenfliesGraph.edgeSet_map])).2 hinc).vertex_mem
...
theorem graph_pointSet_subset (hs : 0 < s)
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter) :
    SchoenfliesGraph.pointSet w.graph w.drawing ⊆ srcDom := by
  rw [graph, SchoenfliesGraph.pointSet_union]
  apply Set.union_subset
  · rw [w.outer_pointSet, ← P.src_isWeaklyAdmissible.outerSet_eq]
    exact (SchoenfliesGraph.pointSet_mono (P.str.outerGraph_le.map P.src.pos)).trans
      P.src_isWeaklyAdmissible.skeletonSet_subset
  · rw [w.inner_pointSet]
    exact Q.localOverlay_pointSet_subset hs (hwindow.trans diff_subset) extra
...
theorem graph_edge_dichotomy (hs : 0 < s)
  ...
  · exact Or.inl (by
      intro x hx
      rw [← w.outer_pointSet]
      exact SchoenfliesGraph.edgeArc_subset_pointSet heOuter hx)
  · obtain ⟨R, hR, rfl⟩ := heInner
    ...
    have harc : edgeArc w.drawing (w.name R) = edgeArc segmentDrawing R := by
      calc
        edgeArc w.drawing (w.name R) =
          ...
          simpa only [SchoenfliesGraph.edgeArc] using congrArg
            (fun f : ℝ → Plane => f '' unitInterval) hdrawing
        _ = edgeArc segmentDrawing R :=
          SchoenfliesGraph.edgeArc_relabelDrawing w.name_inj hR
    rw [harc]
    ...
    apply hx.2
    rw [graph, SchoenfliesGraph.vertexSet_union]
    exact Or.inr (by rwa [SchoenfliesGraph.vertexSet_relabelEdges])
...
theorem graph_edge_subset (hs : 0 < s)
  ...
  · have hfAbstract : f ∈ E(P.str.outerGraph) := by
      rwa [SchoenfliesGraph.edgeSet_map] at hfOuter
    have hzfSrc : z ∈ edgeArc P.src.drawing f := by
      have hdraw := w.drawing_of_outer hfAbstract
      simpa only [SchoenfliesGraph.edgeArc] using
    ...
    have hef : e = f := P.src.isDrawing.unique_edge_at
      (by rw [SchoenfliesGraph.edgeSet_map]; exact he)
      (by rw [SchoenfliesGraph.edgeSet_map]; exact P.str.outerGraph_le.edgeSet_mono hfAbstract)
      hznotOld hze hzfSrc
    subst f
    have hdraw := w.drawing_of_outer hfAbstract
    rw [SchoenfliesGraph.edgeArc, hdraw]
    intro y hy
    exact hy
  · obtain ⟨R, hR, rfl⟩ := hfInner
    ...
    · exfalso
      have heMixed : e ∈ E(w.graph) := Or.inl (by
        rw [SchoenfliesGraph.edgeSet_map]
        exact heOuter)
...
theorem sourceTrace_isTwoConnected (hs : 0 < s)
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter) :
    (SchoenfliesGraph.traceGraph w.graph w.drawing P.src.skeletonSet).IsTwoConnected :=
...
theorem localGrid_subset_graph :
    SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing ⊆
      SchoenfliesGraph.pointSet w.graph w.drawing := by
...
theorem graph_grid_edge_subset (hs : 0 < s)
  ...
  · exfalso
    have hzOuter : z ∈ srcOuter := by
      rw [← w.outer_pointSet]
      exact SchoenfliesGraph.edgeArc_subset_pointSet hfOuter hzf
...
theorem localGridTrace_isTwoConnected (hs : 0 < s)
    (hwindow : Plane.closedSquare p s ⊆ srcDom \ srcOuter) :
    (SchoenfliesGraph.traceGraph w.graph w.drawing
      (SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon))
...
theorem graph_isTwoConnected_of_two_common (hs : 0 < s)
    ...
    (haGrid : a ∈ SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing)
    (hbGrid : b ∈ SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing) :
    w.graph.IsTwoConnected := by
  let T := SchoenfliesGraph.traceGraph w.graph w.drawing P.src.skeletonSet
  let K := SchoenfliesGraph.traceGraph w.graph w.drawing
    (SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing)
  ...
  have hcompat : T.Compatible K :=
    SchoenfliesGraph.Compatible.of_le_le (SchoenfliesGraph.traceGraph_le _) (SchoenfliesGraph.traceGraph_le _)
  have hU2 : (T.union K).IsTwoConnected :=
    hT2.union hcompat hK2 hab haT haK hbT hbK
  apply hU2.of_le_of_vertexSet_subset
    (SchoenfliesGraph.union_le (SchoenfliesGraph.traceGraph_le _) (SchoenfliesGraph.traceGraph_le _))
  intro x hx
  rw [SchoenfliesGraph.vertexSet_union]
  have hxPoint : x ∈ SchoenfliesGraph.pointSet w.graph w.drawing :=
    SchoenfliesGraph.vertexSet_subset_pointSet hx
  ...
  · exact Or.inl (by
      ...
      exact ⟨hx, by
        ...
        exact Or.inl hxCore⟩)
  · exact Or.inr (by
      rw [SchoenfliesGraph.traceGraph_vertexSet, localGrid_eq, pieceListGraph_pointSet]
      exact ⟨hx, hxGrid⟩)
...
theorem graph_isConnected_diff_of_source_connected (hs : 0 < s)
    ...
    (hmeet : ((P.src.skeletonSet \ srcOuter) ∩
      SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing).Nonempty) :
    IsConnected (SchoenfliesGraph.pointSet w.graph w.drawing \ srcOuter) := by
  let Kset := SchoenfliesGraph.pointSet (localGrid p s (localGridCount s epsilon)) segmentDrawing
  ...
  have hcarrier : SchoenfliesGraph.pointSet w.graph w.drawing \ srcOuter =
      (P.src.skeletonSet \ srcOuter) ∪ Kset := by
    ext x
    rw [Set.mem_diff, Set.mem_union, Set.mem_diff]
    constructor
...
theorem isSourceExtension (hs : 0 < s)
    ...
    (hconnected : IsConnected (SchoenfliesGraph.pointSet w.graph w.drawing \ srcOuter)) :
