...
theorem isPath_initBoundary_false_tail :
  ...
  refine .cons (initSkel_isLink_edge (by decide : (2 : Fin 6) + 1 = 3))
    (.cons (initSkel_isLink_edge (by decide : (3 : Fin 6) + 1 = 4))
      (SchoenfliesGraph.IsPath.single initSkel_isLink_chord.symm (by simp)) ?_) ?_
  · simp [SchoenfliesGraph.walkVertices, SchoenfliesGraph.coveredVertices, initSkel_inc_iff, InitialCell.ends,
      InitialCell.edges]
  · simp [SchoenfliesGraph.walkVertices, SchoenfliesGraph.coveredVertices, initSkel_inc_iff, InitialCell.ends,
      InitialCell.edges]
...
theorem isPath_initBoundary_true_tail :
  ...
  refine .cons (initSkel_isLink_edge (by decide : (5 : Fin 6) + 1 = 0))
    (.cons (initSkel_isLink_edge (by decide : (0 : Fin 6) + 1 = 1))
      (SchoenfliesGraph.IsPath.single initSkel_isLink_chord (by simp)) ?_) ?_
  · simp [SchoenfliesGraph.walkVertices, SchoenfliesGraph.coveredVertices, initSkel_inc_iff, InitialCell.ends,
      InitialCell.edges]
  · simp [SchoenfliesGraph.walkVertices, SchoenfliesGraph.coveredVertices, initSkel_inc_iff, InitialCell.ends,
      InitialCell.edges]
...
theorem isLongCycle_initBoundary_false :
  ...
  mem := SchoenfliesGraph.mem_walkVertices_cons_of_mem
...
theorem mem_faceCells_iff_mem_pathCells {k : Bool} {u c : InitialCell}
  ...
  simp only [CellStructure.pathCells, Set.mem_union, Set.mem_setOf_eq,
    SchoenfliesGraph.mem_walkVertices_iff, SchoenfliesGraph.mem_coveredVertices_iff]
  constructor
  · rintro (hc | hc)
    · exact Or.inl hc
...
theorem isPolygonal_tgt_edgeArc {e : InitialCell} (he : e ∈ E(initSkel)) :
    IsPolygonal (SchoenfliesGraph.edgeArc d.tgt.draw e) := by
...
theorem isPolygonal_src_nonboundary_edgeArc :
    IsPolygonal (SchoenfliesGraph.edgeArc d.src.draw .chord) ∧
      SchoenfliesGraph.edgeArc d.src.draw .chord \ C ⊆ inside C := by
...
theorem initial_pair' (hC : IsJordanCurve C) (A : AnchorSet C) :
    ∃ d : InitialData C,
      ...
      (∀ e ∈ E(initSkel), IsPolygonal (SchoenfliesGraph.edgeArc d.tgt.draw e)) ∧
