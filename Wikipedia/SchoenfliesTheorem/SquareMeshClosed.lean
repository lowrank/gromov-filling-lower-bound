...
exactly that statement in the vocabulary of `SchoenfliesGraph.IsIncWalk` — a walk along which
`dist P.1 ·` strictly increases — and `SchoenfliesGraph.IsIncWalk.isPath` turns it into a path. The two
...
`SchoenfliesGraph.IsTwoConnected.of_le_of_vertexSet_subset` then transfers 2-connectivity from the assembly
...
walk into a path, which `SchoenfliesGraph.IsIncWalk.isPath` does. -/
...
theorem meshGraph_outer_cycle_of_mem_modelCurve {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
    ...
    ∃ (e : Piece) (u v x : Plane) (D : List Piece),
      (meshGraph N fresh anchors).IsLongCycle e u v D x ∧
        ...
        SchoenfliesGraph.edgesCover segmentDrawing (e :: D) = modelCurve :=
...
noncomputable def outerCycleData (δ : ℝ) (fresh anchors : List Plane) :
  ...
  if h : ∃ t : Piece × Plane × Plane × Plane × List Piece,
      (squareMesh δ fresh anchors).IsLongCycle t.1 t.2.1 t.2.2.1 t.2.2.2.2 t.2.2.2.1 ∧
        ...
        SchoenfliesGraph.edgesCover segmentDrawing (t.1 :: t.2.2.2.2) = modelCurve
...
theorem meshGraph_ring_cycle {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
    ...
    ∃ (e : Piece) (u v x : Plane) (D : List Piece),
      (meshGraph N fresh anchors).IsLongCycle e u v D x ∧
        ...
        SchoenfliesGraph.edgesCover segmentDrawing (e :: D) = ringSet r := by
  ...
  have hthird : (rsideT r).2 ∈ (overlayGraph (meshSegments N fresh)
      (meshPoints N fresh anchors)).walkVertices (rsideT r).1
        (((WT ++ WL) ++ WB) ++ L.reverse) :=
    SchoenfliesGraph.walkVertices_mono
  ...
  have hcovT : SchoenfliesGraph.edgesCover segmentDrawing WT = (rsideT r).seg := edgesCover_eq_seg hmemT hcT
  have hcovL : SchoenfliesGraph.edgesCover segmentDrawing WL = (rsideL r).seg := edgesCover_eq_seg hmemL hcL
  have hcovB : SchoenfliesGraph.edgesCover segmentDrawing WB = (rsideB r).seg := edgesCover_eq_seg hmemB hcB
  have hcovR : SchoenfliesGraph.edgesCover segmentDrawing WR = (rsideR r).seg := edgesCover_eq_seg hmemR hcR
  refine subset_antisymm (fun z hz => ?_) (fun z hz => ?_)
  · obtain ⟨Q, hQ, hzQ⟩ := SchoenfliesGraph.mem_edgesCover_iff.1 hz
  ...
  · rw [← rsides_cover hrpos.le] at hz
    ...
    · rw [← hcovT] at hz
      obtain ⟨Q, hQ, hzQ⟩ := SchoenfliesGraph.mem_edgesCover_iff.1 hz
      exact SchoenfliesGraph.mem_edgesCover (List.mem_cons_of_mem _
    ...
    · rw [← hcovR] at hz
      ...
      · exact SchoenfliesGraph.mem_edgesCover List.mem_cons_self hzQ
      · exact SchoenfliesGraph.mem_edgesCover (List.mem_cons_of_mem _
...
noncomputable def ringGraph (N : ℕ) (fresh anchors : List Plane) (r : ℝ) : SchoenfliesGraph Plane Piece :=
...
theorem mem_walkVertices_of_mem_points {pieces : List Piece} {points : List Plane}
  ...
  have hcov : SchoenfliesGraph.edgesCover segmentDrawing W = P.seg := edgesCover_eq_seg hP hW
  rw [← hcov] at hqP
  obtain ⟨Q, hQ, hqQ⟩ := SchoenfliesGraph.mem_edgesCover_iff.1 hqP
  ...
  exact SchoenfliesGraph.mem_walkVertices_of_mem_covered
    (SchoenfliesGraph.mem_coveredVertices hQ (overlayGraph_inc hQmem hend))
...
`N⁻¹ • w` with it, and `SchoenfliesGraph.IsTwoConnected.union` applies; from then on every further ring
...
`SchoenfliesGraph.IsTwoConnected.of_le_of_vertexSet_subset` transfers 2-connectivity to the mesh itself. -/
...
theorem spokeGraph_le {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
  ...
  SchoenfliesGraph.pathGraphOf_le (spokeWalk_isPath hN hfresh anchors hu).isWalk
...
theorem smul_mem_coveredVertices_spokeWalk {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
  ...
  rcases SchoenfliesGraph.mem_walkVertices_iff.1
...
theorem meshCore_le {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
  ...
  SchoenfliesGraph.union_le
    (SchoenfliesGraph.union_le (ringGraph_le N fresh anchors 1)
      (SchoenfliesGraph.pathGraphOf_le (meshEar_isPath hN hfresh anchors hz hw hzw).isWalk))
...
theorem meshCore_isTwoConnected {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
  ...
  have hearle : G.pathGraphOf z (meshEar N fresh anchors z w) ≤ G :=
    SchoenfliesGraph.pathGraphOf_le hear.isWalk
  ...
  have h1 : ((ringGraph N fresh anchors 1).union
    ...
    (ringGraph_isTwoConnected hN hfresh anchors (one_mem_meshRadii hN)).ear
      (SchoenfliesGraph.Compatible.of_le_le (ringGraph_le N fresh anchors 1) hearle)
  ...
  have hmemz : rmin • z ∈ V(G.pathGraphOf z (meshEar N fresh anchors z w)) :=
    SchoenfliesGraph.mem_walkVertices_of_mem_covered (SchoenfliesGraph.coveredVertices_mono
  ...
  exact h1.union (SchoenfliesGraph.Compatible.of_le_le
      (SchoenfliesGraph.union_le (ringGraph_le N fresh anchors 1) hearle) (ringGraph_le N fresh anchors rmin))
...
theorem smul_mem_walkVertices_meshEar {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
  ...
  have hear := meshEar_isPath hN hfresh anchors hz hw hzw
  constructor
  · exact SchoenfliesGraph.walkVertices_mono
      (fun _ hQ => List.mem_append_left _ (List.mem_append_left _ hQ))
      (smul_mem_walkVertices_spokeWalk hN hfresh anchors hz hr)
  · by_cases h1 : r = 1
    · rw [h1, one_smul]; exact hear.isWalk.target_mem_walkVertices
    · exact SchoenfliesGraph.mem_walkVertices_of_mem_covered (SchoenfliesGraph.coveredVertices_mono
        (fun _ hQ => List.mem_append_right _ (List.mem_reverse.2 hQ))
...
noncomputable def attachRings (N : ℕ) (fresh anchors : List Plane) (K : SchoenfliesGraph Plane Piece) :
    List ℝ → SchoenfliesGraph Plane Piece
  | [] => K
...
iterated. -/
noncomputable def attachSpokes (N : ℕ) (fresh anchors : List Plane) (K : SchoenfliesGraph Plane Piece) :
    List Plane → SchoenfliesGraph Plane Piece
  | [] => K
  | u :: us => (attachSpokes N fresh anchors K us).union (spokeGraph N fresh anchors u)

theorem le_attachRings (N : ℕ) (fresh anchors : List Plane) (K : SchoenfliesGraph Plane Piece)
    (l : List ℝ) : K ≤ attachRings N fresh anchors K l := by
  induction l with
  | nil => exact le_refl K
  | cons r rs ih => exact ih.trans (SchoenfliesGraph.left_le_union _ _)

theorem le_attachSpokes (N : ℕ) (fresh anchors : List Plane) (K : SchoenfliesGraph Plane Piece)
    (l : List Plane) : K ≤ attachSpokes N fresh anchors K l := by
  induction l with
  | nil => exact le_refl K
  | cons u us ih => exact ih.trans (SchoenfliesGraph.left_le_union _ _)

theorem attachRings_le {N : ℕ} {fresh anchors : List Plane} {K : SchoenfliesGraph Plane Piece}
    (hK : K ≤ meshGraph N fresh anchors) (l : List ℝ) :
    attachRings N fresh anchors K l ≤ meshGraph N fresh anchors := by
  induction l with
  | nil => exact hK
  | cons r rs ih => exact SchoenfliesGraph.union_le ih (ringGraph_le _ _ _ _)

theorem attachSpokes_le {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
    (hfresh : ∀ x ∈ fresh, x ∈ modelCurve) {anchors : List Plane} {K : SchoenfliesGraph Plane Piece}
    (hK : K ≤ meshGraph N fresh anchors) {l : List Plane} (hl : ∀ u ∈ l, u ∈ fresh) :
    attachSpokes N fresh anchors K l ≤ meshGraph N fresh anchors := by
  induction l with
  | nil => exact hK
  | cons u us ih =>
    exact SchoenfliesGraph.union_le (ih fun x hx => hl x (List.mem_cons_of_mem _ hx))
      (spokeGraph_le hN hfresh anchors (hl u List.mem_cons_self))

theorem ringGraph_le_attachRings {N : ℕ} {fresh anchors : List Plane} {K : SchoenfliesGraph Plane Piece}
  ...
  | cons r' rs ih =>
    ...
    · exact (SchoenfliesGraph.Compatible.of_le_le (attachRings_le hK rs)
        (ringGraph_le N fresh anchors r)).right_le_union
    · exact (ih h).trans (SchoenfliesGraph.left_le_union _ _)

theorem spokeGraph_le_attachSpokes {N : ℕ} {fresh anchors : List Plane}
    {K : SchoenfliesGraph Plane Piece} (hK : K ≤ meshGraph N fresh anchors) {l : List Plane} {u : Plane}
  ...
  | cons u' us ih =>
    ...
    · exact (SchoenfliesGraph.Compatible.of_le_le
    ...
    · exact (ih (fun x hx => hl x (List.mem_cons_of_mem _ hx)) h).trans (SchoenfliesGraph.left_le_union _ _)

theorem attachRings_isTwoConnected {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
    (hfresh : ∀ x ∈ fresh, x ∈ modelCurve) {anchors : List Plane} {z w : Plane}
    (hz : z ∈ fresh) (hw : w ∈ fresh) (hzw : z ≠ w) {K : SchoenfliesGraph Plane Piece}
  ...
  | cons r rs ih =>
    ...
    exact hacc.union (SchoenfliesGraph.Compatible.of_le_le
...
theorem attachSpokes_isTwoConnected {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
  ...
  | cons u us ih =>
    ...
    refine hacc.ear (SchoenfliesGraph.Compatible.of_le_le
