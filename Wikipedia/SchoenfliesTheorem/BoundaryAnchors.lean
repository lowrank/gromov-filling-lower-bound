...
theorem CellStructure.Realization.IsAdmissible.isStageOn
    ...
    SchoenfliesGraph.IsStageOn R.graph R.drawing C (inside C) where
  ...
  polygonal := by
    ...
    have heOuterMap : e ∈ E(S.outerGraph.map R.pos) := by
      ...
      exact heOuter
    rw [← h.outerSet_eq]
    exact SchoenfliesGraph.edgeArc_subset_pointSet heOuterMap
  ...
  vertex_mem := by
    ...
    have hxDom : x ∈ C ∪ inside C := h.skeletonSet_subset
      (SchoenfliesGraph.vertexSet_subset_pointSet hxV)
  ...
  edge_split := by
    ...
    · have heOuterMap : e ∈ E(S.outerGraph.map R.pos) := by
        ...
        exact heOuter
      rw [← h.outerSet_eq]
      exact Or.inl (SchoenfliesGraph.edgeArc_subset_pointSet heOuterMap)
    · right
      obtain ⟨a, b, hab⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet heSkel
...
edge. -/
theorem SchoenfliesGraph.IsDrawing.exists_nonboundary_incident_of_mem_closure
    {G : SchoenfliesGraph Plane γ} {drawing : γ → ℝ → Plane} [G.Finite]
    (h : G.IsDrawing drawing) {c : Plane} (hcV : c ∈ V(G)) (hcC : c ∈ C)
  (hclosure : c ∈ closure (SchoenfliesGraph.pointSet G drawing \ C)) :
    ∃ e, G.Inc e c ∧ SchoenfliesGraph.Nonboundary drawing C e := by
...
theorem SchoenfliesGraph.pointSet_traceGraph_eq_interiorPart
    {G : SchoenfliesGraph Plane γ} {drawing : γ → ℝ → Plane}
    (h : G.IsDrawing drawing) (D : Set Plane) :
    SchoenfliesGraph.pointSet (SchoenfliesGraph.traceGraph G drawing D) drawing =
      SchoenfliesGraph.interiorPart G drawing D := by
  apply Set.Subset.antisymm
  · rintro z (hzV | hzE)
    · exact Or.inl hzV
    · obtain ⟨e, he, hze⟩ := Set.mem_iUnion₂.1 hzE
      obtain ⟨x, y, hxy⟩ :=
        (SchoenfliesGraph.traceGraph G drawing D).exists_isLink_of_mem_edgeSet he
      have ht := (SchoenfliesGraph.traceGraph_isLink D).1 hxy
  ...
  · rintro z (hzV | hzE)
    ...
    · obtain ⟨e, ⟨he, heD⟩, hze⟩ := Set.mem_iUnion₂.1 hzE
      ...
      exact Set.mem_iUnion₂.2 ⟨e, by
        rw [SchoenfliesGraph.edgeSet_eq_setOf_exists_isLink]
        exact ⟨x, y, (SchoenfliesGraph.traceGraph_isLink D).2
...
theorem SchoenfliesGraph.IsDrawing.isPolygonal_edgesCover_of_mem
    {G : SchoenfliesGraph Plane γ} {drawing : γ → ℝ → Plane}
    (h : G.IsDrawing drawing) {a b : Plane} {W : List γ}
    (hW : G.IsWalk a W b) (hne : W ≠ [])
    (hpoly : ∀ e ∈ W, IsPolygonal (SchoenfliesGraph.edgeArc drawing e)) :
    IsPolygonal (SchoenfliesGraph.edgesCover drawing W) := by
  induction hW with
...
theorem SchoenfliesGraph.IsWalk.exists_lift_map
    {α α' β : Type*} {G : SchoenfliesGraph α β} {f : α → α'}
  ...
  | nil =>
      ...
      exact ⟨u, SchoenfliesGraph.IsWalk.nil hu, hy⟩
  | cons e W ih =>
      cases h with
      | @cons _ w _ _ _ hlink htail =>
          ...
          exact ⟨v, SchoenfliesGraph.IsWalk.cons hlink₀ hv, hfv⟩

/-- The corresponding lifting statement for a simple path. -/
theorem SchoenfliesGraph.IsPath.exists_lift_map
    {α α' β : Type*} {G : SchoenfliesGraph α β} {f : α → α'}
    (hf : Set.InjOn f V(G)) {u : α} (hu : u ∈ V(G))
  ...
  | nil =>
      ...
      exact ⟨u, SchoenfliesGraph.IsPath.nil hu, hy⟩
  | cons e W ih =>
      cases h with
      | @cons _ w _ _ _ hlink htail hfresh =>
          ...
          subst w
          obtain ⟨v, hv, hfv⟩ := ih hlink₀.right_mem htail
          refine ⟨v, SchoenfliesGraph.IsPath.cons hlink₀ hv ?_, hfv⟩
          intro huTail
          apply hfresh
          rw [SchoenfliesGraph.walkVertices_map]
          exact ⟨u, huTail, rfl⟩
...
theorem CellStructure.SkeletonHomeo.image_edgesCover
    ...
    (hW : ∀ e ∈ W, e ∈ E(S.skel)) :
    g.toFun '' SchoenfliesGraph.edgesCover R₁.drawing W = SchoenfliesGraph.edgesCover R₂.drawing W := by
  induction W with
  | nil => simp
  | cons e W ih =>
      rw [SchoenfliesGraph.edgesCover_cons, SchoenfliesGraph.edgesCover_cons, Set.image_union,
...
theorem SchoenfliesGraph.IsStageOn.exists_crosscut_path
    {G : SchoenfliesGraph Plane γ} {drawing : γ → ℝ → Plane} [G.Finite]
    (h : G.IsStageOn drawing C (inside C)) (hC : IsJordanCurve C)
    {a b : Plane} (hab : a ≠ b) (haC : a ∈ C) (hbC : b ∈ C)
    (ha : ∃ e, G.Inc e a ∧ SchoenfliesGraph.Nonboundary drawing C e)
    (hb : ∃ e, G.Inc e b ∧ SchoenfliesGraph.Nonboundary drawing C e) :
    ∃ W : List γ, G.IsPath a W b ∧
      (∀ e ∈ W, SchoenfliesGraph.Nonboundary drawing C e) ∧
      IsCrosscut C (SchoenfliesGraph.edgesCover drawing W) a b := by
  classical
  by_cases hbig : ∃ (e : γ) (x y : Plane),
      G.IsLink e x y ∧ SchoenfliesGraph.Nonboundary drawing C e ∧ x ∈ C ∧ y ∈ C
  · obtain ⟨e, x, y, hlink, hnonboundary, hxC, hyC⟩ := hbig
    ...
    have hpath : G.IsPath a [e] b := SchoenfliesGraph.IsPath.single hlab hab
    have hpoly : IsPolygonal (SchoenfliesGraph.edgesCover drawing [e]) := by
      rw [SchoenfliesGraph.edgesCover_cons, SchoenfliesGraph.edgesCover_nil, union_empty]
      exact h.polygonal hlab.edge_mem hnonboundary
    have hinter : SchoenfliesGraph.edgesCover drawing [e] \ {a, b} ⊆ inside C := by
      rw [SchoenfliesGraph.edgesCover_cons, SchoenfliesGraph.edgesCover_nil, union_empty]
  ...
  · have hcase : ∀ ⦃e : γ⦄ ⦃x y : Plane⦄, G.IsLink e x y →
        SchoenfliesGraph.Nonboundary drawing C e → x ∈ C → y ∉ C :=
    ...
    let K := SchoenfliesGraph.traceGraph G drawing (inside C)
    have hKle : K ≤ G := SchoenfliesGraph.traceGraph_le _
    letI : K.Finite := SchoenfliesGraph.Finite.of_le hKle
    have hKpoint : SchoenfliesGraph.pointSet K drawing =
        SchoenfliesGraph.interiorPart G drawing (inside C) :=
      SchoenfliesGraph.pointSet_traceGraph_eq_interiorPart h.isDrawing (inside C)
    have hyaK : ya ∈ V(K) := by
    ...
    have hlast : G.IsPath yb [eb] b := SchoenfliesGraph.IsPath.single hlb.symm hybne
    have htail : G.IsPath ya (W ++ [eb]) b := by
      apply hWG.append hlast
      intro y hyW hyLast
      have hyK : y ∈ V(K) := by
        rw [SchoenfliesGraph.walkVertices_congr_of_le hKle
          (fun e he => hWK.edge_mem he)] at hyW
        exact SchoenfliesGraph.walkVertices_subset_vertexSet hWK.left_mem hyW
      have hyD : y ∈ inside C := by
        rw [SchoenfliesGraph.traceGraph_vertexSet] at hyK
        exact hyK.2
      rcases SchoenfliesGraph.mem_walkVertices_iff.1 hyLast with rfl | hyCovered
      · rfl
    ...
    have hafresh : a ∉ G.walkVertices ya (W ++ [eb]) := by
      intro haTail
      rcases SchoenfliesGraph.mem_walkVertices_iff.1 haTail with haya | haCovered
      · exact h.notMem_of_mem_curve haC (haya ▸ hyaD)
      · rw [SchoenfliesGraph.coveredVertices_append] at haCovered
        rcases haCovered with haW | haeb
        · have haK : a ∈ V(K) := by
            have hcoverEq : G.coveredVertices W = K.coveredVertices W :=
              SchoenfliesGraph.coveredVertices_congr_of_le hKle (fun e he => hWK.edge_mem he)
            exact SchoenfliesGraph.coveredVertices_subset_vertexSet (hcoverEq ▸ haW)
          rw [SchoenfliesGraph.traceGraph_vertexSet] at haK
    ...
    let Wfull := ea :: (W ++ [eb])
    have hfull : G.IsPath a Wfull b := SchoenfliesGraph.IsPath.cons hla htail hafresh
    have hfullne : Wfull ≠ [] := List.cons_ne_nil _ _
    have hWnonboundary : ∀ e ∈ Wfull, SchoenfliesGraph.Nonboundary drawing C e := by
      intro e he
      ...
      · have heK := hWK.edge_mem heW
        ...
        have ht := (SchoenfliesGraph.traceGraph_isLink (inside C)).1 hxyK
        intro houter
      ...
      · have heq : e = eb := by simpa using heLast
        exact heq ▸ hbnb
    have hpoly : IsPolygonal (SchoenfliesGraph.edgesCover drawing Wfull) :=
      SchoenfliesGraph.IsDrawing.isPolygonal_edgesCover_of_mem h.isDrawing hfull.isWalk hfullne
        (fun e he => h.polygonal (hfull.edge_mem he) (hWnonboundary e he))
    have hinter : SchoenfliesGraph.edgesCover drawing Wfull \ {a, b} ⊆ inside C := by
      rintro z ⟨hzCover, hzab⟩
      apply h.pointSet_diff_subset
      refine ⟨SchoenfliesGraph.edgesCover_subset_pointSet
        (fun e he => hfull.edge_mem he) hzCover, fun hzC => ?_⟩
      obtain ⟨e, he, hze⟩ := SchoenfliesGraph.mem_edgesCover_iff.1 hzCover
      rcases List.mem_cons.1 he with rfl | he
...
theorem quantitativeFresh_halfSpoke_subset_targetSkeleton [Infinite γ]
  ...
  have hyMesh : y ∈ SchoenfliesGraph.pointSet
  ...
  rw [w.reverse.overlay.transfer.skeletonSet_eq, SchoenfliesGraph.pointSet_relabelEdges]
  exact hyOverlay
...
theorem prescribedSourceAnchorSet_hasAnchorCrosscuts :
  ...
  have htargetStage : SchoenfliesGraph.IsStageOn r.pair.tgt.graph r.pair.tgt.drawing
  ...
  obtain ⟨ea, hea⟩ :=
    SchoenfliesGraph.IsDrawing.exists_nonboundary_incident_of_mem_closure
  ...
  obtain ⟨W, hWsrc, hWnonboundary, hPcross⟩ :=
    SchoenfliesGraph.IsStageOn.exists_crosscut_path hsourceStage hC hab haC hbC
      ⟨ea, hea⟩ ⟨eb, heb⟩
  let Q : Set Plane := SchoenfliesGraph.edgesCover r.pair.src.drawing W
  let Q' : Set Plane := r.pair.homeo.toFun '' Q
  have hQSkel : Q ⊆ r.pair.src.skeletonSet :=
    SchoenfliesGraph.edgesCover_subset_pointSet (fun e he => hWsrc.edge_mem he)
  ...
  have hWnotOuter : ∀ e ∈ W, e ∉ E(r.pair.str.outerGraph) := by
    ...
    have hxOuter : x ∈ r.pair.src.outerSet :=
      SchoenfliesGraph.edgeArc_subset_pointSet (by
        rw [SchoenfliesGraph.edgeSet_map]
        exact heOuter) hx
  ...
  obtain ⟨beta', hAbstractPath, hbeta'Pos⟩ :=
    SchoenfliesGraph.IsPath.exists_lift_map r.pair.src.injOn_pos halpha hsrcPathMapped
  ...
  have htargetPath : r.pair.tgt.graph.IsPath
    ...
    exact SchoenfliesGraph.IsPath.map r.pair.tgt.injOn_pos hAbstractPath
  have htargetPoly : IsPolygonal (SchoenfliesGraph.edgesCover r.pair.tgt.drawing W) :=
    SchoenfliesGraph.IsDrawing.isPolygonal_edgesCover_of_mem r.pair.tgt.isDrawing
      htargetPath.isWalk (hWsrc.ne_nil hab) (fun e he =>
        hrTargetAdmissible.isPolygonal (hWedge e he) (hWnotOuter e he))
  have hQ'image : Q' = SchoenfliesGraph.edgesCover r.pair.tgt.drawing W := by
