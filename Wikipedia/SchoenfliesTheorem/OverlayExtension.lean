...
variable {β δ κ : Type*}
  {G : SchoenfliesGraph Plane β} {Gdraw : β → ℝ → Plane}
  {H : SchoenfliesGraph Plane δ} {Hdraw : δ → ℝ → Plane}
  {K : SchoenfliesGraph Plane κ} {Kdraw : κ → ℝ → Plane}
...
theorem trans (hGH : IsPlaneSubdivisionExtension G Gdraw H Hdraw)
  ...
  edge_subset := by
    ...
    have hzH : z ∈ SchoenfliesGraph.pointSet H Hdraw :=
      hGH.pointSet_subset (SchoenfliesGraph.edgeArc_subset_pointSet he hze)
...
@[simp] theorem mem_currentOverlayPieces {pieces : List Piece} {extra : List Plane} {R : Piece} :
  ...
  rw [currentOverlayPieces, Finset.mem_toList, SchoenfliesGraph.mem_edgeFinset]
...
noncomputable def extendOverlay
    (pieces : List Piece) (extra : List Plane) (joins : List Piece) : SchoenfliesGraph Plane Piece :=
...
theorem cover_currentOverlayPieces (pieces : List Piece) (extra : List Plane) :
    cover (currentOverlayPieces pieces extra) =
      SchoenfliesGraph.pointSet (attachGraph pieces extra) segmentDrawing := by
  ext x
  constructor
  · intro hx
    obtain ⟨R, hR, hxR⟩ := mem_cover_iff.1 hx
    exact SchoenfliesGraph.edgeArc_subset_pointSet (mem_currentOverlayPieces.1 hR) (by
...
theorem extendOverlay_pointSet
    ...
    SchoenfliesGraph.pointSet (extendOverlay pieces extra joins) segmentDrawing =
      SchoenfliesGraph.pointSet (attachGraph pieces extra) segmentDrawing ∪ cover joins := by
...
theorem extendOverlay_isDrawing
    ...
    SchoenfliesGraph.IsDrawing (extendOverlay pieces extra joins) segmentDrawing := by
...
theorem extendOverlay_edge_subset
    ...
    ∀ {A : Piece}, A ∈ E(attachGraph pieces extra) → ∀ {R : Piece},
      ...
      (SchoenfliesGraph.edgeArc segmentDrawing R ∩
        (SchoenfliesGraph.edgeArc segmentDrawing A \ V(extendOverlay pieces extra joins))).Nonempty →
...
theorem attachGraphTrace_isTwoConnected_extendOverlay
    ...
    (SchoenfliesGraph.traceGraph (extendOverlay pieces extra joins) segmentDrawing
      (SchoenfliesGraph.pointSet (attachGraph pieces extra) segmentDrawing)).IsTwoConnected :=
