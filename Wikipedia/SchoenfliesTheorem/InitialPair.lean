...
theorem exists_isSetHomeoOn_modelCurve {C : Set Plane} (hC : IsJordanCurve C) :
  ...
  · rw [continuousOn_iff_continuous_restrict]
    have : (C.restrict fun p => if hp : p ∈ C then (h ⟨p, hp⟩ : Plane) else 0) =
        fun p : ↥C => (h p : Plane) := by
      funext p; simp [Set.restrict, p.2]
    rw [this]
    exact continuous_subtype_val.comp h.continuous
  · rw [continuousOn_iff_continuous_restrict]
    have : (modelCurve.restrict fun q => if hq : q ∈ modelCurve then (h.symm ⟨q, hq⟩ : Plane)
        else 0) = fun q : ↥modelCurve => (h.symm q : Plane) := by
      funext q; simp [Set.restrict, q.2]
    rw [this]
...
def initSkel : SchoenfliesGraph InitialCell InitialCell where
...
def initOuter : SchoenfliesGraph InitialCell InitialCell where
  vertexSet := InitialCell.vertices
...
@[simp] theorem edgeArc_edge (i : Fin 6) :
    SchoenfliesGraph.edgeArc H.draw (.edge i) = H.outer i '' I := rfl

@[simp] theorem edgeArc_chord : SchoenfliesGraph.edgeArc H.draw .chord = H.chordParam '' I := rfl
...
/-- **A vertex lies on an outer edge only if it is one of its two ends.** Not a primitive rule of
...
theorem isDrawing : SchoenfliesGraph.IsDrawing (initSkel.map H.point) H.draw where
  edge_param := by
    intro e he
    rw [SchoenfliesGraph.edgeSet_map, edgeSet_initSkel] at he
    rcases he with ⟨i, rfl⟩ | rfl
  ...
  edge_inter := by
    ...
    rw [SchoenfliesGraph.edgeSet_map, edgeSet_initSkel] at he hf
...
theorem outerSet_realization : H.realization.outerSet = H.outerArcs := by
  change V(initOuter.map H.point) ∪ (⋃ e ∈ E(initOuter.map H.point), SchoenfliesGraph.edgeArc H.draw e) = _
  ...
  · refine Set.iUnion₂_subset fun e he => ?_
    rw [SchoenfliesGraph.edgeSet_map, edgeSet_initOuter] at he
    obtain ⟨i, rfl⟩ := he
  ...
  · refine Set.iUnion_subset fun i => ?_
    ...
    exact Set.subset_biUnion_of_mem (u := fun e => SchoenfliesGraph.edgeArc H.draw e)
...
theorem skeletonSet_realization : H.realization.skeletonSet = H.outerArcs ∪ H.chordSet := by
  change V(initSkel.map H.point) ∪ (⋃ e ∈ E(initSkel.map H.point), SchoenfliesGraph.edgeArc H.draw e) = _
...
with one chord — and `SchoenfliesGraph.isTwoConnected_map_iff` carries it to both realizations at once, so
...
private theorem stepFull {m m' : Fin 6} (h : m + 1 = m') :
  ...
  subst h
  exact SchoenfliesGraph.Reaches.of_isLink (initSkel_isLink_ends (InitialCell.edge_mem_edges m))

/-- The abstract hexagon-with-chord is connected. -/
theorem connected_initSkel : initSkel.Connected := by
  refine SchoenfliesGraph.Connected.of_hub (u := .vert 0) ⟨0, rfl⟩ ?_
  rintro _ ⟨j, rfl⟩
  ...
  fin_cases j
  · exact SchoenfliesGraph.Reaches.refl ⟨0, rfl⟩
  · exact s01
  · exact s01.trans s12
...
private theorem stepDel (k : Fin 6) {m m' : Fin 6} (hmm : m + 1 = m') (hm : m ≠ 0)
  ...
  refine SchoenfliesGraph.Reaches.of_isLink (e := InitialCell.edge (k + m)) ?_
  rw [SchoenfliesGraph.deleteVerts_isLink]
...
theorem deleteVert_connected_initSkel (k : Fin 6) :
  ...
  refine SchoenfliesGraph.Connected.of_hub hhub ?_
  rintro _ ⟨⟨j, rfl⟩, hj⟩
  ...
  fin_cases m
  · exact absurd rfl hm
  · exact SchoenfliesGraph.Reaches.refl hhub
  · exact h12
  · exact h12.trans h23
...
theorem HexData.isTwoConnected_graph (H : HexData) : (H.realization.graph).IsTwoConnected :=
  (SchoenfliesGraph.isTwoConnected_map_iff H.injOn_point).2 isTwoConnected_initSkel
