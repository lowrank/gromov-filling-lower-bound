...
-/
import Wikipedia.SchoenfliesTheorem.PolygonBridge
...
*not* new: they exist as `SchoenfliesGraph.isBounded_closedSquare` and `SchoenfliesGraph.not_isBounded_beyondSquare` in
`Schoenflies/SchoenfliesGraph/OuterFace.lean`. Neither has any graph content — they are plane facts about
