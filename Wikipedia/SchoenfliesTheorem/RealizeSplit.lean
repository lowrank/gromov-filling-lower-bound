...
`SchoenfliesGraph` namespace. If a second consumer appears they belong in `Schoenflies/SchoenfliesGraph/`:

* `SchoenfliesGraph.IsPath.map`, `SchoenfliesGraph.walkVertices_map`, `SchoenfliesGraph.IsPathGraph.map` — a path pushes forward
  along an injective relabelling of the vertices. `SchoenfliesGraph.IsWalk.map` was already on `main` (in
...
* `SchoenfliesGraph.IsDrawing.isArcBetween_walkPointSet` — **the point set of a drawn path is a simple
  ...
  applies). The induction is on `SchoenfliesGraph.IsPath`, and the freshness clause of a path is exactly
...
variable {α α' β : Type*} {G : SchoenfliesGraph α β} {f : α → α'} {u v : α} {W : List β}
...
/-- **A path pushes forward along an injective relabelling.** Unlike `SchoenfliesGraph.IsWalk.map` this
...
the left-hand graph, and `SchoenfliesGraph.map` keeps the edge set, so the two resolutions agree. -/
theorem map_union (G H : SchoenfliesGraph α β) (f : α → α') :
    (G.union H).map f = (G.map f).union (H.map f) := by
  refine SchoenfliesGraph.ext (by simp [Set.image_union]) fun e x y => ?_
...
theorem IsPathGraph.map {P : SchoenfliesGraph α β} (hf : InjOn f V(P)) (h : P.IsPathGraph u W v) :
...
variable {β : Type*} {H K : SchoenfliesGraph Plane β} {drw : β → ℝ → Plane}

/-! ### The point set of a drawn path

`SchoenfliesGraph.pointSet` reads the vertex set and the edge set of a graph. Along a walk both are read
...
edges. For a path graph this is the whole of `SchoenfliesGraph.pointSet` (`walkPointSet_eq_pointSet`). -/
def walkPointSet (H : SchoenfliesGraph Plane β) (drw : β → ℝ → Plane) (u : Plane) (W : List β) : Set Plane :=
...
them. That is precisely the freshness clause of `SchoenfliesGraph.IsPath`: the vertex the path departs
...
theorem walkVertices_graph (u : γ) (W : List γ) :
  ...
  SchoenfliesGraph.walkVertices_map _ _ _
...
theorem cellUnion_pathCells {u v : γ} {W : List γ} (h : S.skel.IsWalk u W v) :
    R.cellUnion (S.pathCells u W) = SchoenfliesGraph.walkPointSet R.graph R.drawing (R.pos u) W := by
  have hsub : S.skel.walkVertices u W ⊆ V(S.skel) :=
    SchoenfliesGraph.walkVertices_subset_vertexSet h.left_mem
  ...
  rw [CellStructure.pathCells, cellUnion_union, hV, SchoenfliesGraph.walkPointSet, R.walkVertices_graph]
  refine Set.Subset.antisymm ?_ ?_
  · refine Set.union_subset (cellUnion_subset fun f hf => ?_) Set.subset_union_left
    obtain ⟨a, b, hl⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet (h.edge_mem hf)
    rw [R.cell_edge hl]
  ...
  · refine Set.union_subset Set.subset_union_right (Set.iUnion₂_subset fun f hf x hx => ?_)
    ...
    · simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hxab
      ...
      · exact Or.inr ⟨a, SchoenfliesGraph.mem_walkVertices_of_mem_covered ⟨f, hf, hl.inc_left⟩, rfl⟩
      · exact Or.inr ⟨b, SchoenfliesGraph.mem_walkVertices_of_mem_covered ⟨f, hf, hl.inc_right⟩, rfl⟩
...
theorem skeletonSet_subset_cellUnion :
  ...
  · obtain ⟨f, hf, hxf⟩ := Set.mem_iUnion₂.1 hx
    ...
    obtain ⟨a, b, hl⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet hf
    by_cases hxab : x ∈ ({R.pos a, R.pos b} : Set Plane)
    · simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hxab
      rcases hxab with rfl | rfl
...
theorem source_mem_cells₂ : d.source ∈ d.cells₂ :=
  Or.inr SchoenfliesGraph.mem_walkVertices_self
...
theorem subcells_face_diff : S.subcells d.face \ {d.face} = d.cells₁ ∪ d.cells₂ := by
  ext σ
  simp only [Set.mem_diff, Set.mem_singleton_iff, CellStructure.mem_subcells_iff, Set.mem_union]
  constructor
  · rintro ⟨⟨-, hsub⟩, hne⟩
...
def earGraph (d : S.SplitData) (earPos : γ → Plane) : SchoenfliesGraph Plane γ := d.ear.map earPos

@[simp] theorem vertexSet_earGraph (earPos : γ → Plane) :
    V(d.earGraph earPos) = earPos '' V(d.ear) := SchoenfliesGraph.vertexSet_map _ _

@[simp] theorem edgeSet_earGraph (earPos : γ → Plane) :
    E(d.earGraph earPos) = E(d.ear) := SchoenfliesGraph.edgeSet_map _ _
...
def earSet (d : S.SplitData) (earPos : γ → Plane) (earDraw : γ → ℝ → Plane) : Set Plane :=
  SchoenfliesGraph.pointSet (d.earGraph earPos) earDraw
...
structure EarCrosscut (d : S.SplitData) (R : S.Realization) (earPos : γ → Plane)
  ...
  isDrawing : SchoenfliesGraph.IsDrawing (d.earGraph earPos) earDraw
...
theorem mem_earSet_of_mem_ear {z : γ} (hz : z ∈ V(d.ear)) : earPos z ∈ d.earSet earPos earDraw :=
  SchoenfliesGraph.vertexSet_subset_pointSet (by rw [d.vertexSet_earGraph]; exact ⟨z, hz, rfl⟩)

theorem edgeArc_subset_earSet {f : γ} (hf : f ∈ E(d.ear)) :
    SchoenfliesGraph.edgeArc earDraw f ⊆ d.earSet earPos earDraw :=
  SchoenfliesGraph.edgeArc_subset_pointSet (by rw [d.edgeSet_earGraph]; exact hf)
...
theorem edgeArc_diff {f a b : γ} (hl : d.ear.IsLink f a b) :
    SchoenfliesGraph.edgeArc earDraw f \ earPos '' V(d.ear) =
      SchoenfliesGraph.edgeArc earDraw f \ {earPos a, earPos b} := by
  have hlm : (d.earGraph earPos).IsLink f (earPos a) (earPos b) := hl.map earPos
  refine Set.Subset.antisymm (Set.diff_subset_diff_right ?_) (fun x ⟨hx, hx'⟩ => ⟨hx, ?_⟩)
  · rintro _ (rfl | rfl)
...
noncomputable def splitCell (d : S.SplitData) (R : S.Realization) (earPos : γ → Plane)
  ...
  else if σ ∈ E(d.ear) then SchoenfliesGraph.edgeArc earDraw σ \ earPos '' V(d.ear)
  else R.cell σ
...
theorem splitCell_earEdge {f : γ} (hf : f ∈ E(d.ear)) :
    d.splitCell R earPos earDraw f = SchoenfliesGraph.edgeArc earDraw f \ earPos '' V(d.ear) := by
  classical
...
theorem edgeArc_splitDrawing_of_mem_skel {f : γ} (hf : f ∈ E(S.skel)) :
    SchoenfliesGraph.edgeArc (d.splitDrawing R earDraw) f = SchoenfliesGraph.edgeArc R.drawing f := by
  rw [SchoenfliesGraph.edgeArc, SchoenfliesGraph.edgeArc, splitDrawing_of_mem_skel hf]

theorem edgeArc_splitDrawing_of_mem_ear {f : γ} (hf : f ∈ E(d.ear)) :
    SchoenfliesGraph.edgeArc (d.splitDrawing R earDraw) f = SchoenfliesGraph.edgeArc earDraw f := by
  rw [SchoenfliesGraph.edgeArc, SchoenfliesGraph.edgeArc, splitDrawing_of_mem_ear hf]
...
theorem splitGraph_eq :
  ...
  rw [SchoenfliesGraph.map_union]
  congr 1
  · exact SchoenfliesGraph.map_eq_of_eqOn fun z hz => hE.splitPos_eq hz
  · exact SchoenfliesGraph.map_eq_of_eqOn fun z hz => splitPos_of_mem_ear hz

/-- **The old drawn skeleton with the drawn ear glued on is a plane graph.** The three clauses
of `SchoenfliesGraph.IsDrawing` all reduce, on a mixed pair, to the same fact: the ear meets the old
...
theorem isDrawing_split :
    SchoenfliesGraph.IsDrawing (R.graph.union (d.earGraph earPos)) (d.splitDrawing R earDraw) := by
  have hEskel : E(R.graph) = E(S.skel) := Realization.edgeSet_graph R
  have hcompat : R.graph.Compatible (d.earGraph earPos) :=
    SchoenfliesGraph.Compatible.of_disjoint_edgeSet (by
      rw [hEskel, d.edgeSet_earGraph]; exact d.disjoint_edgeSet)
  have hle₁ : R.graph ≤ R.graph.union (d.earGraph earPos) := SchoenfliesGraph.left_le_union _ _
  ...
  have hmix : ∀ {p : Plane} {e g : γ}, e ∈ E(S.skel) → g ∈ E(d.ear) →
      p ∈ SchoenfliesGraph.edgeArc R.drawing e → p ∈ SchoenfliesGraph.edgeArc earDraw g →
    ...
    have hp1 : p ∈ R.skeletonSet :=
      SchoenfliesGraph.edgeArc_subset_pointSet (by rwa [hEskel]) hpe
    have hp2 : p ∈ d.earSet earPos earDraw := edgeArc_subset_earSet hg hpg
    obtain ⟨hpV1, hpV2⟩ := hcross hp2 hp1
    obtain ⟨a, b, hlab⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet (show e ∈ E(R.graph) by rwa [hEskel])
    obtain ⟨c, c', hlcc⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet
  ...
  · -- each edge is drawn by its own parametrization
    ...
    · rw [hEskel] at he
      obtain ⟨hc, hi, hl⟩ := R.isDrawing.edge_param (by rwa [SchoenfliesGraph.edgeSet_map])
      rw [splitDrawing_of_mem_skel he]
      exact ⟨hc, hi, SchoenfliesGraph.union_isLink.2 (Or.inl hl)⟩
    · rw [d.edgeSet_earGraph] at he
      ...
      exact ⟨hc, hi, SchoenfliesGraph.union_isLink.2
  ...
  · -- an edge's arc meets the vertices only at its own ends
    ...
    rcases SchoenfliesGraph.union_isLink.1 hl with hl | ⟨-, hl⟩
    · have he : e ∈ E(S.skel) := by rw [← hEskel]; exact hl.edge_mem
      ...
      have hvg : v ∈ V(R.graph) := by
        ...
        · exact hv
        · exact (hcross (SchoenfliesGraph.vertexSet_subset_pointSet hv)
            (SchoenfliesGraph.edgeArc_subset_pointSet (by rwa [hEskel]) hmem)).1
    ...
    · have he : e ∈ E(d.ear) := by rw [← d.edgeSet_earGraph]; exact hl.edge_mem
      ...
      have hvg : v ∈ V(d.earGraph earPos) := by
        ...
        · exact (hcross (edgeArc_subset_earSet he hmem)
            (SchoenfliesGraph.vertexSet_subset_pointSet hv)).2
        · exact hv
  ...
  · -- two distinct edges meet only at shared vertices
    ...
    · rw [hEskel] at he hf
      ...
      obtain ⟨hpV, hIe, hIf⟩ := R.isDrawing.edge_inter (by rwa [SchoenfliesGraph.edgeSet_map])
        (by rwa [SchoenfliesGraph.edgeSet_map]) hef hpe hpf
...
theorem closure_edgeArc_diff {f a b : γ} (hl : d.ear.IsLink f a b) :
    closure (SchoenfliesGraph.edgeArc earDraw f \ {earPos a, earPos b}) = SchoenfliesGraph.edgeArc earDraw f := by
  have harc : IsArcBetween (SchoenfliesGraph.edgeArc earDraw f) (earPos a) (earPos b) :=
    hE.isDrawing.edge_isArcBetween (hl.map earPos)
  refine Set.Subset.antisymm (closure_minimal Set.diff_subset harc.isArc.isCompact.isClosed) ?_
  intro x hx
...
noncomputable def realize (R : S.Realization) (d : S.SplitData) (earPos : γ → Plane)
  ...
  injOn_pos := by
    have hsep : ∀ ⦃x y : γ⦄, x ∈ V(S.skel) → y ∈ V(d.ear) → y ∉ V(S.skel) →
      ...
      have h2 : R.pos x ∈ R.skeletonSet :=
        SchoenfliesGraph.vertexSet_subset_pointSet (by rw [Realization.vertexSet_graph]; exact ⟨x, hx, rfl⟩)
  ...
  cell_edge := by
    ...
    rcases SchoenfliesGraph.union_isLink.1 hl' with hk | ⟨-, hk⟩
...
theorem cellUnion_earCells :
  ...
  · have hσ' : σ ∈ V(d.ear) ∪ E(d.ear) := hσ
    ...
    · rw [realize_cell, splitCell_earEdge hσ']
      exact Set.Subset.trans Set.diff_subset (EarCrosscut.edgeArc_subset_earSet hσ')
  · intro x hx
    have hx' : x ∈ V(d.earGraph earPos) ∪
        ⋃ e ∈ E(d.earGraph earPos), SchoenfliesGraph.edgeArc earDraw e := hx
...
theorem isCrosscutSplit_realize {D : Set Plane} (hcd : R.IsCellDecomposition D)
  ...
  earNonempty := by
    ...
    · rw [realize_cell, hE.splitCell_earVertex hσV]
      exact ⟨earPos σ, rfl⟩
    · obtain ⟨a, b, hl⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet hσE
