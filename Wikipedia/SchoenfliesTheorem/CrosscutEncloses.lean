...
`SchoenfliesGraph.CrosscutEncloses` of `Schoenflies/OuterChain.lean` is that sentence weakened to *at least*
...
`SchoenfliesGraph.crosscutEnclosesOff` is a two-page corollary of it. A second general fact falls out on the
...
from every consumer's obligations, which matters here because `SchoenfliesGraph.IsCycleCrosscut` does not
record it (see below).

## Two defects of the interface handed to this module — read before integrating

**1. `SchoenfliesGraph.CrosscutEncloses` as stated on `main` is false**, and the deliverable is
`SchoenfliesGraph.CrosscutEnclosesOff`, which is it plus one hypothesis: `x ∈ exterior H drawing`. Nothing in
`SchoenfliesGraph.CrosscutEncloses` prevents `x` from lying **on the crosscut's own drawing**: `x` is only
...
**The repair costs the chain nothing.** `SchoenfliesGraph.OuterOnPairs` already puts `x` off the whole chain
(`SchoenfliesGraph.IsPlaneChain.mem_exterior_chain`), and every block lies inside the chain
(`SchoenfliesGraph.pointSet_mono`), so the clause can be produced where it is needed instead of being carried.
`SchoenfliesGraph.IsPlaneChain.descent_of_crosscutOff` below therefore proves `SchoenfliesGraph.Descent` **exactly as it
stands on `main`**, and `SchoenfliesGraph.IsPlaneChain.outer_chain_of_crosscutExists` is `lem:outer-chain` with
`SchoenfliesGraph.CrosscutEncloses` discharged outright. Nothing in `Schoenflies/OuterChain.lean` has to
change; `SchoenfliesGraph.CrosscutEncloses` should simply be deleted, together with
`SchoenfliesGraph.IsPlaneChain.descent_of_crosscut` and `SchoenfliesGraph.IsPlaneChain.outer_chain_of_crosscut`, which the
two theorems here replace.

**2. `SchoenfliesGraph.IsCycleCrosscut` does not record that the two arcs meet only at the two cut points**,
although `SchoenfliesGraph.IsCycleThrough.split_at` — the theorem that builds them — produces exactly that
...
* `SchoenfliesGraph.IsDrawing.mem_edgesCover_of_mem_walkVertices` — the converse of
  `SchoenfliesGraph.IsDrawing.mem_walkVertices_of_mem_edgesCover`; general, belongs in
  `Schoenflies/SchoenfliesGraph/CycleJordan.lean`.
* `SchoenfliesGraph.CrosscutEnclosesOff`, `SchoenfliesGraph.crosscutEnclosesOff` — the geometric half of the descent
  step of `lem:outer-chain`: `SchoenfliesGraph.CrosscutEncloses` with the missing hypothesis restored,
  **proved**.
* `SchoenfliesGraph.IsPlaneChain.descent_of_crosscutOff` — `SchoenfliesGraph.Descent` from `SchoenfliesGraph.CrosscutExists` alone;
  a drop-in replacement for `SchoenfliesGraph.IsPlaneChain.descent_of_crosscut`.
* `SchoenfliesGraph.IsPlaneChain.outer_chain_of_crosscutExists` — **`lem:outer-chain`**, assuming only the
  combinatorial half `SchoenfliesGraph.CrosscutExists`.
-/
...
variable {β : Type*} {H F : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}

/-! ## What a walk visits lies on what it draws

`SchoenfliesGraph.IsDrawing.mem_walkVertices_of_mem_edgesCover` goes the other way; this direction is the
first half of the proof of `SchoenfliesGraph.IsDrawing.pointSet_pathGraphOf`, extracted. -/
...
`SchoenfliesGraph.CrosscutEncloses` of `Schoenflies/OuterChain.lean`, plus the hypothesis it is missing. -/

/-- **`SchoenfliesGraph.CrosscutEncloses` with the clause it is missing**: `x` is off the drawing of `H`.

Without it the statement is false — see the module docstring. `SchoenfliesGraph.CrosscutEncloses` puts `x`
...
`SchoenfliesGraph.CrosscutEncloses` verbatim, so that the substitution into
`SchoenfliesGraph.IsPlaneChain.descent_of_crosscut` is mechanical once `SchoenfliesGraph.Descent` carries the same
clause. -/
def CrosscutEnclosesOff (drawing : β → ℝ → Plane) (x : Plane) : Prop :=
  ∀ (H F : SchoenfliesGraph Plane β), H.Finite → IsDrawing H drawing →
...
by `SchoenfliesGraph.IsDrawing.path_isArcBetween`, they cover the cycle's realisation because their edge lists
do, and they meet exactly at the two cut points by `Schoenflies.two_arcs_inter_of_union` — which is
why `SchoenfliesGraph.IsCycleCrosscut` gets away with not recording that. That the crosscut meets the cycle
only at its ends is the `edges_new` and `interior` clauses read through
`SchoenfliesGraph.IsDrawing.edge_inter`. -/
theorem crosscutEnclosesOff (drawing : β → ℝ → Plane) (x : Plane) :
  ...
  -- **The clause `SchoenfliesGraph.CrosscutEncloses` is missing**: `x` is off the crosscut, because it is off
  -- the whole drawing.
...
The extra hypothesis of `SchoenfliesGraph.CrosscutEnclosesOff` does **not** have to be threaded through
`SchoenfliesGraph.Descent`: the chain hypothesis `SchoenfliesGraph.OuterOnPairs` already puts `x` off the whole chain
(`SchoenfliesGraph.IsPlaneChain.mem_exterior_chain`), and every block of the chain lies inside it. So the
repair costs the interface nothing at all — `SchoenfliesGraph.Descent` is used here exactly as it stands on
`main`, and the two theorems below are drop-in replacements for
`SchoenfliesGraph.IsPlaneChain.descent_of_crosscut` and `SchoenfliesGraph.IsPlaneChain.outer_chain_of_crosscut` with
`SchoenfliesGraph.CrosscutEncloses` discharged. -/

namespace IsPlaneChain

variable {Γ : ℕ → SchoenfliesGraph Plane β} {G : SchoenfliesGraph Plane β} {n : ℕ} {x : Plane}

/-- **The descent step from its two halves**, with the geometric half taken in the corrected form
`SchoenfliesGraph.CrosscutEnclosesOff`. The missing clause "`x` is off the drawing of the block" is supplied
from `SchoenfliesGraph.OuterOnPairs`, so `SchoenfliesGraph.Descent` is produced with its statement on `main` unchanged.
-/
...
discharged.** Only `SchoenfliesGraph.CrosscutExists` — the purely combinatorial construction of the crosscut
— is still assumed.

This is `SchoenfliesGraph.IsPlaneChain.outer_chain_of_crosscut` with the argument `hcen` supplied by
`SchoenfliesGraph.crosscutEnclosesOff`. -/
