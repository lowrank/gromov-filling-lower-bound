...
theorem pos_pair_subset_edgeArc (R : S.Realization) {e a b : γ} (hl : S.skel.IsLink e a b) :
    ({R.pos a, R.pos b} : Set Plane) ⊆ SchoenfliesGraph.edgeArc R.drawing e := by
  obtain ⟨-, -, hlink⟩ := R.isDrawing.edge_param (by rw [SchoenfliesGraph.edgeSet_map]; exact hl.edge_mem)
  have h0 : R.drawing e 0 ∈ SchoenfliesGraph.edgeArc R.drawing e := Set.mem_image_of_mem _ zero_mem_I
  have h1 : R.drawing e 1 ∈ SchoenfliesGraph.edgeArc R.drawing e := Set.mem_image_of_mem _ one_mem_I
...
theorem image_cell (g : SkeletonHomeo R₁ R₂) (hσ : σ ∈ V(S.skel) ∪ E(S.skel)) :
  ...
  · obtain ⟨a, b, hl⟩ := SchoenfliesGraph.exists_isLink_of_mem_edgeSet he
    have harc : SchoenfliesGraph.edgeArc R₁.drawing σ ⊆ R₁.skeletonSet :=
      SchoenfliesGraph.edgeArc_subset_pointSet (by rw [Realization.edgeSet_graph]; exact he)
    rw [R₁.cell_edge hl, R₂.cell_edge hl,
      (g.injOn.mono harc).image_diff_subset (R₁.pos_pair_subset_edgeArc hl),
