...
2. *Two nearby congruent squares meet twice.* The route taken is the coordinate one, not the
   ...
   `SchoenfliesGraph.IsTwoConnected.union` consumes. See `exists_two_common_vertices` and the discussion
   before it.
...
* `exists_two_common_vertices`, `exists_two_cut_points`, `exists_overlayPiece_end_subset` —
  ...
  `lem:union-two-connected` (`SchoenfliesGraph.IsTwoConnected.union`) consumes.
...
`SchoenfliesGraph.isBounded_closedSquare` is the same fact for the centre `0` only. -/
...
theorem not_isBounded_compl_closedSquare (c : Plane) (r : ℝ) :
  ...
  SchoenfliesGraph.not_isBounded_beyondSquare (r + supNorm c)
...
`Schoenflies/SchoenfliesGraph/Redrawing.lean`, which is far above this module in the import order (its
...
graph. Two common points would not be enough for `SchoenfliesGraph.IsTwoConnected.union`, which wants two
...
`SchoenfliesGraph.IsTwoConnected.union` needs two common **vertices**, not two common points.  The
...
This is the form `SchoenfliesGraph.IsTwoConnected.union` consumes: `a ≠ b`, with both `a` and `b` vertices
...
`Γ : ℕ → SchoenfliesGraph Plane Piece` are the chain graphs, all subgraphs of **one** overlay graph `G`
...
the `Γ i` are `SchoenfliesGraph.Compatible` with each other for free (they share edge names because they
share the ambient edge type `Piece`).

```
theorem outer_chain {k : ℕ} (hk : 3 ≤ k) (Γ : ℕ → SchoenfliesGraph Plane Piece)
    (drawing : Piece → ℝ → Plane)
    (hfin : ∀ i < k, (Γ i).Finite)
    (hdraw : ∀ i < k, SchoenfliesGraph.IsDrawing (Γ i) drawing)
    (h2c : ∀ i < k, (Γ i).IsTwoConnected)
    (hshare : ∀ i, i + 1 < k → ∃ a b : Plane, a ≠ b ∧
      a ∈ V(Γ i) ∧ a ∈ V(Γ (i+1)) ∧ b ∈ V(Γ i) ∧ b ∈ V(Γ (i+1)))
    (hfar : ∀ i j, i < k → j < k → i + 2 ≤ j →
      Disjoint (SchoenfliesGraph.pointSet (Γ i) drawing) (SchoenfliesGraph.pointSet (Γ j) drawing))
    {x : Plane}
    (houter : ∀ i, i + 1 < k → ¬ Bornology.IsBounded
      (SchoenfliesGraph.face ((Γ i).union (Γ (i+1))) drawing x)) :
    ¬ Bornology.IsBounded (SchoenfliesGraph.face (chainUnion Γ k) drawing x)
```

with `chainUnion Γ k` the fold of `SchoenfliesGraph.union` over `i < k`, exported as a `def` with
...
* **"outer face" must be stated as `¬ IsBounded (face …)`, not as `face … = face … base` for
  some chosen base point.**  `SchoenfliesGraph.exists_unbounded_face` and `SchoenfliesGraph.unbounded_face_unique`
  make the two equivalent, but the arc-complement proof gets its hypothesis from
  `SchoenfliesGraph.beyondSquare_subset_face` applied to a containing square, which is naturally an
...
* **`hshare` must be about vertices**, which is what `exists_two_common_vertices` and
  ...
  *points* would not compose with `SchoenfliesGraph.IsTwoConnected.union`.
