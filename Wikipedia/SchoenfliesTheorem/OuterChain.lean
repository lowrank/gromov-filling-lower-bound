...
graph `G` — see `SchoenfliesGraph.IsPlaneChain`. Two members therefore agree about the ends of a shared
edge, meet only where `G` lets them meet, and `Γ p ∪ Γ q` is `SchoenfliesGraph.union` with nothing to
...
`SchoenfliesGraph.chainUnion Γ i m` is the block `Γ i ∪ Γ (i+1) ∪ ⋯ ∪ Γ (i+m)`, indexed by its **length**
...
`¬ Bornology.IsBounded (face H drawing x)`. `SchoenfliesGraph.unbounded_face_unique` is what makes that
"the" outer face, and `SchoenfliesGraph.beyondSquare_subset_face` is how a consumer proves it.
...
* the reduction of "`x` lies in a bounded face" to "some cycle encloses `x`"
  (`SchoenfliesGraph.encloses_of_isBounded_face`, from `SchoenfliesGraph.face_cycles'`) and back
  (`SchoenfliesGraph.isBounded_face_of_encloses`);
* 2-connectivity of every block, by iterating `SchoenfliesGraph.IsTwoConnected.union`
...
* the two minimality consequences "`C` contains an edge of `Γ j` outside `Γ (j-1)`" and "`C`
  ...
  down into a shorter block with `SchoenfliesGraph.IsCycleThrough.anti`;
* the splicing of a crosscut onto each of the two arcs (`SchoenfliesGraph.exists_spliced_cycles`, from
  `SchoenfliesGraph.exists_spliced_cycle`) and the edge count that makes the splice a strict improvement
  (`SchoenfliesGraph.edgesOutside_splice_lt`, `SchoenfliesGraph.exists_cycle_edgesOutside_lt`).

`SchoenfliesGraph.Descent` is the middle paragraph of the blueprint's proof — from a cycle `C` of
...
* `Schoenflies/CrosscutExists.lean` — combinatorial: build the crosscut, as
  `SchoenfliesGraph.IsPlaneChain.crosscutExists`. Nothing geometric enters beyond
  `SchoenfliesGraph.IsPlaneChain.disjoint_block_far`, "`Γ j` meets the earlier chain only through
  `Γ (j-1)`".
* `Schoenflies/CrosscutEncloses.lean` — geometric: "the path `R` lies on one side of `C` and is
  ...
  `x`", as `SchoenfliesGraph.crosscutEnclosesOff`. That module also carries the counterexample showing why
...
* `SchoenfliesGraph.chainUnion` — `Γ i ∪ ⋯ ∪ Γ (i+m)`, with `chainUnion_le`, `le_chainUnion`,
  `chainUnion_finite`, `chainUnion_isTwoConnected` (the last is repeated
  `lem:union-two-connected`).
* `SchoenfliesGraph.Encloses` — "that face is the interior of its boundary cycle, which therefore encloses
  `x`", as a property of a graph.
* `SchoenfliesGraph.encloses_of_isBounded_face`, `SchoenfliesGraph.isBounded_face_of_encloses` — the two directions
...
* `SchoenfliesGraph.IsPlaneChain` — the hypotheses of `lem:outer-chain`; `SchoenfliesGraph.OuterOnPairs` — its
  hypothesis on consecutive pairs.
* `SchoenfliesGraph.IsCycleCrosscut` — the crosscut `R` of the cycle `C`, with the two arcs it cuts.
* `SchoenfliesGraph.exists_spliced_cycles` — the two cycles `R ∪ C₁`, `R ∪ C₂`.
* `SchoenfliesGraph.edgesOutside`, `SchoenfliesGraph.edgesOutside_splice_lt`,
  `SchoenfliesGraph.exists_cycle_edgesOutside_lt` — "it has fewer edges outside `Γ (j-1)` than `C`, since
  one nonempty outside portion of `C` has been replaced by `R ⊆ Γ (j-1)`".
* `SchoenfliesGraph.Descent` — the crosscut paragraph of the proof; discharged in
  `Schoenflies/CrosscutEncloses.lean` from `SchoenfliesGraph.CrosscutExists` alone.
* `SchoenfliesGraph.IsPlaneChain.not_encloses` — the double minimisation.
* `SchoenfliesGraph.IsPlaneChain.outer_chain` — **`lem:outer-chain`**, from `SchoenfliesGraph.Descent`;
  `SchoenfliesGraph.IsPlaneChain.outer_chain'` in `Schoenflies/OuterChainClosed.lean` is the same
...
variable {α β : Type*} {Γ : ℕ → SchoenfliesGraph α β} {G K : SchoenfliesGraph α β} {i m p : ℕ}
...
def chainUnion (Γ : ℕ → SchoenfliesGraph α β) (i : ℕ) : ℕ → SchoenfliesGraph α β
  | 0 => Γ i
  | m + 1 => (chainUnion Γ i m).union (Γ (i + m + 1))

@[simp]
theorem chainUnion_zero (Γ : ℕ → SchoenfliesGraph α β) (i : ℕ) : chainUnion Γ i 0 = Γ i := rfl

@[simp]
theorem chainUnion_succ (Γ : ℕ → SchoenfliesGraph α β) (i m : ℕ) :
    chainUnion Γ i (m + 1) = (chainUnion Γ i m).union (Γ (i + m + 1)) := rfl

/-- A block of length one is a consecutive pair. -/
theorem chainUnion_one (Γ : ℕ → SchoenfliesGraph α β) (i : ℕ) :
...
*right*-hand summand of a `SchoenfliesGraph.union` a subgraph, and it is free once every member lies in one
ambient graph. -/
...
`SchoenfliesGraph.IsCycleThrough.anti` is the cycle counterpart of `SchoenfliesGraph.IsPath.anti`; it is what turns
...
structure IsCycleCrosscut (H F : SchoenfliesGraph α β) (e : β) (u v : α) (D : List β) (a b : α)
...
with their edge lists named. Both are produced by `SchoenfliesGraph.exists_spliced_cycle`, applied to the
cycle's own subgraph and to each arc in turn. -/
theorem exists_spliced_cycles {H F : SchoenfliesGraph α β} {e : β} {u v a b : α} {D R D₁ D₂ : List β}
...
def edgesOutside (H : SchoenfliesGraph α β) (W : List β) : Set β := {g | g ∈ W ∧ g ∉ E(H)}

theorem mem_edgesOutside {H : SchoenfliesGraph α β} {W : List β} {g : β} :
    g ∈ edgesOutside H W ↔ g ∈ W ∧ g ∉ E(H) := Iff.rfl

theorem finite_edgesOutside (H : SchoenfliesGraph α β) (W : List β) : (edgesOutside H W).Finite :=
  (W.finite_toSet).subset fun _ hg => hg.1

theorem edgesOutside_append (H : SchoenfliesGraph α β) (W₁ W₂ : List β) :
    edgesOutside H (W₁ ++ W₂) = edgesOutside H W₁ ∪ edgesOutside H W₂ := by
  ext g
  simp only [mem_edgesOutside, List.mem_append, Set.mem_union]
  tauto

theorem edgesOutside_perm {H : SchoenfliesGraph α β} {W₁ W₂ : List β} (hp : W₁.Perm W₂) :
    edgesOutside H W₁ = edgesOutside H W₂ := by
  ext g
  simp only [mem_edgesOutside, hp.mem_iff]

theorem edgesOutside_eq_empty {H : SchoenfliesGraph α β} {W : List β} (h : ∀ g ∈ W, g ∈ E(H)) :
    edgesOutside H W = ∅ := by
  ext g
...
supplies. It holds for a cycle because the detour of `SchoenfliesGraph.IsCycleThrough` is a path and does
not contain the named edge. -/
theorem edgesOutside_splice_lt {H : SchoenfliesGraph α β} {W W' D₁ D₂ R : List β} (hnodup : W.Nodup)
...
variable {β : Type*} {H K : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {x : Plane}
...
def Encloses (H : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (x : Plane) : Prop :=
...
`R ∪ C₁`, `R ∪ C₂` encloses `x`", so a discharger of `SchoenfliesGraph.Descent` has only to produce the
crosscut and decide which side. The splitting of the cycle is `SchoenfliesGraph.IsCycleThrough.split_at`
and the two spliced cycles are `SchoenfliesGraph.exists_spliced_cycle`; both hand back exactly the
permutation statements assumed here. -/
theorem exists_cycle_edgesOutside_lt {H F : SchoenfliesGraph Plane β} {e : β} {u v : Plane} {D : List β}
...
variable {β : Type*} {Γ : ℕ → SchoenfliesGraph Plane β} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
...
`k ≥ 3`. -/
structure IsPlaneChain (Γ : ℕ → SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (G : SchoenfliesGraph Plane β)
    (n : ℕ) : Prop where
...
`SchoenfliesGraph.IsCycleThrough.split_at`, `SchoenfliesGraph.exists_spliced_cycle`, `SchoenfliesGraph.IsDrawing.arcs_of_split`
and `Schoenflies.crosscutSplitsRegion` available. -/
def Descent (Γ : ℕ → SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (n : ℕ) (x : Plane) : Prop :=
  ∀ (i m : ℕ), i + (m + 2) ≤ n →
    ∀ (e : β) (u v : Plane) (D : List β),
...
`SchoenfliesGraph.Descent` is what the main theorem consumes, and `SchoenfliesGraph.descent_of_crosscut` derives it
...
def CrosscutExists (Γ : ℕ → SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (n : ℕ) (x : Plane) :
    Prop :=
  ∀ (i m : ℕ), i + (m + 2) ≤ n →
    ∀ (e : β) (u v : Plane) (D : List β),
      (chainUnion Γ i (m + 2)).IsCycleThrough e u v D →
...
the same statement as `SchoenfliesGraph.CrosscutEnclosesOff` in `Schoenflies/CrosscutEncloses.lean` but
...
The repair costs the consumer nothing, because `SchoenfliesGraph.OuterOnPairs` already places `x` off the
whole chain: `SchoenfliesGraph.IsPlaneChain.descent_of_crosscutOff` proves `SchoenfliesGraph.Descent` below verbatim,
...
counterexample, instead of as a proof placeholder nobody revisited. -/
...
second clause with `SchoenfliesGraph.beyondSquare_subset_face`, and `SchoenfliesGraph.unbounded_face_unique` is what
makes "an unbounded face" *the* outer face. -/
def OuterOnPairs (Γ : ℕ → SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (n : ℕ) (x : Plane) : Prop :=
...
`SchoenfliesGraph.unbounded_face_unique` there is only one unbounded face, so this really does say `x` is
in *the* outer face. -/
