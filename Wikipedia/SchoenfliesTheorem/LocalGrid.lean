...
* `lem:union-two-connected` — used through `SchoenfliesGraph.IsTwoConnected.union` and
  `SchoenfliesGraph.IsTwoConnected.ear`; `SchoenfliesGraph.IsTwoConnected.of_le_of_vertexSet_subset` is the
...
theorem not_isTwoConnected_meshGraph_of_fresh_subsingleton {N : ℕ} (hN : 2 ≤ N)
  ...
  have hinv : ∀ {u v : Plane}, ((meshGraph N fresh anchors).deleteVerts {z}).Reaches u v →
    ...
    | @cons a b c e W hl _ ih =>
      rw [SchoenfliesGraph.deleteVerts_isLink] at hl
  ...
  have := hinv ((h2.deleteVerts_connected' z).reaches
    (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne hcV hcz)
    (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne hdV hdz))
...
variable {α β : Type*} {H K : SchoenfliesGraph α β}
...
theorem IsTwoConnected.of_le_of_vertexSet_subset (hH : H.IsTwoConnected) (hHK : H ≤ K)
  ...
  deleteVerts_connected := fun x _ =>
    (hH.deleteVerts_connected' x).of_spanning_le (deleteVerts_mono hHK _) (by
      ...
      exact Set.diff_subset_diff_left hV)
...
noncomputable def localGrid (p : Plane) (s : ℝ) (k : ℕ) : SchoenfliesGraph Plane Piece :=
...
theorem localGrid_isDrawing (hs : 0 < s) (hk : 1 ≤ k) :
    SchoenfliesGraph.IsDrawing (localGrid p s k) segmentDrawing :=
...
theorem localGrid_fine (hs : 0 < s) (hε : 0 < ε) :
    (localGrid p s (localGridCount s ε)).IsTwoConnected ∧
      SchoenfliesGraph.IsDrawing (localGrid p s (localGridCount s ε)) segmentDrawing ∧
