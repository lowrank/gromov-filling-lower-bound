...
hypothesis and restates the lemma without it (`SchoenfliesGraph.face_cycles'`,
`SchoenfliesGraph.IsDrawing.hasFaceCycles_union'`).
...
`Schoenflies/SchoenfliesGraph/K33Land.lean`. It is `Schoenflies.ClosedPolygon.arcPieces` and its API of
...
`Schoenflies.CrosscutSplitsRegion` can be deleted from `SchoenfliesGraph.face_cycles` and
`SchoenfliesGraph.IsDrawing.hasFaceCycles_union`, and the two primed statements at the end here dropped.
...
* `SchoenfliesGraph.IsDrawing.hasFaceCycles_union'` — one ear of `lem:face-cycles`.
* `SchoenfliesGraph.face_cycles'` — **`lem:face-cycles`**.
* `SchoenfliesGraph.IsFaceCycle.eq_inside_of_isBounded'` — "in particular, every bounded face is the
...
variable {β : Type*} {G B : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {a b : Plane}

/-! ## `lem:face-cycles`, with nothing assumed

`SchoenfliesGraph.IsDrawing.hasFaceCycles_union` and `SchoenfliesGraph.face_cycles` of
...
/-- **One ear**, with no hypothesis assumed: `SchoenfliesGraph.IsDrawing.hasFaceCycles_union` with
...
This is `SchoenfliesGraph.face_cycles` with its hypothesis `Schoenflies.CrosscutSplitsRegion` discharged by
