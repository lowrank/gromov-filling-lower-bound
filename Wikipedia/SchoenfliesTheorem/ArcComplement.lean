...
`SchoenfliesGraph.IsPlaneChain` requires every link `Γ i` to be a subgraph of **one** ambient plane graph
...
* `squareGraph pieces points c r` — the edges of the overlay lying on the boundary of the square
  ...
  (`mem_vertexSet_squareGraph`), which is what `SchoenfliesGraph.IsTwoConnected.union` needs.
* `familyChain c N m r i` — the `i`-th link: `SchoenfliesGraph.chainUnion` of the `m + 1` square graphs of
...
`isPlaneChain_familyChain` is the theorem that this family is a `SchoenfliesGraph.IsPlaneChain`, stated for
...
* `SquaresTwoConnected` — **the part of a polygonal overlay lying on the boundary of one of its
  ...
  corners among the vertices.  It is a cycle, so `SchoenfliesGraph.IsLongCycle.isTwoConnected` finishes;
...
* `SchoenfliesGraph.CrosscutExists` and `SchoenfliesGraph.CrosscutEncloses` — the two halves of the descent step of
  ...
  ∀-form: `CrosscutExists` conditioned on `SchoenfliesGraph.IsPlaneChain` (which is how a discharger will
...
def segGraph (S : Set Piece) : SchoenfliesGraph Plane Piece where
...
theorem segGraph_finite (hS : S.Finite) : SchoenfliesGraph.Finite (segGraph S) where
  finite_vertexSet := by
...
theorem pointSet_segGraph (S : Set Piece) :
    SchoenfliesGraph.pointSet (segGraph S) segmentDrawing = ⋃ P ∈ S, P.seg := by
  ext z
  simp only [SchoenfliesGraph.pointSet, mem_union, mem_iUnion, exists_prop, segGraph_vertexSet,
...
`SchoenfliesGraph.IsPlaneChain` discharged at the bottom of the tower. -/
def squareGraph (pieces : List Piece) (points : List Plane) (c : Plane) (r : ℝ) :
    SchoenfliesGraph Plane Piece :=
...
instance squareGraph_finite : SchoenfliesGraph.Finite (squareGraph pieces points c r) :=
  segGraph_finite squareEdges_finite

theorem pointSet_squareGraph_subset :
    SchoenfliesGraph.pointSet (squareGraph pieces points c r) segmentDrawing ⊆
...
*vertices* that `SchoenfliesGraph.IsTwoConnected.union` consumes. -/
...
noncomputable def familyOverlay (c : ℕ → Plane) (N : ℕ) (r : ℝ) : SchoenfliesGraph Plane Piece :=
  overlayGraph (familyPieces c N r) (familyPoints c N r)

/-- The part of the ambient overlay lying on the `j`-th small square. -/
noncomputable def familySquare (c : ℕ → Plane) (N : ℕ) (r : ℝ) (j : ℕ) : SchoenfliesGraph Plane Piece :=
...
noncomputable def familyChain (c : ℕ → Plane) (N m : ℕ) (r : ℝ) (i : ℕ) : SchoenfliesGraph Plane Piece :=
  SchoenfliesGraph.chainUnion (familySquare c N r) (i * m) m

theorem familySquare_le : familySquare c N r j ≤ familyOverlay c N r := squareGraph_le

instance familySquare_finite : SchoenfliesGraph.Finite (familySquare c N r j) := squareGraph_finite

theorem familyChain_le : familyChain c N m r i ≤ familyOverlay c N r :=
  SchoenfliesGraph.chainUnion_le fun _ _ _ => familySquare_le

theorem familyChain_finite : (familyChain c N m r i).Finite :=
  SchoenfliesGraph.chainUnion_finite fun _ _ _ => familySquare_finite

/-- The drawing of the ambient overlay: every edge is a straight segment. -/
theorem familyOverlay_isDrawing (hr : 0 < r) :
    SchoenfliesGraph.IsDrawing (familyOverlay c N r) segmentDrawing :=
...
theorem pointSet_familySquare (hr : 0 < r) (hj : j ≤ N) :
    SchoenfliesGraph.pointSet (familySquare c N r j) segmentDrawing =
...
theorem exists_mem_frontier_of_mem_pointSet_familyChain {z : Plane}
    (hz : z ∈ SchoenfliesGraph.pointSet (familyChain c N m r i) segmentDrawing) :
    ∃ j, i * m ≤ j ∧ j ≤ i * m + m ∧ z ∈ frontier (Plane.closedSquare (c j) r) := by
  obtain ⟨p, hp, hp', hzp⟩ := SchoenfliesGraph.IsPlaneChain.mem_pointSet_block hz
...
points, each cut point a vertex — so it is a cycle, and `SchoenfliesGraph.IsLongCycle.isTwoConnected`
...
theorem familyChain_isTwoConnected (h2c : SquaresTwoConnected) (hr : 0 < r)
  ...
  SchoenfliesGraph.chainUnion_isTwoConnected (G := familyOverlay c N r)
...
/-! ## Part 4: the chain satisfies `SchoenfliesGraph.IsPlaneChain`
...
is a `SchoenfliesGraph.chainUnion` of `familySquare`s, and every `familySquare` is a `segGraph` on a subset
...
theorem isPlaneChain_familyChain {n : ℕ} (h2c : SquaresTwoConnected) (hr : 0 < r)
    ...
    SchoenfliesGraph.IsPlaneChain (familyChain c N m r) segmentDrawing (familyOverlay c N r) n := by
  ...
  · -- Consecutive links share the square at the sample they have in common.
    ...
    have hle₁ : familySquare c N r (p * m + m) ≤ familyChain c N m r p :=
      SchoenfliesGraph.le_chainUnion (G := familyOverlay c N r) (fun _ _ _ => familySquare_le)
        (by omega) (by omega)
...
`SchoenfliesGraph.beyondSquare_subset_face` is this statement for a square centred at the origin; the
...
theorem outer_of_notMem_closedSquare {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
    {c₀ : Plane} {R : ℝ} (hsub : SchoenfliesGraph.pointSet G drawing ⊆ Plane.closedSquare c₀ R)
    {base : Plane} (hbase : base ∉ Plane.closedSquare c₀ R) :
    base ∈ SchoenfliesGraph.exterior G drawing ∧ ¬ Bornology.IsBounded (SchoenfliesGraph.face G drawing base) := by
  refine ⟨fun h => hbase (hsub h), fun hb => ?_⟩
  have hsub' : (Plane.closedSquare c₀ R)ᶜ ⊆ SchoenfliesGraph.face G drawing base :=
...
theorem notMem_face_of_mem_openSquare {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
    {c₀ : Plane} {R : ℝ} (hfr : frontier (Plane.closedSquare c₀ R) ⊆ SchoenfliesGraph.pointSet G drawing)
    {base z : Plane} (hb : ¬ Bornology.IsBounded (SchoenfliesGraph.face G drawing base))
    (hz : z ∈ Plane.openSquare c₀ R) : z ∉ SchoenfliesGraph.face G drawing base := by
  intro hmem
  have hsub : SchoenfliesGraph.face G drawing base ⊆ (frontier (Plane.closedSquare c₀ R))ᶜ :=
    fun w hw hw' => SchoenfliesGraph.face_subset_exterior _ _ _ hw (hfr hw')
  have hcomp : SchoenfliesGraph.face G drawing base ⊆
...
theorem outerOnPairs_familyChain {n : ℕ} {R : ℝ} {x : Plane}
    ...
    SchoenfliesGraph.OuterOnPairs (familyChain c N m r) segmentDrawing n x := by
  intro p hp
  ...
  · -- Every point of either link is within `r` of one of the centres of the pair.
    intro z hz
    rw [SchoenfliesGraph.chainUnion_one, SchoenfliesGraph.pointSet_union] at hz
    have key : ∀ i, p ≤ i → i ≤ p + 1 →
        z ∈ SchoenfliesGraph.pointSet (familyChain c N m r i) segmentDrawing →
...
theorem outer_face_familyChain {n : ℕ} {x : Plane}
    (hchain : SchoenfliesGraph.IsPlaneChain (familyChain c N m r) segmentDrawing (familyOverlay c N r) n)
    (hout : SchoenfliesGraph.OuterOnPairs (familyChain c N m r) segmentDrawing n x) :
    x ∈ SchoenfliesGraph.exterior (SchoenfliesGraph.chainUnion (familyChain c N m r) 0 n) segmentDrawing ∧
      ¬ Bornology.IsBounded
        (SchoenfliesGraph.face (SchoenfliesGraph.chainUnion (familyChain c N m r) 0 n) segmentDrawing x) :=
...
theorem exists_face_of_notMem_arc {A : Set Plane} (hA : IsArc A) (h2c : SquaresTwoConnected)
  ...
  haveI : (SchoenfliesGraph.chainUnion (familyChain c ((n + 1) * m) m r) 0 n).Finite :=
    hchain.block_finite (i := 0) (m := n) (by omega)
  have hdraw : SchoenfliesGraph.IsDrawing (SchoenfliesGraph.chainUnion (familyChain c ((n + 1) * m) m r) 0 n)
      segmentDrawing := hchain.block_isDrawing (by omega)
  have hface := SchoenfliesGraph.unbounded_face_unique hdraw houtw.2 houtu.2
  -- **Step 9.** The outer face misses the arc.
  have hmiss : Disjoint (SchoenfliesGraph.face (SchoenfliesGraph.chainUnion (familyChain c ((n + 1) * m) m r) 0 n)
    ...
    have hle₁ : familySquare c ((n + 1) * m) r j ≤ familyChain c ((n + 1) * m) m r i :=
      SchoenfliesGraph.le_chainUnion (G := familyOverlay c ((n + 1) * m) r)
        (fun _ _ _ => familySquare_le) h₁ h₂
    have hle₂ : familyChain c ((n + 1) * m) m r i ≤
        SchoenfliesGraph.chainUnion (familyChain c ((n + 1) * m) m r) 0 n :=
      SchoenfliesGraph.le_chainUnion (G := familyOverlay c ((n + 1) * m) r)
        (fun _ _ _ => familyChain_le) (Nat.zero_le _) (by omega)
    have hfrsub : frontier (Plane.closedSquare (c j) r) ⊆
        SchoenfliesGraph.pointSet (SchoenfliesGraph.chainUnion (familyChain c ((n + 1) * m) m r) 0 n) segmentDrawing := by
      rw [← pointSet_familySquare (c := c) hr (le_of_lt hjr)]
      exact SchoenfliesGraph.pointSet_mono (hle₁.trans hle₂)
    ...
    · exact (SchoenfliesGraph.face_subset_exterior _ _ _ hzF)
        (hfrsub (by rw [Plane.frontier_closedSquare]; exact heq))
  exact ⟨_, hdraw.isOpen_face u, SchoenfliesGraph.isConnected_face houtu.1, SchoenfliesGraph.mem_face houtu.1,
    hface ▸ SchoenfliesGraph.mem_face houtw.1, hmiss⟩
