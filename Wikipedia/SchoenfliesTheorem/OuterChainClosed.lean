...
`Schoenflies/OuterChain.lean` proves the outer-chain lemma from `SchoenfliesGraph.Descent`, the middle
...
`Schoenflies/OuterChain.lean` where that version used to stand. Had the gap been a placeholder
...
* `SchoenfliesGraph.IsPlaneChain.outer_chain'` — **`lem:outer-chain`**: if `x` is in the outer face of
  every consecutive pair `Γ p ∪ Γ (p+1)`, it is in the outer face of `Γ 0 ∪ ⋯ ∪ Γ n`. Same
  statement as `SchoenfliesGraph.IsPlaneChain.outer_chain`, with its hypothesis discharged.
-/
