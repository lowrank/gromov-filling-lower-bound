...
branches. Neither `Schoenflies.polygonal_overlay` nor `SchoenfliesGraph.IsTree.three_leaves` is used.
...
* `Schoenflies.not_three_components` — the `K(3,3)` half of `thm:jordan`, through
  `SchoenfliesGraph.IsArcK33.elim` (`cor:k33-subdivision`).
...
excluded by `cor:k33-subdivision` (`SchoenfliesGraph.IsArcK33.elim`). -/
...
theorem not_three_components (harc : ∀ A : Set Plane, IsArc A → IsConnected Aᶜ)
  ...
  refine (SchoenfliesGraph.IsArcK33.elim (x := xx) (y := fun j => f (t 1 j))
...
theorem IsJordanCurve.isSeparating (harc : ∀ A : Set Plane, IsArc A → IsConnected Aᶜ)
  ...
  · apply Subset.antisymm
    ...
    · rw [(Schoenflies.isOpen_inside hCclosed).frontier_eq]
      exact subset_diff.2 ⟨hinside ▸ subset_closure_of_accessible_dense harc hC hz₀.1,
  ...
  · apply Subset.antisymm
    ...
    · rw [(Schoenflies.isOpen_outside hCclosed).frontier_eq]
      exact subset_diff.2 ⟨houtside ▸ subset_closure_of_accessible_dense harc hC hbase,
