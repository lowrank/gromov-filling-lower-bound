...
variable {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
  {e : β} {x y : Plane}
...
theorem connected_of_isPreconnected_pointSet [G.Finite]
  ...
  by_contra hxy
  let A : SchoenfliesGraph Plane β := G.induce (G.component x)
  let B : SchoenfliesGraph Plane β := G.induce (V(G) \ G.component x)
  have hAle : A ≤ G := G.induce_le component_subset_vertexSet
  have hBle : B ≤ G := G.induce_le Set.diff_subset
  letI : A.Finite := SchoenfliesGraph.Finite.of_le hAle
  letI : B.Finite := SchoenfliesGraph.Finite.of_le hBle
...
def traceGraph (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (A : Set Plane) :
    SchoenfliesGraph Plane β :=
...
theorem IsTwoConnected.spanning_mono {α δ : Type*} {A B : SchoenfliesGraph α δ}
...
variable {γ : Type*} {S : CellStructure γ} {R : S.Realization}
  {outer dom : Set Plane} {H : SchoenfliesGraph Plane γ} {Hdraw : γ → ℝ → Plane}
  ...
  · obtain ⟨e, heR, hze⟩ := Set.mem_iUnion₂.1 hzE
    ...
    exact (hH.edge_subset heS hf ⟨z, hzf, by
      ...
      exact ⟨hze, by simp [hza, hzb]⟩, hznotH⟩).trans
        (SchoenfliesGraph.edgeArc_subset_pointSet heR)
...
theorem trace_pointSet (hH : IsSourceExtension R outer dom H Hdraw) :
    SchoenfliesGraph.pointSet (SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet) Hdraw = R.skeletonSet :=
  SchoenfliesGraph.pointSet_traceGraph_eq hH.isDrawing R.skeletonSet hH.skeletonSet_subset
    (trace_absorb hH)
...
theorem edge_trace_pointSet (hH : IsSourceExtension R outer dom H Hdraw)
    {e : γ} (he : e ∈ E(S.skel)) :
    SchoenfliesGraph.pointSet (SchoenfliesGraph.traceGraph H Hdraw (edgeArc R.drawing e)) Hdraw =
      edgeArc R.drawing e := by
  have heR : e ∈ E(R.graph) := by rwa [R.edgeSet_graph]
  refine SchoenfliesGraph.pointSet_traceGraph_eq hH.isDrawing _
    ((SchoenfliesGraph.edgeArc_subset_pointSet heR).trans hH.skeletonSet_subset) ?_
  intro f hf hmeet
...
theorem exists_edge_trace (hH : IsSourceExtension R outer dom H Hdraw)
    ...
    ∃ D : List γ, H.IsPath (R.pos a) D (R.pos b) ∧
      SchoenfliesGraph.edgesCover Hdraw D = edgeArc R.drawing e := by
  let K := SchoenfliesGraph.traceGraph H Hdraw (edgeArc R.drawing e)
  have hKle : K ≤ H := SchoenfliesGraph.traceGraph_le _
  letI : H.Finite := hH.finite
  letI : K.Finite := SchoenfliesGraph.Finite.of_le hKle
  ...
  have haK : R.pos a ∈ V(K) := by
    rw [SchoenfliesGraph.traceGraph_vertexSet]
    exact ⟨haH, hOldArc.left_mem⟩
  have hbK : R.pos b ∈ V(K) := by
    rw [SchoenfliesGraph.traceGraph_vertexSet]
    exact ⟨hbH, hOldArc.right_mem⟩
  have hKconn : K.Connected := SchoenfliesGraph.connected_of_isPreconnected_pointSet
  ...
  have hPathArc : IsArcBetween (SchoenfliesGraph.edgesCover Hdraw D) (R.pos a) (R.pos b) :=
    hH.isDrawing.path_isArcBetween hDH (hDH.ne_nil hne)
  have hcoverSub : SchoenfliesGraph.edgesCover Hdraw D ⊆ edgeArc R.drawing e := by
    rw [← hpoint]
    exact SchoenfliesGraph.edgesCover_subset_pointSet fun g hg => hD.edge_mem hg
...
theorem pathGraph_edge_trace_le (hH : IsSourceExtension R outer dom H Hdraw)
    ...
    (hcover : SchoenfliesGraph.edgesCover Hdraw D = edgeArc R.drawing e) :
    H.pathGraphOf (R.pos a) D ≤
      SchoenfliesGraph.traceGraph H Hdraw (edgeArc R.drawing e) := by
  have hne : R.pos a ≠ R.pos b := R.isDrawing.ne_of_isLink (hab.map R.pos)
  have hPpoint : pointSet (H.pathGraphOf (R.pos a) D) Hdraw =
      SchoenfliesGraph.edgesCover Hdraw D :=
    hH.isDrawing.pointSet_pathGraphOf hD.isWalk (hD.ne_nil hne)
  have hPle : H.pathGraphOf (R.pos a) D ≤ H := SchoenfliesGraph.pathGraphOf_le hD.isWalk
  refine ⟨?_, ?_⟩
  · intro z hz
    rw [SchoenfliesGraph.traceGraph_vertexSet]
  ...
  · intro f x y hlink
    ...
    have hfD : f ∈ D := by
      rwa [SchoenfliesGraph.pathGraphOf_edgeSet hD.isWalk] at hfP
    have hlinkH : H.IsLink f x y := hPle.isLink_mono hlink
    have hsub : edgeArc Hdraw f ⊆ edgeArc R.drawing e := by
      rw [← hcover]
      exact fun z hz => SchoenfliesGraph.mem_edgesCover hfD hz
    have harc := hH.isDrawing.edge_isArcBetween hlinkH
    exact (SchoenfliesGraph.traceGraph_isLink _).2
...
theorem old_vertex_mem_trace (hH : IsSourceExtension R outer dom H Hdraw)
    {a : γ} (ha : a ∈ V(S.skel)) :
    R.pos a ∈ V(SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet) := by
  rw [SchoenfliesGraph.traceGraph_vertexSet]
...
theorem reaches_trace_of_isWalk (hH : IsSourceExtension R outer dom H Hdraw)
    ...
    (SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet).Reaches (R.pos a) (R.pos b) := by
  induction hW with
  ...
  | @cons a w b e W hlink htail ih =>
    ...
    have hPle : H.pathGraphOf (R.pos a) D ≤
        SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet :=
      (pathGraph_edge_trace_le hH hlink hD hcover).trans
        (SchoenfliesGraph.traceGraph_mono (SchoenfliesGraph.edgeArc_subset_pointSet
...
theorem old_vertices_reach_trace (hH : IsSourceExtension R outer dom H Hdraw)
  ...
  have hS2 : S.skel.IsTwoConnected :=
    (SchoenfliesGraph.isTwoConnected_map_iff R.injOn_pos).1 hR2
...
theorem exists_reaches_old_vertex (hH : IsSourceExtension R outer dom H Hdraw)
    {x : Plane} (hx : x ∈ V(SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet)) :
    ∃ a ∈ V(S.skel),
      (SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet).Reaches x (R.pos a) := by
  rw [SchoenfliesGraph.traceGraph_vertexSet] at hx
  ...
  · rcases hx.2 with hxV | hxE
    ...
    · obtain ⟨e, heR, hxe⟩ := Set.mem_iUnion₂.1 hxE
      ...
      have hxP : x ∈ V(H.pathGraphOf (R.pos a) D) := by
        rw [SchoenfliesGraph.pathGraphOf_vertexSet]
        exact hxwalk
      have hPle : H.pathGraphOf (R.pos a) D ≤
          SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet :=
        (pathGraph_edge_trace_le hH hab hD hcover).trans
          (SchoenfliesGraph.traceGraph_mono (SchoenfliesGraph.edgeArc_subset_pointSet heR))
      have haP : R.pos a ∈ V(H.pathGraphOf (R.pos a) D) :=
        SchoenfliesGraph.mem_vertexSet_pathGraphOf_self
...
theorem exists_reaches_old_vertex_delete
    ...
    (hx : x ∈ V(SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet)) (hxc : x ≠ c) :
    ∃ a ∈ V(S.skel),
      ((SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet).deleteVerts {c}).Reaches x (R.pos a) := by
  rw [SchoenfliesGraph.traceGraph_vertexSet] at hx
  by_cases hxold : x ∈ V(R.graph)
  · rw [R.vertexSet_graph] at hxold
    obtain ⟨a, ha, rfl⟩ := hxold
    exact ⟨a, ha, .refl (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne
  ...
  · rcases hx.2 with hxV | hxE
    ...
    · obtain ⟨e, heR, hxe⟩ := Set.mem_iUnion₂.1 hxE
      ...
      · exact ⟨a, hab.left_mem, hreach.mono (SchoenfliesGraph.deleteVerts_mono hPle _)⟩
      · exact ⟨b, hab.right_mem, hreach.mono (SchoenfliesGraph.deleteVerts_mono hPle _)⟩
...
theorem reaches_trace_delete_of_deleteVerts_isWalk
    ...
    ((SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet).deleteVerts {R.pos z}).Reaches
      (R.pos a) (R.pos b) := by
  induction hW with
  | nil ha =>
      rw [SchoenfliesGraph.mem_deleteVerts_singleton] at ha
      exact .refl (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne
        (old_vertex_mem_trace hH ha.1) (fun h => ha.2 (by
          simpa using R.injOn_pos ha.1 hzS h)))
  | @cons a w b e W hlink htail ih =>
    rw [SchoenfliesGraph.deleteVerts_isLink] at hlink
    have hlinkS := hlink.1
    ...
    have hznot : R.pos z ∉ H.walkVertices (R.pos a) D := by
      ...
      have hzarc : R.pos z ∈ edgeArc R.drawing e := by
        rw [← hcover, ← hPpoint]
        exact Or.inl (by rwa [SchoenfliesGraph.pathGraphOf_vertexSet])
    ...
    have hreach : ((H.pathGraphOf (R.pos a) D).deleteVerts {R.pos z}).Reaches
      ...
      ⟨D, hD.pathGraphOf.isWalk.deleteVerts_singleton (by
        rw [SchoenfliesGraph.walkVertices_pathGraphOf]
        exact hznot)⟩
    exact (hreach.mono (SchoenfliesGraph.deleteVerts_mono hPle _)).trans ih
...
theorem reaches_trace_delete_of_deleteEdges_isWalk
    ...
    ((SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet).deleteVerts {c}).Reaches
      (R.pos a) (R.pos b) := by
  induction hW with
  | @nil x hx =>
      exact .refl (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne
...
theorem trace_isTwoConnected
    ...
    (SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet).IsTwoConnected := by
  let K := SchoenfliesGraph.traceGraph H Hdraw R.skeletonSet
  have hS2 : S.skel.IsTwoConnected :=
    (SchoenfliesGraph.isTwoConnected_map_iff R.injOn_pos).1 hR2
  ...
  · obtain ⟨a, ha⟩ := hS2.connected.nonempty
    refine SchoenfliesGraph.Connected.of_hub (old_vertex_mem_trace hH ha) ?_
    intro x hx
  ...
  · intro c hcK
    ...
    have hpDel : p ∈ V(K.deleteVerts {c}) :=
      SchoenfliesGraph.mem_deleteVerts_singleton_of_ne hpK hpc
    refine SchoenfliesGraph.Connected.of_hub hpDel ?_
    intro x hx
    rw [SchoenfliesGraph.mem_deleteVerts_singleton] at hx
    ...
    have habReach : (K.deleteVerts {c}).Reaches (R.pos a) (R.pos b) := by
      ...
      · rw [R.vertexSet_graph] at hcold
        ...
        rw [SchoenfliesGraph.mem_deleteVerts_singleton] at haDel hbDel
        have haz : a ≠ z := by
          intro h
          apply haDel.2
          rw [h, hzc]
        ...
        have haSdel : a ∈ V(S.skel.deleteVerts {z}) :=
          SchoenfliesGraph.mem_deleteVerts_singleton_of_ne ha haz
        have hbSdel : b ∈ V(S.skel.deleteVerts {z}) :=
          SchoenfliesGraph.mem_deleteVerts_singleton_of_ne hb hbz
      ...
      · have hcskel : c ∈ R.skeletonSet := by
          rw [SchoenfliesGraph.traceGraph_vertexSet] at hcK
          exact hcK.2
        ...
        · obtain ⟨e₀, he₀R, hce₀⟩ := Set.mem_iUnion₂.1 hcE
          ...
          have hcyc : S.skel.LiesOnCycle e₀ :=
            (SchoenfliesGraph.liesOnCycle_iff_deleteEdges_reaches huv).2 (hS2.no_bridge huv)
...
variable {β δ : Type*} {G : SchoenfliesGraph Plane β} {Gdraw : β → ℝ → Plane}
  {K : SchoenfliesGraph Plane δ} {Kdraw : δ → ℝ → Plane}
...
structure IsPlaneSubdivisionExtension (G : SchoenfliesGraph Plane β) (Gdraw : β → ℝ → Plane)
    (K : SchoenfliesGraph Plane δ) (Kdraw : δ → ℝ → Plane) : Prop where
...
theorem trace_absorb (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw) :
  ...
  · obtain ⟨e, he, hze⟩ := Set.mem_iUnion₂.1 hzE
    exact (h.edge_subset he hf ⟨z, hzf, hze, hznotK⟩).trans
      (SchoenfliesGraph.edgeArc_subset_pointSet he)
...
theorem trace_pointSet (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw) :
    pointSet (SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)) Kdraw = pointSet G Gdraw :=
  SchoenfliesGraph.pointSet_traceGraph_eq h.isDrawing _ h.pointSet_subset h.trace_absorb
...
theorem edge_trace_pointSet (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw)
    {e : β} (he : e ∈ E(G)) :
    pointSet (SchoenfliesGraph.traceGraph K Kdraw (edgeArc Gdraw e)) Kdraw = edgeArc Gdraw e := by
  refine SchoenfliesGraph.pointSet_traceGraph_eq h.isDrawing _
    ((SchoenfliesGraph.edgeArc_subset_pointSet he).trans h.pointSet_subset) ?_
  intro f hf hmeet
...
theorem exists_edge_trace (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw)
  ...
  let T := SchoenfliesGraph.traceGraph K Kdraw (edgeArc Gdraw e)
  have hTK : T ≤ K := SchoenfliesGraph.traceGraph_le _
  letI : K.Finite := h.finite
  letI : T.Finite := SchoenfliesGraph.Finite.of_le hTK
  ...
  have hTconn : T.Connected := SchoenfliesGraph.connected_of_isPreconnected_pointSet
...
theorem pathGraph_edge_trace_le (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw)
    ...
    K.pathGraphOf a D ≤ SchoenfliesGraph.traceGraph K Kdraw (edgeArc Gdraw e) := by
  have hne : a ≠ b := h.oldIsDrawing.ne_of_isLink hab
  have hPpoint : pointSet (K.pathGraphOf a D) Kdraw = edgesCover Kdraw D :=
    h.isDrawing.pointSet_pathGraphOf hD.isWalk (hD.ne_nil hne)
  have hPK : K.pathGraphOf a D ≤ K := SchoenfliesGraph.pathGraphOf_le hD.isWalk
  refine ⟨?_, ?_⟩
  · intro z hz
    rw [SchoenfliesGraph.traceGraph_vertexSet]
...
theorem old_vertex_mem_trace (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw)
    {a : Plane} (ha : a ∈ V(G)) :
    a ∈ V(SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)) := by
  rw [SchoenfliesGraph.traceGraph_vertexSet]
...
theorem reaches_trace_of_isWalk (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw)
    ...
    (SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)).Reaches a b := by
  induction hW with
  | nil ha => exact .refl (h.old_vertex_mem_trace ha)
  | @cons a w b e W hlink htail ih =>
    obtain ⟨D, hD, hcover⟩ := h.exists_edge_trace hlink
    have hPle : K.pathGraphOf a D ≤ SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw) :=
      (h.pathGraph_edge_trace_le hlink hD hcover).trans
        (SchoenfliesGraph.traceGraph_mono (SchoenfliesGraph.edgeArc_subset_pointSet hlink.edge_mem))
...
theorem exists_reaches_old_vertex (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw)
    {x : Plane} (hx : x ∈ V(SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw))) :
    ∃ a ∈ V(G), (SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)).Reaches x a := by
  rw [SchoenfliesGraph.traceGraph_vertexSet] at hx
    ...
    · obtain ⟨e, he, hxe⟩ := Set.mem_iUnion₂.1 hxE
      ...
      have hPle : K.pathGraphOf a D ≤ SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw) :=
        (h.pathGraph_edge_trace_le hab hD hcover).trans
          (SchoenfliesGraph.traceGraph_mono (SchoenfliesGraph.edgeArc_subset_pointSet he))
      have haP : a ∈ V(K.pathGraphOf a D) := SchoenfliesGraph.mem_vertexSet_pathGraphOf_self
      exact ⟨a, hab.left_mem,
        (hD.isPathGraph_pathGraphOf.connected.reaches hxP haP).mono hPle⟩
...
theorem exists_reaches_old_vertex_delete
    ...
    (hx : x ∈ V(SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw))) (hxc : x ≠ c) :
    ∃ a ∈ V(G),
      ((SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)).deleteVerts {c}).Reaches x a := by
  rw [SchoenfliesGraph.traceGraph_vertexSet] at hx
  by_cases hxold : x ∈ V(G)
  · exact ⟨x, hxold, .refl (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne
...
theorem reaches_trace_delete_of_deleteVerts_isWalk
    ...
    ((SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)).deleteVerts {z}).Reaches a b := by
  induction hW with
  | nil ha =>
      rw [SchoenfliesGraph.mem_deleteVerts_singleton] at ha
      exact .refl (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne
  ...
  | @cons a w b e W hlink htail ih =>
    ...
    have hPle : K.pathGraphOf a D ≤ SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw) :=
      (h.pathGraph_edge_trace_le hlinkG hD hcover).trans
        (SchoenfliesGraph.traceGraph_mono (SchoenfliesGraph.edgeArc_subset_pointSet hlinkG.edge_mem))
...
theorem reaches_trace_delete_of_deleteEdges_isWalk
    ...
    ((SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)).deleteVerts {c}).Reaches a b := by
  induction hW with
  | @nil x hx =>
      exact .refl (SchoenfliesGraph.mem_deleteVerts_singleton_of_ne
...
theorem trace_isTwoConnected (h : IsPlaneSubdivisionExtension G Gdraw K Kdraw)
    ...
    (SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)).IsTwoConnected := by
  let T := SchoenfliesGraph.traceGraph K Kdraw (pointSet G Gdraw)
  ...
  · obtain ⟨a, ha⟩ := hG2.connected.nonempty
    refine SchoenfliesGraph.Connected.of_hub (h.old_vertex_mem_trace ha) ?_
    intro x hx
  ...
  · intro c hcT
    ...
    have hpDel : p ∈ V(T.deleteVerts {c}) :=
      SchoenfliesGraph.mem_deleteVerts_singleton_of_ne hpT hpc
    refine SchoenfliesGraph.Connected.of_hub hpDel ?_
    intro x hx
    rw [SchoenfliesGraph.mem_deleteVerts_singleton] at hx
    ...
    have habReach : (T.deleteVerts {c}).Reaches a b := by
      ...
      · have haDel := hxa.right_mem
        ...
        have haGdel : a ∈ V(G.deleteVerts {c}) :=
          SchoenfliesGraph.mem_deleteVerts_singleton_of_ne ha hac
        have hbGdel : b ∈ V(G.deleteVerts {c}) :=
          SchoenfliesGraph.mem_deleteVerts_singleton_of_ne hb hbc
      ...
      · have hcpoint : c ∈ pointSet G Gdraw := by
          rw [SchoenfliesGraph.traceGraph_vertexSet] at hcT
          exact hcT.2
        ...
        · obtain ⟨e₀, he₀, hce₀⟩ := Set.mem_iUnion₂.1 hcE
          ...
          have hcyc : G.LiesOnCycle e₀ :=
            (SchoenfliesGraph.liesOnCycle_iff_deleteEdges_reaches huv).2 (hG2.no_bridge huv)
...
theorem skeleton_isTwoConnected {d : S.SubdivData}
  ...
  have hfresh : d.left ∉ d.skeleton.walkVertices d.newVertex [d.newEdge₂] := by
    intro h
    rw [SchoenfliesGraph.walkVertices_cons d.isLink_newEdge₂, SchoenfliesGraph.walkVertices_nil] at h
    rcases h with h | h
  ...
  have hdel : S.skel.deleteEdges {d.edge} ≤ d.skeleton := by
    refine ⟨?_, ?_⟩
    · rw [SchoenfliesGraph.vertexSet_deleteEdges, d.skeleton_vertexSet]
      exact Set.subset_insert _ _
    · intro e x y hlink
      change S.skel.IsLink e x y ∧ e ∉ ({d.edge} : Set γ) at hlink
      exact d.skeleton_isLink_of_old (by simpa using hlink.2) hlink.1
  have hPle : P ≤ d.skeleton := SchoenfliesGraph.pathGraphOf_le hpath.isWalk
  have hcompat : (S.skel.deleteEdges {d.edge}).Compatible P :=
    SchoenfliesGraph.Compatible.of_le_le hdel hPle
  have hnew : ∀ x ∈ V(P), x ≠ d.left → x ≠ d.right → x ∉ V(S.skel) := by
    intro x hx hxl hxr hxS
    rw [SchoenfliesGraph.pathGraphOf_vertexSet, SchoenfliesGraph.walkVertices_cons d.isLink_newEdge₁,
      SchoenfliesGraph.walkVertices_cons d.isLink_newEdge₂, SchoenfliesGraph.walkVertices_nil] at hx
  ...
  have hBle : (S.skel.deleteEdges {d.edge}).union P ≤ d.skeleton :=
    SchoenfliesGraph.union_le hdel hPle
  ...
  · apply Set.mem_union_right
    rw [SchoenfliesGraph.pathGraphOf_vertexSet, SchoenfliesGraph.walkVertices_cons d.isLink_newEdge₁,
      SchoenfliesGraph.walkVertices_cons d.isLink_newEdge₂, SchoenfliesGraph.walkVertices_nil]
    simp
...
theorem outerSet_realize {d : S.SubdivData} {R : S.Realization}
  ...
  · have hedge : (⋃ f ∈ E(S.outerGraph), SchoenfliesGraph.edgeArc R.drawing f) =
        SchoenfliesGraph.edgeArc R.drawing d.edge ∪
          ⋃ f ∈ E(S.outerGraph) \ {d.edge}, SchoenfliesGraph.edgeArc R.drawing f := by
      rw [← Set.biUnion_insert, Set.insert_diff_singleton, Set.insert_eq_of_mem he]
    have hnew : (⋃ f ∈ E(d.outer), SchoenfliesGraph.edgeArc (d.realizeDrawing R t) f) =
        SchoenfliesGraph.edgeArc R.drawing d.edge ∪
          ⋃ f ∈ E(S.outerGraph) \ {d.edge}, SchoenfliesGraph.edgeArc R.drawing f := by
    ...
    have hM : R.drawing d.edge t ∈ SchoenfliesGraph.edgeArc R.drawing d.edge :=
      ⟨t, Set.Ioo_subset_Icc_self ht, rfl⟩
    change SchoenfliesGraph.pointSet (d.outer.map (d.realizePos R t)) (d.realizeDrawing R t) =
      SchoenfliesGraph.pointSet (S.outerGraph.map R.pos) R.drawing
    rw [SchoenfliesGraph.pointSet, SchoenfliesGraph.pointSet, SchoenfliesGraph.vertexSet_map, SchoenfliesGraph.vertexSet_map,
      SchoenfliesGraph.edgeSet_map, SchoenfliesGraph.edgeSet_map, hvertex, hnew, hedge, Set.insert_union,
  ...
  · change SchoenfliesGraph.pointSet (d.outer.map (d.realizePos R t)) (d.realizeDrawing R t) =
    ...
    have hgraph : S.outerGraph.map (d.realizePos R t) = S.outerGraph.map R.pos :=
      SchoenfliesGraph.map_eq_of_eqOn fun z hz => d.realizePos_of_mem_vertexSet
        (S.outerGraph_le.vertexSet_mono hz)
    rw [hgraph]
    apply SchoenfliesGraph.pointSet_congr
    intro f hf
    rw [SchoenfliesGraph.edgeSet_map] at hf
...
theorem isWeaklyAdmissible_realize {d : S.SubdivData} {R : S.Realization}
  ...
  isTwoConnected := by
    have hS2 : S.skel.IsTwoConnected :=
      (SchoenfliesGraph.isTwoConnected_map_iff R.injOn_pos).1 hR.isTwoConnected
...
structure CommonSubdivisionData
    (P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
    (H : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) where
  /-- The part of `H` supported on the old source skeleton. -/
  graph : SchoenfliesGraph Plane γ
...
noncomputable def commonSubdivisionData [Infinite γ]
    {P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom}
    {H : SchoenfliesGraph Plane γ} {Hdraw : γ → ℝ → Plane}
    (hH : IsSourceExtension P.src srcOuter srcDom H Hdraw) :
    CommonSubdivisionData P H Hdraw := by
  classical
  let K := SchoenfliesGraph.traceGraph H Hdraw P.src.skeletonSet
  have hKle : K ≤ H := SchoenfliesGraph.traceGraph_le _
  letI : H.Finite := hH.finite
  letI : K.Finite := SchoenfliesGraph.Finite.of_le hKle
  ...
  let w := Classical.choice (GeneratedPair.exists_subdivideSetData P
    (SchoenfliesGraph.finite_vertexSet K) hvertices)
  exact {
    graph := K
    pair := w.pair
    parent := w.parent
