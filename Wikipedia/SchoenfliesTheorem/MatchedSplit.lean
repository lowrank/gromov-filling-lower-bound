...
of every ear edge: `SchoenfliesGraph.IsDrawing.edge_param` is orientation-free, so `earDraw₁ f 0` and
...
`SchoenfliesGraph.pointSet_congr` — the point set of a plane graph depends on the drawing only through the
arcs of the graph's own edges. It is stated in the root `SchoenfliesGraph` namespace next to
`SchoenfliesGraph.pointSet_union` (which lives in `Schoenflies/FaceCycles.lean`); if a second consumer
appears it belongs there or in `Schoenflies/SchoenfliesGraph/Drawing.lean`.
-/
...
variable {β : Type*} {G : SchoenfliesGraph Plane β}
...
theorem Realization.pos_mem_skeletonSet (R : S.Realization) {v : γ} (hv : v ∈ V(S.skel)) :
  ...
  SchoenfliesGraph.vertexSet_subset_pointSet (by rw [Realization.vertexSet_graph]; exact ⟨v, hv, rfl⟩)
...
ear; `SchoenfliesGraph.pointSet_union` splits the point set of that union; and `SchoenfliesGraph.pointSet_congr`
...
theorem skeletonSet_realize {R : S.Realization} {earPos : γ → Plane} {earDraw : γ → ℝ → Plane}
  ...
  have hstep : (d.realize R earPos earDraw hE).skeletonSet =
      SchoenfliesGraph.pointSet (R.graph.union (d.earGraph earPos)) (d.splitDrawing R earDraw) := by
    rw [← hE.splitGraph_eq]
    rfl
  rw [hstep, SchoenfliesGraph.pointSet_union]
  congr 1
  · exact SchoenfliesGraph.pointSet_congr fun e he =>
...
structure EarHomeo (d : S.SplitData) (earPos₁ : γ → Plane) (earDraw₁ : γ → ℝ → Plane)
  ...
  edgeArc_image : ∀ ⦃f⦄, f ∈ E(d.ear) →
    toFun '' SchoenfliesGraph.edgeArc earDraw₁ f = SchoenfliesGraph.edgeArc earDraw₂ f
...
theorem image_earSet :
  ...
  rw [earSet, earSet, SchoenfliesGraph.pointSet, SchoenfliesGraph.pointSet, Set.image_union, Set.image_iUnion₂]
  congr 1
...
def symm : d.EarHomeo earPos₂ earDraw₂ earPos₁ earDraw₁ where
  ...
  earPos_apply := by
    ...
    apply m.leftInvOn
    exact SchoenfliesGraph.vertexSet_subset_pointSet (by
...
noncomputable def splitHomeo (g : SkeletonHomeo R₁ R₂)
  ...
  edgeArc_image := by
    ...
    · change splitMap g m '' SchoenfliesGraph.edgeArc (d.splitDrawing R₁ earDraw₁) e =
        SchoenfliesGraph.edgeArc (d.splitDrawing R₂ earDraw₂) e
      rw [edgeArc_splitDrawing_of_mem_skel he', edgeArc_splitDrawing_of_mem_skel he',
        Set.image_congr (fun x hx => splitMap_of_mem_skeletonSet
          (SchoenfliesGraph.edgeArc_subset_pointSet (by rwa [Realization.edgeSet_graph]) hx)),
        g.edgeArc_image he']
    · change splitMap g m '' SchoenfliesGraph.edgeArc (d.splitDrawing R₁ earDraw₁) e =
        SchoenfliesGraph.edgeArc (d.splitDrawing R₂ earDraw₂) e
