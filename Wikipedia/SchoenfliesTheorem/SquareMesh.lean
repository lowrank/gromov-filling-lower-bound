...
* **2-connectivity is not proved at all.** It is not present as a hypothesis either — a
  ...
  by `SchoenfliesGraph.IsTwoConnected.union`, each added cycle sharing at least two vertices with what is
...
theorem mem_smul_segment {a b : ℝ} {z x : Plane} :
  ...
  · rintro ⟨t, ⟨p, q, hp, hq, hpq, rfl⟩, rfl⟩
    ...
    simp only [smul_smul, add_smul, smul_eq_mul]
...
/-- **The mesh graph.** -/
noncomputable def meshGraph (N : ℕ) (fresh anchors : List Plane) : SchoenfliesGraph Plane Piece :=
  overlayGraph (meshSegments N fresh) (meshPoints N fresh anchors)

instance meshGraph_finite (N : ℕ) (fresh anchors : List Plane) :
    SchoenfliesGraph.Finite (meshGraph N fresh anchors) :=
...
theorem meshGraph_isDrawing {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
    ...
    SchoenfliesGraph.IsDrawing (meshGraph N fresh anchors) segmentDrawing :=
...
theorem meshGraph_pointSet (N : ℕ) (fresh anchors : List Plane) :
    SchoenfliesGraph.pointSet (meshGraph N fresh anchors) segmentDrawing = cover (meshSegments N fresh) :=
...
theorem meshGraph_face_small {N : ℕ} (hN : 2 ≤ N) {fresh : List Plane}
    ...
    (hbase : base ∈ SchoenfliesGraph.exterior (meshGraph N fresh anchors) segmentDrawing)
    (hbdd : Bornology.IsBounded (SchoenfliesGraph.face (meshGraph N fresh anchors) segmentDrawing base)) :
    SchoenfliesGraph.face (meshGraph N fresh anchors) segmentDrawing base ⊆ Plane.openSquare 0 1 ∧
      Metric.diam (SchoenfliesGraph.face (meshGraph N fresh anchors) segmentDrawing base) < δ := by
  have hNR0 : (0 : ℝ) < N := by exact_mod_cast Nat.lt_of_lt_of_le two_pos hN
  set F := SchoenfliesGraph.face (meshGraph N fresh anchors) segmentDrawing base with hF
  have hpt : SchoenfliesGraph.pointSet (meshGraph N fresh anchors) segmentDrawing
      = cover (meshSegments N fresh) := meshGraph_pointSet N fresh anchors
  have hsq : SchoenfliesGraph.pointSet (meshGraph N fresh anchors) segmentDrawing
  ...
  have hb1 : Plane.supNorm base < 1 := by
    ...
    · exfalso
      ...
      exact SchoenfliesGraph.not_isBounded_beyondSquare 1
        (hbdd.subset (SchoenfliesGraph.beyondSquare_subset_face hsq hmem))
  have hdisj : ∀ x ∈ F, x ∉ cover (meshSegments N fresh) := by
    intro x hx
    have := SchoenfliesGraph.face_subset_exterior _ _ _ hx
    rwa [SchoenfliesGraph.exterior, hpt, mem_compl_iff] at this
  obtain ⟨hin, hdist⟩ := radial_diam_bound hN hNδ.le hdense
    (SchoenfliesGraph.isConnected_face hbase).isPreconnected hdisj (SchoenfliesGraph.mem_face hbase) hb1
...
the outer ring. -/
noncomputable def squareMesh (δ : ℝ) (fresh anchors : List Plane) : SchoenfliesGraph Plane Piece :=
  meshGraph (meshCount δ) fresh anchors

instance squareMesh_finite (δ : ℝ) (fresh anchors : List Plane) :
    SchoenfliesGraph.Finite (squareMesh δ fresh anchors) := meshGraph_finite _ _ _
...
theorem squareMesh_isDrawing {fresh : List Plane} (hfresh : ∀ z ∈ fresh, z ∈ modelCurve)
    ...
    SchoenfliesGraph.IsDrawing (squareMesh δ fresh anchors) segmentDrawing :=
  meshGraph_isDrawing (two_le_meshCount δ) hfresh anchors

theorem squareMesh_pointSet (δ : ℝ) (fresh anchors : List Plane) :
    SchoenfliesGraph.pointSet (squareMesh δ fresh anchors) segmentDrawing
...
theorem modelCurve_subset_squareMesh_pointSet (δ : ℝ) (fresh anchors : List Plane) :
    modelCurve ⊆ SchoenfliesGraph.pointSet (squareMesh δ fresh anchors) segmentDrawing := by
...
theorem squareMesh_pointSet_subset {fresh : List Plane} (hfresh : ∀ z ∈ fresh, z ∈ modelCurve)
    (δ : ℝ) (anchors : List Plane) :
    SchoenfliesGraph.pointSet (squareMesh δ fresh anchors) segmentDrawing ⊆ Plane.closedSquare 0 1 := by
...
theorem squareMesh_face_small {fresh : List Plane} (hfresh : ∀ z ∈ fresh, z ∈ modelCurve)
    ...
    (hbase : base ∈ SchoenfliesGraph.exterior (squareMesh δ fresh anchors) segmentDrawing)
    (hbdd : Bornology.IsBounded (SchoenfliesGraph.face (squareMesh δ fresh anchors) segmentDrawing base)) :
    SchoenfliesGraph.face (squareMesh δ fresh anchors) segmentDrawing base ⊆ Plane.openSquare 0 1 ∧
      Metric.diam (SchoenfliesGraph.face (squareMesh δ fresh anchors) segmentDrawing base) < δ :=
...
theorem squareMesh_isConnected_diff {fresh : List Plane}
    ...
    (hz₀ : z₀ ∈ fresh) :
    IsConnected
      (SchoenfliesGraph.pointSet (squareMesh δ fresh anchors) segmentDrawing \ modelCurve) := by
  rw [squareMesh_pointSet]
