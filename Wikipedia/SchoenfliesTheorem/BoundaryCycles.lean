...
invariant in the representation already used by `SchoenfliesGraph.IsLongCycle`.
...
theorem pathCells_append {u w v : γ} {W₁ W₂ : List γ}
  ...
  · rintro σ (hσ | hσ)
    ...
    · rcases SchoenfliesGraph.walkVertices_append_subset hσ with hσ | hσ
      · exact Or.inl (Or.inr hσ)
      · exact Or.inr (Or.inr hσ)
  · rintro σ (hσ | hσ)
    · rcases hσ with hσ | hσ
      · exact Or.inl (List.mem_append_left W₂ hσ)
      · exact Or.inr (SchoenfliesGraph.walkVertices_mono (List.subset_append_left W₁ W₂) hσ)
    · rcases hσ with hσ | hσ
      · exact Or.inl (List.mem_append_right W₁ hσ)
      · rcases SchoenfliesGraph.mem_walkVertices_iff.1 hσ with rfl | hcov
        · exact Or.inr
            (SchoenfliesGraph.walkVertices_mono (List.subset_append_left W₁ W₂)
              h₁.target_mem_walkVertices)
        · exact Or.inr (SchoenfliesGraph.mem_walkVertices_of_mem_covered
            (SchoenfliesGraph.coveredVertices_append.symm ▸ Or.inr hcov))
...
theorem pathCells_eq_of_perm {u₁ v₁ u₂ v₂ : γ} {W₁ W₂ : List γ}
  ...
  ext σ
  simp only [Set.mem_union, Set.mem_setOf_eq, SchoenfliesGraph.mem_coveredVertices_iff]
  constructor
  · rintro (hσ | ⟨e, he, hi⟩)
...
theorem mem_walk_of_vertex_sub (h : S.FaceCycle F) {a : γ} (ha : a ∈ V(S.skel))
  ...
  · rcases SchoenfliesGraph.mem_walkVertices_cons h.isCycle.isLink.symm haV with rfl | haW
...
`SplitData`.  The second path returned by `SchoenfliesGraph.IsCycleThrough.split_at` is reversed so that
  ...
  have hinter : S.pathCells a D₁ ∩ S.pathCells a D₂.reverse = {a, b} := by
    ...
    · rintro σ ⟨hσ₁, hσ₂⟩
      ...
      · rcases hσ₂' with he₂ | hv₂
        ...
        · exact hmeet σ hv₁ hv₂
    · rintro σ (rfl | rfl)
      · exact ⟨Or.inr SchoenfliesGraph.mem_walkVertices_self, Or.inr SchoenfliesGraph.mem_walkVertices_self⟩
