...
* `SchoenfliesGraph.IsDrawing.exists_cover`, `SchoenfliesGraph.IsDrawing.exists_isLocalRadius`,
  `SchoenfliesGraph.IsDrawing.finite_localDirs` — the same statements for a finite plane graph all of
  whose edges are polygonal.
* `SchoenfliesGraph.IsDrawing.openSegment_subset_face` — the access arc of
...
theorem IsLocalRadius.ball_diff_eq_cone (h : IsLocalRadius S x r) :
  ...
  ext z
  simp only [Set.mem_diff, mem_ball, Plane.mem_cone_iff, mem_setOf_eq, sub_ne_zero]
  constructor
  · rintro ⟨hzb, hzS⟩
...
variable {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {x : Plane}
...
theorem IsDrawing.exists_cover [G.Finite] (h : IsDrawing G drawing)
  ...
  refine ⟨(SchoenfliesGraph.edgeFinset G).toList.flatMap fun e => segsOf (vs e), ?_⟩
  rw [pointSet]
  congr 1
...
theorem IsDrawing.exists_segmentCover [G.Finite] (h : IsDrawing G drawing)
  ...
  classical
  choose! vs hvs using hpoly
  let ps := (SchoenfliesGraph.edgeFinset G).toList.flatMap fun e => segsOf (vs e)
...
theorem IsDrawing.exists_isLocalRadius [G.Finite] (h : IsDrawing G drawing)
  ...
  exact Schoenflies.exists_isLocalRadius (SchoenfliesGraph.finite_vertexSet G) hps hx
...
theorem IsDrawing.finite_localDirs [G.Finite] (h : IsDrawing G drawing)
  ...
  obtain ⟨ps, hps⟩ := h.exists_cover hpoly
  exact Schoenflies.localDirs_finite (SchoenfliesGraph.finite_vertexSet G) hps x
