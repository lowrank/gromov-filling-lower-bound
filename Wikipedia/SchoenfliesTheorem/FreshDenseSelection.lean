...
theorem IsJordanCurve.subset_closure_sdiff_finite
  ...
  have hcleanDense : Dense (eligible' \ forbidden') :=
    heligibleDense.diff_finite hforbiddenFinite
...
theorem exists_clean_freshDense_of_accessibleTargetBoundary_dense
  ...
  have hcleanDense : modelCurve ⊆
    ...
    isJordanCurve_modelCurve.subset_closure_sdiff_finite
      haccessibleSubset haccessible (SchoenfliesGraph.finite_vertexSet (G := P.tgt.graph))
  obtain ⟨fresh, hfreshClean, hdense, hnet⟩ := exists_finite_freshDenseNet_of_dense
    (Set.diff_subset.trans haccessibleSubset) hcleanDense hdelta
...
theorem diam_closure_targetFace_lt (w : MeshOverlayTransferData Q anchors)
  ...
  have hmeshSkel :
      SchoenfliesGraph.pointSet (squareMesh w.delta w.fresh anchors) segmentDrawing ⊆
        w.pair.tgt.skeletonSet := by
    rw [w.transfer.skeletonSet_eq, SchoenfliesGraph.pointSet_relabelEdges]
    exact Q.squareMesh_subset_meshOverlay w.delta w.fresh anchors
  have hcellExterior : w.pair.tgt.cell F ⊆
      SchoenfliesGraph.exterior (squareMesh w.delta w.fresh anchors) segmentDrawing := by
    intro x hxCell hxMesh
    exact Set.disjoint_left.1
      (w.pair.tgt.disjoint_cell_skeletonSet w.pair.tgt_isCellDecomposition hF)
      hxCell (hmeshSkel hxMesh)
  have hzExterior :
      z ∈ SchoenfliesGraph.exterior (squareMesh w.delta w.fresh anchors) segmentDrawing :=
    hcellExterior hz
  have hcellFace : w.pair.tgt.cell F ⊆
      SchoenfliesGraph.face (squareMesh w.delta w.fresh anchors) segmentDrawing z :=
    (w.pair.tgt_isFaceJordan.isConnected hF).isPreconnected.subset_connectedComponentIn
      hz hcellExterior
  have hfaceCurveCompl :
      SchoenfliesGraph.face (squareMesh w.delta w.fresh anchors) segmentDrawing z ⊆ modelCurveᶜ := by
    intro x hxFace hxCurve
    exact SchoenfliesGraph.face_subset_exterior _ _ _ hxFace
      (modelCurve_subset_squareMesh_pointSet w.delta w.fresh anchors hxCurve)
  have hfaceOpen :
      SchoenfliesGraph.face (squareMesh w.delta w.fresh anchors) segmentDrawing z ⊆
        Plane.openSquare 0 1 := by
    have hcomp := (SchoenfliesGraph.isConnected_face hzExterior).isPreconnected.subset_connectedComponentIn
      (SchoenfliesGraph.mem_face hzExterior) hfaceCurveCompl
