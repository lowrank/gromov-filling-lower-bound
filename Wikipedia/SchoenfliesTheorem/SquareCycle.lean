...
-/
import Wikipedia.SchoenfliesTheorem.ArcComplement
...
so it is a cycle, and `SchoenfliesGraph.IsLongCycle.isTwoConnected` finishes.  What is missing on `main`
...
* `SchoenfliesGraph.IsIncWalk` is a walk along which a chosen real function strictly increases.  Such a
  walk is automatically a **path** (`SchoenfliesGraph.IsIncWalk.isPath`): the freshness clause of
  `SchoenfliesGraph.IsPath` says the vertex a step departs from is not visited later, and a vertex visited
...
* The four sides are chained separately and concatenated.  The concatenation is a closed walk
  at the north-east corner, so its last edge is split off (`SchoenfliesGraph.IsIncWalk.split_last`) and
  becomes the distinguished edge of `SchoenfliesGraph.IsCycleThrough`.
...
* `SchoenfliesGraph.IsIncWalk` — no blueprint statement; it is the device that makes
...
variable {α β : Type*} {G H : SchoenfliesGraph α β} {f g : α → ℝ} {u v w x : α} {e : β}
  {W W₁ W₂ : List β}

/-- **A walk along which `f` strictly increases.**  The point of the definition is
`SchoenfliesGraph.IsIncWalk.isPath`: a walk that cannot come back is a path, and no argument about the
distinctness of the vertices has to be made anywhere else. -/
inductive IsIncWalk (G : SchoenfliesGraph α β) (f : α → ℝ) : α → List β → α → Prop
...
round the square into the edge plus detour that `SchoenfliesGraph.IsCycleThrough` asks for. -/
...
theorem exists_incWalk_of_chain {G : SchoenfliesGraph Plane Piece} (hab : a ≠ b) (hbG : b ∈ V(G)) :
  ...
  · obtain ⟨Q, hQE, w, hQw, hlt⟩ := exists_next_piece hab hC.finite hC.sub_ab hC.memP
    ...
    have hC' : IsPieceChain a b w (E \ {Q}) :=
      { finite := hC.finite.subset Set.diff_subset
    ...
    have hcard' : (E \ {Q}).ncard < n := by
      rw [← hcard]; exact Set.ncard_diff_singleton_lt_of_mem hQE hC.finite
    obtain ⟨W, hWmem, hWwalk⟩ :=
      ih _ hcard' w (E \ {Q}) rfl hC' (fun R hR => hlink R hR.1)
    refine ⟨Q :: W, fun R => ?_, .cons hlinkQ hlt hWwalk⟩
    simp only [List.mem_cons, hWmem, Set.mem_diff, Set.mem_singleton_iff]
    constructor
    · rintro (rfl | ⟨h, -⟩)
      exacts [hQE, h]
    · intro hR
...
theorem walkVertices_squareGraph_subset {u : Plane} {W : List Piece} {T : Set Plane}
  ...
  rintro x hx
  rcases SchoenfliesGraph.mem_walkVertices_iff.1 hx with rfl | ⟨Q, hQ, y, hy⟩
  · exact hu
...
the distinguished edge of `SchoenfliesGraph.IsCycleThrough`. -/
...
what makes the first usable — `SchoenfliesGraph.IsLongCycle.isTwoConnected` proves the *cycle graph*
...
theorem exists_isLongCycle_squareGraph (hnd : ∀ P ∈ pieces, P.Nondeg)
  ...
  have hNWmem : sqNW c r ∈ K.walkVertices (sqNE c r) D :=
    SchoenfliesGraph.walkVertices_mono hin₁ hc₁.isWalk.target_mem_walkVertices
  ...
  have hVsub : V(K) ⊆ K.walkVertices (sqNE c r) D := by
    ...
    · exact SchoenfliesGraph.mem_walkVertices_of_mem_covered ⟨Q, h, inc_of_mem_squareEdges hQ hy⟩
    · rw [List.mem_singleton] at h
      ...
      exacts [hD.isWalk.target_mem_walkVertices, SchoenfliesGraph.mem_walkVertices_self]
  refine le_antisymm hcyc.cycleGraph_le ⟨?_, ?_⟩
  · rw [hcyc.cycleGraph_vertexSet]
    exact hVsub
  · intro Q x y hlink
    simp only [SchoenfliesGraph.cycleGraph, SchoenfliesGraph.pathGraphOf_isLink]
...
`SchoenfliesGraph.IsLongCycle.isTwoConnected` finishes. -/
