...
theorem invert_image_outside (harc : ∀ A : Set Plane, IsArc A → IsConnected Aᶜ)
  ...
  refine (Set.union_diff_cancel_right ?_).symm
...
theorem invert_image_union_outside (harc : ∀ A : Set Plane, IsArc A → IsConnected Aᶜ)
  ...
  rw [Set.image_union, invert_image_outside harc hC ha, Set.union_diff_distrib,
    Set.diff_singleton_eq_self (notMem_invert_image ha.1)]
...
theorem sdiff_singleton (h : IsHomeoOn f g S T) {u v : Plane} (hu : u ∈ S) (huv : f u = v) :
  ...
  refine h.mono Set.diff_subset Set.diff_subset (fun z hz => ⟨h.mapsTo hz.1, ?_⟩)
