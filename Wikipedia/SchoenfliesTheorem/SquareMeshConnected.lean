...
The ear construction is replaced here by two applications of `SchoenfliesGraph.IsTwoConnected.union`,
which is shorter and needs no ear at all:

* one **cell** — the boundary of a single rectangle of the grid — is a quadrilateral, and
  `SchoenfliesGraph.isTwoConnected_of_quad` proves any quadrilateral 2-connected directly from the
...
`gridGraph_isCycleThrough_boundary` is a `SchoenfliesGraph.IsCycleThrough` of the grid graph — the
...
`SchoenfliesGraph.IsTwoConnected.attach_cycles` below: it takes a 2-connected hub `K` and finitely many
...
* `lem:union-two-connected` — used through `SchoenfliesGraph.IsTwoConnected.union`; iterated here as
  `SchoenfliesGraph.IsTwoConnected.attach_cycles`, which is the blueprint's "adding these finitely many
...
variable {α β : Type*} {G : SchoenfliesGraph α β}
...
definition of `SchoenfliesGraph.IsTwoConnected`, because a four-vertex graph is small enough that
...
`SchoenfliesGraph.chainUnion_isTwoConnected` (`Schoenflies/OuterChain.lean`) iterates the same lemma along
...
def attachUnion (K : SchoenfliesGraph α β) (Γ : ℕ → SchoenfliesGraph α β) : ℕ → SchoenfliesGraph α β
  | 0 => K
  | (m + 1) => (attachUnion K Γ m).union (Γ m)

@[simp] theorem attachUnion_zero (K : SchoenfliesGraph α β) (Γ : ℕ → SchoenfliesGraph α β) :
    attachUnion K Γ 0 = K := rfl

@[simp] theorem attachUnion_succ (K : SchoenfliesGraph α β) (Γ : ℕ → SchoenfliesGraph α β) (m : ℕ) :
    attachUnion K Γ (m + 1) = (attachUnion K Γ m).union (Γ m) := rfl

theorem le_attachUnion (K : SchoenfliesGraph α β) (Γ : ℕ → SchoenfliesGraph α β) (m : ℕ) :
    K ≤ attachUnion K Γ m := by
  induction m with
  | zero => exact le_rfl
  | succ m ih => exact ih.trans (left_le_union _ _)

theorem attachUnion_le {K : SchoenfliesGraph α β} {Γ : ℕ → SchoenfliesGraph α β} {m : ℕ} (hK : K ≤ G)
    (hΓ : ∀ q, q < m → Γ q ≤ G) : attachUnion K Γ m ≤ G := by
  induction m with
  | zero => exact hK
  | succ m ih => exact union_le (ih fun q hq => hΓ q (by omega)) (hΓ m (by omega))

theorem piece_le_attachUnion {K : SchoenfliesGraph α β} {Γ : ℕ → SchoenfliesGraph α β} {m q : ℕ} (hK : K ≤ G)
...
theorem IsTwoConnected.attach_cycles {K : SchoenfliesGraph α β} {Γ : ℕ → SchoenfliesGraph α β} {m : ℕ}
...
`q s, q (s+1), …`. The freshness clause of `SchoenfliesGraph.IsPath` is then exactly the injectivity of
`p` on the index range, because an edge of the chain is incident with nothing but its own two
ends (`SchoenfliesGraph.Inc.eq_or_eq_of_isLink`). -/
...
def pieceListGraph (edges : List Piece) : SchoenfliesGraph Plane Piece where
...
instance pieceListGraph_finite (l : List Piece) : SchoenfliesGraph.Finite (pieceListGraph l) where
...
theorem pieceListGraph_pointSet (l : List Piece) :
    SchoenfliesGraph.pointSet (pieceListGraph l) segmentDrawing = cover l := by
  ext z
  simp only [SchoenfliesGraph.pointSet, Set.mem_union, Set.mem_iUnion, exists_prop, pieceListGraph_vertexSet,
...
theorem pieceListGraph_union (l l' : List Piece) :
  ...
  refine SchoenfliesGraph.eq_of_le_of_subset_subset
    (SchoenfliesGraph.union_le (pieceListGraph_mono (List.subset_append_left _ _))
      (pieceListGraph_mono (List.subset_append_right _ _))) ?_ ?_
  · rw [pieceListGraph_vertexSet, endSet_append, SchoenfliesGraph.vertexSet_union]
    exact subset_rfl
  · rintro P hP
...
def gridGraph (xc yc : ℕ → ℝ) (m n : ℕ) : SchoenfliesGraph Plane Piece := pieceListGraph (gridEdges xc yc m n)
...
`SchoenfliesGraph.isTwoConnected_of_quad` with the four corners named. Only the two consecutive-coordinate
...
theorem cellGraph_isTwoConnected {xc yc : ℕ → ℝ} {i j : ℕ} (hx : xc i ≠ xc (i + 1))
  ...
  refine SchoenfliesGraph.isTwoConnected_of_quad (a := gridPt xc yc i j) (b := gridPt xc yc (i + 1) j)
...
Two chains of `SchoenfliesGraph.IsTwoConnected.union`, and the two shared vertices are in each case the
...
theorem gridGraph_isCycleThrough_boundary {xc yc : ℕ → ℝ} {m n : ℕ} (hm : 1 ≤ m) (hn : 1 ≤ n)
  ...
  · -- the detour is a path: `SchoenfliesGraph.isPath_range'` with the vertices of the walk
    have := SchoenfliesGraph.isPath_range' (G := gridGraph xc yc m n) (p := gridBoundaryPt xc yc m n)
...
theorem edgesCover_gridBoundary_eq_iUnion (xc yc : ℕ → ℝ) {m n : ℕ} (hL : 1 ≤ 2 * m + 2 * n) :
  ...
  ext z
  simp only [SchoenfliesGraph.mem_edgesCover_iff, List.mem_cons, gridBoundaryDetour, List.mem_map,
...
that single step settles both remaining clauses of `SchoenfliesGraph.IsDrawing`. -/
...
theorem gridGraph_isDrawing (hxs : StrictMonoOn xc (Set.Iic m))
    ...
    SchoenfliesGraph.IsDrawing (gridGraph xc yc m n) segmentDrawing := by
  refine ⟨?_, ?_, ?_⟩
  · intro P hP
...
`SchoenfliesGraph.IsDrawing.cycle_isJordanCurve` applied to the boundary cycle. -/
theorem gridBoundary_isJordanCurve (hxs : StrictMonoOn xc (Set.Iic m))
    (hys : StrictMonoOn yc (Set.Iic n)) (hm : 1 ≤ m) (hn : 1 ≤ n) :
    IsJordanCurve (SchoenfliesGraph.edgesCover segmentDrawing
      (gridBoundaryLast xc yc m n :: gridBoundaryDetour xc yc m n)) :=
