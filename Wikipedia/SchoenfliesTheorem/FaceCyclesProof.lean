...
`SchoenfliesGraph.face_cycles` **carries a hypothesis, `Schoenflies.CrosscutSplitsRegion`, which nothing
...
* **The realisation of a cycle is a separating curve** (`SchoenfliesGraph.IsDrawing.cycle_isSeparating`),
  which is what the base case needed and what `main` could not say. It composes
  `SchoenfliesGraph.IsDrawing.cycle_isJordanCurve` with `Schoenflies.exists_closedPolygon` and
...
* **The base case** (`SchoenfliesGraph.IsDrawing.hasFaceCycles_cycleGraph`): the cycle subgraph occupies
  exactly the cycle's realisation, so its faces are the two regions of that curve.
* **Cutting a cycle at two of its vertices** (`SchoenfliesGraph.IsCycleThrough.split_at`) and **splicing an
  ear onto an arc** (`SchoenfliesGraph.exists_spliced_cycle`) — the combinatorics of "both bounded by
  cycles" — together with the geometric reading of the split
  (`SchoenfliesGraph.IsDrawing.arcs_of_split`).
* **The step's topology**: the enlarged exterior is the old one minus the ear
  (`SchoenfliesGraph.IsDrawing.pointSet_pathGraphOf`), a face the ear misses survives with its cycle
  (`SchoenfliesGraph.IsFaceCycle.mono`), and the face the ear cuts is cut only inside itself
  (`Schoenflies.connectedComponentIn_diff`).

## For the integrator

`SchoenfliesGraph.IsPath.append`, `SchoenfliesGraph.IsPath.split_meet`, `SchoenfliesGraph.IsWalk.walkVertices_eq_covered`,
`SchoenfliesGraph.IsWalk.walkVertices_reverse_eq`, `SchoenfliesGraph.coveredVertices_congr_of_le`,
`SchoenfliesGraph.walkVertices_congr_of_le` and `SchoenfliesGraph.edgesCover_append` / `…_perm` / `…_reverse` are
general and belong in `Schoenflies/SchoenfliesGraph/Walk.lean` and `Schoenflies/SchoenfliesGraph/CycleJordan.lean`.
...
* `Schoenflies.IsArcBetween.eq_of_subset`, `Schoenflies.IsArcBetween.exists_poly_eq`,
  `SchoenfliesGraph.IsDrawing.exists_poly_eq_edgesCover`, `SchoenfliesGraph.IsDrawing.isPolygonal_edgesCover` — §1,
  the polygonality of what a walk draws.
* `SchoenfliesGraph.IsDrawing.cycle_isSeparating` — `thm:polygonal-jordan` for the realisation of a cycle:
  "This is true for the initial cycle by Theorem `thm:polygonal-jordan`."
* `SchoenfliesGraph.IsFaceCycle`, `SchoenfliesGraph.HasFaceCycles` — the conclusion of `lem:face-cycles`, at one face
  and at all of them.
* `SchoenfliesGraph.IsDrawing.hasFaceCycles_cycleGraph` — the base case.
* `SchoenfliesGraph.IsCycleThrough.split_at`, `SchoenfliesGraph.IsDrawing.arcs_of_split`,
  `SchoenfliesGraph.exists_spliced_cycle` — "Theorem `thm:polygonal-crosscut` replaces `F` by exactly two
...
* `SchoenfliesGraph.IsDrawing.hasFaceCycles_union` — one ear.
* `SchoenfliesGraph.face_cycles` — `lem:face-cycles`, modulo `Schoenflies.CrosscutSplitsRegion`.
-/
...
theorem connectedComponentIn_diff (S P : Set Plane) (z : Plane) :
  ...
  · have hcomp : connectedComponentIn (S \ P) z ⊆ connectedComponentIn S z :=
      isPreconnected_connectedComponentIn.subset_connectedComponentIn
        (mem_connectedComponentIn hz) ((connectedComponentIn_subset _ _).trans Set.diff_subset)
...
Two general facts about paths that `Schoenflies/SchoenfliesGraph/Walk.lean` does not have, and that the
splitting of a cycle at two of its vertices runs on. **The integrator should hoist both into
`Schoenflies/SchoenfliesGraph/Walk.lean`**, next to `SchoenfliesGraph.IsPath.split`, which is the weaker form of the
second. -/

section GeneralWalk

variable {α β : Type*} {G : SchoenfliesGraph α β} {u v w x y : α} {e : β} {W W₁ W₂ : List β}
...
`SchoenfliesGraph.IsPath.split` with the meeting condition in place of its weaker last clause. -/
...
theorem IsCycleThrough.split_aux {G : SchoenfliesGraph α β} {e : β} {u v c d : α} {D X Y Z : List β}
...
theorem IsCycleThrough.split_at {G : SchoenfliesGraph α β} {e : β} {u v a b : α} {D : List β}
...
theorem coveredVertices_congr_of_le {H G : SchoenfliesGraph α β} (hHG : H ≤ G) (hW : ∀ f ∈ W, f ∈ E(H)) :
...
theorem walkVertices_congr_of_le {H G : SchoenfliesGraph α β} (hHG : H ≤ G) (hW : ∀ f ∈ W, f ∈ E(H)) :
...
arc and back along the ear; presented as `SchoenfliesGraph.IsCycleThrough`, its named edge is the ear's
first, and its detour is the arc followed by the rest of the ear reversed. -/
theorem exists_spliced_cycle {B G : SchoenfliesGraph α β} (hBG : B ≤ G) {a b : α} {D₁ D' : List β}
...
variable {β : Type*} {G B : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane}
...
missing from it: `SchoenfliesGraph.IsDrawing.cycle_isJordanCurve` says the realisation is a Jordan curve
and `SchoenfliesGraph.IsDrawing.isPolygonal_edgesCover` says it is polygonal, and
...
theorem IsCycleThrough.isWalk_cons {α : Type*} {G : SchoenfliesGraph α β} {e : β} {u v : α} {D : List β}
...
structure IsFaceCycle (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (z : Plane)
...
def HasFaceCycles (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) : Prop :=
...
The combinatorial split of `SchoenfliesGraph.IsCycleThrough.split_at` becomes the geometric one: the two
...
there.** The hypotheses are the output of `SchoenfliesGraph.IsCycleThrough.split_at`, read in the plane. -/
...
**Read this before using `SchoenfliesGraph.face_cycles`.** Everything below the base case is proved
...
its realisation separates the plane (`SchoenfliesGraph.IsDrawing.cycle_isSeparating`), that the ear lies in
