...
The graph is `overlayGraph`, a `SchoenfliesGraph Plane Piece`: its edges are the oriented, deduplicated
...
The three clauses of `SchoenfliesGraph.IsDrawing` come out as follows.
...
noncomputable def overlayGraph (pieces : List Piece) (points : List Plane) :
    SchoenfliesGraph Plane Piece where
...
@[simp] theorem edgeArc_segmentDrawing (P : Piece) :
    SchoenfliesGraph.edgeArc segmentDrawing P = P.seg :=
...
theorem overlayGraph_isDrawing (pieces : List Piece) (points : List Plane)
    ...
    SchoenfliesGraph.IsDrawing (overlayGraph pieces points) segmentDrawing := by
  refine ⟨?_, ?_, ?_⟩
...
theorem overlayGraph_pointSet (pieces : List Piece) (points : List Plane) :
    SchoenfliesGraph.pointSet (overlayGraph pieces points) segmentDrawing = cover pieces := by
  rw [← overlayPieces_cover pieces points]
  ext z
  simp only [SchoenfliesGraph.pointSet, mem_union, mem_iUnion, exists_prop, overlayGraph_vertexSet,
...
Without this the whole face and outer-face machinery of `Schoenflies/SchoenfliesGraph/Drawing.lean` and
`Schoenflies/SchoenfliesGraph/OuterFace.lean` is inapplicable to the overlay, since every statement there
...
instance overlayGraph_finite (pieces : List Piece) (points : List Plane) :
    SchoenfliesGraph.Finite (overlayGraph pieces points) where
...
theorem polygonal_overlay (pieces : List Piece) (hnd : ∀ P ∈ pieces, P.Nondeg) :
    ∃ G : SchoenfliesGraph Plane Piece, SchoenfliesGraph.Finite G ∧ SchoenfliesGraph.IsDrawing G segmentDrawing ∧
      SchoenfliesGraph.pointSet G segmentDrawing = cover pieces := by
