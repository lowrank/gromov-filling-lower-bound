...
* **The induction scheme, steps 2 and 3** — `Schoenflies.transfer_of_ears`. With `earStep`
  ...
  (`SchoenfliesGraph.IsTwoConnected.ear_decomposition`) transfers the whole extension. This is the backbone
...
theorem isWeaklyAdmissible_realize (hE : d.EarCrosscut R earPos earDraw)
  ...
  outerSet_eq := by
    change SchoenfliesGraph.pointSet ((S.splitFace d).outerGraph.map (d.splitPos R earPos))
    ...
    have hgraph : S.outerGraph.map (d.splitPos R earPos) = S.outerGraph.map R.pos :=
      SchoenfliesGraph.map_eq_of_eqOn fun z hz => hE.splitPos_eq (S.outerGraph_le.vertexSet_mono hz)
    rw [hgraph]
    calc
      SchoenfliesGraph.pointSet (S.outerGraph.map R.pos) (d.splitDrawing R earDraw) =
          SchoenfliesGraph.pointSet (S.outerGraph.map R.pos) R.drawing := by
        apply SchoenfliesGraph.pointSet_congr
        intro e he
        rw [SchoenfliesGraph.edgeSet_map] at he
...
theorem exists_matched_target (hsrc : d.EarCrosscut R₁ srcPos srcDraw)
    ...
    ∃ tgtPos : γ → Plane, ∃ tgtDraw : γ → ℝ → Plane,
      ...
      ∀ ⦃e⦄, e ∈ E(d.ear) → IsPolygonal (SchoenfliesGraph.edgeArc tgtDraw e) := by
  classical
  ...
  have hhomeo : d.EarHomeo srcPos srcDraw tgtPos tgtDraw := {
    ...
    edgeArc_image := by
      intro e _
      exact (SchoenfliesGraph.edgeArc_mapDrawing m e).symm
  }
  refine ⟨tgtPos, tgtDraw, hhomeo, htgt, ?_⟩
  intro e he
  obtain ⟨x, y, hxy⟩ := d.ear.exists_isLink_of_mem_edgeSet he
  have hedge : IsArcBetween (SchoenfliesGraph.edgeArc tgtDraw e) (tgtPos x) (tgtPos y) :=
...
structure IsSourceExtension {S : CellStructure γ} (R : S.Realization) (outer dom : Set Plane)
    (H : SchoenfliesGraph Plane γ) (Hdraw : γ → ℝ → Plane) : Prop where
  /-- `H` is finite. -/
  finite : H.Finite
  /-- `H` is a plane graph. -/
...
structure EarStepData (T : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
  ...
  srcEarSet_eq : splitData.earSet srcPos srcDraw = SchoenfliesGraph.edgesCover Hdraw D
...
theorem exists_source_face_of_ear
    ...
    ∃ u v F, u ∈ V(T.str.skel) ∧ v ∈ V(T.str.skel) ∧ u ≠ v ∧
      ...
      SchoenfliesGraph.edgesCover Hdraw D \ {a, b} ⊆ T.src.cell F ∧
...
theorem source_ear_edge_polygonal
    ...
    (hinside : SchoenfliesGraph.edgesCover Hdraw D \ {a, b} ⊆ T.src.cell F) :
...
def CommonSubdivision (P : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
  ...
  ∃ (K : SchoenfliesGraph Plane γ) (T₀ : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom) (par₀ : γ → γ),
    K.IsTwoConnected ∧ K ≤ H ∧ IsPartialTransferOf T₀ P K Hdraw par₀

/-- **Step 3, one ear insertion**, as an interface.

The data handed to the step is exactly what `SchoenfliesGraph.IsTwoConnected.ear_decomposition` supplies:
...
structure SourceEarStepData (T : GeneratedPair S₀ srcOuter srcDom tgtOuter tgtDom)
  ...
  srcDraw : γ → ℝ → Plane
  srcCrosscut : splitData.EarCrosscut T.src srcPos srcDraw
  srcEdgePolygonal : ∀ ⦃e⦄, e ∈ E(splitData.ear) → IsPolygonal (SchoenfliesGraph.edgeArc srcDraw e)
  srcEarSet_eq : splitData.earSet srcPos srcDraw = SchoenfliesGraph.edgesCover Hdraw D
  vertexSet_subset :
    V(B.union (H.pathGraphOf a D)) ⊆
      V((splitData.realize T.src srcPos srcDraw srcCrosscut).graph)
...
theorem exists_sourceEarStepData [Infinite γ]
  ...
  let srcDraw : γ → ℝ → Plane := SchoenfliesGraph.relabelDrawing Q ename Hdraw
  have hQdraw : SchoenfliesGraph.IsDrawing Q Hdraw := hH.isDrawing.mono hQle
  have hrelDraw : SchoenfliesGraph.IsDrawing relabelled srcDraw := by
  ...
  have hsrcSet : d.earSet srcPos srcDraw = SchoenfliesGraph.edgesCover Hdraw D := by
  ...
  have hsrcEdgePoly : ∀ ⦃e⦄, e ∈ E(d.ear) →
      IsPolygonal (SchoenfliesGraph.edgeArc srcDraw e) := by
    intro e he
  ...
  have hsrcPoly : IsPolygonal (d.earSet srcPos srcDraw) := by
    ...
    exact hQdraw.isPolygonal_edgesCover
      (fun f hfQ => hsrcEdgeOrig f (by rwa [SchoenfliesGraph.pathGraphOf_edgeSet hpath.isWalk] at hfQ))
...
ear contains an old edge, `SchoenfliesGraph.ear_edges_notMem_or_union_eq` shows that the union did not
...
`SchoenfliesGraph.polygonal_side_accessibility_target` consumes, and it is the shape a target realization
...
theorem exists_target_crosscut [G.Finite] (h : IsDrawing G drawing)
  ...
  obtain ⟨ws, -, -, -, hsub, harc, hmeet⟩ :=
    exists_crosscut_of_polyAccessible hopen hconn (hdisj.mono_right hJ) hvw hvJ hwJ
      (SchoenfliesGraph.polygonal_side_accessibility_target h hpoly hQ hQK hcell hF hv)
      (SchoenfliesGraph.polygonal_side_accessibility_target h hpoly hQ hQK hcell hF hw)
...
/-- A face and two distinct boundary vertices of a generated pair have the target polygonal
...
theorem GeneratedPair.exists_target_crosscut
  ...
  letI : (P.str.skel.map P.tgt.pos).Finite := {
    finite_vertexSet := by
      rw [SchoenfliesGraph.vertexSet_map]
...
theorem exists_overlay_of_biUnion_finite {ι : Type*} {s : Set ι} {A : ι → Set Plane}
    ...
    ∃ G : SchoenfliesGraph Plane Piece, SchoenfliesGraph.Finite G ∧ IsDrawing G segmentDrawing ∧
...
example [Nonempty γ] {S₀ : CellStructure γ} {srcOuter srcDom tgtOuter tgtDom : Set Plane}
    ...
    {H : SchoenfliesGraph Plane γ} {Hdraw : γ → ℝ → Plane} {par : γ → γ}
