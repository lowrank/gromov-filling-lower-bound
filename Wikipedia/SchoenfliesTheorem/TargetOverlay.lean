...
theorem exists_targetSegmentCover
  ...
  letI : SchoenfliesGraph.Finite P.tgt.graph :=
...
@[simp] theorem mem_squareMeshPieces {delta : ℝ} {fresh anchors : List Plane} {R : Piece} :
  ...
  simp [squareMeshPieces, SchoenfliesGraph.mem_edgeFinset]
...
theorem cover_squareMeshPieces (delta : ℝ) (fresh anchors : List Plane) :
    cover (squareMeshPieces delta fresh anchors) =
      SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing := by
  ext x
  constructor
  · intro hx
...
noncomputable def meshOverlay (Q : TargetSegmentCover P) (delta : ℝ)
    (fresh anchors : List Plane) : SchoenfliesGraph Plane Piece :=
...
theorem meshOverlay_isDrawing (Q : TargetSegmentCover P)
    ...
    SchoenfliesGraph.IsDrawing (Q.meshOverlay delta fresh anchors) segmentDrawing :=
...
theorem meshOverlay_pointSet (Q : TargetSegmentCover P)
    ...
    SchoenfliesGraph.pointSet (Q.meshOverlay delta fresh anchors) segmentDrawing =
      P.tgt.skeletonSet ∪
        SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing := by
...
theorem targetSkeleton_subset_meshOverlay (Q : TargetSegmentCover P)
    (delta : ℝ) (fresh anchors : List Plane) :
    P.tgt.skeletonSet ⊆
      SchoenfliesGraph.pointSet (Q.meshOverlay delta fresh anchors) segmentDrawing := by
...
theorem squareMesh_subset_meshOverlay (Q : TargetSegmentCover P)
    (delta : ℝ) (fresh anchors : List Plane) :
    SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing ⊆
      SchoenfliesGraph.pointSet (Q.meshOverlay delta fresh anchors) segmentDrawing := by
  rw [Q.meshOverlay_pointSet]
...
theorem targetTrace_isTwoConnected (Q : TargetSegmentCover P)
    {fresh : List Plane} (hfresh : ∀ z ∈ fresh, z ∈ modelCurve)
    (delta : ℝ) (anchors : List Plane) :
    (SchoenfliesGraph.traceGraph (Q.meshOverlay delta fresh anchors) segmentDrawing
...
theorem squareMeshTrace_isTwoConnected (Q : TargetSegmentCover P)
    ...
    (anchors : List Plane) :
    (SchoenfliesGraph.traceGraph (Q.meshOverlay delta fresh anchors) segmentDrawing
      (SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing)).IsTwoConnected :=
...
theorem meshOverlay_isTwoConnected (Q : TargetSegmentCover P)
  ...
  let T := SchoenfliesGraph.traceGraph (Q.meshOverlay delta fresh anchors) segmentDrawing
    P.tgt.skeletonSet
  let M := SchoenfliesGraph.traceGraph (Q.meshOverlay delta fresh anchors) segmentDrawing
    (SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing)
  ...
  have hwTarget : w ∈ P.tgt.skeletonSet := by
    ...
    rw [P.tgt_isWeaklyAdmissible.outerSet_eq]
    exact hfresh w hw
  have hzMesh : z ∈ SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing :=
    SchoenfliesGraph.vertexSet_subset_pointSet hzSquare
  have hwMesh : w ∈ SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing :=
    SchoenfliesGraph.vertexSet_subset_pointSet hwSquare
  have hzT : z ∈ V(T) := by
  ...
  have hxPoint : x ∈ SchoenfliesGraph.pointSet (Q.meshOverlay delta fresh anchors) segmentDrawing :=
    SchoenfliesGraph.vertexSet_subset_pointSet hx
  rw [Q.meshOverlay_pointSet] at hxPoint
  rcases hxPoint with hxTarget | hxMesh
  · exact Or.inl (by rw [SchoenfliesGraph.traceGraph_vertexSet]; exact ⟨hx, hxTarget⟩)
  · exact Or.inr (by rw [SchoenfliesGraph.traceGraph_vertexSet]; exact ⟨hx, hxMesh⟩)
  ...
  · refine Or.inr ⟨?_, ?_⟩
    ...
    · intro x hx
      refine ⟨Q.meshOverlay_pointSet_subset hfresh delta anchors
        (SchoenfliesGraph.edgeArc_subset_pointSet hR hx.1), ?_⟩
      intro hxOuter
      have hxMeshOuter :
...
theorem newTargetBoundaryAnchored_meshOverlay (Q : TargetSegmentCover P)
  ...
  · obtain ⟨A, hA, hRA⟩ := hOld
    ...
    have hRbase : edgeArc segmentDrawing R ⊆ P.tgt.skeletonSet := by
      rw [edgeArc_segmentDrawing]
      exact hRA.trans (hAe.trans (SchoenfliesGraph.edgeArc_subset_pointSet (by
...
theorem newTargetBoundaryAnchored_relabelledMeshOverlay (Q : TargetSegmentCover P)
  ...
  obtain ⟨R', hR', hR'name, hR'inc⟩ :=
    (SchoenfliesGraph.relabelEdges_inc (Q.meshOverlay delta fresh anchors) name hname
      (name R) z).1 hinc
  ...
  have hRnot : ¬ edgeArc segmentDrawing R ⊆ modelCurve := by
    intro hsub
    apply hnot
    rwa [SchoenfliesGraph.edgeArc_relabelDrawing hname hR]
  rcases Q.meshOverlay_edge_source_squareMesh hR with hOld | hMesh
  · obtain ⟨A, hA, hRA⟩ := hOld
    obtain ⟨e, he, hAe⟩ := Q.source A hA
    have hRbase :
      ...
      rw [SchoenfliesGraph.edgeArc_relabelDrawing hname hR, edgeArc_segmentDrawing]
      exact hRA.trans (hAe.trans (SchoenfliesGraph.edgeArc_subset_pointSet (by
        rw [P.tgt.edgeSet_graph]
        exact he)))
...
theorem noNewNonouterIncidenceAtBoundary_relabelledMeshOverlay
  ...
  have hSnot : ¬ edgeArc segmentDrawing S ⊆ modelCurve := by
    intro hsub
    apply hknot
    rwa [SchoenfliesGraph.edgeArc_relabelDrawing hname hS]
  have hzRarc : z ∈ edgeArc segmentDrawing R := hdraw.inc_mem_edgeArc hR'inc
  have hzRseg : z ∈ R.seg := by rwa [edgeArc_segmentDrawing] at hzRarc
  rcases Q.meshOverlay_edge_source_squareMesh hR with hROld | hRMesh
  · obtain ⟨A, hA, hRA⟩ := hROld
...
@[simp] theorem mem_openTargetEdgePieces
  ...
  simp only [openTargetEdgePieces, List.mem_map, Finset.mem_toList, Finset.mem_filter,
    SchoenfliesGraph.mem_edgeFinset]
  constructor
...
noncomputable def finiteOpenTargetCover
  ...
  subset_open := by
    ...
    exact Set.diff_subset_diff_left (SchoenfliesGraph.edgeArc_subset_pointSet (by
...
theorem mesh_hits_openTarget_of_locallyWider
    ...
    ∀ z ∈ P.tgt.skeletonSet \ modelCurve,
      ∃ A : Set Plane, A ⊆ P.tgt.skeletonSet \ modelCurve ∧
        IsPreconnected A ∧ z ∈ A ∧
          (A ∩ (SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing \
            modelCurve)).Nonempty := by
  intro z hz
  obtain ⟨A, hA, hAconn, hzA, x, hxA, y, hyA, hxy⟩ := hwide z hz
  refine ⟨A, hA, hAconn, hzA, ?_⟩
  have hmeet :
      (A ∩ SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing).Nonempty := by
    by_contra hempty
...
theorem meshOverlay_isConnected_diff_of_hits (Q : TargetSegmentCover P)
      ...
      ∃ A : Set Plane, A ⊆ P.tgt.skeletonSet \ modelCurve ∧
        IsPreconnected A ∧ z ∈ A ∧
          (A ∩ (SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing \
            modelCurve)).Nonempty) :
    IsConnected
      (SchoenfliesGraph.pointSet (Q.meshOverlay delta fresh anchors) segmentDrawing \ modelCurve) := by
  let M := SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing \ modelCurve
  ...
  have heq :
      (P.tgt.skeletonSet ∪
          SchoenfliesGraph.pointSet (squareMesh delta fresh anchors) segmentDrawing) \ modelCurve =
...
theorem isSourceExtension_relabelledMeshOverlay
    ...
    (hconnected : IsConnected
      (SchoenfliesGraph.pointSet (Q.meshOverlay delta fresh anchors) segmentDrawing \ modelCurve)) :
  ...
  finite := SchoenfliesGraph.Finite.relabelEdges hname
  ...
  skeletonSet_subset := by
    rw [SchoenfliesGraph.pointSet_relabelEdges hname]
    exact Q.targetSkeleton_subset_meshOverlay delta fresh anchors
  edge_subset := by
    intro e he d hd hmeet
    obtain ⟨R, hR, rfl⟩ := hd
    rw [SchoenfliesGraph.edgeArc_relabelDrawing hname hR,
      SchoenfliesGraph.vertexSet_relabelEdges] at hmeet
    rw [SchoenfliesGraph.edgeArc_relabelDrawing hname hR]
