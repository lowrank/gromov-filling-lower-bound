...
(`SchoenfliesGraph.IsStageOn`), and nothing below mentions the sequence of stages that produces it.
...
* `SchoenfliesGraph.IsStageOn` — a finite plane graph carrying `C` as its outer boundary.
* `SchoenfliesGraph.Nonboundary`, `SchoenfliesGraph.interiorPart`, `SchoenfliesGraph.attachEdges`,
  `SchoenfliesGraph.isPreconnected_interiorPart` — the interior subgraph and its connectedness, which is
  the substance of the second case.
* `SchoenfliesGraph.IsStageOn.exists_crosscut` — **`lem:skeleton-crosscuts`, clause 1**: the source-side
...
variable {β : Type*} {G : SchoenfliesGraph Plane β} {drawing : β → ℝ → Plane} {C D : Set Plane}
...
structure IsStageOn (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (C D : Set Plane) : Prop where
...
theorem diff_subset_edgeArc [G.Finite] (h : IsStageOn G drawing C D)
  ...
  have hRclosed : IsClosed R := by
    refine (SchoenfliesGraph.finite_vertexSet (G := G)).isCompact.isClosed.union ?_
    refine (Set.Finite.isCompact_biUnion ?_
      fun f hf => h.isDrawing.isCompact_edgeArc hf.1).isClosed
    exact (SchoenfliesGraph.finite_edgeSet (G := G)).subset fun f hf => hf.1
...
so `interiorPart` is that set rather than a `SchoenfliesGraph`. -/
...
def interiorPart (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (D : Set Plane) : Set Plane :=
...
theorem isClosed_interiorPart [G.Finite] (h : IsDrawing G drawing) :
  ...
  refine ((SchoenfliesGraph.finite_vertexSet (G := G)).inter_of_left D).isCompact.isClosed.union ?_
  refine (Set.Finite.isCompact_biUnion ?_ fun e he => h.isCompact_edgeArc he.1).isClosed
  exact (SchoenfliesGraph.finite_edgeSet (G := G)).subset fun e he => he.1
...
def attachEdges (G : SchoenfliesGraph Plane β) (drawing : β → ℝ → Plane) (S : Set Plane) : Set Plane :=
...
theorem exists_crosscut [G.Finite] (h : IsStageOn G drawing C D) {a b : Plane}
  ...
  · -- Case 2: extract the crosscut from `Y = e_a ∪ |Λ| ∪ e_b`.
    ...
    have hfin : (Sum.inl '' (V(G) ∩ D) ∪ Sum.inr '' EE : Set (Plane ⊕ β)).Finite :=
      (((SchoenfliesGraph.finite_vertexSet (G := G)).inter_of_left D).image _).union
        (((SchoenfliesGraph.finite_edgeSet (G := G)).subset fun e he => he.1).image _)
