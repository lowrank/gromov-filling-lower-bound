...
-/
import SchoenfliesCompat.Graph.Maps
...
are the vertices and the edges of an abstract multigraph `skel : SchoenfliesGraph γ γ`, the 2-cells are a
...
The price of the choice is paid here, once: `SchoenfliesGraph.isTwoConnected_map_iff` and
`SchoenfliesGraph.connected_map_iff` transport connectivity along an injective relabelling of the
...
and vertex-deletion lemmas they rest on, are general facts about `SchoenfliesGraph.map` in the root
`SchoenfliesGraph` namespace and belong in `Schoenflies/SchoenfliesGraph/` if a second consumer appears.
...
* `SchoenfliesGraph.isTwoConnected_map_iff`, `SchoenfliesGraph.connected_map_iff` — connectivity is invariant under
...
variable {α α' β : Type*} {G : SchoenfliesGraph α β} {f : α → α'} {u v x : α} {W : List β}
...
theorem map_deleteVerts_singleton (hf : InjOn f V(G)) (hx : x ∈ V(G)) :
  ...
  refine SchoenfliesGraph.ext ?_ ?_
  · rw [vertexSet_deleteVerts, vertexSet_map, vertexSet_map, vertexSet_deleteVerts]
    rw [hf.image_diff_subset (Set.singleton_subset_iff.2 hx), Set.image_singleton]
  · intro e a b
...
variable {δ : Type*} {H K : SchoenfliesGraph Plane δ} {drawing : δ → ℝ → Plane}
...
def graph : SchoenfliesGraph Plane γ := S.skel.map R.pos
...
theorem cell_subset_skeletonSet {R : S.Realization} {κ : γ}
  ...
  · obtain ⟨a, b, hl⟩ := exists_isLink_of_mem_edgeSet he
    rw [R.cell_edge hl]
    exact Set.diff_subset.trans (edgeArc_subset_pointSet
...
theorem image_pointSet_map {H : SchoenfliesGraph γ γ} (hH : H ≤ S.skel) :
...
theorem image_nonboundary : g.toFun '' R₁.nonboundary = R₂.nonboundary := by
  have : g.toFun '' (R₁.skeletonSet \ R₁.outerSet) = R₂.skeletonSet \ R₂.outerSet := by
    rw [g.injOn.image_diff_subset R₁.outerSet_subset_skeletonSet, g.image_skeletonSet,
      g.image_outerSet]
  exact this
...
theorem isConnected_nonboundary_iff (g : SkeletonHomeo R₁ R₂) :
  ...
  · intro h
    ...
    exact h.image _ (g.continuousOn_toFun.mono Set.diff_subset)
  · intro h
    rw [← g.symm.image_nonboundary]
    exact h.image _ (g.symm.continuousOn_toFun.mono Set.diff_subset)
