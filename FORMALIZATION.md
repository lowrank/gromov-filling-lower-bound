...
| Proposition 3.2, Orthogonal Jacobian budget | **Partial** | The algebraic Bessel/Jacobian inequality and genuine planar metric-profile derivative budget are verified in `JacobianBudget.lean` and `ProfileFourier.lean`; `RiemannianLipschitzDerivative.lean` supplies the sharp intrinsic pointwise derivative-norm input at differentiability points. Parameter differentiation and global intrinsic surface integration remain unformalized. |
