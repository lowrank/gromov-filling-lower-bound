...
All 18 current numbered statements are listed in the [current manuscript ledger](docs/current-manuscript.md); [Revised Section 4](docs/section4.md) gives the nonlinear details. The new endpoints are `riemannianSurfaceArea_gt_one_div_twenty_five_of_conventionally_oriented_isometric_filling` and `riemannianSharpenedNonlinearCertificate_le_surfaceArea_of_conventionally_oriented_isometric_filling`, both in `RiemannianSharpenedNonlinear.lean`. Their geometric hypotheses match the previous endpoints exactly.
