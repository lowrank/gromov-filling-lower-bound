...
| Planar and local Riemannian analysis | Lipschitz area inequality, intrinsic two-Jacobian and chain rule, one-chart weighted area measure and integral transfer, exact planar determinant identity, Jacobian budgets |
...
- canonical Riemannian area measure and compatible-atlas globalization (the
  intrinsic pointwise `J₂`, its chain rule, and the exact weighted
  measure/integral transfer and area inequality on one parametrized chart are
  verified);
