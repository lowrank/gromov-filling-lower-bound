...
-/
/-
Compatibility note: this pinned mathlib 4.29 module is namespace-isolated as
`SchoenfliesGraph` so that it can coexist with the independent graph type in
the pinned HOL Light Jordan-curve development.
-/
module
...
This module defines `SchoenfliesGraph α β` for a vertex type `α` and an edge type `β`,
and gives basic API for incidence, adjacency and extensionality.
The design broadly follows [Chou1994].

## Main definitions

For `G : SchoenfliesGraph α β`, ...
...
* `SchoenfliesGraph.noEdge V` is the graph with vertex set `V` and no edges.
* `SchoenfliesGraph.bouquet v E` is the graph with vertex set `{v}` and edge set `E`,
  where every edge is a loop at `v`.
* `SchoenfliesGraph.banana u v E` is the graph with vertex set `{u, v}` and edge set `E`,
...
A specific advantage is that this allows subgraphs of `G : SchoenfliesGraph α β` to also exist on
an equal footing with `G` as terms in `SchoenfliesGraph α β`,
and so there is no need for a `SchoenfliesGraph.subgraph` type and all the associated
...
refer to the `vertexSet` and `edgeSet` of `G : SchoenfliesGraph α β`.
...
-/
structure SchoenfliesGraph (α β : Type*) where
  /-- The vertex set. -/
  vertexSet : Set α
...
initialize_simps_projections SchoenfliesGraph (IsLink → isLink)

namespace SchoenfliesGraph

variable {G H : SchoenfliesGraph α β}

/-- `V(G)` denotes the `vertexSet` of a graph `G`. -/
scoped notation "V(" G ")" => SchoenfliesGraph.vertexSet G

/-- `E(G)` denotes the `edgeSet` of a graph `G`. -/
scoped notation "E(" G ")" => SchoenfliesGraph.edgeSet G
...
def Inc (G : SchoenfliesGraph α β) (e : β) (x : α) : Prop := ∃ y, G.IsLink e x y
...
lemma inc_eq_inc_iff_isLink_eq_isLink {G₁ G₂ : SchoenfliesGraph α β} :
...
def IsLoopAt (G : SchoenfliesGraph α β) (e : β) (x : α) : Prop := G.IsLink e x x
...
see `SchoenfliesGraph.isNonloopAt_iff_inc_not_isLoopAt`. -/
def IsNonloopAt (G : SchoenfliesGraph α β) (e : β) (x : α) : Prop := ∃ y ≠ x, G.IsLink e x y
...
def Adj (G : SchoenfliesGraph α β) (x y : α) : Prop := ∃ e, G.IsLink e x y
...
@[simp]
lemma mk_eq_self (G : SchoenfliesGraph α β) {E : Set β} (hE : ∀ e, e ∈ E ↔ ∃ x y, G.IsLink e x y) :
    SchoenfliesGraph.mk V(G) G.IsLink E
...
to the definition of `SchoenfliesGraph`, so it doesn't require equality of the edge sets.) -/
@[ext]
protected lemma ext {G₁ G₂ : SchoenfliesGraph α β} (hV : V(G₁) = V(G₂))
...
lemma ext_inc {G₁ G₂ : SchoenfliesGraph α β} (hV : V(G₁) = V(G₂)) (h : ∀ e x, G₁.Inc e x ↔ G₂.Inc e x) :
    G₁ = G₂ :=
  SchoenfliesGraph.ext hV fun _ _ _ ↦ by simp_rw [isLink_iff_inc, h]

/-- `SchoenfliesGraph.copy` produces a graph equal to `G` but with provided definitional choices
...
@[simps]
def copy (G : SchoenfliesGraph α β) {vertexSet : Set α} {edgeSet : Set β} {IsLink : β → α → α → Prop}
    (hvertexSet : V(G) = vertexSet) (hedgeSet : E(G) = edgeSet)
    (hIsLink : ∀ e x y, G.IsLink e x y ↔ IsLink e x y) : SchoenfliesGraph α β where
  vertexSet := vertexSet
  edgeSet := edgeSet
  IsLink := IsLink
...
@[simp]
lemma copy_eq (G : SchoenfliesGraph α β) {V : Set α} {E : Set β} {IsLink : β → α → α → Prop}
...
def Compatible (G H : SchoenfliesGraph α β) : Prop :=
...
lemma Compatible.refl (G : SchoenfliesGraph α β) : G.Compatible G :=
  fun _ _ _ _ _ => .rfl

@[simp]
lemma Compatible.rfl {G : SchoenfliesGraph α β} : G.Compatible G := .refl _

instance : Std.Refl (Compatible : SchoenfliesGraph α β → SchoenfliesGraph α β → Prop) where
  refl _ := .rfl
...
instance : Std.Symm (Compatible : SchoenfliesGraph α β → SchoenfliesGraph α β → Prop) where
...
@[simps (attr := grind =)]
def noEdge (vertexSet : Set α) (β : Type*) : SchoenfliesGraph α β where
  vertexSet := vertexSet
  edgeSet := ∅
  IsLink _ _ _ := False
  isLink_symm := by simp
...
lemma edgeSet_eq_empty : E(G) = ∅ ↔ G = noEdge V(G) β := by
  refine ⟨fun h ↦ SchoenfliesGraph.ext rfl ?_, fun h ↦ by rw [h, noEdge_edgeSet]⟩
...
@[simps (attr := grind =)]
def banana (u v : α) (edgeSet : Set β) : SchoenfliesGraph α β where
  vertexSet := {u, v}
  edgeSet := edgeSet
...
lemma banana_comm (u v : α) (edgeSet : Set β) : banana u v edgeSet = banana v u edgeSet :=
  SchoenfliesGraph.ext_inc (pair_comm ..) <| by simp [or_comm]
...
@[simp]
lemma banana_empty : banana u v ∅ = SchoenfliesGraph.noEdge {u, v} β := by
  ext <;> simp
...
abbrev bouquet (v : α) (edgeSet : Set β) : SchoenfliesGraph α β :=
  banana v v edgeSet
...
lemma eq_bouquet_of_subsingleton (hv : v ∈ V(G)) (hss : V(G).Subsingleton) :
  ...
  refine SchoenfliesGraph.ext_inc (by simpa) fun e x ↦ ⟨fun h ↦ ?_, fun h ↦ ?_⟩
