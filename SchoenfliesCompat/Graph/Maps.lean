...
This file defines vertex map between graphs `SchoenfliesGraph α β`. Morphisms between graphs will also be
...
variable {α α' α'' β : Type*} {G H : SchoenfliesGraph α β} {f g : α → α'} {u v : α} {e : β} {x y : α'}
...
/-- Map `G : SchoenfliesGraph α β` to a `SchoenfliesGraph α' β` with the same edge set by applying a function `f : α → α'`
  to each vertex. Edges between identified vertices become loops. -/
@[expose, simps! (attr := grind =)]
def map (f : α → α') (G : SchoenfliesGraph α β) : SchoenfliesGraph α' β where
  vertexSet := f '' V(G)
  edgeSet := E(G)
  IsLink e := Relation.Map (G.IsLink e) f f
  isLink_symm _ he := Relation.map_symmetric (G.isLink_symm he) f
...
@[simp]
lemma vertexSet_map (f : α → α') (G : SchoenfliesGraph α β) :
    V(G.map f) = f '' V(G) := map_vertexSet ..

@[simp]
lemma edgeSet_map (f : α → α') (G : SchoenfliesGraph α β) :
    E(G.map f) = E(G) := map_edgeSet ..
...
lemma map_eq_of_eqOn (h : EqOn f g V(G)) : G.map f = G.map g := by
  apply SchoenfliesGraph.ext
  · ext z
    constructor
    · rintro ⟨x, hx, rfl⟩
      exact ⟨x, hx, (h hx).symm⟩
    · rintro ⟨x, hx, rfl⟩
      exact ⟨x, hx, h hx⟩
  · intro e x y
    constructor
    · rintro ⟨u, v, huv, hu, hv⟩
      exact ⟨u, v, huv, (h huv.left_mem).symm.trans hu,
        (h huv.right_mem).symm.trans hv⟩
    · rintro ⟨u, v, huv, hu, hv⟩
      exact ⟨u, v, huv, (h huv.left_mem).trans hu,
        (h huv.right_mem).trans hv⟩
