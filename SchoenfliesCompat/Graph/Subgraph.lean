...
public import SchoenfliesCompat.Graph.Basic
public import Mathlib.Tactic.TFAE

/-!
# Subgraphs of multigraphs

This file develops the basic theory of subgraphs for multigraphs `SchoenfliesGraph α β`:
...
- `H ≤s G` (`SchoenfliesGraph.IsSpanningSubgraph`): `H` has the same vertex set as `G`.
- `H ≤i G` (`SchoenfliesGraph.IsInducedSubgraph`): `H` contains every ambient link between its vertices.
- `H ≤c G` (`SchoenfliesGraph.IsClosedSubgraph`): `H` is a union of components of `G`.

## Implementation notes

Following the overall design of `SchoenfliesGraph`, subgraphs are terms of the same type `SchoenfliesGraph α β`
rather than a separate `Subgraph` structure. This allows us to reuse notation and lemmas
uniformly and to express the subgraph order directly as a partial order on `SchoenfliesGraph α β`.
...
variable {α β : Type*} {x y z u v w : α} {e f : β} {G G₁ G₂ H H₁ H₂ K : SchoenfliesGraph α β} {F F₁ F₂ : Set β}
   {X Y : Set α}
...
`H ≤ G` instead. -/
@[mk_iff]
structure IsSubgraph (H G : SchoenfliesGraph α β) : Prop where
...
lemma IsSubgraph.antisymm (h₁ : H.IsSubgraph G) (h₂ : G.IsSubgraph H) : H = G :=
  SchoenfliesGraph.ext (h₁.1.antisymm h₂.1) fun _ _ _ ↦ ⟨(h₁.2 ·), (h₂.2 ·)⟩

/-- `H ≤ G` means `H` is a subgraph of `G`. It is defined as `V(H) ⊆ V(G)` and every link of `H`
being a link of `G`. -/
instance : PartialOrder (SchoenfliesGraph α β) where
  le := IsSubgraph
...
lemma IsSubgraph.inc_congr (hHG : H ≤ G) (he : e ∈ E(H)) : H.Inc e x ↔ G.Inc e x := by
  simp_rw [SchoenfliesGraph.Inc, hHG.isLink_iff he]
...
lemma IsSubgraph.isLoopAt_congr (hHG : H ≤ G) (he : e ∈ E(H)) :
  ...
  unfold SchoenfliesGraph.IsLoopAt
...
lemma IsSubgraph.isNonloopAt_congr (hHG : H ≤ G) (he : e ∈ E(H)) :
  ...
  simp_rw [SchoenfliesGraph.IsNonloopAt, hHG.isLink_iff he]
...
/-- `H ≤s G` (`SchoenfliesGraph.IsSpanningSubgraph`) is a subgraph of `G` with the same vertex set. -/
@[mk_iff]
structure IsSpanningSubgraph (H G : SchoenfliesGraph α β) : Prop extends H ≤ G where
  vertexSet_eq : V(H) = V(G)

@[inherit_doc IsSpanningSubgraph]
infixl:50 " ≤s " => SchoenfliesGraph.IsSpanningSubgraph
...
instance : IsPartialOrder (SchoenfliesGraph α β) (· ≤s ·) where
...
/-- `H ≤i G` (`SchoenfliesGraph.IsInducedSubgraph`) is a subgraph of `G` such that every link of `G`
involving two vertices of `H` is also a link of `H`. -/
@[mk_iff]
structure IsInducedSubgraph (H G : SchoenfliesGraph α β) : Prop extends H ≤ G where
  isLink_of_mem_mem : ∀ ⦃e x y⦄, G.IsLink e x y → x ∈ V(H) → y ∈ V(H) → H.IsLink e x y

@[inherit_doc IsInducedSubgraph]
scoped infixl:50 " ≤i " => SchoenfliesGraph.IsInducedSubgraph
...
instance : IsPartialOrder (SchoenfliesGraph α β) (· ≤i ·) where
...
/-- `H ≤c G` (`SchoenfliesGraph.IsClosedSubgraph`) is a union of components of `G`. -/
@[mk_iff]
structure IsClosedSubgraph (H G : SchoenfliesGraph α β) : Prop extends IsInducedSubgraph H G where
  closed : ∀ ⦃e x⦄, G.Inc e x → x ∈ V(H) → e ∈ E(H)

@[inherit_doc IsClosedSubgraph]
scoped infixl:50 " ≤c " => SchoenfliesGraph.IsClosedSubgraph
...
instance : IsPartialOrder (SchoenfliesGraph α β) (· ≤c ·) where
