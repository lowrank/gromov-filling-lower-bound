...
public import SchoenfliesCompat.Graph.Subgraph
...
variable {α β : Type*} {x y : α} {e : β} {G H : SchoenfliesGraph α β} {F F₀ : Set β} {X : Set α}

open Set Function

namespace SchoenfliesGraph

/-- Restrict `G : SchoenfliesGraph α β` to the edges in a set `E₀` without removing vertices -/
@[expose, simps (attr := grind =)]
def restrict (G : SchoenfliesGraph α β) (E₀ : Set β) : SchoenfliesGraph α β where
  vertexSet := V(G)
  edgeSet := E(G) ∩ E₀
...
@[simp]
lemma restrict_eq_self_iff (G : SchoenfliesGraph α β) (E₀ : Set β) : G.restrict E₀ = G ↔ E(G) ⊆ E₀ :=
  ⟨fun h ↦ by simpa using h.ge.edgeSet_mono,
    fun h ↦ (Compatible.of_le restrict_le).ext (by simp) (by simpa)⟩

@[simp]
lemma restrict_self (G : SchoenfliesGraph α β) : G.restrict E(G) = G :=
  (Compatible.of_le_le (G := G) (by simp) (by simp)).ext rfl (by simp)

@[simp]
lemma restrict_edgeSet_inter (G : SchoenfliesGraph α β) (F : Set β) : G.restrict (E(G) ∩ F) = G.restrict F :=
  (Compatible.of_le_le (G := G) (by simp) (by simp)).ext (by simp) (by simp)

@[simp]
lemma restrict_inter_edgeSet (G : SchoenfliesGraph α β) (F : Set β) :
...
@[gcongr]
lemma restrict_mono_right (G : SchoenfliesGraph α β) (hss : F₀ ⊆ F) : G.restrict F₀ ≤ G.restrict F where
...
@[expose, simps! (attr := grind =)]
def deleteEdges (G : SchoenfliesGraph α β) (F : Set β) : SchoenfliesGraph α β :=
...
@[simp]
lemma vertexSet_deleteEdges (G : SchoenfliesGraph α β) (F : Set β) :
    V(G.deleteEdges F) = V(G) := deleteEdges_vertexSet ..

@[simp]
lemma restrict_edgeSet_sdiff_eq_deleteEdges (G : SchoenfliesGraph α β) (F : Set β) :
...
lemma restrict_eq_deleteEdges (G : SchoenfliesGraph α β) (F : Set β) :
...
lemma deleteEdges_isLoopAt : (G.deleteEdges F).IsLoopAt e x ↔ G.IsLoopAt e x ∧ e ∉ F := by
  simp only [← restrict_edgeSet_sdiff_eq_deleteEdges, restrict_isLoopAt, mem_diff,
    and_congr_right_iff, and_iff_right_iff_imp]
  exact fun h _ ↦ h.edge_mem

@[simp]
lemma deleteEdges_deleteEdges (G : SchoenfliesGraph α β) (F₁ F₂ : Set β) :
    (G.deleteEdges F₁).deleteEdges F₂ = G.deleteEdges (F₁ ∪ F₂) := by
  simp only [← restrict_edgeSet_sdiff_eq_deleteEdges, diff_eq_compl_inter, restrict_inter_edgeSet,
    restrict_edgeSet, restrict_restrict, compl_union]
...
@[expose, simps! (attr := grind =) isLink]
protected def induce (G : SchoenfliesGraph α β) (X : Set α) : SchoenfliesGraph α β where
  vertexSet := X
  IsLink e x y := G.IsLink e x y ∧ x ∈ X ∧ y ∈ X
  isLink_symm _ _ _ _ h := ⟨h.1.symm, h.2.2, h.2.1⟩
...
lemma edgeSet_induce (G : SchoenfliesGraph α β) (X : Set α) :
    E(G.induce X) = {e | ∃ x y, G.IsLink e x y ∧ x ∈ X ∧ y ∈ X} := rfl

@[simp, grind =]
lemma induce_vertexSet (G : SchoenfliesGraph α β) : G.induce V(G) = G := by
...
def deleteVerts (G : SchoenfliesGraph α β) (X : Set α) : SchoenfliesGraph α β := G.induce (V(G) \ X)

@[simp, grind =]
lemma vertexSet_deleteVerts (G : SchoenfliesGraph α β) (X : Set α) : V(G.deleteVerts X) = V(G) \ X := by
  unfold deleteVerts
  rfl

@[simp, grind =]
lemma deleteVerts_isLink (G : SchoenfliesGraph α β) (X : Set α) :
...
@[simp]
lemma edgeSet_deleteVerts (G : SchoenfliesGraph α β) (X : Set α) :
    E(G.deleteVerts X) = {e | ∃ x y, G.IsLink e x y ∧ x ∉ X ∧ y ∉ X} := by
  simp [edgeSet_eq_setOf_exists_isLink]

@[simp, grind =]
lemma deleteVerts_empty (G : SchoenfliesGraph α β) : G.deleteVerts (∅ : Set α) = G := by
  simp [deleteVerts]
