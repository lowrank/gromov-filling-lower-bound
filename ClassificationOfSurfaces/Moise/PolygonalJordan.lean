...
import ClassificationOfSurfaces.Compat.CircleArc
...
noncomputable def outgoingVector (i : ZMod J.n) : Plane :=
  J.vertex (i + 1) - J.vertex i

/-- The ray from vertex `i` toward the preceding vertex. -/
noncomputable def incomingRayVector (i : ZMod J.n) : Plane :=
  J.vertex (i - 1) - J.vertex i
...
theorem crossingX_right (v w : Plane) (hne : v 1 ≠ w 1) : crossingX v w (w 1) = w 0 := by
  unfold crossingX
  have hden : w 1 - v 1 ≠ 0 := sub_ne_zero.mpr hne.symm
  field_simp [hden]
  ring
