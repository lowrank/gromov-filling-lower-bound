...
theorem faceCenter_ne_vertexPoint (t : K.Face) (v : K.UsedVertex) :
  ...
  change (K.faceCenter t).1 v.1 = (K.vertexPoint v).1 v.1 at hcoord
...
theorem faceCenter_injective : Function.Injective K.faceCenter := by
  ...
  · intro v hvt
    ...
    change (K.faceCenter t).1 v = (K.faceCenter u).1 v at hcoord
...
theorem faceCenter_ne_mem_edgeCarrier (t : K.Face) (e : K.Edge)
  ...
  have hsub : t.1 ⊆ e.1 := by
    ...
    change (K.faceCenter t).1 v = p.1 v at hcoord
...
theorem faceCenter_ne_mem_faceEdge (t : K.Face) (i : ZMod 3)
  ...
  change (K.faceCenter t).1 (K.faceVertex t (i + 2)) =
    p.1 (K.faceVertex t (i + 2)) at hcoord
  have hzero :
...
theorem fanCenterWeight_eq_zero_of_faceMap_eq_of_parent_ne
  ...
  · apply le_of_not_gt
    ...
    have hsubset : f.1.1 ⊆ g.1.1 := by
      ...
      change (M.fanFaceMap f x).1 (K.faceVertex f.1 i) =
        (M.fanFaceMap g y).1 (K.faceVertex f.1 i) at hcoord
...
theorem extendFaceCoordinates_eq_center_add_smul_normalizedBase
  ...
  have hfirst : d •
        (Pi.single (M.fanFirstVertex f).1
          (x (M.fanFirstVertex f) / d) : K.realization → ℝ) =
      (Pi.single (M.fanFirstVertex f).1
        (x (M.fanFirstVertex f)) : K.realization → ℝ) := by
    funext v
  ...
  have hsecond : d •
        (Pi.single (M.fanSecondVertex f).1
          (x (M.fanSecondVertex f) / d) : K.realization → ℝ) =
      (Pi.single (M.fanSecondVertex f).1
        (x (M.fanSecondVertex f)) : K.realization → ℝ) := by
    funext v
  ...
  change
    (Pi.single (M.fanCenterVertex f).1
          (x (M.fanCenterVertex f)) : K.realization → ℝ) +
        (Pi.single (M.fanFirstVertex f).1
          (x (M.fanFirstVertex f)) : K.realization → ℝ) +
      (Pi.single (M.fanSecondVertex f).1
          (x (M.fanSecondVertex f)) : K.realization → ℝ) =
      (Pi.single (M.fanCenterVertex f).1
          (x (M.fanCenterVertex f)) : K.realization → ℝ) +
        ((1 - x (M.fanCenterVertex f)) •
            (Pi.single (M.fanFirstVertex f).1
              (x (M.fanFirstVertex f) /
                (1 - x (M.fanCenterVertex f))) : K.realization → ℝ) +
          (1 - x (M.fanCenterVertex f)) •
            (Pi.single (M.fanSecondVertex f).1
              (x (M.fanSecondVertex f) /
                (1 - x (M.fanCenterVertex f))) : K.realization → ℝ))
