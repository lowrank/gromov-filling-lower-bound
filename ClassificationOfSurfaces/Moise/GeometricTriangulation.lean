...
* non-example: `ℝ` and `ℚ` have no geometric triangulation (they are not compact).
-/
  ...
  exact Relation.reflTransGen_closed
    ...
    (fun a b hab ↦ by
      ...
      exact hstar v a b (Finset.mem_inter.mp hv).1
        (Finset.mem_inter.mp hv).2) (hinter f g)
...
theorem faces_isDualConnected_of_isVertexStarConnected [ConnectedSpace S]
  ...
  letI : ConnectedSpace T.realization :=
    T.homeo.symm.surjective.connectedSpace T.homeo.symm.continuous
