    ...
    [[SignedDart.pos (Fin.last n).castSucc,
      ...
      [P1.expandWord (Fin.last n) [.pos (Fin.last n)],
        P1.expandWord (Fin.last n)
          (SignedDart.neg (Fin.last n) :: X.map P1.castSuccDart)]
            ...
            · let kW : Fin W.length :=
              ...
              have hsourceBoundary :
                ...
                simp [source, FiniteCyclicPresentation.boundary, kW,
                  finCongr_apply_mk, List.get_cons_succ]
              have htargetBoundary :
                  (target X W).boundary
                      (baseFaceEquiv X W k.succ) =
                    W.get kW := by
                have hindex :
                    baseFaceEquiv X W k.succ = kW.succ := by
                  apply Fin.ext
                  rfl
                change
                  (X :: W).get (baseFaceEquiv X W k.succ) = W.get kW
                rw [hindex]
