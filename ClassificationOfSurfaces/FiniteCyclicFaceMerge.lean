    ...
    · rw [hq]
      ...
      | zero =>
          ...
          have htargetFace :
              P2.oldFace (markedTarget U V W)
                  ...
                  ⟨(0 : (target U V W).Face).val, by
                    change 0 < (W.map P2.retainWord).length + 1
                    simp⟩ =
                P2.oldFace (markedTarget U V W)
                  (markedTargetCut U V W)
                  (markedTargetCut U V W).face.face := by
            apply Fin.ext
            rfl
          refine hsourceFace.symm ▸ ?_
          refine htargetFace.symm ▸ ?_
          rw [source_boundary_selected, P2.split_boundary_selected]
          simp only [targetCut, markedTargetCut,
            ...
            P2.freshEdge]
          have hexpand :=
            P1.expandWord_append (P1.freshEdge n)
              (P2.retainWord U) [.pos (P1.freshEdge n)]
          refine hexpand.symm ▸ ?_
