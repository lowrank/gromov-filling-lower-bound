...
theorem afterSecond_isRotated_thirdSource {n : ℕ} (a b c : Fin n)
  ...
  simp only [afterSecond, thirdSource, Crosscap.negativeTarget,
    Crosscap.negativeSource, Dyck.oneFace_boundary_zero]
  rw [inverseWord_append, Dyck.inverseWord_singleton,
    inverseWord_inverseWord]
  simp only [SignedDart.flip, List.cons_append, List.nil_append,
    List.append_assoc]
