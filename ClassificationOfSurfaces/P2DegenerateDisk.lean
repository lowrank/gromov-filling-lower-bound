  ...
  · have hzval :
    ...
    have hrle : r ≤ 1 := by
      ...
      have hprod :
        ...
        have hrpos : 0 < r := lt_trans (by norm_num) hrlt
        have hrsub : 0 < r - 1 := sub_pos.mpr hrlt
        have hqsub : 0 < 1 - q.val.re := sub_pos.mpr hqre
        exact mul_pos (mul_pos (mul_pos (by norm_num) hrpos) hrsub) hqsub
      linarith
    nlinarith [hrSq]
  · have hzval :
        z.val = 1 + (r : ℂ) * (1 - q.val) := by
    ...
    have hprod :
      ...
      have hqsub : 0 < 1 - q.val.re := sub_pos.mpr hqre
      exact mul_pos (mul_pos (mul_pos (by norm_num) hrpos) hrplus) hqsub
    linarith
