...
/-- A boundary obtained by a boundary-respecting continuous map from the free
upper boundary loop of a glued-strip point quotient inherits the odd-degree
obstruction as soon as the glued lower loop carries one. -/
theorem hasOddBoundaryDegreeObstruction_of_comp_continuous_cylinderStripGluedPointBoundary
    {m : ℕ} (P : CylinderStripLowerBoundaryPairing m)
    {Y : Type*} [TopologicalSpace Y]
    {boundary : UnitAddCircle → Y}
    (f : CylinderStripGluedPointSpace P → Y)
    (hf : Continuous f)
...
boundary-degree obstruction. -/
theorem hasOddBoundaryDegreeObstruction_of_comp_continuous_adjacentPreservingCylinderStripGluedPointBoundary
    {m : ℕ} (hm : Odd m)
    {X : Type*} [TopologicalSpace X]
    {boundary : UnitAddCircle → X}
...
inductive CylinderStripLowerBoundaryEdgeStep {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :
    Fin (m + 1) → Fin (m + 1) → Prop
  | pair (j : Fin (m + 1)) : CylinderStripLowerBoundaryEdgeStep P j (P.edgePair j)

/-- Generated relation on lower boundary vertices coming from the endpoint
identifications induced by the lower-edge pairing. -/
inductive CylinderStripLowerBoundaryVertexStep {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :
    Fin (m + 1) → Fin (m + 1) → Prop
  | start (j : Fin (m + 1)) : CylinderStripLowerBoundaryVertexStep P j (P.pairedStart j)
  | finish (j : Fin (m + 1)) :
      CylinderStripLowerBoundaryVertexStep P (cyclicSucc j) (P.pairedFinish j)

/-- Equivalence relation on lower boundary edges generated by the pairing. -/
def cylinderStripLowerBoundaryEdgeSetoid {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) : Setoid (Fin (m + 1)) :=
  Relation.EqvGen.setoid (CylinderStripLowerBoundaryEdgeStep P)

/-- Equivalence relation on lower boundary vertices generated by the endpoint
identifications of the pairing. -/
def cylinderStripLowerBoundaryVertexSetoid {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) : Setoid (Fin (m + 1)) :=
  Relation.EqvGen.setoid (CylinderStripLowerBoundaryVertexStep P)

/-- Quotient type of paired lower boundary edges. -/
abbrev CylinderStripLowerBoundaryEdgeClass {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :=
  Quotient (cylinderStripLowerBoundaryEdgeSetoid P)

/-- Quotient type of lower boundary vertices after imposing the endpoint
identifications induced by the pairing. -/
abbrev CylinderStripLowerBoundaryVertexClass {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :=
  Quotient (cylinderStripLowerBoundaryVertexSetoid P)

noncomputable instance {m : ℕ} (P : CylinderStripLowerBoundaryPairing m) :
    Fintype (CylinderStripLowerBoundaryEdgeClass P) := by
  let _ : DecidableRel (cylinderStripLowerBoundaryEdgeSetoid P).r := Classical.decRel _
  infer_instance

noncomputable instance {m : ℕ} (P : CylinderStripLowerBoundaryPairing m) :
    Fintype (CylinderStripLowerBoundaryVertexClass P) := by
  let _ : DecidableRel (cylinderStripLowerBoundaryVertexSetoid P).r := Classical.decRel _
  infer_instance

/-- Generated relation on all strip edges: only lower angular edges are paired,
all other edges remain as they are. -/
def cylinderStripEdgeGluingSetoid {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) : Setoid (CylinderStripEdge n m) :=
  Relation.EqvGen.setoid fun e₁ e₂ ↦
    ∃ j : Fin (m + 1),
      e₁ = CylinderStripEdge.angular (0 : Fin (n + 2)) j ∧
      e₂ = CylinderStripEdge.angular (0 : Fin (n + 2)) (P.edgePair j)

/-- Generated relation on all strip vertices: only lower-boundary vertices are
identified, according to the endpoint data induced by the pairing. -/
def cylinderStripVertexGluingSetoid {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) : Setoid (CylinderStripVertex n m) :=
  Relation.EqvGen.setoid fun v w ↦
    (∃ j : Fin (m + 1),
      v = ((0 : Fin (n + 2)), j) ∧
...
/-- Quotient edge type for future strip gluings. -/
abbrev CylinderStripGluedEdge (n : ℕ) {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :=
  Quotient (cylinderStripEdgeGluingSetoid (n := n) P)

/-- Quotient vertex type for future strip gluings. -/
abbrev CylinderStripGluedVertex (n : ℕ) {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :=
  Quotient (cylinderStripVertexGluingSetoid (n := n) P)

noncomputable instance {n m : ℕ} (P : CylinderStripLowerBoundaryPairing m) :
    Fintype (CylinderStripGluedEdge n P) := by
  let _ : DecidableRel (cylinderStripEdgeGluingSetoid (n := n) P).r := Classical.decRel _
  infer_instance
...
@[simp] theorem mk_cylinderStripLowerBoundaryEdgeClass_pair {m : ℕ}
    ...
    Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) j =
      Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) (P.edgePair j) := by
  apply Quotient.sound
...
@[simp] theorem mk_cylinderStripLowerBoundaryVertexClass_start {m : ℕ}
    ...
    Quotient.mk (cylinderStripLowerBoundaryVertexSetoid P) j =
      Quotient.mk (cylinderStripLowerBoundaryVertexSetoid P) (P.pairedStart j) := by
  apply Quotient.sound
  exact Relation.EqvGen.rel _ _ (CylinderStripLowerBoundaryVertexStep.start j)

@[simp] theorem mk_cylinderStripLowerBoundaryVertexClass_finish {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (j : Fin (m + 1)) :
    Quotient.mk (cylinderStripLowerBoundaryVertexSetoid P) (cyclicSucc j) =
      Quotient.mk (cylinderStripLowerBoundaryVertexSetoid P) (P.pairedFinish j) := by
  apply Quotient.sound
...
def cylinderStripLowerBoundaryEdgeTag {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :
    CylinderStripEdge n m → Option (CylinderStripLowerBoundaryEdgeClass P)
  | .radial _ _ => none
  | .angular i j =>
...
theorem cylinderStripEdgeGluingStep_preserves_lowerBoundaryEdgeTag {n m : ℕ}
    ...
    (h : ∃ j : Fin (m + 1),
      e₁ = CylinderStripEdge.angular (0 : Fin (n + 2)) j ∧
      e₂ = CylinderStripEdge.angular (0 : Fin (n + 2)) (P.edgePair j)) :
    cylinderStripLowerBoundaryEdgeTag P e₁ = cylinderStripLowerBoundaryEdgeTag P e₂ := by
  rcases h with ⟨j, rfl, rfl⟩
  change some (Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) j) =
    some (Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) (P.edgePair j))
  rw [mk_cylinderStripLowerBoundaryEdgeClass_pair]

theorem eqvGen_cylinderStripEdgeGluing_preserves_lowerBoundaryEdgeTag {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m)
    {e₁ e₂ : CylinderStripEdge n m}
    (h : Relation.EqvGen
  ...
  induction h with
  | rel _ _ hstep =>
      exact cylinderStripEdgeGluingStep_preserves_lowerBoundaryEdgeTag P hstep
  | refl _ => rfl
  | symm _ _ _ ih => exact ih.symm
  | trans _ _ _ _ _ ih₁ ih₂ => exact ih₁.trans ih₂

def cylinderStripGluedLowerBoundaryEdgeTag (n : ℕ) {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :
    CylinderStripGluedEdge n P → Option (CylinderStripLowerBoundaryEdgeClass P) :=
  Quotient.lift
    (cylinderStripLowerBoundaryEdgeTag P)
    (by
      intro a b hab
...
@[simp] theorem mk_cylinderStripGluedLowerEdge_eq_iff {n m : ℕ}
  ...
  constructor
  · intro h
    have hsome := congrArg (cylinderStripGluedLowerBoundaryEdgeTag n P) h
    change some (Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) j) =
...
@[simp] theorem cylinderStripGluedLowerBoundaryEdgeTag_mk_lower {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (j : Fin (m + 1)) :
    cylinderStripGluedLowerBoundaryEdgeTag n P
      (Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
        (CylinderStripEdge.angular (0 : Fin (n + 2)) j)) =
      some (Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) j) :=
  rfl

@[simp] theorem cylinderStripGluedLowerBoundaryEdgeTag_mk_radial {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (i : Fin (n + 1)) (j : Fin (m + 1)) :
    cylinderStripGluedLowerBoundaryEdgeTag n P
      (Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
        (CylinderStripEdge.radial i j)) = none :=
  rfl

@[simp] theorem cylinderStripGluedLowerBoundaryEdgeTag_mk_angular_ne_zero {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (i : Fin (n + 2)) (j : Fin (m + 1))
    (hi0 : i ≠ 0) :
    cylinderStripGluedLowerBoundaryEdgeTag n P
      (Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
...
theorem fin_last_succ_ne_zero {n : ℕ} :
...
def cylinderStripGluedBoundaryEdges (n : ℕ) {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :
    Finset (CylinderStripGluedEdge n P) :=
  Finset.univ.image (cylinderStripGluedBoundaryEdge n P)

theorem cylinderStripGluedBoundaryEdges_eq (n : ℕ) {m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) :
    cylinderStripGluedBoundaryEdges n P =
      Finset.univ.image (cylinderStripGluedBoundaryEdge n P) := rfl

theorem mk_cylinderStripGluedEdge_mem_faceEdges_iff {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m)
  ...
  constructor
  · intro h
    rw [cylinderStripGluedFaceEdges_eq] at h
    rcases Finset.mem_image.mp h with ⟨k, _hk, hk⟩
    refine ⟨cylinderStripFaceEdge f k, ?_, ?_⟩
    · rw [cylinderStripFaceEdges_eq]
      exact Finset.mem_image.mpr ⟨k, Finset.mem_univ _, rfl⟩
    · simpa [cylinderStripGluedFaceEdge] using hk
  · rintro ⟨e', he', heq⟩
    rw [cylinderStripFaceEdges_eq] at he'
    rcases Finset.mem_image.mp he' with ⟨k, _hk, hk⟩
    subst e'
    rw [cylinderStripGluedFaceEdges_eq]
    exact Finset.mem_image.mpr ⟨k, Finset.mem_univ _, by
      simpa [cylinderStripGluedFaceEdge] using heq⟩

@[simp] theorem mem_cylinderStripGluedBoundaryEdges_mk_radial_iff {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (i : Fin (n + 1)) (j : Fin (m + 1)) :
    Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
        (CylinderStripEdge.radial i j) ∈
      cylinderStripGluedBoundaryEdges n P ↔ False := by
  constructor
  · intro h
    rw [cylinderStripGluedBoundaryEdges_eq] at h
    rcases Finset.mem_image.mp h with ⟨k, _hk, hk⟩
    have hEq : cylinderStripBoundaryEdge (n := n) (m := m) k =
        CylinderStripEdge.radial i j := by
      exact (mk_cylinderStripGluedEdge_eq_radial_iff P
        (cylinderStripBoundaryEdge (n := n) (m := m) k) i j).1 <| by
          simpa [cylinderStripGluedBoundaryEdge] using hk
    simpa [cylinderStripBoundaryEdge] using hEq
  · intro h
    exact False.elim h

@[simp] theorem mem_cylinderStripGluedBoundaryEdges_mk_lower_iff {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (j : Fin (m + 1)) :
    Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
        (CylinderStripEdge.angular (0 : Fin (n + 2)) j) ∈
      cylinderStripGluedBoundaryEdges n P ↔ False := by
  ...
  · intro h
    rw [cylinderStripGluedBoundaryEdges_eq] at h
    rcases Finset.mem_image.mp h with ⟨k, _hk, hk⟩
    have htag := congrArg (cylinderStripGluedLowerBoundaryEdgeTag n P) hk
    rw [cylinderStripGluedBoundaryEdge, cylinderStripBoundaryEdge,
      cylinderStripGluedLowerBoundaryEdgeTag_mk_angular_ne_zero P
        (Fin.last (n + 1)) k fin_last_succ_ne_zero,
      cylinderStripGluedLowerBoundaryEdgeTag_mk_lower] at htag
    cases htag
  · intro h
    exact False.elim h

@[simp] theorem mem_cylinderStripGluedBoundaryEdges_mk_angular_ne_zero_iff {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (i : Fin (n + 2)) (j : Fin (m + 1))
    (hi0 : i ≠ 0) :
    Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
        (CylinderStripEdge.angular i j) ∈
      cylinderStripGluedBoundaryEdges n P ↔ i = Fin.last (n + 1) := by
  constructor
  · intro h
    rw [cylinderStripGluedBoundaryEdges_eq] at h
    rcases Finset.mem_image.mp h with ⟨k, _hk, hk⟩
    have hEq : cylinderStripBoundaryEdge (n := n) (m := m) k =
        CylinderStripEdge.angular i j := by
      exact (mk_cylinderStripGluedEdge_eq_angular_of_ne_zero_iff P
        (cylinderStripBoundaryEdge (n := n) (m := m) k) i j hi0).1 <| by
          simpa [cylinderStripGluedBoundaryEdge] using hk
    have hi : i = Fin.last (n + 1) := by
      simpa [cylinderStripBoundaryEdge] using
        congrArg (fun e => match e with
          | .radial _ _ => (0 : Fin (n + 2))
          | .angular i _ => i) hEq.symm
    exact hi
  · intro hi
    subst i
    rw [cylinderStripGluedBoundaryEdges_eq]
    exact Finset.mem_image.mpr ⟨j, Finset.mem_univ _, by
      simp [cylinderStripGluedBoundaryEdge, cylinderStripBoundaryEdge]⟩
...
theorem exists_mem_cylinderStripFaceEdges_eq_gluedLowerEdge_iff {n m : ℕ}
    ...
    (∃ e ∈ cylinderStripFaceEdges (i, k),
      Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e =
        Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
          (CylinderStripEdge.angular (0 : Fin (n + 2)) j)) ↔
      i = 0 ∧ (k = j ∨ k = P.edgePair j) := by
  constructor
  · rintro ⟨e, he, hq⟩
    have htag := congrArg (cylinderStripGluedLowerBoundaryEdgeTag n P) hq
    cases e with
    | radial i' l =>
        rw [cylinderStripGluedLowerBoundaryEdgeTag_mk_radial,
          cylinderStripGluedLowerBoundaryEdgeTag_mk_lower] at htag
        cases htag
    | angular i' l =>
        by_cases hi0 : i' = 0
        · subst hi0
          rw [cylinderStripGluedLowerBoundaryEdgeTag_mk_lower,
            cylinderStripGluedLowerBoundaryEdgeTag_mk_lower] at htag
          have hclass : Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) l =
              Quotient.mk (cylinderStripLowerBoundaryEdgeSetoid P) j := Option.some.inj htag
          have hlj : l = j ∨ l = P.edgePair j :=
            (mk_cylinderStripLowerBoundaryEdgeClass_eq_iff P j l).1 hclass.symm
          have hemem : CylinderStripEdge.angular (0 : Fin (n + 2)) l ∈ cylinderStripFaceEdges (i, k) := by
            simpa using he
          rcases (mem_cylinderStripFaceEdges_angular_iff (0 : Fin (n + 2)) i l k).1 hemem with ⟨hk, hlevel⟩
          subst k
          rcases hlevel with hl | hu
          · have hi : i = 0 := by
        ...
        · rw [cylinderStripGluedLowerBoundaryEdgeTag_mk_angular_ne_zero P i' l hi0,
            cylinderStripGluedLowerBoundaryEdgeTag_mk_lower] at htag
          cases htag
  · rintro ⟨hi, hk⟩
    subst i
    rcases hk with rfl | hk
    · refine ⟨CylinderStripEdge.angular (0 : Fin (n + 2)) k, ?_, rfl⟩
      exact (mem_cylinderStripFaceEdges_angular_iff (0 : Fin (n + 2)) (0 : Fin (n + 1)) k k).2
        ⟨rfl, Or.inl rfl⟩
    · subst k
      refine ⟨CylinderStripEdge.angular (0 : Fin (n + 2)) (P.edgePair j), ?_, (mk_cylinderStripGluedEdge_pair P j).symm⟩
    ...
    by
      ...
      exact (Finset.univ.filter fun f : CylinderStripFace n m ↦
        ∃ e ∈ cylinderStripFaceEdges f,
          Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e =
            Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
              (CylinderStripEdge.angular (0 : Fin (n + 2)) j)).card = 2 := by
  classical
  have hfilter :
      (Finset.univ.filter fun f : CylinderStripFace n m ↦
        ∃ e ∈ cylinderStripFaceEdges f,
          Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e =
            Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
              (CylinderStripEdge.angular (0 : Fin (n + 2)) j)) =
        ({((0 : Fin (n + 1)), j), ((0 : Fin (n + 1)), P.edgePair j)} : Finset (CylinderStripFace n m)) := by
    ext f
    rcases f with ⟨i, k⟩
    simp only [Finset.mem_filter, Finset.mem_univ, true_and, Finset.mem_insert, Finset.mem_singleton]
    constructor
    · intro hf
      rcases (exists_mem_cylinderStripFaceEdges_eq_gluedLowerEdge_iff P i k j).1 hf with ⟨hi, hk⟩
      subst i
      rcases hk with rfl | hk
      · exact Or.inl rfl
    ...
    · intro hf
      ...
      · cases hf
        exact (exists_mem_cylinderStripFaceEdges_eq_gluedLowerEdge_iff P 0 j j).2 ⟨rfl, Or.inl rfl⟩
      · cases hf
        exact (exists_mem_cylinderStripFaceEdges_eq_gluedLowerEdge_iff P 0 (P.edgePair j) j).2 ⟨rfl, Or.inr rfl⟩
  rw [hfilter]
  apply Finset.card_pair
  intro h
  have hj : j = P.edgePair j := by
    simpa using congrArg Prod.snd h
  exact P.self_ne_edgePair j hj
...
theorem cylinderStripGluedEdge_faceCount_of_pos_rep {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (hm : 0 < m)
    (e : CylinderStripEdge n m) :
    (Finset.univ.filter fun f : CylinderStripFace n m ↦
      Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e ∈
        cylinderStripGluedFaceEdges n P f).card =
      if Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e ∈
          cylinderStripGluedBoundaryEdges n P then 1 else 2 := by
  classical
  have hfilter :
      (Finset.univ.filter fun f : CylinderStripFace n m ↦
        Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e ∈
      ...
      (Finset.univ.filter fun f : CylinderStripFace n m ↦
        ∃ e' ∈ cylinderStripFaceEdges f,
          Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e' =
            Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e) := by
    ext f
    simp [mk_cylinderStripGluedEdge_mem_faceEdges_iff]
  rw [hfilter]
  cases e with
  | radial i j =>
      rw [cylinderStripGluedRadial_faceCount_of_pos P hm i j, if_neg]
      simpa using (mem_cylinderStripGluedBoundaryEdges_mk_radial_iff P i j)
  | angular i j =>
      by_cases hi0 : i = 0
      · subst hi0
        have hb : Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
              (CylinderStripEdge.angular (0 : Fin (n + 2)) j) ∉
              cylinderStripGluedBoundaryEdges n P := by
          intro hmem
          exact (mem_cylinderStripGluedBoundaryEdges_mk_lower_iff P j).1 hmem
        rw [cylinderStripGluedLowerEdge_faceCount P j, if_neg hb]
      · rw [cylinderStripGluedAngular_faceCount_of_ne_zero P i j hi0]
        by_cases hiLast : i = Fin.last (n + 1)
        · have hb : Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
              (CylinderStripEdge.angular i j) ∈ cylinderStripGluedBoundaryEdges n P :=
            (mem_cylinderStripGluedBoundaryEdges_mk_angular_ne_zero_iff P i j hi0).2 hiLast
          rw [if_pos hiLast, if_pos hb]
        · have hb : Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P)
              (CylinderStripEdge.angular i j) ∉ cylinderStripGluedBoundaryEdges n P := by
            intro hmem
            exact hiLast ((mem_cylinderStripGluedBoundaryEdges_mk_angular_ne_zero_iff P i j hi0).1 hmem)
          rw [if_neg hiLast, if_neg hb]

theorem cylinderStripGluedEdge_faceCount_of_pos {n m : ℕ}
    (P : CylinderStripLowerBoundaryPairing m) (hm : 0 < m)
    (e : CylinderStripGluedEdge n P) :
    (Finset.univ.filter fun f : CylinderStripFace n m ↦
      e ∈ cylinderStripGluedFaceEdges n P f).card =
      if e ∈ cylinderStripGluedBoundaryEdges n P then 1 else 2 := by
  classical
  let e' : CylinderStripEdge n m := e.out
  have hout : Quotient.mk (cylinderStripEdgeGluingSetoid (n := n) P) e' = e :=
    Quotient.out_eq e
...
structure CylinderStripGluedMeshData
    {X : Type*} [PseudoMetricSpace X]
    (boundary : UnitAddCircle → X) (ε : ℝ) where
  n : ℕ
  m : ℕ
  pairing : CylinderStripLowerBoundaryPairing m
  hm : 0 < m
  vertexPoint : CylinderStripGluedVertex n pairing → X
  edgeToX : CylinderStripGluedEdge n pairing → ClosedUnitInterval → X
  hedgeToX : ∀ e, Continuous (edgeToX e)
  hedgeStart : ∀ e, edgeToX e closedUnitIntervalStart =
    vertexPoint (cylinderStripGluedEdgeChosenEnds n pairing e).1
  hedgeFinish : ∀ e, edgeToX e closedUnitIntervalFinish =
    vertexPoint (cylinderStripGluedEdgeChosenEnds n pairing e).2
  faceCenter : CylinderStripFace n m → X
  mesh : ∀ (f : CylinderStripFace n m) (e : CylinderStripGluedEdge n pairing),
    e ∈ cylinderStripGluedFaceEdges n pairing f → ∀ x : ClosedUnitInterval,
      dist (faceCenter f) (edgeToX e x) < ε
  boundaryPath : ∀ k x,
    edgeToX (cylinderStripGluedBoundaryEdge n pairing k) x =
      boundary ((angularSubdivisionParameter m k x : ℝ) : UnitAddCircle)

/-- The explicit quotient glued-strip mesh data needed downstream, bundled at
one mesh scale. -/
def CylinderStripGluedMeshData.toAbstractVariableDirectedQuotientGeometricPolygonalModel
    {X : Type*} [PseudoMetricSpace X]
    {boundary : UnitAddCircle → X} {ε : ℝ}
    (D : CylinderStripGluedMeshData boundary ε) :
    AbstractVariableDirectedQuotientGeometricPolygonalModel boundary ε :=
  cylinderStripGluedAbstractVariableDirectedQuotientGeometricModel_of_mesh
...
mesh-data interface. -/
theorem hasCylinderStripGluedArbitrarilyFineData_of_cylinderStripGlued_data
    {X : Type*} [PseudoMetricSpace X] {boundary : UnitAddCircle → X}
  ...
  exact ⟨{
    ...
    edgeToX := edgeToX
    hedgeToX := hedgeToX
    hedgeStart := hedgeStart
    hedgeFinish := hedgeFinish
    faceCenter := faceCenter
    mesh := hmesh
    boundaryPath := hboundaryPath
  }⟩

/-- The bundled glued-strip mesh-data interface already implies the
quotient-friendly directed abstract arbitrarily-fine polygonal-model
interface. -/
theorem hasAbstractVariableDirectedQuotientArbitrarilyFinePolygonalModels_of_cylinderStripGluedArbitrarilyFineData
    {X : Type*} [PseudoMetricSpace X] {boundary : UnitAddCircle → X}
    (hmodels : HasCylinderStripGluedArbitrarilyFineData boundary) :
    HasAbstractVariableDirectedQuotientArbitrarilyFinePolygonalModels boundary := by
  intro ε hε
...
/-- Consequently bundled explicit glued-strip quotient mesh data at every
positive scale already yields the odd boundary-degree obstruction on the
remaining free boundary. -/
theorem hasOddBoundaryDegreeObstruction_of_cylinderStripGluedArbitrarilyFineData
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X}
    (hmodels : HasCylinderStripGluedArbitrarilyFineData boundary) :
    HasOddBoundaryDegreeObstruction boundary :=
