...
/-- A compact source with an arbitrary open chart cover and an odd
boundary-degree obstruction already yields the `N`-mode orientation-free lower
bound once the summed Jacobian budget is stated on the extracted finite
subcover. -/
theorem FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal_ofCompactOpenCover
    {N : ℕ} {X : Type*} [TopologicalSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : Type*}
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    (rowMap : Fin N → X → ℂ)
    ...
    (hbudget :
      (∑ j : Fin N,
        (((FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
          hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece sourcePiece_open
          sourcePiece_cover targetPiece chart targetPiece_open localMap K
          local_lipschitz local_agree).toFiniteComplexSourceChartCertificateData).toFiniteComplexLocalCertificateData).jacobianMass j) ≤ area) :
    ENNReal.ofReal (finiteUniversalConstant N) ≤ area := by
  let D :=
    FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece sourcePiece_open
      sourcePiece_cover targetPiece chart targetPiece_open localMap K
      local_lipschitz local_agree
  simpa [D] using
    (FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal
      D area hbudget)

/-- A compact source with an arbitrary open chart cover and an odd
boundary-degree obstruction already yields the rowwise bounded witness region
once the finite subcover has been extracted. -/
theorem FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.exists_bounded_region_volume_le_jacobianMass_ofCompactOpenCover
    {N : ℕ} {X : Type*} [TopologicalSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : Type*}
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    (rowMap : Fin N → X → ℂ)
    ...
    ∃ region : Set ℂ,
      ...
      volume region ≤
        (((FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
          hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece sourcePiece_open
          sourcePiece_cover targetPiece chart targetPiece_open localMap K
          local_lipschitz local_agree).toFiniteComplexSourceChartCertificateData).toFiniteComplexLocalCertificateData).jacobianMass j := by
  let D :=
    FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece sourcePiece_open
      sourcePiece_cover targetPiece chart targetPiece_open localMap K
      local_lipschitz local_agree
  simpa [D] using
    (FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.exists_bounded_region_volume_le_jacobianMass
      D j)

/-- A compact source with an arbitrary open chart cover and an odd
boundary-degree obstruction also carries any common additive defect term through
that extracted finite subcover. -/
theorem FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal_add_ofCompactOpenCover
    {N : ℕ} {X : Type*} [TopologicalSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : Type*}
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    (rowMap : Fin N → X → ℂ)
    ...
    (hbudget :
      (∑ j : Fin N,
        (((FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
          hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece sourcePiece_open
          sourcePiece_cover targetPiece chart targetPiece_open localMap K
          local_lipschitz local_agree).toFiniteComplexSourceChartCertificateData).toFiniteComplexLocalCertificateData).jacobianMass j) + defect ≤ area) :
    ENNReal.ofReal (finiteUniversalConstant N) + defect ≤ area := by
  let D :=
    FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece sourcePiece_open
      ...
      local_lipschitz local_agree
  simpa [D] using
    (FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal_add
      D area defect hbudget)
...
/-- A compact-source family of open chart covers with an odd boundary-degree
obstruction already yields the full orientation-free universal bound once the
summed Jacobian budgets are stated on the extracted finite subcovers. -/
theorem ComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal_ofCompactOpenCover
    {X : Type*} [TopologicalSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : ℕ → Type*}
    (area : ℝ≥0∞)
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    ...
    ENNReal.ofReal universalConstant ≤ area := by
  let S :=
    ComplexSourceChartSystemOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      area hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece
      sourcePiece_open sourcePiece_cover targetPiece chart targetPiece_open
      localMap K local_lipschitz local_agree budget
  simpa [S] using
    (ComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal S)

/-- The same compact-source family of open chart covers also carries any
common additive defect term unchanged through the extracted finite subcovers. -/
theorem ComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal_add_ofCompactOpenCover
    {X : Type*} [TopologicalSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : ℕ → Type*}
    (area defect : ℝ≥0∞)
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    ...
    (hbudget : ∀ N : ℕ,
      (∑ j : Fin N,
        (((FiniteComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
          ...
          (K N)
          (fun j i ↦ local_lipschitz N j i)
          (fun j i x ↦ local_agree N j i x)).toFiniteComplexSourceChartCertificateData).toFiniteComplexLocalCertificateData).jacobianMass j) + defect ≤ area) :
    ENNReal.ofReal universalConstant + defect ≤ area := by
  let S :=
    ComplexSourceChartSystemOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      area hobstruction rowMap rowMap_cont rowMap_boundary sourcePiece
      ...
      localMap K local_lipschitz local_agree budget
  simpa [S] using
    (ComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal_add
      S hbudget)
...
/-- The same generic source-chart certificate already transfers from any
...
theorem ComplexSourceChartSystem.universal_ennreal_of_comp_continuous_adjacentPreservingCylinderStripGluedPointBoundary
    {m : ℕ} (hm : Odd m)
    {Y : Type*} [TopologicalSpace Y]
    {boundary : UnitAddCircle → Y} {area : ℝ≥0∞}
    (S : ComplexSourceChartSystem Y boundary area)
...
/-- The same rowwise bounded-region certificate also transfers from any
boundary obtained by a boundary-respecting continuous map from the free upper
loop of the odd adjacent-preserving glued-strip quotient. -/
theorem ComplexSourceChartSystem.exists_bounded_region_volume_le_jacobianMass_of_comp_continuous_adjacentPreservingCylinderStripGluedPointBoundary
    {m : ℕ} (hm : Odd m)
    {Y : Type*} [TopologicalSpace Y]
    {boundary : UnitAddCircle → Y} {area : ℝ≥0∞}
    (S : ComplexSourceChartSystem Y boundary area)
...
theorem ComplexSourceChartSystem.exists_bounded_region_volume_le_jacobianMass_of_homeomorphCylinderStripGluedPointBoundaryObstructionData
    {Y : Type*} [TopologicalSpace Y]
    {boundary : UnitAddCircle → Y} {area : ℝ≥0∞}
    (S : ComplexSourceChartSystem Y boundary area)
    (D : HomeomorphCylinderStripGluedPointBoundaryObstructionData boundary)
    ...
    ∃ region : Set ℂ,
      ...
      Bornology.IsBounded region ∧ 0 ∈ region ∧
      volume region ≤ (S.data N).jacobianMass j :=
  S.exists_bounded_region_volume_le_jacobianMass_of_homeomorph_cylinderStripGluedPointBoundary
    D.pairing D.homeomorph D.boundaryHomeomorph D.lowerObstruction N j
...
/-- The same disk-homeomorphic route also feeds the generic source-chart
certificate through the bundled explicit glued-strip quotient disk interface. -/
theorem ComplexSourceChartSystem.universal_ennreal_of_closedUnitDisk_homeomorph_cylinderStripGlued
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {area : ℝ≥0∞}
    (S : ComplexSourceChartSystem X boundary area)
    (e : ClosedUnitDisk ≃ₜ X)
    ...
    ENNReal.ofReal universalConstant ≤ area :=
  S.universal_ennreal_of_odd_boundary_degree_obstruction
    (hasOddBoundaryDegreeObstruction_of_closedUnitDisk_homeomorph_cylinderStripGlued
      e hboundaryMap)
...
/-- The additive-defect generic source-chart certificate also transfers
from any boundary obtained by a boundary-respecting continuous map from the
free upper loop of the odd adjacent-preserving glued-strip quotient. -/
theorem ComplexSourceChartSystem.universal_ennreal_add_of_comp_continuous_adjacentPreservingCylinderStripGluedPointBoundary
    {m : ℕ} (hm : Odd m)
    {Y : Type*} [TopologicalSpace Y]
    {boundary : UnitAddCircle → Y} {area defect : ℝ≥0∞}
    (S : ComplexSourceChartSystem Y boundary area)
...
theorem ComplexSourceChartSystem.universal_ennreal_add_of_homeomorphCylinderStripGluedPointBoundaryObstructionData
    {Y : Type*} [TopologicalSpace Y]
    {boundary : UnitAddCircle → Y} {area defect : ℝ≥0∞}
    (S : ComplexSourceChartSystem Y boundary area)
    (D : HomeomorphCylinderStripGluedPointBoundaryObstructionData boundary)
  ...
  S.universal_ennreal_add_of_homeomorph_cylinderStripGluedPointBoundary
    D.pairing D.homeomorph D.boundaryHomeomorph D.lowerObstruction hbudget
...
/-- The same disk-homeomorphic route also supplies the additive-defect
generic source-chart certificate through the bundled explicit glued-strip
quotient disk interface. -/
theorem ComplexSourceChartSystem.universal_ennreal_add_of_closedUnitDisk_homeomorph_cylinderStripGlued
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {area defect : ℝ≥0∞}
    (S : ComplexSourceChartSystem X boundary area)
    (e : ClosedUnitDisk ≃ₜ X)
...
/-- A compact source with an arbitrary open chart cover and an odd
boundary-degree obstruction already yields the `N`-mode metric source-chart
lower bound once the summed Jacobian budget is stated on the extracted finite
subcover. -/
theorem FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal_ofCompactOpenCover
    {N : ℕ} {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : Type*}
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    (sourcePiece : ι → Set X)
    ...
    (hbudget :
      (∑ j : Fin N,
        (FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
          hobstruction sourcePiece sourcePiece_open sourcePiece_cover targetPiece
          chart targetPiece_open localMap K local_lipschitz local_agree).jacobianMass
            hboundary j) ≤ area) :
    ENNReal.ofReal (finiteUniversalConstant N) ≤ area := by
  let D :=
    FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      hobstruction sourcePiece sourcePiece_open sourcePiece_cover targetPiece
      chart targetPiece_open localMap K local_lipschitz local_agree
  simpa [D] using
    (FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal
      D hboundary area hbudget)

/-- A compact source with an arbitrary open chart cover and an odd
boundary-degree obstruction already yields the rowwise bounded witness region
for the genuine metric Fourier maps once the finite subcover has been
extracted. -/
theorem FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.exists_bounded_region_volume_le_jacobianMass_ofCompactOpenCover
    {N : ℕ} {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : Type*}
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    (sourcePiece : ι → Set X)
  ...
  let D :=
    FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      ...
      chart targetPiece_open localMap K local_lipschitz local_agree
  simpa [D] using
    (FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.exists_bounded_region_volume_le_jacobianMass
      D hboundary j)

/-- A compact source with an arbitrary open chart cover and an odd
boundary-degree obstruction also carries any common additive defect term through
that extracted finite metric subcover. -/
theorem FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal_add_ofCompactOpenCover
    {N : ℕ} {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : Type*}
    (hobstruction : HasOddBoundaryDegreeObstruction boundary)
    (sourcePiece : ι → Set X)
  ...
  let D :=
    FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      ...
      chart targetPiece_open localMap K local_lipschitz local_agree
  simpa [D] using
    (FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.finite_universal_ennreal_add
      D hboundary area defect hbudget)
...
/-- A compact-source family of open chart covers with an odd boundary-degree
obstruction already yields the full metric source-chart universal bound once
the summed Jacobian budgets are stated on the extracted finite subcovers. -/
theorem MetricComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal_ofCompactOpenCover
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : ℕ → Type*}
    (area : ℝ≥0∞)
    (hboundary : IsometricCircleBoundary boundary)
    ...
    ENNReal.ofReal universalConstant ≤ area := by
  let S :=
    MetricComplexSourceChartSystemOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      area hboundary hobstruction sourcePiece sourcePiece_open sourcePiece_cover
      targetPiece chart targetPiece_open localMap K local_lipschitz local_agree
      budget
  simpa [S] using
    (MetricComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal S)

/-- The same compact-source family of open chart covers also carries any
common additive defect term unchanged through the extracted finite metric
subcovers. -/
theorem MetricComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal_add_ofCompactOpenCover
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {ι : ℕ → Type*}
    (area defect : ℝ≥0∞)
    (hboundary : IsometricCircleBoundary boundary)
    ...
    (hbudget : ∀ N : ℕ,
      (∑ j : Fin N,
        (FiniteMetricComplexSourceChartDataOfOddBoundaryDegreeObstruction.ofCompactOpenCover
          ...
          (K N)
          (fun j i ↦ local_lipschitz N j i)
          (fun j i x ↦ local_agree N j i x)).jacobianMass hboundary j) + defect ≤ area) :
    ENNReal.ofReal universalConstant + defect ≤ area := by
  let S :=
    MetricComplexSourceChartSystemOfOddBoundaryDegreeObstruction.ofCompactOpenCover
      area hboundary hobstruction sourcePiece sourcePiece_open sourcePiece_cover
      ...
      budget
  simpa [S] using
    (MetricComplexSourceChartSystemOfOddBoundaryDegreeObstruction.universal_ennreal_add
      S hbudget)
...
/-- The same metric source-chart certificate already transfers from any
boundary obtained by a boundary-respecting continuous map from the free upper
loop of the odd adjacent-preserving glued-strip quotient. -/
theorem MetricComplexSourceChartSystem.universal_ennreal_of_comp_continuous_adjacentPreservingCylinderStripGluedPointBoundary
    {m : ℕ} (hm : Odd m)
    {Y : Type*} [PseudoMetricSpace Y]
    {boundary : UnitAddCircle → Y} {area : ℝ≥0∞}
    (S : MetricComplexSourceChartSystem Y boundary area)
...
/-- The same metric rowwise bounded-region certificate also transfers
from any boundary obtained by a boundary-respecting continuous map from the
free upper loop of the odd adjacent-preserving glued-strip quotient. -/
theorem MetricComplexSourceChartSystem.exists_bounded_region_volume_le_jacobianMass_of_comp_continuous_adjacentPreservingCylinderStripGluedPointBoundary
    {m : ℕ} (hm : Odd m)
    {Y : Type*} [PseudoMetricSpace Y]
    {boundary : UnitAddCircle → Y} {area : ℝ≥0∞}
    (S : MetricComplexSourceChartSystem Y boundary area)
...
theorem MetricComplexSourceChartSystem.exists_bounded_region_volume_le_jacobianMass_of_homeomorphCylinderStripGluedPointBoundaryObstructionData
    {Y : Type*} [PseudoMetricSpace Y]
    {boundary : UnitAddCircle → Y} {area : ℝ≥0∞}
    (S : MetricComplexSourceChartSystem Y boundary area)
    (D : HomeomorphCylinderStripGluedPointBoundaryObstructionData boundary)
...
/-- The same disk-homeomorphic route also feeds the metric source-chart
certificate through the bundled explicit glued-strip quotient disk interface. -/
theorem MetricComplexSourceChartSystem.universal_ennreal_of_closedUnitDisk_homeomorph_cylinderStripGlued
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {area : ℝ≥0∞}
    (S : MetricComplexSourceChartSystem X boundary area)
    (e : ClosedUnitDisk ≃ₜ X)
...
/-- The additive-defect metric source-chart certificate also transfers
from any boundary obtained by a boundary-respecting continuous map from the
free upper loop of the odd adjacent-preserving glued-strip quotient. -/
theorem MetricComplexSourceChartSystem.universal_ennreal_add_of_comp_continuous_adjacentPreservingCylinderStripGluedPointBoundary
    {m : ℕ} (hm : Odd m)
    {Y : Type*} [PseudoMetricSpace Y]
    {boundary : UnitAddCircle → Y} {area defect : ℝ≥0∞}
    (S : MetricComplexSourceChartSystem Y boundary area)
...
theorem MetricComplexSourceChartSystem.universal_ennreal_add_of_homeomorphCylinderStripGluedPointBoundaryObstructionData
    {Y : Type*} [PseudoMetricSpace Y]
    {boundary : UnitAddCircle → Y} {area defect : ℝ≥0∞}
    (S : MetricComplexSourceChartSystem Y boundary area)
    (D : HomeomorphCylinderStripGluedPointBoundaryObstructionData boundary)
...
/-- The same disk-homeomorphic route also supplies the additive-defect metric
source-chart certificate through the bundled explicit glued-strip quotient
disk interface. -/
theorem MetricComplexSourceChartSystem.universal_ennreal_add_of_closedUnitDisk_homeomorph_cylinderStripGlued
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {boundary : UnitAddCircle → X} {area defect : ℝ≥0∞}
    (S : MetricComplexSourceChartSystem X boundary area)
    (e : ClosedUnitDisk ≃ₜ X)
...
/-- The finite orientation-free Fourier certificate for complex-plane
domains whose boundary is the image of a compact source carrying the
stronger directed abstract arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem finite_universal_ennreal_of_complex_planar_maps_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  exact finite_universal_ennreal_of_complex_planar_maps_of_odd_boundary_degree_obstruction
    N area boundary'
    (hasOddBoundaryDegreeObstruction_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
      f hf hboundaryMap hmodels)
    G hG hboundary K hGLipschitz hbudget

/-- The finite orientation-free Fourier certificate for complex-plane
domains whose boundary is the image of a compact source carrying the
undirected arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem finite_universal_ennreal_of_complex_planar_maps_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
...
stronger directed abstract arbitrarily-fine polygonal-model interface. -/
theorem finite_universal_ennreal_of_complex_planar_maps_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
...
/-- The finite orientation-free Fourier certificate for complex-plane
domains whose boundary is identified with a compact source carrying the
undirected arbitrarily-fine polygonal-model interface. -/
theorem finite_universal_ennreal_of_complex_planar_maps_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
...
/-- Infinite orientation-free Fourier certificate for complex-plane domains
whose boundary is the image of a compact source carrying the stronger
directed abstract arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem universal_ennreal_of_complex_planar_maps_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  rcases hfinite N with ⟨G, hG, hboundary, K, hGLipschitz, hbudget⟩
  exact finite_universal_ennreal_of_complex_planar_maps_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    N area f hf hboundaryMap hmodels G hG hboundary K hGLipschitz hbudget

/-- Infinite orientation-free Fourier certificate for complex-plane domains
whose boundary is the image of a compact source carrying the undirected
arbitrarily-fine polygonal-model interface under a boundary-respecting
continuous map into the plane. -/
theorem universal_ennreal_of_complex_planar_maps_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  rcases hfinite N with ⟨G, hG, hboundary, K, hGLipschitz, hbudget⟩
  exact finite_universal_ennreal_of_complex_planar_maps_of_compact_continuous_arbitrarilyFine
    N area f hf hboundaryMap hmodels G hG hboundary K hGLipschitz hbudget

/-- Infinite orientation-free Fourier certificate for complex-plane domains
whose boundary is identified with a compact source carrying the stronger
directed abstract arbitrarily-fine polygonal-model interface. -/
theorem universal_ennreal_of_complex_planar_maps_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  intro N
  rcases hfinite N with ⟨G, hG, hboundary, K, hGLipschitz, hbudget⟩
  exact finite_universal_ennreal_of_complex_planar_maps_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    N area e hboundaryHomeomorph hmodels G hG hboundary K hGLipschitz hbudget

/-- Infinite orientation-free Fourier certificate for complex-plane domains
whose boundary is identified with a compact source carrying the undirected
arbitrarily-fine polygonal-model interface. -/
theorem universal_ennreal_of_complex_planar_maps_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  intro N
  rcases hfinite N with ⟨G, hG, hboundary, K, hGLipschitz, hbudget⟩
  exact finite_universal_ennreal_of_complex_planar_maps_of_homeomorph_arbitrarilyFine
    N area e hboundaryHomeomorph hmodels G hG hboundary K hGLipschitz hbudget
...
/-- Planar Lemma 5.4 for the genuine metric distance-profile map when the
boundary is the image of a compact source carrying the stronger directed
abstract arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
    ...
    ENNReal.ofReal (mixedBoundaryArea (givensMatrix N) j) ≤
      ∫⁻ x, ENNReal.ofReal
        |(fderiv ℝ (givensMetricFourierMap boundary' N j) x).det| ∂volume := by
  exact
    givens_mixedBoundaryArea_le_complex_jacobian_of_odd_boundary_degree_obstruction
      j boundary'
      (hasOddBoundaryDegreeObstruction_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
        f hf hboundaryMap hmodels)
      (givensMetricFourierMap boundary' N j)
...
/-- Bounded-region form of planar Lemma 5.4 for the genuine metric Fourier
map when the boundary is the image of a compact source carrying the stronger
directed abstract arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem exists_givensMetricFourierMap_bounded_region_volume_le_complex_jacobian_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
    ...
    ∃ region : Set ℂ,
      ...
      volume region ≤
        ∫⁻ x, ENNReal.ofReal
          |(fderiv ℝ (givensMetricFourierMap boundary' N j) x).det| ∂volume := by
  exact
    exists_givens_bounded_region_volume_le_complex_jacobian_of_odd_boundary_degree_obstruction
      j boundary'
      (hasOddBoundaryDegreeObstruction_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
        f hf hboundaryMap hmodels)
      (givensMetricFourierMap boundary' N j)
...
/-- Planar Lemma 5.4 for the genuine metric distance-profile map when the
boundary is the image of a compact source carrying the undirected
arbitrarily-fine polygonal-model interface under a boundary-respecting
continuous map into the plane. -/
theorem givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
...
undirected arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem exists_givensMetricFourierMap_bounded_region_volume_le_complex_jacobian_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
...
/-- Planar Lemma 5.4 for the genuine metric distance-profile map when the
boundary is identified with a compact source carrying the stronger directed
abstract arbitrarily-fine polygonal-model interface. -/
theorem givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
...
stronger directed abstract arbitrarily-fine polygonal-model interface. -/
theorem exists_givensMetricFourierMap_bounded_region_volume_le_complex_jacobian_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
    ...
    ∃ region : Set ℂ,
      ...
      volume region ≤
        ∫⁻ x, ENNReal.ofReal
          |(fderiv ℝ (givensMetricFourierMap boundary' N j) x).det| ∂volume := by
  let _ : CompactSpace ℂ := Homeomorph.compactSpace e
  exact exists_givensMetricFourierMap_bounded_region_volume_le_complex_jacobian_of_odd_boundary_degree_obstruction
    j boundary'
    (hasOddBoundaryDegreeObstruction_of_homeomorph_abstractVariableDirectedArbitrarilyFine
      e hboundaryHomeomorph hmodels)
    hboundary'
...
/-- Planar Lemma 5.4 for the genuine metric distance-profile map when the
boundary is identified with a compact source carrying the undirected
arbitrarily-fine polygonal-model interface. -/
theorem givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
...
map when the boundary is identified with a compact source carrying the
undirected arbitrarily-fine polygonal-model interface. -/
theorem exists_givensMetricFourierMap_bounded_region_volume_le_complex_jacobian_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
...
/-- Finite slack-refined planar certificate when the boundary is the image of
a compact source carrying the stronger directed abstract arbitrarily-fine
polygonal-model interface under a boundary-respecting continuous map into the
plane. -/
theorem finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro j
  exact
    givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
      j f hf hboundaryMap hmodels hboundary'

/-- Infinite slack-refined planar certificate when the boundary is the image
of a compact source carrying the stronger directed abstract arbitrarily-fine
polygonal-model interface under a boundary-respecting continuous map into the
plane. -/
theorem universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_maps_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
      N area f hf hboundaryMap hmodels hboundary' harea

/-- Finite slack-refined planar certificate when the boundary is the image of
a compact source carrying the undirected arbitrarily-fine polygonal-model
interface under a boundary-respecting continuous map into the plane. -/
theorem finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro j
  exact
    givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_compact_continuous_arbitrarilyFine
      j f hf hboundaryMap hmodels hboundary'

/-- Infinite slack-refined planar certificate when the boundary is the image
of a compact source carrying the undirected arbitrarily-fine polygonal-model
interface under a boundary-respecting continuous map into the plane. -/
theorem universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_maps_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_compact_continuous_arbitrarilyFine
      N area f hf hboundaryMap hmodels hboundary' harea

/-- Finite slack-refined planar certificate when the boundary is identified
with a compact source carrying the stronger directed abstract arbitrarily-fine
polygonal-model interface. -/
theorem finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  refine (finite_universal_ennreal_of_givens_coverage_budget_add N
    ...
    (fun j ↦ ∫⁻ x, ENNReal.ofReal
      |(fderiv ℝ (givensMetricFourierMap boundary' N j) x).det| ∂volume) ?_ hbudget).trans harea
  intro j
  exact givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    j e hboundaryHomeomorph hmodels hboundary'

/-- Infinite slack-refined planar certificate when the boundary is identified
with a compact source carrying the stronger directed abstract arbitrarily-fine
polygonal-model interface. -/
theorem universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_maps_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_homeomorph_abstractVariableDirectedArbitrarilyFine
      N area e hboundaryHomeomorph hmodels hboundary' harea

/-- Finite slack-refined planar certificate when the boundary is identified
with a compact source carrying the undirected arbitrarily-fine polygonal-model
interface. -/
theorem finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  refine (finite_universal_ennreal_of_givens_coverage_budget_add N
    ...
    (fun j ↦ ∫⁻ x, ENNReal.ofReal
      |(fderiv ℝ (givensMetricFourierMap boundary' N j) x).det| ∂volume) ?_ hbudget).trans harea
  intro j
  exact givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_homeomorph_arbitrarilyFine
    j e hboundaryHomeomorph hmodels hboundary'

/-- Infinite slack-refined planar certificate when the boundary is identified
with a compact source carrying the undirected arbitrarily-fine polygonal-model
interface. -/
theorem universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_maps_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_add_lintegral_distanceSlackDerivativeEnergyDefect_of_metric_complex_planar_map_of_homeomorph_arbitrarilyFine
      N area e hboundaryHomeomorph hmodels hboundary' harea
...
/-- The complete finite planar certificate for the actual metric Fourier
family when the boundary is the image of a compact source carrying the
stronger directed abstract arbitrarily-fine polygonal-model interface under
a boundary-respecting continuous map into the plane. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
...
/-- Infinite-mode planar conclusion for the actual metric Fourier family when
the boundary is the image of a compact source carrying the stronger directed
abstract arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_of_metric_complex_planar_map_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
      N area f hf hboundaryMap hmodels hboundary' (hbudget N)
...
undirected arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
...
/-- Infinite-mode planar conclusion for the actual metric Fourier family when
the boundary is the image of a compact source carrying the undirected
arbitrarily-fine polygonal-model interface under a boundary-respecting
continuous map into the plane. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_of_metric_complex_planar_map_of_compact_continuous_arbitrarilyFine
      N area f hf hboundaryMap hmodels hboundary' (hbudget N)

/-- The complete finite planar certificate for the actual metric Fourier family
when the boundary is identified with a compact source carrying the stronger
directed abstract arbitrarily-fine polygonal-model interface. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  apply finite_universal_ennreal_of_givens_coverage_budget N area
    (fun j ↦ ∫⁻ x, ENNReal.ofReal
      |(fderiv ℝ (givensMetricFourierMap boundary' N j) x).det| ∂volume)
  · exact fun j ↦
      givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_homeomorph_abstractVariableDirectedArbitrarilyFine
        j e hboundaryHomeomorph hmodels hboundary'
  · exact hbudget

/-- Infinite-mode planar conclusion for the actual metric Fourier family when
the boundary is identified with a compact source carrying the stronger
directed abstract arbitrarily-fine polygonal-model interface. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  apply universal_ennreal_bound_of_finite_certificates
  intro N
  exact finite_universal_ennreal_of_metric_complex_planar_map_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    N area e hboundaryHomeomorph hmodels hboundary' (hbudget N)

/-- The complete finite planar certificate for the actual metric Fourier family
when the boundary is identified with a compact source carrying the undirected
arbitrarily-fine polygonal-model interface. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  apply finite_universal_ennreal_of_givens_coverage_budget N area
    (fun j ↦ ∫⁻ x, ENNReal.ofReal
      |(fderiv ℝ (givensMetricFourierMap boundary' N j) x).det| ∂volume)
  · exact fun j ↦
      givensMetricFourierMap_mixedBoundaryArea_le_complex_jacobian_of_homeomorph_arbitrarilyFine
        j e hboundaryHomeomorph hmodels hboundary'
  · exact hbudget

/-- Infinite-mode planar conclusion for the actual metric Fourier family when
the boundary is identified with a compact source carrying the undirected
arbitrarily-fine polygonal-model interface. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  apply universal_ennreal_bound_of_finite_certificates
  intro N
  exact finite_universal_ennreal_of_metric_complex_planar_map_of_homeomorph_arbitrarilyFine
    N area e hboundaryHomeomorph hmodels hboundary' (hbudget N)
...
abstract arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  have hbudget' :
      ∀ᵐ x ∂volume.restrict (Set.univ : Set ℂ),
        (∑ k : Fin N, complexDerivativeEnergy
          (fderiv ℝ (oddProfileFourierMap boundary' (oddMode k)) x)) ≤ 2 :=
    ae_restrict_of_ae hbudget
  refine (finite_universal_ennreal_of_metric_complex_planar_map_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    N (volume (Set.univ : Set ℂ)) f hf hboundaryMap hmodels hboundary' ?_).trans harea
  simpa only [Measure.restrict_univ] using
    sum_lintegral_givensMetricFourierMap_le_volume_of_coordinateEnergy
      boundary' hboundary' N Set.univ MeasurableSet.univ hbudget'
...
abstract arbitrarily-fine polygonal-model interface under a
boundary-respecting continuous map into the plane. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_coordinateEnergy_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_compact_continuous_abstractVariableDirectedArbitrarilyFine
      N area f hf hboundaryMap hmodels hboundary' (hbudget N) harea
...
arbitrarily-fine polygonal-model interface under a boundary-respecting
continuous map into the plane. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  have hbudget' :
      ∀ᵐ x ∂volume.restrict (Set.univ : Set ℂ),
        (∑ k : Fin N, complexDerivativeEnergy
          (fderiv ℝ (oddProfileFourierMap boundary' (oddMode k)) x)) ≤ 2 :=
    ae_restrict_of_ae hbudget
  refine (finite_universal_ennreal_of_metric_complex_planar_map_of_compact_continuous_arbitrarilyFine
    N (volume (Set.univ : Set ℂ)) f hf hboundaryMap hmodels hboundary' ?_).trans harea
  simpa only [Measure.restrict_univ] using
    sum_lintegral_givensMetricFourierMap_le_volume_of_coordinateEnergy
      boundary' hboundary' N Set.univ MeasurableSet.univ hbudget'
...
arbitrarily-fine polygonal-model interface under a boundary-respecting
continuous map into the plane. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_coordinateEnergy_of_compact_continuous_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (f : X → ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_compact_continuous_arbitrarilyFine
      N area f hf hboundaryMap hmodels hboundary' (hbudget N) harea

/-- The finite planar certificate with coordinate-energy budget when the
boundary is identified with a compact source carrying the stronger directed
abstract arbitrarily-fine polygonal-model interface. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  have hbudget' :
      ∀ᵐ x ∂volume.restrict (Set.univ : Set ℂ),
        (∑ k : Fin N, complexDerivativeEnergy
          (fderiv ℝ (oddProfileFourierMap boundary' (oddMode k)) x)) ≤ 2 :=
    ae_restrict_of_ae hbudget
  refine (finite_universal_ennreal_of_metric_complex_planar_map_of_homeomorph_abstractVariableDirectedArbitrarilyFine
...
boundary is identified with a compact source carrying the stronger directed
abstract arbitrarily-fine polygonal-model interface. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_coordinateEnergy_of_homeomorph_abstractVariableDirectedArbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_homeomorph_abstractVariableDirectedArbitrarilyFine
      N area e hboundaryHomeomorph hmodels hboundary' (hbudget N) harea

/-- The finite planar certificate with coordinate-energy budget when the
boundary is identified with a compact source carrying the undirected
arbitrarily-fine polygonal-model interface. -/
theorem finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (N : ℕ) (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  have hbudget' :
      ∀ᵐ x ∂volume.restrict (Set.univ : Set ℂ),
        (∑ k : Fin N, complexDerivativeEnergy
          (fderiv ℝ (oddProfileFourierMap boundary' (oddMode k)) x)) ≤ 2 :=
    ae_restrict_of_ae hbudget
  refine (finite_universal_ennreal_of_metric_complex_planar_map_of_homeomorph_arbitrarilyFine
    N (volume (Set.univ : Set ℂ)) e hboundaryHomeomorph hmodels hboundary' ?_).trans harea
  simpa only [Measure.restrict_univ] using
    sum_lintegral_givensMetricFourierMap_le_volume_of_coordinateEnergy
      boundary' hboundary' N Set.univ MeasurableSet.univ hbudget'
...
boundary is identified with a compact source carrying the undirected
arbitrarily-fine polygonal-model interface. -/
theorem universal_ennreal_of_metric_complex_planar_maps_of_coordinateEnergy_of_homeomorph_arbitrarilyFine
    {X : Type*} [PseudoMetricSpace X] [CompactSpace X]
    (area : ℝ≥0∞)
    {boundary : UnitAddCircle → X} {boundary' : UnitAddCircle → ℂ}
    (e : X ≃ₜ ℂ)
  ...
  intro N
  exact
    finite_universal_ennreal_of_metric_complex_planar_map_of_coordinateEnergy_of_homeomorph_arbitrarilyFine
      N area e hboundaryHomeomorph hmodels hboundary' (hbudget N) harea
