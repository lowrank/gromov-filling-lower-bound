...
/-- If the boundary is identified with the standard closed-disk boundary by a
homeomorphism, the explicit disk polygonal-model construction also supplies
the quotient-friendly directed abstract arbitrarily fine interface, so the
Jordan region at the origin is covered without any extra topological
hypothesis. -/
theorem givensBoundaryCurve_jordan_region_subset_range_of_closedUnitDisk_homeomorph_abstractVariableDirectedQuotient
    {Y : Type*} [PseudoMetricSpace Y] [CompactSpace Y]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → Y}
    (e : ClosedUnitDisk ≃ₜ Y)
...
from the quotient-friendly directed disk polygonal-model interface. -/
theorem exists_givensBoundaryCurve_bounded_region_subset_range_of_closedUnitDisk_homeomorph_abstractVariableDirectedQuotient
    {Y : Type*} [PseudoMetricSpace Y] [CompactSpace Y]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → Y}
    (e : ClosedUnitDisk ≃ₜ Y)
...
theorem givensBoundaryCurve_jordan_region_subset_range_of_homeomorphCylinderStripGluedPointBoundaryObstructionData
    ...
    (hzero : 0 ∈ region₁) :
    region₁ ⊆ Set.range G :=
  givensBoundaryCurve_jordan_region_subset_range_of_homeomorph_cylinderStripGluedPointBoundary
...
/-- Separation-free coverage also transfers from the bundled point-quotient
homeomorphism-plus-obstruction interface. -/
theorem exists_givensBoundaryCurve_bounded_region_subset_range_of_homeomorphCylinderStripGluedPointBoundaryObstructionData
    {Y : Type*} [TopologicalSpace Y]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → Y}
    (D : HomeomorphCylinderStripGluedPointBoundaryObstructionData boundary)
    ...
    ∃ region : Set ℂ,
      ...
      Bornology.IsBounded region ∧ 0 ∈ region ∧
      region ⊆ Set.range G :=
  exists_givensBoundaryCurve_bounded_region_subset_range_of_homeomorph_cylinderStripGluedPointBoundary
    D.pairing j D.homeomorph D.boundaryHomeomorph D.lowerObstruction
    G hG hboundary
...
/-- If the boundary is identified with the standard closed-square boundary by a
homeomorphism, the Jordan region at the origin is already covered through the
quotient-friendly directed polygonal-model interface furnished by the square
model. -/
theorem givensBoundaryCurve_jordan_region_subset_range_of_closedUnitSquare_homeomorph_abstractVariableDirectedQuotient
    {Y : Type*} [PseudoMetricSpace Y] [CompactSpace Y]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → Y}
    (e : ClosedUnitSquare ≃ₜ Y)
...
follows from the quotient-friendly directed polygonal-model route. -/
theorem exists_givensBoundaryCurve_bounded_region_subset_range_of_closedUnitSquare_homeomorph_abstractVariableDirectedQuotient
    {Y : Type*} [PseudoMetricSpace Y] [CompactSpace Y]
    {N : ℕ} (j : Fin N)
    {boundary : UnitAddCircle → Y}
    (e : ClosedUnitSquare ≃ₜ Y)
